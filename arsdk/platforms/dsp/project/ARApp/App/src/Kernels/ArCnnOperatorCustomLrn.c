...
void dcnn_exec_scale_interal(char* input_data, float* output_data, int data_size, float scale_factor, int zero_point, int precision)
{
...
void dcnn_exec_quantization_interal(float* float_data, char* quant_data, int data_size, float scale_factor, int zero_point, int precision)
{
...
static AR_S32 CustomLrnForward(AR_U32 input_data, AR_U32 output_data,	AR_NPU_CB_PARAM_S *pCBParams, void *pOpParams)
{
	...
	if (!pLrnInput)
	{
		xDebugPrint("Malloc lrn input error.\n");
		return -1;
	}
	...
	dcnn_hwc_to_chw(input_data, pLrnInput, &pCBParams->astInputTensor[0]);


	float *input_data_temp = (float *)malloc(data_size * sizeof(double));
	if (!input_data_temp)
	{
		xDebugPrint("Malloc input buffer failed.\n");
		return -1;
	}
	float *output_data_temp = (float *)malloc(data_size * sizeof(double));
	if (!output_data_temp)
	{
		xDebugPrint("Malloc output buffer failed.\n");
	...
	dcnn_chw_to_hwc(pLrnOutput, output_data, &pCBParams->astOutputTensor[0]);
...
AR_S32 ArCnnOperatorCustomLrn(AR_U32 pIn, AR_U32 pOut, AR_NPU_CB_PARAM_S * pCBParams, void*  pOpParams)
{
...
AR_CNN_OPERATOR_DECLARE(ArCnnOperatorCustomLrn) = {
    .pName = "LRN",
    .pCnnCbHandler = ArCnnOperatorCustomLrn,
};
