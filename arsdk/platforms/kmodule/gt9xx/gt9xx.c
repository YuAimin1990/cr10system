...
static int gt_debug = 0;
module_param(gt_debug, int, 0600);

static int width = -1;
module_param(width, int, 0600);

static int height = -1;
module_param(height, int, 0600);

static int swapxy = -1;
module_param(swapxy, int, 0600);

static int mirrorx2y = -1;
module_param(mirrorx2y, int, 0600);
			 ...
			 u8 *key_value)
{
	...
	*key_value = point_data[3 + 8 * touch_num];
	...
	if(gt_debug)
        dev_info(&ts->client->dev, "-----touch_num:%d-----\n", touch_num);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	if (mirrorx2y != -1) {
		pdata->mirror_x2y = mirrorx2y;
	} else {
		of_property_read_u32(np, "goodix,mirror-x2y", &pdata->mirror_x2y);
	}
	if (pdata->mirror_x2y)
		dev_info(dev, "mirror_x2y-x2y enabled\n");
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "gt_debug: %d, width: %d, height: %d, swapxy: %d, mirrorx2y: %d", gt_debug, width, height, swapxy, mirrorx2y);
