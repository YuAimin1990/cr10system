...
#define MAX_GESTURE_DATA_SIZE 100
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	u8 ges_data[MAX_GESTURE_DATA_SIZE];
    s32 ret = 0;
...
//         error otherwise.
__maybe_unused static long gt1x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_COMPAT
__maybe_unused static long gt1x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static const struct proc_ops gt1x_fops = {
	.proc_read = gt1x_gesture_data_read,
	.proc_write = gt1x_gesture_data_write,
	.proc_ioctl = gt1x_ioctl,
#ifdef CONFIG_COMPAT
    .proc_compat_ioctl = gt1x_compat_ioctl,
#endif
};
#else
