...
#ifdef CONFIG_DRM_PANEL_NOTIFY
static int gt1x_check_dt(struct device_node *np)
{
        int i;
        int count;
                                ...
                                struct panel_event_notification *notification, void *client_data)
{
        ...
        GTP_INFO("gsl notification");
        if (!notification) {
                GTP_ERROR("Invalid notification");
				return;
        }
        ...
        case DRM_PANEL_EVENT_BLANK:
                if (notification->notif_data.early_trigger)
                        GTP_INFO("resume notification pre commit");
                else
                    	gt1x_suspend();
                break;
        case DRM_PANEL_EVENT_UNBLANK:
                if (notification->notif_data.early_trigger)
                        GTP_INFO("suspend notification pre commit");
                else
                        gt1x_resume();
                break;
        default:
                GTP_INFO("notification serviced :%d",notification->notif_type);
                break;
        }
}
...
static int gt1x_register_powermanger(void)
{
	...
	if (active_panel) {
		cookie = panel_event_notifier_register(
				...
				NULL);
        if (IS_ERR(cookie))
                GTP_ERROR("Failed to register panel event notifier");
	}
#elif defined(CONFIG_FB)
