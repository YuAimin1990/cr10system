...
static struct proc_ops gt1x_tool_fops = {
...
Output:
    Return write length.
********************************************************/
static ssize_t gt1x_tool_write(struct file *filp, const char __user * buff, size_t len, loff_t * ppos)
{
	u64 ret = 0;
	GTP_DEBUG_FUNC();
...
static ssize_t gt1x_tool_read(struct file *filp, char __user * buffer, size_t count, loff_t * ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		GTP_DEBUG("unsupported operation 2");
		return -1;
		...
		if (copy_to_user(buffer, GTP_DRIVER_VERSION, tmp_len + 1)) {
			GTP_ERROR("copy to user failed");
		}
		return (tmp_len + 1);
	}
