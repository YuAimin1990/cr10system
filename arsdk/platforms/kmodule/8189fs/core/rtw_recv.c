...
static void rtw_signal_stat_timer_hdl(void *ctx)
{
	...
	if (adapter->recvpriv.is_signal_dbg) {
		...
		RTW_INFO(FUNC_ADPT_FMT" tmp_s:%d, ratio_curr_stat:%d, avg_signal_strength:%d, ratio_pre_stat:%d, recvpriv->signal_strength:%d, ratio_total:%d\n" , FUNC_ADPT_ARG(adapter), tmp_s, ratio_curr_stat,
		avg_signal_strength, ratio_pre_stat, recvpriv->signal_strength, ratio_total);
		...
		RTW_INFO(FUNC_ADPT_FMT" tmp_q:%d, ratio_curr_stat:%d, avg_signal_qual:%d, ratio_pre_stat:%d, recvpriv->signal_qual:%d, ratio_total:%d\n" , FUNC_ADPT_ARG(adapter), tmp_q, ratio_curr_stat,
		avg_signal_qual, ratio_pre_stat, recvpriv->signal_qual, ratio_total);
		...
		if (avg_signal_strength == 0) {
			tmp_s = 1;
			recvpriv->signal_strength = 0;
		}
		if (avg_signal_qual == 0) tmp_q = 0;
		recvpriv->rssi = (s8)translate_percentage_to_dbm(tmp_s);
		if (recvpriv->rssi != 0) {
			prerssi = recvpriv->rssi;
			RTW_INFO(FUNC_ADPT_FMT" prerssi:%3d\n", FUNC_ADPT_ARG(adapter), prerssi);
		}
		else if (recvpriv->rssi == 0) {
			recvpriv->rssi = prerssi;
			RTW_INFO(FUNC_ADPT_FMT" recvpriv->rssi:%3d\n", FUNC_ADPT_ARG(adapter), recvpriv->rssi);
		}
