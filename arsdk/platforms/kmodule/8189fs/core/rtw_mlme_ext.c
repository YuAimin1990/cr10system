	...
	rtw_rson_scan_wk_cmd(padapter, RSON_SCAN_PROCESS);
		...
		if (rtw_chk_roam_flags(padapter, RTW_ROAM_ACTIVE)) {
			...
			if ((precvpriv->signal_strength_data.avg_val < pmlmepriv->roam_rssi_threshold)
				&& (rtw_get_passing_time_ms(pmlmepriv->last_roaming) >= pmlmepriv->roam_scan_int*1000)) {
#ifdef CONFIG_RTW_80211K
...
bypass_active_keep_alive:
			...
			if (rx_chk == _FAIL) {
				pmlmeext->retry++;
				RTW_PRINT(FUNC_ADPT_FMT"limit:%d, retry:%d\n", FUNC_ADPT_ARG(padapter), rx_chk_limit, pmlmeext->retry);
