...
#define CHECK_OWNER(local) \
    do { \
        ...
        if (0 != memcmp(&owner, local, sizeof(owner))) { \
            AR_LOG_ERROR("return user uuid failed\n"); \
            AR_LOG_PrintErrCode(AR_ERR_CIPHER_ILLEGAL_UUID);\
            return AR_ERR_CIPHER_ILLEGAL_UUID; \
        } \
    } while (0)
...
/*! \the max cipher soft channel count*/
#define CRYPTO_HARD_CHANNEL_MAX         (0x10)

/*! \serure mmz or not, not used */
#define SEC_MMZ                         (0x00)

#define DISABLE_DEBUG_INFO

#ifdef DISABLE_DEBUG_INFO
#define AR_PROC_SUPPORT                 0
#else
#define AR_PROC_SUPPORT                 1
#endif
...
#define LOG_D_MODULE_ID             AR_ID_CIPHER
...
#define AR_LOG_ERR_MEM              LOG_ERRCODE_DEF(0x0001)      /**< Memory Operation Error */
#define AR_LOG_ERR_SEM              LOG_ERRCODE_DEF(0x0002)      /**< Semaphore Operation Error */
#define AR_LOG_ERR_FILE             LOG_ERRCODE_DEF(0x0003)      /**< File Operation Error */
#define AR_LOG_ERR_LOCK             LOG_ERRCODE_DEF(0x0004)      /**< Lock Operation Error */
#define AR_LOG_ERR_PARAM            LOG_ERRCODE_DEF(0x0005)      /**< Invalid Parameter */
#define AR_LOG_ERR_TIMER            LOG_ERRCODE_DEF(0x0006)      /**< Timer error */
#define AR_LOG_ERR_THREAD           LOG_ERRCODE_DEF(0x0007)      /**< Thread Operation Error */
#define AR_LOG_ERR_TIMEOUT          LOG_ERRCODE_DEF(0x0008)      /**< Time Out Error */
#define AR_LOG_ERR_DEVICE           LOG_ERRCODE_DEF(0x0009)      /**< Device Operation Error */
#define AR_LOG_ERR_STATUS           LOG_ERRCODE_DEF(0x0010)      /**< Status Error */
#define AR_LOG_ERR_IOCTRL           LOG_ERRCODE_DEF(0x0011)      /**< IO Operation Error */
#define AR_LOG_ERR_INUSE            LOG_ERRCODE_DEF(0x0012)      /**< In use */
#define AR_LOG_ERR_EXIST            LOG_ERRCODE_DEF(0x0013)      /**< Have exist */
#define AR_LOG_ERR_NOEXIST          LOG_ERRCODE_DEF(0x0014)      /**< no exist */
#define AR_LOG_ERR_UNSUPPORTED      LOG_ERRCODE_DEF(0x0015)      /**< Unsupported */
#define AR_LOG_ERR_UNAVAILABLE      LOG_ERRCODE_DEF(0x0016)      /**< Unavailable */
#define AR_LOG_ERR_UNINITED         LOG_ERRCODE_DEF(0x0017)      /**< Uninited */
#define AR_LOG_ERR_DATABASE         LOG_ERRCODE_DEF(0x0018)      /**< Database Operation Error */
#define AR_LOG_ERR_OVERFLOW         LOG_ERRCODE_DEF(0x0019)      /**< Overflow */
#define AR_LOG_ERR_EXTERNAL         LOG_ERRCODE_DEF(0x0020)      /**< External Error */
#define AR_LOG_ERR_UNKNOWNED        LOG_ERRCODE_DEF(0x0021)      /**< Unknow Error */
#define AR_LOG_ERR_FLASH            LOG_ERRCODE_DEF(0x0022)      /**< Flash Operation Error*/
#define AR_LOG_ERR_ILLEGAL_IMAGE    LOG_ERRCODE_DEF(0x0023)      /**< Illegal Image */
#define AR_LOG_ERR_ILLEGAL_UUID     LOG_ERRCODE_DEF(0x0023)      /**< Illegal UUID */
#define AR_LOG_ERR_NOPERMISSION     LOG_ERRCODE_DEF(0x0023)      /**< No Permission */

/**< Function trace log, strictly prohibited to expand */
#define AR_LOG_PrintFuncWar(Func, ErrCode)  AR_LOG_WARN("Call %s return [0x%08X]\n", #Func, (unsigned int)ErrCode);
#define AR_LOG_PrintFuncErr(Func, ErrCode)  AR_LOG_ERROR("Call %s return [0x%08X]\n", #Func, (unsigned int)ErrCode);
#define AR_LOG_PrintErrCode(ErrCode)        AR_LOG_ERROR("Error Code: [0x%08X]\n", (unsigned int)ErrCode);

/**< Used for displaying more detailed error information */
#define AR_ERR_PrintS32(val)                AR_LOG_ERROR("%s = %d\n",        #val, val)
#define AR_ERR_PrintU32(val)                AR_LOG_ERROR("%s = %u\n",        #val, val)
#define AR_ERR_PrintS64(val)                AR_LOG_ERROR("%s = %lld\n",      #val, val)
#define AR_ERR_PrintU64(val)                AR_LOG_ERROR("%s = %llu\n",      #val, val)
#define AR_ERR_PrintH32(val)                AR_LOG_ERROR("%s = 0x%08X\n",    #val, val)
#define AR_ERR_PrintH64(val)                AR_LOG_ERROR("%s = 0x%016llX\n", #val, val)
#define AR_ERR_PrintStr(val)                AR_LOG_ERROR("%s = %s\n",        #val, val)
#define AR_ERR_PrintVoid(val)               AR_LOG_ERROR("%s = %p\n",        #val, val)
#define AR_ERR_PrintFloat(val)              AR_LOG_ERROR("%s = %f\n",        #val, val)
#define AR_ERR_PrintInfo(val)               AR_LOG_ERROR("<%s>\n", val)

/**< Used for displaying more detailed warning information */
#define AR_LOG_PrintS32(val)                AR_LOG_WARN("%s = %d\n",        #val, val)
#define AR_LOG_PrintU32(val)                AR_LOG_WARN("%s = %u\n",        #val, val)
#define AR_LOG_PrintS64(val)                AR_LOG_WARN("%s = %lld\n",      #val, val)
#define AR_LOG_PrintU64(val)                AR_LOG_WARN("%s = %llu\n",      #val, val)
#define AR_LOG_PrintH32(val)                AR_LOG_WARN("%s = 0x%08X\n",    #val, val)
#define AR_LOG_PrintH64(val)                AR_LOG_WARN("%s = 0x%016llX\n", #val, val)
#define AR_LOG_PrintStr(val)                AR_LOG_WARN("%s = %s\n",        #val, val)
#define AR_LOG_PrintVoid(val)               AR_LOG_WARN("%s = %p\n",        #val, val)
#define AR_LOG_PrintFloat(val)              AR_LOG_WARN("%s = %f\n",        #val, val)
#define AR_LOG_PrintInfo(val)               AR_LOG_WARN("<%s>\n", val)

/**< Only used for self debug, Can be expanded as needed */
#define AR_DBG_PrintS32(val)                AR_LOG_DEBUG("%s = %d\n",       #val, val)
#define AR_DBG_PrintU32(val)                AR_LOG_DEBUG("%s = %u\n",       #val, val)
#define AR_DBG_PrintS64(val)                AR_LOG_DEBUG("%s = %lld\n",     #val, val)
#define AR_DBG_PrintU64(val)                AR_LOG_DEBUG("%s = %llu\n",     #val, val)
#define AR_DBG_PrintH32(val)                AR_LOG_DEBUG("%s = 0x%08X\n",   #val, val)
#define AR_DBG_PrintH64(val)                AR_LOG_DEBUG("%s = 0x%016llX\n",#val, val)
#define AR_DBG_PrintStr(val)                AR_LOG_DEBUG("%s = %s\n",       #val, val)
#define AR_DBG_PrintVoid(val)               AR_LOG_DEBUG("%s = %p\n",       #val, val)
#define AR_DBG_PrintFloat(val)              AR_LOG_DEBUG("%s = %f\n",       #val, val)
#define AR_DBG_PrintInfo(val)               AR_LOG_DEBUG("<%s>\n", val)

#if (LOG_D_FUNCTRACE == 1) || (LOG_D_UNFTRACE == 1)
#define AR_UNF_FuncEnter()                  AR_LOG_DEBUG(" >>>>>>[Enter]\n")    /**< Only used for unf interface */
#define AR_UNF_FuncExit()                   AR_LOG_DEBUG(" <<<<<<[Exit]\n")     /**< Only used for unf interface */
#else
#define AR_UNF_FuncEnter()
#define AR_UNF_FuncExit()
#endif

//#define LOG_D_FUNCTRACE 1

#if LOG_D_FUNCTRACE
#define AR_LOG_FuncEnter()                  AR_LOG_DEBUG(" =====>[Enter]\n")    /**< Used for all interface except unf */
#define AR_LOG_FuncExit()                   AR_LOG_DEBUG(" =====>[Exit]\n")     /**< Used for all interface except unf */
#else
#define AR_LOG_FuncEnter()
#define AR_LOG_FuncExit()
#endif

#define AR_LOG_CHECK(fnFunc)                            \
do                                                      \
{                                                       \
    AR_S32 s32iErrCode = fnFunc;                        \
    if (AR_SUCCESS != s32iErrCode)                      \
    {                                                   \
        AR_LOG_PrintFuncErr(#fnFunc, s32iErrCode);      \
    }                                                   \
} while (0)


#define AR_LOG_CHECK_PARAM(bVal)                        \
do                                                      \
{                                                       \
    if (bVal)                                           \
    {                                                   \
...
#define AR_LOG_CHECK_INITED(u32InitCount)               \
do                                                      \
{                                                       \
    if (0 == u32InitCount)                              \
    {                                                   \
        AR_LOG_PrintErrCode(AR_ERR_CIPHER_NOT_INIT);    \
        return AR_ERR_CIPHER_NOT_INIT;                  \
    }                                                   \
} while (0)

#define AR_LOG_PrintBlock(data, length)
