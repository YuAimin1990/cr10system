...
#if 1
    if(memblock_overlaps_region(&memblock.reserved, new_start, zone->nbytes) ||
       memblock_overlaps_region(&memblock.memory, new_start, zone->nbytes))
    {
...
#ifdef MMZ_GAP_SUPPORT
int gap_is_dirty(unsigned long phys_addr, int is_header)
{
    unsigned int* vir_addr = ioremap(phys_addr, MMZ_GAP_OFFSET);
    int ret = 0;
    int i = 0;
    for(i=0; i<MMZ_GAP_OFFSET/4-is_header; ++i)
    {
        if(0 != vir_addr[i])
        {
            printk(KERN_ERR "gap is dirty, start index:%d address:0x%08lX, value:%02x!\n", i, phys_addr+i*4, vir_addr[i]);
            ret = 1;
            break;
        }
    }
...
int get_mmb_size(unsigned long phys_addr)
{
    int mmz_size = 0;

    unsigned int* vir_addr = ioremap(phys_addr, MMZ_GAP_OFFSET);
    mmz_size = vir_addr[MMZ_GAP_OFFSET/4-1];
    iounmap(vir_addr);
...
int mmz_read_proc(struct seq_file *sfile)
{
    ...
    list_for_each_entry(p, &mmz_list, list) {
        ...
        list_for_each_entry(mmb, &p->mmb_list, list) {
            #ifdef MMZ_GAP_SUPPORT
            seq_printf(sfile, "   |-MMB: " HIL_MMB_FMT_S " gap0:%d gap1:%d \n", hil_mmb_fmt_arg(mmb), gap_is_dirty(mmb->phys_addr, 1), gap_is_dirty(mmb->phys_addr+get_mmb_size(mmb->phys_addr)+MMZ_GAP_OFFSET, 0));
            #else
