...
static int mmz_flush_invalid_mmb_dirty(struct dirty_area *p_area)
{
    if (p_area == NULL) {
        return -EINVAL;
    }
...
#ifdef CONFIG_64BIT
    //__dma_map_area(p_area->dirty_virt_start, p_area->dirty_size, DMA_TO_DEVICE);
    __dma_unmap_area(p_area->dirty_virt_start, p_area->dirty_size, DMA_FROM_DEVICE);
#endif
    return 0;
}
...
static long mmz_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) == 'm') {
        ...
        if(_IOC_NR(cmd) == 50) {
            ...
            pmi = get_mmbinfo(mmb->phys_addr, pmu);
            if(pmi == NULL && area.dirty_virt_start != NULL) {
                ret = -EFAULT;
                goto __error_exit;
            }else if (pmi == NULL && area.dirty_virt_start == NULL) {
                unsigned long prot, flags, pgoff;
                addr = 0;
                ...
                area.dirty_virt_start = (void *)(addr + (area.dirty_phys_start - mmb->phys_addr));//ioremap(mmb->phys_addr, mmb->length);
                //printk(KERN_INFO "%p %p\n", area.dirty_virt_start, mmb->kvirt);
            }else{
                area.dirty_virt_start = pmi->mapped + (area.dirty_phys_start - pmi->phys_addr);
            }
        ...
        struct dirty_area area;
        //__phys_addr_type__ orig_addr;
        unsigned long offset;//virt_addr
        struct mm_struct *mm = current->mm;
        ...
        if(_IOC_NR(cmd) == 51) {
            ...
            mmz_flush_invalid_mmb_dirty(&area);
            up_read(&mm->mmap_sem);
        }else {
            unsigned long addr = 0, len = 0;
