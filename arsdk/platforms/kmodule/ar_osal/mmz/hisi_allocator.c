...
#ifdef MMZ_GAP_SUPPORT
static int clean_gap(unsigned long phys_addr, int size)
{
    unsigned int* vir_addr = ioremap(phys_addr, MMZ_GAP_OFFSET);
    int ret = 0;
    int i = 0;
    for(i=0; i<MMZ_GAP_OFFSET/4; ++i)
    {
        vir_addr[i] = 0;
    }
    vir_addr[MMZ_GAP_OFFSET/4-1] = size;
    iounmap(vir_addr);
                              ...
                              hil_mmz_t *_user_mmz)
{
...
#ifdef MMZ_GAP_SUPPORT
    mmb = kmalloc(sizeof(hil_mmb_t)*2, GFP_KERNEL);
#else
...
#ifdef MMZ_GAP_SUPPORT
    memset(mmb, 0, sizeof(hil_mmb_t)*2);
#else
    ...
    if (do_mmb_alloc(mmb)) {
        ...
        memcpy(mmb+1, mmb, sizeof(hil_mmb_t));
        (mmb+1)->length = size - MMZ_GAP_TOTAL_SIZE;
        (mmb+1)->phys_addr += MMZ_GAP_OFFSET;

        clean_gap(mmb->phys_addr, temp_size);
        clean_gap(mmb->phys_addr+temp_size+MMZ_GAP_OFFSET, 0);
    }
    return mmb+1;
#else
    return mmb;
...
#ifdef MMZ_GAP_SUPPORT
    if(gap_is_dirty(mmb->phys_addr, 1)){
        printk(KERN_ERR "error name:%s, check gap0 is dirty when mmb free\n", mmb->name);
        WARN_ON(1);
    }

    if(gap_is_dirty(mmb->phys_addr+get_mmb_size(mmb->phys_addr)+MMZ_GAP_OFFSET, 0)){
        printk(KERN_ERR "error name:%s, check gap1 is dirty when mmb free\n", mmb->name);
        WARN_ON(1);
    }
#endif
