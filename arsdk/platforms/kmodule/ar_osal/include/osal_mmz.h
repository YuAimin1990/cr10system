...
#define MMZ_GAP_TOTAL_SIZE         (8*1024)
#define MMZ_GAP_OFFSET             (MMZ_GAP_TOTAL_SIZE/2)
...
#else
#define hil_mmb_fmt_arg(p) (p)->phys_addr+MMZ_GAP_OFFSET, mmz_grain_align((p)->phys_addr + (p)->length -MMZ_GAP_OFFSET) - 1, (unsigned long)(uintptr_t)((p)->kvirt), (p)->flags, ((p)->length-MMZ_GAP_TOTAL_SIZE) / SZ_1K, (p)->name
#endif
...
#define IOC_MMB_INVALID_DCACHE_DIRTY _IOW('i', 51, struct dirty_area)
#define IOC_MMB_FLUSH_DCACHE_DIRTY_PA _IOW('d', 52, struct dirty_area)
#define IOC_MMB_INVALID_DCACHE_DIRTY_PA _IOW('i', 53, struct dirty_area)
