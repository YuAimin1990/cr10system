...
static ssize_t psblk_generic_blk_read(char *buf, size_t bytes, loff_t pos)
{
	...
	if (pos >= pstore_device_info->zone.pmsg_size + pstore_device_info->zone.console_size + pstore_device_info->zone.ftrace_size){
        return ramoops_blk_read(buf, bytes, pos - (pstore_device_info->zone.pmsg_size + pstore_device_info->zone.console_size + pstore_device_info->zone.ftrace_size));
    }
#endif
...
#if IS_ENABLED(CONFIG_PSTORE_RAM)
	if (pos >= pstore_device_info->zone.pmsg_size + pstore_device_info->zone.console_size + pstore_device_info->zone.ftrace_size){
        return ramoops_blk_write(buf, bytes, pos - (pstore_device_info->zone.pmsg_size + pstore_device_info->zone.console_size + pstore_device_info->zone.ftrace_size));
    }
#endif
...
{
	int ret = -ENODEV;
    long ram_dump_size = 0;
    long ram_dump_count = 0;
...
#if IS_ENABLED(CONFIG_PSTORE_RAM)
	dev->zone.panic_write = psblk_generic_blk_write;
    ramoops_get_dump_size_count(&ram_dump_size, &ram_dump_count);
    kmsg_size = ram_dump_size/SZ_1K;
	dev->zone.total_size = pmsg_size > 0 ? pmsg_size * SZ_1K : 0;
    dev->zone.total_size += ftrace_size > 0 ? ftrace_size * SZ_1K : 0;
    dev->zone.total_size += console_size > 0 ? console_size * SZ_1K : 0;
    dev->zone.total_size += ram_dump_size * ram_dump_count;
#else
    dev->zone.total_size = i_size_read(psblk_file->f_mapping->host);
#endif
	ret = __register_pstore_device(dev);
	if (ret)
		goto err_fput;
...
static __init const char *early_boot_devpath(const char *initial_devname)
{
    ...
    dev_t dev;
again:
	dev = name_to_dev_t(initial_devname);
	if (!dev) {
	...
	sys_unlink(devname);
	sys_mknod(devname, S_IFBLK | 0600, new_encode_dev(dev));
