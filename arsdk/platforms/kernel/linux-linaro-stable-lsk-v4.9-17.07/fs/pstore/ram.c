...
module_param_named(mem_address, mem_address, ullong, 0400);
...
struct ramoops_context {
	...
	struct pstore_device_info pdev;
};
...
int ramoops_get_dump_size_count(long *dump_size, long *dump_count)
{
	struct ramoops_context *ctx = &oops_cxt;
    *dump_size = ctx->record_size;
    *dump_count = ctx->max_dump_cnt;

	return 0;
}
EXPORT_SYMBOL_GPL(ramoops_get_dump_size_count);

ssize_t ramoops_blk_read(char *buf, size_t bytes, loff_t pos)
{
	struct persistent_ram_zone *prz = NULL;
	struct ramoops_context *ctx = &oops_cxt;
    int prz_id = pos/ctx->record_size;
	if (prz_id > ctx->max_dump_cnt-1)
	    return -EINVAL;

	prz = ctx->dprzs[prz_id];

	memcpy(buf, (char *)prz->vaddr, bytes);

	return bytes;
}
EXPORT_SYMBOL_GPL(ramoops_blk_read);

ssize_t ramoops_blk_write(const char *buf, size_t bytes,
		loff_t pos)
{
	struct ramoops_context *ctx = &oops_cxt;
	struct persistent_ram_zone *prz = NULL;
	...
	memcpy((char *)prz->vaddr, buf, bytes);

	return bytes;
}
EXPORT_SYMBOL_GPL(ramoops_blk_write);
...
#undef parse_u32
	...
	if (!strcmp(parent_node->name, "reserved-memory") &&
...
static int ramoops_probe(struct platform_device *pdev)
{
	...
	pr_info("pstore ram and blk enabled at the same time\n");
#else
