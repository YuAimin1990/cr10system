...
static int psz_kmsg_recover_meta(struct psz_context *cxt)
{
	...
	for (i = 0; i < cxt->kmsg_max_cnt; i++) {
		...
		if (hdr->magic != PSTORE_KMSG_HEADER_MAGIC) {
			pr_debug("found invalid zone: %s: id %lu, off %lld, size %zu, sig %llx-%llx\n",
					zone->name, i, zone->off,
					zone->buffer_size, buf->sig, zone->buffer->sig);
			continue;
		}
...
static int psz_kmsg_recover(struct psz_context *cxt)
{
	...
	if (ret) {
		pr_info("psz_kmsg_recover_meta ret %d\n", ret);
		goto recover_fail;
    }

	ret = psz_kmsg_recover_data(cxt);
	if (ret) {
		pr_info("psz_kmsg_recover_data ret %d\n", ret);
		goto recover_fail;
    }
		...
		struct pstore_record *record)
{
	...
	if (unlikely(rem < cnt)) {
		ret = psz_zone_write(zone, FLUSH_PART, buf, rem, start);
        if (ret != 0) {
            return ret;
        }
		buf += rem;
	...
	ret = psz_zone_write(zone, FLUSH_PART, buf, cnt, start);
    if (ret != 0) {
        return ret;
    }
	...
	if (is_full_data) {
		...
		ret = psz_zone_write(zone, FLUSH_META, NULL, 0, 0);
        if (ret != 0) {
            return ret;
        }
	}
