...
#if defined(RPC_FS_PATCH)
	return fuse_simple_request(fc->main_fc, &args);
#else
		   ...
		   unsigned int flags)
{
    ...
    struct fuse_ioctl_ext* ctl = NULL;
#endif
...
#if defined(RPC_FS_PATCH)
    iov = iov_page;
    ctl = (struct fuse_ioctl_ext*)arg;

    // restricted cases
    // 1. 64bit .ko + 64bit kernel + 32bit libfuse.so
    if (flags & FUSE_IOCTL_RESTRICTED_COMPAT) {
        iov->iov_base = (void __user *)arg;
        iov->iov_len = sizeof(struct fuse_ioctl_ext);
        (iov + 1)->iov_base = (void __user *)ctl->buf;
        (iov + 1)->iov_len = ctl->len;

        in_iov = iov;
        in_iovs = 2;
        out_iov = iov;
        out_iovs = 2;
    // 2. 32bit app + 64bit kernel + 32bit libfuse.so
    } else if (flags & FUSE_IOCTL_UNRESTRICTED) {
        iov->iov_base = (void __user *)arg;
        iov->iov_len = sizeof(struct fuse_ioctl_ext);

        in_iov = iov;
        in_iovs = 1;
    // 3. 64bit .ko/app + 64bit kernel + 64bit libfuse.so
    } else if (flags & FUSE_IOCTL_RESTRICTED) {
        iov->iov_base = (void __user *)arg;
        iov->iov_len = sizeof(struct fuse_ioctl_ext);
        (iov + 1)->iov_base = (void __user *)ctl->buf;
        (iov + 1)->iov_len = ctl->len;
 ...
 out:
    ...
    // copy ioctl internal return value
#if defined(RPC_FS_PATCH)
    if (outarg.result < 0) {
        int ret = 0;
        struct fuse_ioctl_ext ctrl;
        ctrl.ret = outarg.result;
        outarg.result = -EINVAL;
        ret = copy_to_user((void *)(uintptr_t)arg, &ctrl, sizeof(struct fuse_ioctl_ext));
        if (ret) {
            outarg.result = -ENOMEM;
        }
    }
#endif
