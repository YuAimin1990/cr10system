...
#include <linux/artosyn-pinshare.h>
#include <linux/gpio/consumer.h>
#include <linux/dma-mapping.h>
#include "artosyn-qspi.h"
#include <linux/artosyn-mcp.h>
#include "ar_nor_dma.h"
...
#define ARTOSYN_QSPI_CMD_FAST        0xeb

/* Read/Write regs, for Spansion 4k disable */
#define ARTOSYN_CMD_READ_ANY_REG_SPANSION    (0x65)
#define ARTOSYN_CMD_WRITE_ANY_REG_SPANSION   (0x71)

//Artosyn qspi controller registers
...
#define ARTOSYN_QSPI_ERR(fmt, ...) \
	printk(KERN_ERR"[ARTOSYN QSPI][err] %s %d, " fmt, __func__, __LINE__, ##__VA_ARGS__)

#define ARTOSYN_QSPI_DBG(fmt, ...) \
	printk(KERN_DEBUG"[ARTOSYN QSPI][dbg] %s %d, " fmt, __func__, __LINE__, ##__VA_ARGS__)


void art_nor_dma_write(struct art_nor_host *host, unsigned int value,unsigned int reg)
{
	writel(value, (void __iomem *)((unsigned int)host->nor_dma_base + reg));
}
unsigned int art_nor_dma_read(struct art_nor_host *host, unsigned int reg)
{
	return readl((void __iomem *)((unsigned int)host->nor_dma_base + reg));
}
void art_nor_dma_lowbit_write(struct art_nor_host *host, unsigned int value)
{
	writel(value, (void __iomem *)((unsigned int)host->nor_dma_base_low));
}
unsigned int art_nor_dma_lowbit_read(struct art_nor_host *host)
{
	return readl((void __iomem *)((unsigned int)host->nor_dma_base_low));
}


static inline uint32_t artosyn_qspi_readl(struct artosyn_qspi *aq, uint32_t reg)
{
	return readl(aq->regs + reg);
}

static inline void artosyn_qspi_writel(struct artosyn_qspi *aq, uint32_t reg, uint32_t val)
{
	writel(val, aq->regs + reg);
}

static inline uint16_t artosyn_qspi_readw(struct artosyn_qspi *aq, uint32_t reg)
{
	return readw(aq->regs + reg);
}

static inline void artosyn_qspi_writew(struct artosyn_qspi *aq, uint32_t reg, uint16_t val)
{
	writew(val, aq->regs + reg);
}

static inline uint8_t artosyn_qspi_readb(struct artosyn_qspi *aq, uint32_t reg)
{
	return readb(aq->regs + reg);
}

static inline void artosyn_qspi_writeb(struct artosyn_qspi *aq, uint32_t reg, uint8_t val)
{
...
{
	struct artosyn_qspi *aq = nor->priv;
	unsigned int tmp, busy;
	//reconstruct read status register command - 05h
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(12), 0x710);
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(12), 0x0028);
	artosyn_qspi_writel(aq, QSPI_CMD_ADDR_REG(12), (0xC0<<16) | 0xFFFF);

	do{
		tmp = artosyn_qspi_readl(aq, QSPI_CMD_REG(12)); //active 05h
		busy = tmp & 0x1;
	}while(busy);
}
...
static void nor_select_chip(struct spi_nor *nor, int chip)
{
	...
	struct artosyn_qspi *aq = nor->priv;
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(31), 0x700);
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(31), 0x2610);

	switch (chip) {
		case 0:
			/* TODO: choose the NOR Flash(DIE-0) */
			artosyn_qspi_writel(aq, QSPI_CMD_REG(31), 0x0);
			check_busy(nor);
			break;
		case 1:
			/* TODO: choose the NOR Flash(DIE-1) */
			artosyn_qspi_writel(aq, QSPI_CMD_REG(31), 0x1);
			check_busy(nor);
			break;
		default:
			break;
...
static int artosyn_qspi_read_reg(struct spi_nor *nor, u8 opcode,
			       u8 *buf, int len)
{
	struct artosyn_qspi *aq = nor->priv;
	...
	case SPINOR_OP_RDID:
		...
		break;
	default:
		ARTOSYN_QSPI_ERR("Unsupported cmd 0x%x\n", opcode);
		BUG();
	}

	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(0), instr_lw);
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(0), instr_hw);

	if(len == 1) {
		*(uint8_t *)&tmp = artosyn_qspi_readb(aq, QSPI_CMD_REG(0));
	}
	else if(len <= 2) {
		*(uint16_t *)&tmp = artosyn_qspi_readw(aq, QSPI_CMD_REG(0));
	}
	else if(len <= 4) {
		*(uint32_t *)&tmp = artosyn_qspi_readl(aq, QSPI_CMD_REG(0));
	}
	else if(len <= 8) {
...
static int artosyn_qspi_write_reg(struct spi_nor *nor, u8 opcode,
				u8 *buf, int len)
{
	struct artosyn_qspi *aq = nor->priv;
	uint32_t instr_lw, instr_hw;
	...
	if(!buf) {
		artosyn_qspi_writel(aq, QSPI_CMD_REG(0), 0x1);
	}
	else{
		if(len <= 4)
			artosyn_qspi_writel(aq, QSPI_CMD_REG(0), tmp);
		else
			*(unsigned long long *)(aq->regs +  QSPI_CMD_REG(0)) = tmp;

		/* Read status */
		while(1) {
			artosyn_qspi_read_reg(nor, SPINOR_OP_RDSR, &status, 1);
			status &= SR_WIP;
			if(status == 0)
				break;
			udelay(100);
...
static ssize_t _artosyn_qspi_write_3B(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	...
	pinfunc = artosyn_get_pinshare(PIN_NO(aq->pin_no));
	...
	while (tmplen1) {
		...
		/* Write enable */
		artosyn_qspi_write_reg(nor, SPINOR_OP_WREN, NULL, 0);

		udelay(1);

		/* Config qspi cs as gpio */
		artosyn_set_pinshare(PIN_NO(aq->pin_no), aq->pin_func);
		...
		*(uint8_t*)(aq->mem + to) = *((uint8_t*)write_buf);
		...
		for(i = 0; i < tmplen2; i++) {
			...
			/* Send one byte */
			artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(31), instr_lw);
			artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(31), instr_hw);
			artosyn_qspi_writel(aq, QSPI_CMD_REG(31), 0x1);
			write_buf++;
		}
		...
		/* Restore pinshare */
		artosyn_set_pinshare(PIN_NO(aq->pin_no), pinfunc);
...
static int artosyn_qspi_write_3B(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	unsigned int ret;
	...
	if ((to+len) <= NOR_FLASH_32M){
		ret1 = _artosyn_qspi_write_3B(nor,to,len,write_buf);
		ret = ret1;
	} else if ((to < NOR_FLASH_32M)&&((to+len)> NOR_FLASH_32M)) {
		len1 = NOR_FLASH_32M - to;
		ret2_1 = _artosyn_qspi_write_3B(nor, to, len1, write_buf);
		write_buf += len1;

		to2 = NOR_FLASH_32M;
		len2= len - len1;
		nor_select_chip(nor, 1);
		ret2_2 = _artosyn_qspi_write_3B(nor, to2, len2, write_buf);
		nor_select_chip(nor, 0);
		ret = (ret2_1+ ret2_2);
	} else if(to >= NOR_FLASH_32M) {
		to = to - NOR_FLASH_32M;
		nor_select_chip(nor, 1);
		ret3 = _artosyn_qspi_write_3B(nor, to, len, write_buf);
		nor_select_chip(nor, 0);
		ret = ret3;
	}
...
static ssize_t _artosyn_qspi_write_4B(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	ssize_t ret;
	int i, tmplen1, tmplen2;
...
static int artosyn_qspi_write_4B(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	unsigned int ret;
	unsigned int ret1 = 0, ret2_1 = 0, ret2_2 = 0, ret3 = 0;
	...
	if ((to+len) <= NOR_FLASH_32M){
		ret1 = _artosyn_qspi_write_4B(nor,to,len,write_buf);
		ret = ret1;
	} else if ((to < NOR_FLASH_32M)&&((to+len)> NOR_FLASH_32M)){
		len1 = NOR_FLASH_32M - to;
		ret2_1 = _artosyn_qspi_write_4B(nor, to, len1, write_buf);
		write_buf += len1;

		to2 = NOR_FLASH_32M;
		len2= len - len1;
		nor_select_chip(nor, 1);
		ret2_2 = _artosyn_qspi_write_4B(nor, to2, len2, write_buf);
		nor_select_chip(nor, 0);
		ret = (ret2_1+ ret2_2);
	} else if(to >= NOR_FLASH_32M) {
		to = to - NOR_FLASH_32M;
		nor_select_chip(nor, 1);
		ret3 = _artosyn_qspi_write_4B(nor, to, len, write_buf);
		nor_select_chip(nor, 0);
		ret = ret3;
	}
...
static ssize_t _artosyn_qspi_write(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	struct artosyn_qspi *aq = nor->priv;
	ssize_t ret;
...
static int artosyn_qspi_write(struct spi_nor *nor, loff_t to, size_t len,
				const u_char *write_buf)
{
	unsigned int ret1 = 0, ret2_1 = 0, ret2_2 = 0, ret3 = 0;
	unsigned int to2 = 0, len1 = 0, len2 = 0;
	...
	if ((to+len) <= NOR_FLASH_32M){
		ret1 = _artosyn_qspi_write(nor,to,len,write_buf);
		ret = ret1;
	} else if ((to < NOR_FLASH_32M)&&((to+len)> NOR_FLASH_32M)){
		len1 = NOR_FLASH_32M - to;
		ret2_1 = _artosyn_qspi_write(nor, to, len1, write_buf);
		write_buf += len1;

		to2 = NOR_FLASH_32M;
		len2= len - len1;
		nor_select_chip(nor, 1);
		ret2_2 = _artosyn_qspi_write(nor, to2, len2, write_buf);
		nor_select_chip(nor, 0);
		ret = (ret2_1+ ret2_2);
	} else if(to >= NOR_FLASH_32M){
		to = to - NOR_FLASH_32M;
		nor_select_chip(nor, 1);
		ret3 = _artosyn_qspi_write(nor, to, len, write_buf);
		nor_select_chip(nor, 0);
		ret = ret3;
	}
...
static ssize_t artosyn_qspi_dma_read(struct spi_nor *nor, loff_t from, size_t len, u_char *read_buf)
{
	struct artosyn_qspi *aq = nor->priv;
	struct art_nor_host *host= aq->host;
	int ret,i=0;
	...
	while(len) {
		...
		*(unsigned int *)((u_char *)host->temp_buf + trans_size - 4)= ~check_buf;
		...
		ret = ar_ahb_dma_m2m(host, (u_char *)aq->map_phys + from,
...
static ssize_t artosyn_qspi_read(struct spi_nor *nor, loff_t from, size_t len,
			       u_char *read_buf)
{
	struct artosyn_qspi *aq = nor->priv;
	int i;
...
static ssize_t _artosyn_qspi_dma_cpu_read(struct spi_nor *nor, loff_t from, size_t len,
			       u_char *read_buf)
{
	struct artosyn_qspi *aq = nor->priv;
	size_t len1 = len;
	...
	if(len) {
		left = len % 4;
		len -= left;
		artosyn_qspi_dma_read(nor, from, len, read_buf);
		from += len;
		read_buf += len;
		while(left-- > 0) {
			*read_buf++ = *((u_char *)aq->mem + from);
...
static ssize_t artosyn_qspi_dma_cpu_read(struct spi_nor *nor, loff_t from, size_t len, u_char *read_buf)
{
	...
	if ((from + len) <= NOR_FLASH_32M){
		ret1 = _artosyn_qspi_dma_cpu_read(nor, from, len, read_buf);
		ret = ret1;
	}

	if ((from < NOR_FLASH_32M)&&((from + len)> NOR_FLASH_32M)){
		len1 = NOR_FLASH_32M - from;
		ret2_1 = _artosyn_qspi_dma_cpu_read(nor, from, len1, read_buf);
		read_buf += len1;

		from2 = NOR_FLASH_32M;
		len2= len - len1;
		nor_select_chip(nor, 1);
		ret2_2 = _artosyn_qspi_dma_cpu_read(nor, from2, len2, read_buf);
		nor_select_chip(nor, 0);
		ret = (ret2_1+ ret2_2);
	}
	...
	if(from >= NOR_FLASH_32M){
		...
		nor_select_chip(nor, 1);
		ret3 = _artosyn_qspi_dma_cpu_read(nor, from, len, read_buf);
		nor_select_chip(nor, 0);
		ret = ret3;
	}
...
/* Single mode only */
static int _artosyn_qspi_erase(struct spi_nor *nor, loff_t offs)
{
	struct artosyn_qspi *aq = nor->priv;
	uint32_t instr_lw, instr_hw;
	uint8_t status;
	...
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(0), instr_hw);
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(0), instr_lw);
	artosyn_qspi_writel(aq, QSPI_CMD_ADDR_REG(0), offs);
	artosyn_qspi_writel(aq, QSPI_CMD_REG(0), 0x1);
...
static int artosyn_qspi_erase(struct spi_nor *nor, loff_t offs)
{
	...
	if (offs < NOR_FLASH_32M){
		ret1 = _artosyn_qspi_erase(nor,offs);
		ret = ret1;
	}
	...
	if(offs >= NOR_FLASH_32M){
		...
		nor_select_chip(nor, 1);
		ret2 = _artosyn_qspi_erase(nor, offs);
		nor_select_chip(nor, 0);
		ret =ret2;
	}
...
static int artosyn_qspi_set_clk(struct artosyn_qspi *aq, int clock)
{
	...
	val = artosyn_qspi_readl(aq, QSPI_CFG_REG);
	val = (val & ~CLOCK_SPEED_MASK) | setting;
	artosyn_qspi_writel(aq, QSPI_CFG_REG, val);
	return 0;
}

static int artosyn_qspi_init(struct artosyn_qspi *aq)
{
	unsigned int val = 0;
	unsigned char clock = 0;

	ARTOSYN_QSPI_DBG("qspi ctrl init: registers=%x,mem=%x,clock %d\n",
		aq->regs,aq->mem, aq->clk_rate);


	artosyn_set_pinshare(PIN_NO(76), PIN_FUNC_QSPI_SCK);
	artosyn_set_pinshare(PIN_NO(77), PIN_FUNC_QSPI_CS);
	artosyn_set_pinshare(PIN_NO(80), PIN_FUNC_QSPI_DATA_0);
	artosyn_set_pinshare(PIN_NO(81), PIN_FUNC_QSPI_DATA_1);
	artosyn_set_pinshare(PIN_NO(82), PIN_FUNC_QSPI_DATA_2);
	artosyn_set_pinshare(PIN_NO(83), PIN_FUNC_QSPI_DATA_3);

	artosyn_qspi_writel(aq, QSPI_INIT_REG, 0x1);
	artosyn_qspi_writel(aq, QSPI_ACCESS_EN_REG, 0xacce55);

	/* Use 25M as default clk to read flash id */
	artosyn_qspi_set_clk(aq, 25000000);
...
static void artosyn_qspi_set_4b(struct spi_nor *nor, int enable)
{
	struct artosyn_qspi *aq = nor->priv;

	if(enable)
		artosyn_qspi_write_reg(nor, SPINOR_OP_EN4B, NULL, 0);
	else
		artosyn_qspi_write_reg(nor, SPINOR_OP_EX4B, NULL, 0);
}

#pragma GCC push_options
#pragma GCC optimize ("O0")
static void artosyn_qspi_disable_4k(struct spi_nor *nor)
{
	struct artosyn_qspi *aq = nor->priv;
	...
	instr_hw = HW_BUS_WIDTH_RESERVED |
			   ...
			   HW_DIR_WRITE |
			   HW_CMD_BIT_WIDTH_1_BIT |
			   ARTOSYN_CMD_WRITE_ANY_REG_SPANSION << HW_CMD_SHIFT;
	...
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(0), instr_hw);
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(0), instr_lw);
	artosyn_qspi_writel(aq, QSPI_CMD_ADDR_REG(0), 0x4);
	artosyn_qspi_writel(aq, QSPI_CMD_REG(0), val);
...
static void artosyn_qspi_drive_enhance_winbond(struct spi_nor *nor)
{
...
#define WINB_CMD_WR_REG3 0x11
	...
	instr_lw = 0;
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_L(31), instr_lw); //CMD 01h
	artosyn_qspi_writel(aq, QSPI_CMD_INIT_REG_H(31), instr_hw); //CMD 01h
	artosyn_qspi_writel(aq, QSPI_CMD_REG(31), 0x1);
...
static int artosyn_qspi_prepare(struct spi_nor *nor, enum spi_nor_ops ops)
{
	mcp_mutex_get();
	return 0;
}

static void artosyn_qspi_unprepare(struct spi_nor *nor, enum spi_nor_ops ops)
{
	mcp_mutex_release();
}
#pragma GCC pop_options

static int artosyn_qspi_probe(struct platform_device *pdev)
{
	...
	if(!host->dma_llp)
		host->dma_llp = dmam_alloc_coherent(nor->dev, sizeof(struct ahb_dma_llp) * AHB_DMA_MAX_LLP_ND, &host->dma_buffer, GFP_KERNEL);
	if(!host->dma_llp)
		return -ENOMEM;
	...
	nor->priv = aq;
	mtd->priv = nor;
	nor->read_reg  = artosyn_qspi_read_reg;
	nor->write_reg = artosyn_qspi_write_reg;
	nor->write     = artosyn_qspi_write;
	nor->erase     = artosyn_qspi_erase;
	nor->read      = artosyn_qspi_dma_cpu_read;
	nor->prepare = artosyn_qspi_prepare;
	nor->unprepare = artosyn_qspi_unprepare;

	mcp_mutex_get();
	err = artosyn_qspi_init(aq);
	if (err)
		goto exit;

	for(err = 0; err < 30; err++)
		ARTOSYN_QSPI_DBG("%d %x %x\n", err,
				artosyn_qspi_readl(aq,QSPI_CMD_INIT_REG_H(err)),
				artosyn_qspi_readl(aq,QSPI_CMD_INIT_REG_L(err)));

	/* Read id */
	artosyn_qspi_read_reg(nor, SPINOR_OP_RDID, aq->id, 8);
	...
	if(addr_cnt == LW_ADDRESS_CNT_3B)
		nor->write = artosyn_qspi_write_3B;
	else
		nor->write = artosyn_qspi_write_4B;
#endif

	/* Configure read registers, read 1 byte for each cycle */
	if(nor->flash_read == SPI_NOR_QUAD) {
		nor->read_opcode = ARTOSYN_QSPI_CMD_FAST;
		...
		artosyn_qspi_writel(aq, QSPI_RD_LW_REG, instr_lw);
		artosyn_qspi_writel(aq, QSPI_RD_HW_REG, instr_hw);
		artosyn_qspi_writel(aq, QSPI_UPDATE_RD_REG, 0x1);
	}
	else {
	...
	if(nor->program_opcode == SPINOR_OP_BP || SPINOR_OP_PP_4B) {
		...
		artosyn_qspi_writel(aq, QSPI_WR_LW_REG, instr_lw);
		artosyn_qspi_writel(aq, QSPI_WR_HW_REG, instr_hw);
		artosyn_qspi_writel(aq, QSPI_UPDATE_WR_REG, 0x1);
	}
	else {
		BUG();
	}
	...
	if(nor->addr_width == 4) {
		artosyn_qspi_set_4b(nor, 1);
	}
	else {
		artosyn_qspi_set_4b(nor, 0);
	}

	/* Disable 4k sector for Spansion */
	if(aq->id[0] == SNOR_MFR_SPANSION) {
		artosyn_qspi_disable_4k(nor);
	}

	if (aq->id[0] == SNOR_MFR_WINBOND) {
		artosyn_qspi_drive_enhance_winbond(nor);
	}
	...
	if (of_property_read_u32(np, "pinno", &pinno)) {
		ARTOSYN_QSPI_ERR("No pin no in dts\n");
	...
	ARTOSYN_QSPI_ERR("qspi pinshare pinno = %d\n", pinno);

	if (of_property_read_u32(np, "pinfunc", &pinfunc)) {
		ARTOSYN_QSPI_ERR("No pin func in dts\n");
		mcp_mutex_release();
		return -EINVAL;
	}
	...
	ARTOSYN_QSPI_ERR("qspi pinshare pinfunc = %d\n", pinfunc);

	gpio = of_get_named_gpio_flags(np, "cs-gpios", 0, &flags);

	ARTOSYN_QSPI_ERR("qspi gpio = %d, %d\n", gpio, flags);
	...
	artosyn_qspi_set_clk(aq, aq->clk_rate);
	...
	for_each_available_child_of_node(np, node) {
		if (of_property_read_string(node, "part-name", &part_name)) {
			ARTOSYN_QSPI_DBG("No mtd partition name\n");
			part_name= NULL;
		}
		...
		if(part_offs + part_size > mtd->size) {
			ARTOSYN_QSPI_ERR("Mtd part %s exceed flash size\n");
			return err;
		}

		err = mtd_add_partition(mtd, part_name, part_offs, part_size);
		if(err < 0) {
			ARTOSYN_QSPI_ERR("Add mtd part %s fail\n");
			goto exit;
		}
		part_offs += part_size;
		part_cnt--;
...
static int artosyn_qspi_remove(struct platform_device *pdev)
{
...
static const struct of_device_id artosyn_qspi_dt_ids[] = {
	{ .compatible = "artosyn,sirius-qspi" },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, artosyn_qspi_dt_ids);

static struct platform_driver artosyn_qspi_driver = {
	.driver = {
		.name	= "artosyn,sirius-qspi",
		.of_match_table	= artosyn_qspi_dt_ids,
	},
	.probe		= artosyn_qspi_probe,
	.remove		= artosyn_qspi_remove,
};

module_platform_driver(artosyn_qspi_driver);

MODULE_AUTHOR("Artosyn");
MODULE_DESCRIPTION("Artosyn QSPI Controller driver");
