				      ...
				      struct axi_dma_desc *first)
{
	...
	reg = (DWAXIDMAC_TT_FC_MEM_TO_MEM_DMAC << CH_CFG_H_TT_FC_POS |
	       ...
	       DWAXIDMAC_OSR_LEN_16 << CH_CFG_H_SRC_OSR_POS |
	       DWAXIDMAC_OSR_LEN_16 << CH_CFG_H_DTS_OSR_POS);
			 ...
			 dma_addr_t src_adr, size_t len, unsigned long flags)
{
	...
	while (len) {
		...
		//printk(KERN_INFO "xfer_width: %d src_adr %p dst_adr %p xfer_len %d\n", 
		//		xfer_width, src_adr, dst_adr, xfer_len);
...
#ifdef __USING_AR9301_PLATFORM_DMA__
...
static int dw_probe(struct platform_device *pdev)
{
	...
	if(irq_count > DMAC_MAX_CHANNELS)
		-ENOMEM;

	printk(KERN_INFO "AXI DMA get %d interrupts\n", irq_count);
	...
	for(i = 0; i < irq_count; ++i)
	{
		...
		if (chip->irq[i] < 0)
			return chip->irq[i];
		printk(KERN_INFO "AXI DMA get irq[%d]: %d \n", i, chip->irq[i]);
	}
#else
...
#ifdef __USING_AR9301_PLATFORM_DMA__
	if(hdata->nr_channels != irq_count)
		return -ENOMEM;
