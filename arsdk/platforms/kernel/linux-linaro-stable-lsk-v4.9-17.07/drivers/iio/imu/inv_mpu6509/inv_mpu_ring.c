...
*/
	...
	result = regmap_write(st->map, st->reg->fifo_config, INV_MPU6509_FIFO_MODE_STREAM_TO_FIFO << INV_MPU6509_BIT_OFFSET_FIFO_MODE);
	if (result)
		goto reset_fifo_fail;
...
int inv_mpu6509_update_ref_tmst(struct inv_mpu6509_state *st, s64* timestamp)
{
	...
	if (!result) {
		*timestamp = ktime_get_raw_ns();
		pr_debug("after %lld %lld fifo_count_l=%d fifo_count_h=%d", *timestamp, *timestamp - timestamp1, fifo_count_l, fifo_count_h);
	} else {
