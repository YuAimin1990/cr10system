 ...
 * 18. 10. 2017: Robin Bai (haibin.bai@artosyn.cn)
 *   Modify it to use DTS configuration and seperate the MDIO driver to ../../phy/mdio-sirius.c
 *
 * Originally based on:
 *
...
//hbbai: for debug purpose, use random mac in order to avoid mac conflict
#undef ETHERNET_DEBUG_RANDOM_MAC_ADDR
...
struct oeth_private {
	...
	int phy_rst_gpio;
	struct oeth_regs_tag gmac_regs[15];
...
#define areth_read(lp, reg)						\
	readl_relaxed(((void __iomem *)((lp)->base_addr)) + (reg))
#define areth_write(lp, reg, val)					\
	writel_relaxed((val), ((void __iomem *)((lp)->base_addr)) + (reg))

static int ar_debug_level = -1;
module_param(ar_debug_level, int, 0);
MODULE_PARM_DESC(ar_debug_level, "ar_eth debug level (0=none,...,16=all)");

#define AR_DEFAULT_MSG_ENABLE	(NETIF_MSG_DRV | \
				 NETIF_MSG_PROBE | \
				 NETIF_MSG_LINK | \
				 NETIF_MSG_TIMER | \
				 NETIF_MSG_IFDOWN | \
...
static int oeth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (0 == atomic_read(&cep->tx_free)) {
		netif_stop_queue(dev);
		netdev_err(cep->dev, "NETDEV_TX_BUSY\n");
...
static void oeth_rx_unalign(struct net_device *dev, int pkt_len)
{
	...
	if (likely(skb_alloc)) {
		...
		dma_addr_t new_skb_baddr  =	dma_map_single(dev->dev.parent,
					skb_alloc->data,
		...
		skb_put(skb_alloc, pkt_len - ETH_FCS_LEN);
 ...
 * phy software reset
*/
static void oeth_phy_software_reset(struct net_device * ndev)
{
	unsigned int reg = 0;
	struct oeth_private *cep = oeth_get_private(ndev);

	reg =  areth_read(cep, SIRIUS_ETH_RST_CTRL);
	areth_write(cep, SIRIUS_ETH_RST_CTRL, reg & 0xfffffffe);

	//realtek RTL8201 must be asserted low for at least 10ms.
	//mdelay(15);

	areth_write(cep, SIRIUS_ETH_RST_CTRL, reg | 0x1);
}
							...
							bool flag_rx_enhance)
{
	...
	tx_reg =  areth_read(cep, SIRIUS_ETH_TX_BRIDGE);
	rx_reg =  areth_read(cep, SIRIUS_ETH_RX_BRIDGE);

	if (flag_tx_enhance)
		areth_write(cep, SIRIUS_ETH_TX_BRIDGE, tx_reg | 0x1);
	else
		areth_write(cep, SIRIUS_ETH_TX_BRIDGE, tx_reg & 0xfffffffe);

	if (flag_rx_enhance)
		areth_write(cep, SIRIUS_ETH_RX_BRIDGE, rx_reg | 0x1);
	else
		areth_write(cep, SIRIUS_ETH_RX_BRIDGE, rx_reg & 0xfffffffe);
}
...
int oeth_probe(struct platform_device *pdev)
{
	...
	if(cep->phy_interface < 0)
		cep->phy_interface = PHY_INTERFACE_MODE_RGMII;

	if(cep->phy_interface == PHY_INTERFACE_MODE_RMII) { 
		st = areth_read(cep, SIRIUS_ETH_RMII_CTRL);
		areth_write(cep, SIRIUS_ETH_RMII_CTRL, st|(SIRIUS_ETH_RMII_SEL)); 
	} 

	cep->msg_enable = netif_msg_init(ar_debug_level, AR_DEFAULT_MSG_ENABLE);
	...
	areth_write(cep, SIRIUS_ETH_PWR_CTRL, 0xffffff00);
	oeth_phy_software_reset(ndev);
	...
	(cep->gmac_regs)[ 0].reg_ind = OETH_MODER;
	(cep->gmac_regs)[ 1].reg_ind = OETH_IPGT;
	(cep->gmac_regs)[ 2].reg_ind = OETH_PACKETLEN;
	(cep->gmac_regs)[ 3].reg_ind = OETH_COLLCONF;
	(cep->gmac_regs)[ 4].reg_ind = OETH_TX_BD_NUM;
	(cep->gmac_regs)[ 5].reg_ind = OETH_CTRLMODER;
	(cep->gmac_regs)[ 6].reg_ind = OETH_MIIMODER;
	(cep->gmac_regs)[ 7].reg_ind = OETH_MAC_ADDR0;
	(cep->gmac_regs)[ 8].reg_ind = OETH_MAC_ADDR1;
	(cep->gmac_regs)[ 9].reg_ind = OETH_HASH0;
	(cep->gmac_regs)[10].reg_ind = OETH_HASH1;
	(cep->gmac_regs)[11].reg_ind = OETH_SPEED_SEL;
	(cep->gmac_regs)[12].reg_ind = OETH_INT_MASK2;
	(cep->gmac_regs)[13].reg_ind = OETH_INT_MASK3;
	(cep->gmac_regs)[14].reg_ind = OETH_INT_MASK;
...
static int oeth_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	for (i = 0; i < (sizeof(cep->gmac_regs) / sizeof(struct oeth_regs_tag)); i++)
		cep->gmac_regs[i].reg_val = readl(cep->base_addr + cep->gmac_regs[i].reg_ind);

	oeth_set_int_mask(cep->base_addr, 0);
	oeth_clear_moder(cep->base_addr, OETH_MODER_RXEN | OETH_MODER_TXEN);
	oeth_flush_tx_rx(ndev);
	oeth_free_buf(ndev);
	if(ndev->phydev) {
...
static int oeth_resume(struct platform_device *pdev)
{
	...
	// hardware register re-init
	if(cep->phy_interface == PHY_INTERFACE_MODE_RMII) {
		val = areth_read(cep, SIRIUS_ETH_RMII_CTRL);
		areth_write(cep, SIRIUS_ETH_RMII_CTRL, val | (SIRIUS_ETH_RMII_SEL));
	}
	areth_write(cep, SIRIUS_ETH_PWR_CTRL, 0xffffff00);
	oeth_phy_software_reset(ndev);
	oeth_set_bridge(ndev, cep->tx_enhance, cep->rx_enhance);
	...
	for (i = 0; i < (sizeof(cep->gmac_regs) / sizeof(struct oeth_regs_tag)); i++)
		writel(
			cep->gmac_regs[i].reg_val,
			cep->base_addr + cep->gmac_regs[i].reg_ind
		);

	request_irq(ndev->irq, oeth_interrupt, 0, "eth", ndev);

	if(ndev->phydev) {
		if (! phy_start_aneg(ndev->phydev))
...
MODULE_DESCRIPTION("Artosyn Sirius GMAC driver based on beth");
MODULE_LICENSE("GPL");
