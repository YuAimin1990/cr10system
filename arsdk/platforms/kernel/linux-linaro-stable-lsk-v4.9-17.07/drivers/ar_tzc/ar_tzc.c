...
u32 tzc400_print_intr_reason(u32 tzc_idx, int filter)
{
	u32 int_status;
	u32 fail_addr_l;
	u32 fail_addr_h;
...
static irqreturn_t tzc_irq_handler(int irq, void *arg)
{
	...
	tzc400_print_intr_reason(AR_TZC_400_4F, GMAC_FILTER_IDX );

	tzc_readl(AR_TZC_400_4F, TZ_INT_STATUS, int_status);
	tzc_writel(AR_TZC_400_4F, int_status, TZ_INT_CLEAR);

	return IRQ_HANDLED;
}


static int artosyn_tzc_probe(struct platform_device *pdev)
{
	int ret = 0;
...
static const struct of_device_id artosyn_tzc_match_table[] = {
	{.compatible = "artosyn,tzc-400"},
	{},
};


static int artosyn_tzc_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver artosyn_tzc_driver = {
	.probe = artosyn_tzc_probe,
	.remove = artosyn_tzc_remove,
	.driver = {
		.name = "tzc-artosyn",
		.of_match_table = artosyn_tzc_match_table,
	},
};

static int __init artosyn_tzc_init(void)
{
	return platform_driver_register(&artosyn_tzc_driver);
}

static void __exit artosyn_tzc_exit(void)
{
	platform_driver_unregister(&artosyn_tzc_driver);
}

MODULE_AUTHOR("artosyn");
MODULE_DESCRIPTION("artosyn tzc common driver");
MODULE_LICENSE("GPL");

subsys_initcall(artosyn_tzc_init);
module_exit(artosyn_tzc_exit);
