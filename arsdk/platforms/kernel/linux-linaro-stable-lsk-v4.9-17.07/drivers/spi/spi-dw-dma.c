...
static irqreturn_t dma_transfer(struct dw_spi *dws)
{
	...
	printk(KERN_EMERG "%s: FIFO overrun/underrun, irq_status %d, txlen %d, rxlen %d, tx[0] %d, tx[1] %d, tx[2] %d, tx[3] %d\n"
		...
		struct spi_transfer *xfer)
{
	...
	if (!xfer->tx_buf || (xfer->tx_len < dws->fifo_len && xfer->len < dws->fifo_len))//txbuf null or eeprom mode never use tx dma
		return NULL;
