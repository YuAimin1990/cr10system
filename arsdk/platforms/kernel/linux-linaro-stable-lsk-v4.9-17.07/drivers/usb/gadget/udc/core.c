...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	if (ret && driver->pending.next != LIST_POISON1 && driver->pending.prev != LIST_POISON2) {
		list_del(&driver->pending);
		ret = 0;
	}
	else if (driver->pending.next == LIST_POISON1 && driver->pending.prev == LIST_POISON2)
	{
		ret = 0;
	}
	mutex_unlock(&udc_lock);
