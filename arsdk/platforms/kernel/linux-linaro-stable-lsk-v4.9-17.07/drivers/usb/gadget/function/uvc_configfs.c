				    ...
				    const struct uvcg_config_group_type *type)
{
	...
	config_group_init_type_name(group, type->name, (struct config_item_type *)&type->type);
						    ...
						    const char *name)
{
	...
	h->desc.bcdUVC			= cpu_to_le16(0x0100);//cpu_to_le16(0x0105);//
	h->desc.dwClockFrequency	= cpu_to_le32(1000000);

	config_item_init_type_name(&h->item, name, (struct config_item_type *)&uvcg_control_header_type);
...
static int uvcg_control_class_create_children(struct config_group *parent)
{
	...
	for (i = 0; i < ARRAY_SIZE(names); ++i) {
		...
		config_group_init_type_name(&group->group, group->name,
					    (struct config_item_type *)&uvcg_control_class_type);
...
*uvcg_streaming_header_make(struct config_group *group, const char *name)
{
	...
	config_item_init_type_name(&h->item, name, (struct config_item_type *)&uvcg_streaming_header_type);
...
struct uvcg_frame {
	...
	union{
		struct {
			...
			u32	dw_min_bit_rate;
			u32	dw_max_bit_rate;
			/* dw_max_video_frame_buffer_size is only for uncompressed and mjpeg format */
...
#define UVCG_FRAME_ATTR_ONLY_MJPEG_UNCOMP(cname, aname, bits) \
static ssize_t uvcg_frame_##cname##_show(struct config_item *item, char *page)\
{									\
	struct uvcg_frame *f = to_uvcg_frame(item);			\
	struct f_uvc_opts *opts;					\
...
UVCG_FRAME_ATTR_ONLY_MJPEG_UNCOMP(dw_max_video_frame_buffer_size, dwMaxVideoFrameBufferSize, 32);

#define UVCG_FRAME_ATTR_ONLY_FRAMEBASED(cname, aname, bits) \
static ssize_t uvcg_frame_##cname##_show(struct config_item *item, char *page)\
{									\
	struct uvcg_frame *f = to_uvcg_frame(item);			\
	struct f_uvc_opts *opts;					\
...
UVCG_FRAME_ATTR_ONLY_FRAMEBASED(dw_bytes_perline, dwBytesPerLine, 32);

#undef UVCG_FRAME_ATTR

#undef UVCG_FRAME_ATTR_ONLY_MJPEG_UNCOMP

#undef UVCG_FRAME_ATTR_ONLY_FRAMEBASED
						 ...
						 char *page)
{
	...
	mutex_lock(&opts->lock);
	if (fmt->type == UVCG_FRAMEBASED) {
		for (result = 0, i = 0; i < frm->frame2.b_frame_interval_type; ++i) {
						  ...
						  const char *page, size_t len)
{
	...
	if (fmt->type == UVCG_FRAMEBASED)
		ch->frame2.b_frame_interval_type = n;
	else
					   ...
					   const char *name)
{
	...
	if (fmt->type == UVCG_FRAMEBASED) {
		h->frame2.b_descriptor_type		= USB_DT_CS_INTERFACE;
		h->frame2.b_frame_index			= 1;
		h->frame2.w_width			= 640;
		h->frame2.w_height			= 360;
		h->frame2.dw_min_bit_rate		= 18432000;
		h->frame2.dw_max_bit_rate		= 55296000;
		h->frame2.dw_default_frame_interval	= 333333;
		h->frame2.dw_bytes_perline		= 0;
	} else {
	...
	if (fmt->type == UVCG_UNCOMPRESSED) {
		...
		h->frame2.b_descriptor_subtype = UVC_VS_FRAME_FRAME_BASED;
	...
	if (fmt->type == UVCG_FRAMEBASED)
		config_item_init_type_name(&h->item, name, (struct config_item_type *)&uvcg_frame_type2);
	else
		config_item_init_type_name(&h->item, name, (struct config_item_type *)&uvcg_frame_type);
...
static void uvcg_format_set_indices(struct config_group *fmt)
{
	...
	list_for_each_entry(ci, &fmt->cg_children, ci_entry) {
		...
		if (ci->ci_type != &uvcg_frame_type && ci->ci_type != &uvcg_frame_type2)
			continue;

		frm = to_uvcg_frame(ci);
		if(UVCG_FRAMEBASED == frm->fmt_type)
			frm->frame2.b_frame_index = i++;
		else
...
UVCG_FRAMEBASED_ATTR(b_bits_per_pixel, bBitsPerPixel, 8);
...
UVCG_FRAMEBASED_ATTR(b_variable_size, bVariableSize, 8);
...
static struct configfs_attribute *uvcg_framebased_attrs[] = {
	...
	&uvcg_framebased_attr_b_variable_size,
						   ...
						   const char *name)
{
	...
	config_group_init_type_name(&h->fmt.group, name,
				    (struct config_item_type *)&uvcg_framebased_type);
	...
	struct f_uvc_opts *o)
{
	...
	list_for_each_entry(f, &h->formats, entry) {
        ...
        ret = fun(&o->uvc_color_matching, priv2, priv3, k++, UVCG_COLOR);
		if (ret)
			return ret;
	}
	...
	case UVCG_FRAME: {
		...
		*size += sizeof(frm->frame);
		if (frm->fmt_type == UVCG_FRAMEBASED)
			*size += frm->frame2.b_frame_interval_type * sz;
		else
			*size += frm->frame.b_frame_interval_type * sz;
	}
	break;
	case UVCG_COLOR: {
		struct uvc_color_matching_descriptor * clm = priv1;
		*size += sizeof(*clm);
	}
	break;
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_FRAME: {
		...
		*dest += sz;
		if (frm->fmt_type == UVCG_FRAMEBASED)
			sz = frm->frame2.b_frame_interval_type *
		...
		*dest += sz;
		if (frm->fmt_type == UVCG_UNCOMPRESSED)
			...
			h->bLength = UVC_DT_FRAME_FRAMEBASED_SIZE(
				frm->frame2.b_frame_interval_type);
	}
	break;
	case UVCG_COLOR: {
		struct uvc_color_matching_descriptor * clm = priv1;
					   ...
					   struct config_item *target)
{
	...
	ret = __uvcg_iter_strm_cls(target_hdr, &size, &count, __uvcg_cnt_strm, opts);
	if (ret)
		goto unlock;

	count += 1; /* only for NULL */
...
static int uvcg_streaming_class_create_children(struct config_group *parent)
{
	...
	for (i = 0; i < ARRAY_SIZE(names); ++i) {
		...
		config_group_init_type_name(&group->group, group->name,
					    (struct config_item_type *)&uvcg_streaming_class_type);
		configfs_add_default_group(&group->group, parent);
	}
...
UVCG_OPTS_ATTR(streaming_maxpayload, streaming_maxpayload, 524287);

#undef UVCG_OPTS_ATTR

static int interface_string_copy(const char *s, char **s_copy, int limit)
{
	int ret;
	char *str;
	char *copy = *s_copy;
...
#define UVCG_OPTS_ATTR2(cname, aname, limit)				\
static ssize_t f_uvc_opts_##cname##_show(				\
	struct config_item *item, char *page)				\
{									\
	struct f_uvc_opts *opts = to_f_uvc_opts(item);			\
...
{									\
	...
	if (opts->refcnt) {						\
		...
		goto end;						\
									...
									\
	ret = interface_string_copy(page, &opts->cname, limit);		\
	if (ret)							\
		goto end;						\
	ret = len;							\
									\
...
UVCG_OPTS_ATTR2(control_interface_name, control_interface_name, 126);
UVCG_OPTS_ATTR2(streaming_interface_name, streaming_interface_name, 126);

#undef UVCG_OPTS_ATTR2
...
static struct configfs_attribute *uvc_attrs[] = {
	...
	&f_uvc_opts_attr_control_interface_name,
	&f_uvc_opts_attr_streaming_interface_name,
	NULL,
};
...
int uvcg_attach_configfs(struct f_uvc_opts *opts)
{
	...
	config_group_init_type_name(&opts->func_inst.group, uvc_func_type.name,
				    (struct config_item_type *)&uvc_func_type.type);
