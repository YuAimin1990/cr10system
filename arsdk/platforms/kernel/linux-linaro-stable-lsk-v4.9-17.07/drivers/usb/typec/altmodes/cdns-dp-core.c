...
struct cdn_dp_data sirius_cdn_dp = {
    .max_phy = 1,
};

static const struct of_device_id cdn_dp_dt_ids[] = {
    { .compatible = "artosyn,sirius-dp",
        .data = (void *)&sirius_cdn_dp },
    {}
};
...
static int cdn_dp_enable_phy(struct cdn_dp_device *dp, struct cdn_dp_port *port)
{
    ...
    //set mode
    sirius_typec_set_config_mode(dp->tcpc);
    ...
    //fw access phy
    sirius_typec_common_phy_access(dp->tcpc, 1);
...
err_power_on:
    sirius_typec_common_phy_access(dp->tcpc, 0);
...
static void cdn_dp_pd_event_work(struct work_struct *work)
{
    ...
    while(dp->is_icc_connected == 0)
    {
        ...
        status = ar_icc_client_receive(dp->dp_icc_client, (unsigned char *)&recv_msg, sizeof(recv_msg), dp->dp_icc_msg_id);
    ...
    if (port->extcon && extcon_get_cable_state_(port->extcon, EXTCON_DISP_DP) && 0 == dp->is_dp_connected)
    {
        ...
        ar_icc_client_send(dp->dp_icc_client, (unsigned char *)&send_msg, sizeof(send_msg), dp->dp_icc_msg_id, ICC_CORE_A_3);
...
static int cdn_dp_probe(struct platform_device *pdev)
{
    ...
    dp->dp_icc_client = ar_icc_client_create();
    dp->dp_icc_msg_id = DP_ICC_MESSAGE_ID;
    if(0 != ar_icc_client_register_msgid(dp->dp_icc_client, dp->dp_icc_msg_id))
    {
        printk(KERN_ERR "=====================================>icc register failed!\n");
    }

    if(NULL != dev->parent)
        dp->tcpc = (struct sirius_tcpc *)dev_get_drvdata(dev->parent);
