...
static const unsigned int sirius_tcpm_cable[] = {
    EXTCON_USB,
    EXTCON_USB_HOST,
    EXTCON_USB_VBUS_EN,
    EXTCON_CHG_USB_SDP,
 ...
 */
static const struct typec_altmode_desc sirius_tcpc_altmode[] =
{
    {
...
static const struct tcpc_config sirius_tcpc_otg_config = {
    .src_pdo = src_pdo,
    ...
    .alt_modes = sirius_tcpc_altmode,
    //.alt_modes = NULL,
};

static const struct tcpc_config sirius_tcpc_host_config = {
    .src_pdo = src_pdo,
    .nr_src_pdo = ARRAY_SIZE(src_pdo),
    .snk_pdo = snk_pdo,
    .nr_snk_pdo = ARRAY_SIZE(snk_pdo),
...
static const struct tcpc_config sirius_tcpc_device_config = {
    .src_pdo = src_pdo,
    .nr_src_pdo = ARRAY_SIZE(src_pdo),
    .snk_pdo = snk_pdo,
    .nr_snk_pdo = ARRAY_SIZE(snk_pdo),
...
static int sirius_tcpm_notify(struct sirius_tcpc *tcpc, struct sirius_tcpc_notify new)
{
    ...
    if(memcmp(&tcpc->notify, &new, sizeof(struct sirius_tcpc_notify)))
    {
...
int sirius_typec_common_phy_access(struct sirius_tcpc *tcpc, unsigned char flag)
{
    if(NULL == tcpc)
    {
        printk(KERN_ERR "sirius_typec_common_phy_access failed!");
        return -1;
    }
...
int sirius_typec_set_config_mode(struct sirius_tcpc *tcpc)
{
...
int sirius_tcpd_phy_reset(struct sirius_tcpc *tcpc)
{
...
static int CC1_vconn_enable(struct sirius_tcpc *tcpc, bool enable)
{
...
static int CC2_vconn_enable(struct sirius_tcpc *tcpc, bool enable)
{
...
{
    struct sirius_tcpc *tcpc = container_of(dev, struct sirius_tcpc, tcpc);
    int status, cc_set;
    int rv;
    int port = tcpc->port_index;
    unsigned int sink;
...
static int tcpm_set_polarity(struct tcpc_dev *dev, enum typec_cc_polarity polarity)
{
    ...
    unsigned int                val;
    int                         ret;
    struct sirius_tcpc_notify   new = tcpc->notify;
...
static irqreturn_t sirius_dp_irq(int irq, void *data)
{
    struct sirius_tcpc	        *tcpc = (struct sirius_tcpc *)data;
    struct sirius_tcpc_notify   new;
	int val;
...
static irqreturn_t sirius_tcpc_irq(int irq, void *data)
{
    struct sirius_tcpc    *tcpc = (struct sirius_tcpc *)data;
    int                 status;
    int                 port;
...
static void init_tcpc_dev(struct tcpc_dev * sirius_tcpc_dev)
{
    sirius_tcpc_dev->init = tcpm_init;
    sirius_tcpc_dev->get_vbus = tcpm_get_vbus_level;
    //sirius_tcpc_dev->get_current_limit = tcpm_get_current_limit;
    sirius_tcpc_dev->set_cc = tcpm_set_cc;
    sirius_tcpc_dev->get_cc = tcpm_get_cc;
    sirius_tcpc_dev->set_polarity = tcpm_set_polarity;
    sirius_tcpc_dev->set_vconn = tcpm_set_vconn;
    sirius_tcpc_dev->set_vbus = tcpm_set_vbus;
    sirius_tcpc_dev->set_pd_rx = tcpm_set_rx_enable;
    sirius_tcpc_dev->set_roles = tcpm_set_role;
    sirius_tcpc_dev->start_drp_toggling = tcpm_start_drp_toggling;
    sirius_tcpc_dev->pd_transmit = tcpm_transmit;
}

static void sirius_tcpc_usb_dp_switch_work(struct work_struct *work)
{
    struct sirius_tcpc				*tcpc =	container_of(work, struct sirius_tcpc, usb_dp_switch_work);
    struct sirius_tcpc_notify       new = tcpc->notify;
...
static int sirius_tcpc_usb_dp_switch_notifier(struct notifier_block *nb,
                        unsigned long event, void *ptr)
{
    struct sirius_tcpc *tcpc =
		container_of(nb, struct sirius_tcpc, usb_dp_switch_nb);
...
static int sirius_tcpc_extcon_register(struct sirius_tcpc * tcpc)
{
    int         ret;
    struct device       *dev = tcpc->dev;
    struct extcon_dev   *edev = tcpc->extcon;

    tcpc->usb_dp_switch_nb.notifier_call = sirius_tcpc_usb_dp_switch_notifier;
...
static void loadtcpcfw(const struct firmware *cdns_fw, void *context)
{
    ...
    tcpc = (struct sirius_tcpc *)context;
    dev = tcpc->dev;
    if(NULL == dev)
    {
    ...
    if(USB_DR_MODE_DP_ONLY == tcpc->dr_mode)
    {
        ...
        ret = devm_request_threaded_irq(dev, tcpc->irq, NULL,
                                        sirius_dp_irq, IRQF_ONESHOT | IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
                                        "dp", tcpc);
        if (ret != 0)
        {
...
static int sirius_tcpc_probe(struct platform_device *pdev)
{
    ...
    memset(&tcpc->notify, 0xff, sizeof(struct sirius_tcpc_notify));
    ...
    tcpc->extcon = devm_extcon_dev_allocate(dev, sirius_tcpm_cable);
    ...
    else
    {
        ...
        {
            tcpc->tcpc.config = &sirius_tcpc_device_config;
            printk(KERN_INFO "tcpc device mode!\n");
        }
        else if(tcpc->dr_mode == USB_DR_MODE_OTG)
        {
            tcpc->tcpc.config = &sirius_tcpc_otg_config;
            printk(KERN_INFO "tcpc otg mode!\n");
        }
        else
        {
            tcpc->tcpc.config = &sirius_tcpc_host_config;
    ...
    INIT_WORK(&tcpc->usb_dp_switch_work, sirius_tcpc_usb_dp_switch_work);
    sirius_tcpc_extcon_register(tcpc);
    ...
    printk(KERN_INFO "first sirius_tcpc_probe success %p -----------------\n", tcpc);

    return ret;

}

static int sirius_tcpc_remove(struct platform_device *pdev)
{
    struct sirius_tcpc          *tcpc = platform_get_drvdata(pdev);
...
static int sirius_pm_suspend(struct device *dev)
{
...
static int sirius_pm_resume(struct device *dev)
{
    //struct fusb302_chip *chip = dev->driver_data;
...
static const struct of_device_id sirius_tcpc_ids[] = {
    { .compatible = "artosyn, sirius-tcpc" },
    { }
};
MODULE_DEVICE_TABLE(of, sirius_tcpm_ids);

static const struct dev_pm_ops sirius_pm_ops = {
    .suspend = sirius_pm_suspend,
    .resume = sirius_pm_resume,
};

static struct platform_driver sirius_tcpc_driver = {
    .probe          = sirius_tcpc_probe,
    .remove         = sirius_tcpc_remove,
    .driver         = {
        .name   = "sirius-tcpc",
        .pm     = &sirius_pm_ops,
        .of_match_table = sirius_tcpc_ids,
    },
};

module_platform_driver(sirius_tcpc_driver);

MODULE_ALIAS("platform:sirius tcpc");
