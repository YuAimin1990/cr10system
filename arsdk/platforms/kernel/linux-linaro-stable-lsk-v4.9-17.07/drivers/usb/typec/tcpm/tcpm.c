				  ...
				  int cnt)
{
	...
	tcpm_log(port, "0x%08x 0x%08x 0x%08x 0x%08x\n", payload[0], payload[1], payload[2], payload[3]);
	tcpm_log(port, "0x%08x 0x%08x 0x%08x 0x%08x\n",
		le32_to_cpu(payload[0]), le32_to_cpu(payload[1]),
		le32_to_cpu(payload[2]), le32_to_cpu(payload[3]));
			...
			u32 *response)
{
	...
	if (adev)
	{
		...
		tcpm_log(port, "typec_altmode_vdm ret: %d adev: 0x%p\n", ret, adev);
	}
...
struct tcpm_port *tcpm_register_port(struct device *dev, struct tcpc_dev *tcpc)
{
	...
	if (tcpc->config && tcpc->config->alt_modes) {
		...
		tcpm_log(port, "enter alt_modes register!(%x %d)\n", paltmode->svid, ARRAY_SIZE(port->port_altmode));
		...
		while (paltmode->svid && i < ARRAY_SIZE(port->port_altmode)) {
			...
			tcpm_log(port,
				"%s: success to register port alternate mode 0x%x port_altmode[%d]:0x%p 0x%p",
				dev_name(dev), paltmode->svid, i, port->port_altmode[i], port->port_altmode);
