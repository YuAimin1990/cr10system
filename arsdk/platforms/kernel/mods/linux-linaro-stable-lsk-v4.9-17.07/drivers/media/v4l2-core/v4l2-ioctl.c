...
#define VIDIOC_AR_CAM_TUNING_REQ _IOWR('V', BASE_VIDIOC_PRIVATE + 1, uint32_t)
long video_ioctl2(struct file *file,
	       unsigned int cmd, unsigned long arg)
{
	char sbuf[12];
    unsigned int *p_val=(unsigned int *)sbuf;
    int len=0;    
	void __user *user_ptr = NULL;    
	void	*parg = NULL;    
	long	err  = -EINVAL;
    if(cmd==VIDIOC_AR_CAM_TUNING_REQ)
    {
        if(copy_from_user(sbuf, (void __user *)arg, 12))
           return -EFAULT;
        
        len=p_val[2];
        parg = kmalloc(len, GFP_KERNEL);
        if (NULL == parg)
            return -ENOMEM;
        
        if(copy_from_user(parg, (void __user *)arg, len))
           return -EFAULT;

        err=__video_do_ioctl(file,cmd,parg);
       
        if(copy_to_user((void __user *)arg, parg,len))
           err = -EFAULT;
