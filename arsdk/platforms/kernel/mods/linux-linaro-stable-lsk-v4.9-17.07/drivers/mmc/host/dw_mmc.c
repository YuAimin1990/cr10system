...
static u32 dw_mci_prepare_command(struct mmc_host *mmc, struct mmc_command *cmd)
{
	...
	cmdr |= SDMMC_CMD_CARD_NUM(slot->id);

	return cmdr;
}

static u32 dw_mci_prep_stop_abort(struct mmc_host *mmc, struct mmc_command *cmd)
{
	struct mmc_command *stop;
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
...
static void dw_mci_setup_bus(struct dw_mci_slot *slot, bool force_clkinit)
{
	...
	if (drv_data && drv_data->set_phase)
		drv_data->set_phase(slot);
	...
	} else if (clock != host->current_speed ||
		   clock != slot->__clk_old || force_clkinit) {
				   ...
				   struct mmc_command *cmd)
{
	...
	if (mrq->stop)
		...
		host->stop_cmdr = dw_mci_prep_stop_abort(slot->mmc, cmd);
}
...
static void dw_mci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!dw_mci_get_cd(mmc) && (mrq->cmd->opcode != SDMMC_CMD_UPDATE_CLK)) {
...
//					  SDMMC_CTRL_ALL_RESET_FLAGS);
	...
	if (slot->host->state == STATE_WAITING_CMD11_DONE &&
	    slot->host->cmd11_slot_id == slot->id && ios->clock != 0)
	...
	__acquires(&host->lock)
{
	...
	if (host->state == STATE_SENDING_CMD11) {
		slot = mmc_priv(prev_mmc);
		host->state = STATE_WAITING_CMD11_DONE;
		host->cmd11_slot_id = slot->id;
		dev_dbg(host->dev, "list wait slot%d cmd11 done\n",
			slot->id);
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	init_slots = 0;
	for (i = 0; i < host->num_slots; i++) {
		dw_mci_get_cd(host->slot[i]->mmc);
		ret = mmc_add_host(host->slot[i]->mmc);
		if (ret)
			dev_dbg(host->dev, "slot %d start failed\n", i);
		else
			init_slots++;
	}
	if (init_slots) {
		dev_dbg(host->dev, "%d slots started\n", init_slots);
	} else {
		dev_dbg(host->dev,
			"attempted to start %d slots, but failed on all\n",
			host->num_slots);
		goto err_dmaunmap;
	}
