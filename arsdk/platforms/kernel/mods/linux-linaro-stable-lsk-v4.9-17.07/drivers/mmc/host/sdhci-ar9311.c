...
#define SDHCI_PROXIMA_LOC_REG			(0x1018)
#define  SDHCI_TOP_LOC_ARCACHE_MASK		(0x0F)
#define  SDHCI_TOP_LOC_AWCACHE_MASK		(0x0F << 4)
#define  SDHCI_TOP_LOC_ARLOCK_MASK		(0x03 << 8)
#define  SDHCI_TOP_LOC_AWLOCK_MASK		(0x03 << 10)
#define  SDHCI_TOP_LOC_PWR_POR_MASK		(0x01 << 12)
#define  SDHCI_TOP_LOC_CLK_POR_MASK		(0x01 << 13)
...
static void sdhci_proxima_set_clock(struct sdhci_host *host, unsigned int clock)
{
	 ...
	 */
	reg_val = sdhci_readl(host, SDHCI_PROXIMA_LOC_REG);
	if (clock < 25000000 && proxima_dev_priv->out_phase)
		reg_val &= ~(SDHCI_TOP_LOC_CLK_POR_MASK);
	else
		reg_val |= SDHCI_TOP_LOC_CLK_POR_MASK;
	sdhci_writel(host, reg_val, SDHCI_PROXIMA_LOC_REG);
...
static void sdhci_proxima_global_rst(struct sdhci_host *host)
{
	...
	writel(RGU_EMMC_CORE_RSTN, proxima_dev_priv->rgu_reset0_n_ctrl);
	writel(RGU_EMMC_HRSTN, proxima_dev_priv->rgu_reset1_n_ctrl);
...
static int mmc_hs200_1v8;

/* Setup the speed mode by controller */
static int __init sdhci_proxima_mode_setup(char *str)
{
	if (*str)
		return 0;
...
__setup("mmc-hs200-1_8v", sdhci_proxima_mode_setup);
...
//	u16 ctrl = 0;
	...
	if (of_find_property(pdev->dev.of_node, "out-phase", NULL))
		proxima_dev_priv->out_phase = 1;
	else
		proxima_dev_priv->out_phase = 0;
...
#if !defined(FPGA_EMMC) && !defined(VELOCE_EMMC)
...
static const struct of_device_id sdhci_proxima_of_match_table[] = {
	{ .compatible = "artosyn,proxima-sdhc", },
	{}
};
...
MODULE_AUTHOR("dongyangmeng <dongyang.meng@artosyn.cn>");
