 ...
 *
 *
 * The artosyn Type-C PHY has two PLL clocks. The first PLL clock
...
//#include <soc/artosyn/artosyn_phy_typec.h>
...
struct artosyn_usb3phy_port_cfg {
...
struct artosyn_typec_phy {
	...
	struct artosyn_usb3phy_port_cfg port_cfgs;
...
static struct platform_device * phy_artosyn_sirius_pdev;
...
static int tcphy_dp_set_power_state(struct artosyn_typec_phy *tcphy,
 ...
 * ----------------------------------------------------------------------
 */
static void tcphy_set_lane_mapping(struct artosyn_typec_phy *tcphy, u8 mode)
{
	/*
...
#if 0
static int tcphy_dp_set_lane_count(struct artosyn_typec_phy *tcphy,
				   u8 lane_count)
{
	u32 reg;
...
static int tcphy_dp_set_link_rate(struct artosyn_typec_phy *tcphy,
				  int link_rate)
{
...
#endif
static void tcphy_cfg_24m(struct artosyn_typec_phy *tcphy)
{
	u32 i, rdata;
...
static void tcphy_cfg_usb3_pll(struct artosyn_typec_phy *tcphy)
{
	u32 i;
...
static void tcphy_cfg_dp_pll(struct artosyn_typec_phy *tcphy, int link_rate)
{
	const struct phy_reg *phy_cfg;
	u32 clk_ctrl;
...
static void tcphy_tx_usb3_cfg_lane(struct artosyn_typec_phy *tcphy, u32 lane)
{
...
static void tcphy_rx_usb3_cfg_lane(struct artosyn_typec_phy *tcphy, u32 lane)
{
...
static void tcphy_dp_cfg_lane(struct artosyn_typec_phy *tcphy, int link_rate,
...
static void tcphy_dp_aux_set_flip(struct artosyn_typec_phy *tcphy)
{
...
static void tcphy_dp_aux_calibration(struct artosyn_typec_phy *tcphy)
{
	u16 val;
	u16 tx_ana_ctrl_reg_1;
...
static int tcphy_phy_init(struct artosyn_typec_phy *tcphy, u8 mode)
{
	//struct artosyn_usb3phy_port_cfg *cfg = &tcphy->port_cfgs;
	int ret, i;
	u32 val;
...
static void tcphy_phy_deinit(struct artosyn_typec_phy *tcphy)
{
	void __iomem	           *tmp_addr;
...
static int tcphy_get_mode(struct artosyn_typec_phy *tcphy)
{
...
static int _artosyn_usb3_phy_power_on(struct artosyn_typec_phy *tcphy)
{
	struct artosyn_usb3phy_port_cfg *cfg = &tcphy->port_cfgs;
	...
	printk(KERN_ERR "artosyn_usb_phy_power_on cfg: 0x%p\n", cfg);
...
static int artosyn_usb3_phy_power_on(struct phy *phy)
{
  struct artosyn_typec_phy *tcphy = phy_get_drvdata(phy);
  int ret;
  int tries;

  for (tries = 0; tries < POWER_ON_TRIES; tries++) {
	  ret = _artosyn_usb3_phy_power_on(tcphy);
	  if (!ret)
		  break;
  }
...
static int artosyn_usb3_phy_power_off(struct phy *phy)
{
	struct artosyn_typec_phy *tcphy = phy_get_drvdata(phy);
...
static const struct phy_ops artosyn_usb3_phy_ops = {
	.power_on   = artosyn_usb3_phy_power_on,
	.power_off  = artosyn_usb3_phy_power_off,
	.owner	  = THIS_MODULE,
};

static int artosyn_dp_phy_power_on(struct phy *phy)
{
	struct artosyn_typec_phy *tcphy = phy_get_drvdata(phy);
	struct artosyn_usb3phy_port_cfg *cfg = &tcphy->port_cfgs;
	int new_mode, ret = 0;
	...
	printk(KERN_ERR "artosyn_dp_phy_power_on cfg: 0x%p\n", cfg);
...
static int artosyn_dp_phy_power_off(struct phy *phy)
{
	struct artosyn_typec_phy *tcphy = phy_get_drvdata(phy);
	int ret;
...
static const struct phy_ops artosyn_dp_phy_ops = {
	.power_on   = artosyn_dp_phy_power_on,
	.power_off  = artosyn_dp_phy_power_off,
	.owner	  = THIS_MODULE,
};


static int tcphy_parse_dt(struct artosyn_typec_phy *tcphy,
			struct device *dev)
{
	//struct artosyn_usb3phy_port_cfg *cfg = &tcphy->port_cfgs;
	//int ret;
	...
	*/
	/*ret = of_property_read_u32_array(dev->of_node, "artosyn,phy-config",
  ...
  static void typec_phy_pre_init(struct artosyn_typec_phy *tcphy)
  {
	  //struct artosyn_usb3phy_port_cfg *cfg = &tcphy->port_cfgs;
...
static int artosyn_typec_phy_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct device_node *child_np;
	struct artosyn_typec_phy *tcphy;
...
static int artosyn_typec_phy_remove(struct platform_device *pdev)
{
...
static const struct of_device_id artosyn_typec_phy_dt_ids[] = {
  { .compatible = "artosyn, sirius-phy" },
  {}
};

MODULE_DEVICE_TABLE(of, artosyn_typec_phy_dt_ids);


static struct platform_driver artosyn_typec_phy_driver = {
	.probe		= artosyn_typec_phy_probe,
	.remove		= artosyn_typec_phy_remove,
	.driver		= {
		.name	= "artosyn-sirius-typec-phy",
		.of_match_table = artosyn_typec_phy_dt_ids,
	},
};

module_platform_driver(artosyn_typec_phy_driver);

MODULE_AUTHOR("Chris Zhong <zyw@rock-chips.com>");
MODULE_AUTHOR("Kever Yang <kever.yang@rock-chips.com>");
MODULE_DESCRIPTION("artosyn USB TYPE-C PHY driver");
