...
extern void art_nor_dma_write(struct art_nor_host *host, unsigned int value,unsigned int reg);

extern unsigned int art_nor_dma_read(struct art_nor_host *host, unsigned int reg);

extern void art_nor_dma_lowbit_write(struct art_nor_host *host, unsigned int value);

extern unsigned int art_nor_dma_lowbit_read(struct art_nor_host *host);
...
#define reg_nor_dma_write_field(host,addr, value, field) do {\
			art_nor_dma_write(host,(art_nor_dma_read(host,addr) & ~(MSK(field##_LEN, field##_LSB))) | \
				((value) << (field##_LSB)),addr);\
			} while(0)


/* Here Assume the dst and the src are the same MSIZE and TR_WIDTH, SINGLE BLOCK transfer*/
static void inline ahb_dma_multi_block_transfer(struct art_nor_host *host, int ch)
{
	art_nor_dma_write(host, host->dma_buffer, ADDR_LLP_L(ch));
	art_nor_dma_write(host, DATA_ChEnReg, ADDR_ChEnReg);

}

static void ahb_dma_clear_irq(struct art_nor_host *host)
{
	art_nor_dma_write(host, DATA_ClearBlock, ADDR_ClearBlock);
	art_nor_dma_write(host, DATA_ClearErr, ADDR_ClearErr);
	art_nor_dma_write(host, DATA_ClearTfr, ADDR_ClearTfr);
	art_nor_dma_write(host, DATA_ClearErr, ADDR_ClearSrcTran);
	art_nor_dma_write(host, DATA_ClearTfr, ADDR_ClearDstTran);
}


static void ahb_dma_wait_finish(struct art_nor_host *host, int ch)
{
	volatile unsigned int val = 0;

	while(1) {
		val = art_nor_dma_read(host, ADDR_RawTfr);
...
static int ahb_dma_config(struct art_nor_host *host, int ch, unsigned int src_hs, unsigned int dst_hs,
						single_block_ctl_t *cfg, hand_shake_cfg_t *hs_cfg)
{
	...
	art_nor_dma_lowbit_write(host, val);

	ahb_dma_clear_irq(host);

	// Enable transfer complete interrupt and error interrupt
 	art_nor_dma_write(host, mask, ADDR_MaskTfr);

 	art_nor_dma_write(host, mask, ADDR_MaskErr);

 	art_nor_dma_write(host, mask, ADDR_MaskBlock);

	art_nor_dma_write(host, mask, ADDR_MaskSrcTran);
	art_nor_dma_write(host, mask, ADDR_MaskDstTran);
	...
	art_nor_dma_write(host, reg_value, ADDR_CTL_L(ch));

	reg_value = art_nor_dma_read(host, ADDR_CFG_L(ch));
	reg_value = reg_value & (~(BIT(CFG_L_RELOAD_DST_LSB) | BIT(CFG_L_RELOAD_SRC_LSB)));
	art_nor_dma_write(host, reg_value, ADDR_CFG_L(ch));
	...
	art_nor_dma_write(host, DATA_DmaCfgReg, ADDR_DmaCfgReg);

	return 0;
}


void ar_ahb_dma_init(struct art_nor_host *host)
{
...
int ar_ahb_dma_m2m(struct art_nor_host *host,unsigned char *src, unsigned char *dst, unsigned int size, u_char *read_buf, unsigned int check_buf)
{
	struct ahb_dma_llp *dma_llp = host->dma_llp;
	int i, n;
