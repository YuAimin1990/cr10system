...
static int __init artosyn_mempool_init(void)
{
	int ret;
	...
	if(ret < 0) {
		printk("register artosyn memory pool driver fail\n");
	}
	return ret;
}

static void __exit artosyn_mempool_exit(void)
{
	misc_deregister(&mempool_core.dev);
}

MODULE_AUTHOR("artosyn");
MODULE_DESCRIPTION("artosyn driver to map physical memory to user address space");
MODULE_LICENSE("GPL");

module_init(artosyn_mempool_init);
module_exit(artosyn_mempool_exit);
