...
static void ar9301_qspi_set_spi_mode(struct ar9301_qspi *aq, int mod)
{
	unsigned int tmp;
...
static void ar9301_qspi_set_flash_cs(struct ar9301_qspi *aq, int cs)
{
	int tmp;
...
static void ar9301_qspi_clear_fifo(struct ar9301_qspi *aq)
{
	unsigned int tmp;
...
static void ar9301_qspi_int_enable(struct ar9301_qspi *aq)
{
...
static void ar9301_qspi_clear_interrupt(struct ar9301_qspi *aq)
{
	/* write 1 clear interrupt */
	writel(1, aq->regs + QSPI_INT_ST);
}

static void ar9301_qspi_dtr_enable(struct ar9301_qspi *aq)
{
	int tmp;
...
static void ar9301_qspi_dtr_disable(struct ar9301_qspi *aq)
{
	int tmp;
...
static void ar9301_qspi_set_clk(struct ar9301_qspi *aq, unsigned int rate)
{
...
static void ar9301_qspi_tasklet(unsigned long data)
{
	int ret;
	struct ar9301_qspi *aq = (struct ar9301_qspi *)data;

	if (aq->dbg->dbg_flag & DBG_TASKLET)
		pr_info("%s [%d]: enter tasklet\n", __func__, __LINE__);

	ret = ar9301_qspi_read_fifo(aq, aq->buf, aq->rwlen);
	if (ret)
		BUG();
	complete(&aq->completion);
}

static irqreturn_t ar9301_qspi_interrupt(int irq, void *dev_id)
{
	struct ar9301_qspi *aq = dev_id;
	int status;
	...
	if (status & QSPI_INT_ST_DONE_INT_ST)
		ar9301_qspi_clear_interrupt(aq);
	else {
...
static int ar9301_qspi_adjust_op_size(struct spi_mem *mem, struct spi_mem_op *op)
{
	struct ar9301_qspi *aq = spi_master_get_devdata(mem->spi->master);
...
static bool ar9301_qspi_supports_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	return true;
}
...
static int ar9301_qspi_exec_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	int ret, mod = 0;
	struct ar9301_qspi *aq = spi_master_get_devdata(mem->spi->master);;
...
#define SPI_NAND_CS1 	3
	...
	if (spi->chip_select != aq->cur_cs) {
		ar9301_qspi_set_flash_cs(aq, spi->chip_select);
	...
	if (op->data.dir == SPI_MEM_DATA_IN) {
		if (op->data.nbytes < DMA_TRANS_MIN) {
			...
			if (aq->irq && aq->dbg->pio) {
				...
				/* TODO: read times */
				ret = ar9301_qspi_dma_read(aq, (unsigned int)aq->dma_buffer, op->data.nbytes - mod);
				if (ret)
					goto out;
		...
		if ((aq->irq && aq->dbg->pio) || (op->data.nbytes < DMA_TRANS_MIN)) {
			...
			if (ret)
				goto out;
			ret = ar9301_qspi_dma_write(aq, (unsigned int)aq->dma_buffer, op->data.nbytes - mod);
			if (ret)
				goto out;
			ar9301_qspi_dma_disable(aq);
...
static const char *ar9301_qspi_get_name(struct spi_mem *mem)
{
	return "artosyn_qspi";
}

static int ar9301_qspi_fdt_parse(struct platform_device *pdev, struct ar9301_qspi *aq)
{
	int ret;
	struct resource *res;
...
static int ar9301_qspi_debugfs_init(struct ar9301_qspi *aq);
static void ar9301_qspi_debugfs_exit(struct ar9301_qspi *aq);

static const struct spi_master_mem_ops ar9301_qspi_mem_ops = {
	.adjust_op_size = ar9301_qspi_adjust_op_size,
	.supports_op = ar9301_qspi_supports_op,
	.exec_op = ar9301_qspi_exec_op,
	.get_name = ar9301_qspi_get_name,
};
...
static int ar9301_qspi_probe(struct platform_device *pdev)
{
	...
	ret = ar9301_qspi_fdt_parse(pdev, aq);
	if (ret)
		goto exit;

	ret = ar9301_qspi_debugfs_init(aq);
	if (ret) {
		dev_err(&pdev->dev, "ar9301 qspi debugfs init failed\n");
		goto exit;
	}
	...
	if (aq->irq) {
		...
		tasklet_init(&aq->tasklet, ar9301_qspi_tasklet, (unsigned long)aq);
		ret = devm_request_irq(&pdev->dev, aq->irq, ar9301_qspi_interrupt, IRQF_SHARED, pdev->name, aq);
		if (ret)
			goto exit;
	}
	...
	ctlr->mem_ops = &ar9301_qspi_mem_ops;
...
exit:
	...
	dev_err(&pdev->dev, "Artosyn qspi probe failed\n");
	return ret;
}

static int ar9301_qspi_remove(struct platform_device *pdev)
{
	struct ar9301_qspi *aq = platform_get_drvdata(pdev);

	dmam_free_coherent(aq->dev, aq->dmabuf_len, aq->dmabuf, aq->dma_buffer);
	ar9301_qspi_debugfs_exit(aq);

	return 0;
}

static const struct of_device_id ar9301_qspi_dt_ids[] = {
	{ .compatible = "artosyn,ar9301-qspi" },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, ar9301_qspi_dt_ids);

static struct platform_driver ar9301_qspi_driver = {
	.driver = {
		.name	= "artosyn,ar9301-qspi",
		.of_match_table	= ar9301_qspi_dt_ids,
	},
	.probe		= ar9301_qspi_probe,
	.remove		= ar9301_qspi_remove,
};
			     ...
			     size_t count, loff_t *ppos)
{
	...
	struct ar9301_qspi *aq = (struct ar9301_qspi *)file->private_data;
	struct ar9301_qspi_debug_info *d = aq->dbg;
...
static const struct file_operations ar9301_qspi_dfs_fops = {
	.read   = dfs_file_read,
...
static int ar9301_qspi_debugfs_init(struct ar9301_qspi *aq)
{
	...
	aq->dbg = devm_kzalloc(aq->dev, sizeof(struct ar9301_qspi_debug_info), GFP_KERNEL);
	if (IS_ERR(aq->dbg))
		return PTR_ERR(aq->dbg);

	aq->dbg->dfs_dir = debugfs_create_dir("ar9301_qspi", NULL);
	if (IS_ERR_OR_NULL(aq->dbg->dfs_dir)) {
		err = aq->dbg->dfs_dir ? PTR_ERR(aq->dbg->dfs_dir) : -ENODEV;

		pr_err("aq9301 qspi error: cannot create \"ar9301_qspi\" debugfs directory, error %d\n",
		       err);
		return err;
	}
...
MODULE_DESCRIPTION("ar9301 QSPI Controller driver");
