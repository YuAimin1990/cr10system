		...
		struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	if(dws->is_slave) {
		wait_for_completion_interruptible(&master->xfer_completion);
		return 0;
	} else {
		return 1;
	}
...
{
	struct dw_spi *dws = spi_master_get_devdata(mem->spi->master);
	struct dw_spi_cfg cfg;
	unsigned long flags;
	int ret;
...
int dw_spi_add_host(struct device *dev, struct dw_spi *dws)
{
	...
	master->mode_bits = SPI_CPOL | SPI_CPHA | SPI_LOOP | SPI_CS_HIGH;
