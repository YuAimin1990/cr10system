...
static irqreturn_t icc_peer_ceva_isr(int irq, void *dev_id)
{
	...
	if(status & 0x1FF) //uop and gvi exceptions
	{
		...
		//print ceva program access voliation address and clear irq
		val = readl(priv->ceva_cpm_base + 0x520);
		printk("CEVA PROGRAM VOLIATION ADDRESS: 0x%x\n", val);
		writel(0, priv->ceva_cpm_base + 0x524); //clear irq

		//print ceva data access voliation address and clear irq
		val = readl(priv->ceva_cpm_base + 0xC80);
		printk("CEVA DATA VOLIATION ADDRESS: 0x%x\n", val);
		writel(0, priv->ceva_cpm_base + 0xC84); //clear irq

		//print ceva stack stat and end address
		val = readl(priv->ceva_cpm_base + 0xD48);
		val1 = readl(priv->ceva_cpm_base + 0xD44);
		printk("CEVA STACK STRAT-END ADDRESS: 0x%x-0x%x\n", val, val1);
...
static int icc_debug_peer_show_ceva(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "peer: %s, core id=%d, irq=%d, trigger reg=(0x%x, 0x%x), clr irq reg=(0x%x, 0x%x)\n",
