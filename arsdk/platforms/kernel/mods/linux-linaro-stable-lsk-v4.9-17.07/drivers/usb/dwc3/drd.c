...
static void dwc3_otg_enable_events(struct dwc3 *dwc, u32 enable_mask)
{
    ...
    printk(KERN_CRIT "dwc3_otg_enable_events: 0x%08x 0x%08x 0x%08x\n", dwc->regs, DWC3_OEVTEN, reg);

    reg = dwc3_readl(dwc->regs, DWC3_OEVTEN);
    printk(KERN_CRIT "dwc3_otg_enable_events reg: 0x%08x\n", reg);*/
}
...
static int dwc3_drd_get_current_prt_mode(struct dwc3 *dwc)
{
    int reg;
    reg = gpio_get_value(dwc->idpin_gpio);

    //printk(KERN_CRIT "current_prt_mode : %d\n", reg);
...
static irqreturn_t dwc3_otg_irq(int irq, void *_dwc)
{
    ...
    if(1 == reg && dwc->current_dr_role == DWC3_GCTL_PRTCAP_HOST)
    {
        dwc->otg_restart_host = 1;
        ret = IRQ_WAKE_THREAD;
    }
    else if(0 == reg && dwc->current_dr_role == DWC3_GCTL_PRTCAP_DEVICE)
    {
        ret = IRQ_WAKE_THREAD;
    }
    else
...
void dwc3_otg_init(struct dwc3 *dwc)
{
    ...
    //printk(KERN_CRIT "otg_init prtcap_mode: %d\n", dwc3_drd_get_current_prt_mode(dwc));

    switch (dwc3_drd_get_current_prt_mode(dwc))
    {
    ...
    dwc3_set_prtcap(dwc, dwc3_drd_get_current_prt_mode(dwc));
...
void dwc3_otg_update(struct dwc3 *dwc, bool ignore_idstatus)
{
    ...
    if (!ignore_idstatus)
    {
        reg = gpio_get_value(dwc->idpin_gpio);
        id = !!(reg);
        printk(KERN_CRIT "DWC3_ID_PIN: 0x%08x\n", reg);
	...
	if (!ignore_idstatus) {
        ...
        printk(KERN_CRIT "DWC3_OSTS: 0x%08x\n", reg);
    ...
    //printk(KERN_CRIT "dwc3_otg_update current_otg_role: %d\n", dwc->current_otg_role);
    //printk(KERN_CRIT "dwc3_otg_update desired_otg_role: %d\n", dwc->desired_otg_role);
...
int dwc3_drd_init(struct dwc3 *dwc)
{
    ...
    //printk(KERN_CRIT "dwc3_drd_init\n");

    dwc3_set_prtcap(dwc, dwc3_drd_get_current_prt_mode(dwc));
    dwc->current_dr_role = dwc3_drd_get_current_prt_mode(dwc);

    irq_gpio = of_get_named_gpio(dwc->dev->of_node, "idpin-gpios", 0);
    if(irq_gpio >= 0)
    {
        printk(KERN_CRIT "irq_gpio num: %d\n", irq_gpio);

        if(gpio_is_valid(irq_gpio) && (0 == gpio_request(irq_gpio, "idpin-gpios")))
        {
        ...
        if(irq >= 0)
        {
            printk(KERN_CRIT "dwc3_otg_get_irq num: %d %d\n", irq, gpio_to_irq(irq_gpio));
    		...
    		phy_calibrate(dwc->usb2_generic_phy);
#else
            //printk(KERN_CRIT "gpio irq num: %d\n", irq);

            //ret = devm_request_any_context_irq(dwc->dev, irq, dwc3_drd_notifier, unsigned long irqflags, const char * devname, void * dev_id);

            dwc->otg_irq = irq;
            dwc->idpin_gpio = irq_gpio;
            ...
            dwc3_set_mode(dwc, dwc3_drd_get_current_prt_mode(dwc));
#endif
        }
        else
        {
            printk(KERN_CRIT "gpio_to_irq failed: %d\n", irq);
			return ret;
        }
    }
    else
    ...
    printk(KERN_CRIT "dwc3_get_extcon\n");
