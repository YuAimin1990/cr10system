...
static int dp_altmode_notify(struct dp_altmode *dp)
{
	...
	//indicate find dp svid
	extcon_set_state(dp->tcpc->extcon, EXTCON_CHG_USB_FAST, true);

	//indicate disable usb
	property.intval = 0;
	extcon_set_property(dp->tcpc->extcon, EXTCON_CHG_USB_FAST,
			EXTCON_PROP_USB_TYPEC_POLARITY, property);

	ret = extcon_sync(dp->tcpc->extcon, EXTCON_CHG_USB_FAST);
	if(ret)
		printk(KERN_ERR "1.EXTCON_CHG_USB_FAST extcon_sync failed\n");

	printk(KERN_INFO "=======================>dp state:%d\n", state);
...
static int dp_altmode_configure(struct dp_altmode *dp, u8 con)
{
	...
	u8 pin_assign = 0, typec_receptacle = 0;
	...
	case DP_STATUS_CON_BOTH: /* NOTE: First acting as DP source */
		...
		if(!pin_assign) //type c receptacle
		{
			pin_assign = DP_CAP_UFP_D_PIN_ASSIGN(dp->alt->vdo) &
				DP_CAP_UFP_D_PIN_ASSIGN(dp->port->vdo);
			typec_receptacle = 1;
	...
	if (!DP_CONF_GET_PIN_ASSIGN(dp->data.conf)) {
		...
		if(typec_receptacle)
			conf |= (DP_CONF_SET_PIN_ASSIGN(pin_assign) << 8);
		else
			  ...
			  const u32 hdr, const u32 *vdo, int count)
{
	...
	printk(KERN_INFO "dp->state: %d cmd_type: %d cmd: %d\n",
		dp->state, cmd_type, cmd);
...
static int dp_altmode_probe(struct typec_altmode *alt)
{
	...
	int ret;
	struct device * tcpc_dev = NULL;

	/* FIXME: Port can only be DFP_U. */
	printk(KERN_INFO "dp altmode probe 0x%08x 0x%08x\n",
		port->vdo, alt->vdo);
	...
	printk(KERN_INFO "dp altmode probe 1\n");

	ret = sysfs_create_group(&alt->dev.kobj, &dp_altmode_group);
	if (ret)
		return ret;

	printk(KERN_INFO "dp altmode probe 2\n");

	dp = devm_kzalloc(&alt->dev, sizeof(*dp), GFP_KERNEL);
	if (!dp)
		return -ENOMEM;

	printk(KERN_INFO "dp altmode probe 3\n");
	...
	if(NULL != alt->dev.parent && NULL != alt->dev.parent->parent &&
	   NULL != alt->dev.parent->parent->parent)
	{
		tcpc_dev = alt->dev.parent->parent->parent;
        if(tcpc_dev->driver_data)
		    dp->tcpc = (struct sirius_tcpc *)dev_get_drvdata(tcpc_dev);
		printk(KERN_INFO "PROBE =======================>dev: %p", dp->tcpc);
	}
...
static void dp_altmode_remove(struct typec_altmode *alt)
{
	...
	printk(KERN_INFO "=======================>dp dp_altmode_remove\n");

	//indicate remove dp svid
	extcon_set_state(dp->tcpc->extcon, EXTCON_CHG_USB_FAST, false);
	...
	ret = extcon_sync(dp->tcpc->extcon, EXTCON_CHG_USB_FAST);
	if(ret)
		printk(KERN_ERR "3.EXTCON_CHG_USB_FAST extcon_sync failed\n");
