...
void cdn_dp_clock_reset(struct cdn_dp_device *dp)
{
    ...
    //wkwk
	//writel(0x0000003d, dp->regs + DP_AUX_DIVIDE_2M);
	//writel(0x000000ae, dp->regs + DP_AUX_FREQUENCY_1M_MAX);
	//writel(0x00000050, dp->regs + DP_AUX_FREQUENCY_1M_MIN);
...
err_dpcd_read:
	printk(KERN_ERR "dpcd read failed: %d\n", ret);
	return ret;
}
...
err_dpcd_write:
	if (ret)
		printk(KERN_ERR "dpcd write failed: %d\n", ret);
	return ret;
}
...
err_get_hpd:
	printk(KERN_ERR "get aux status failed: %d\n", ret);
	return ret;
}
			 ...
			 u32 i_size, const u32 *d_mem, u32 d_size)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "failed to loaded the FW reg = %x\n",
			      reg);
		return -EINVAL;
	}
...
err_set_firmware_active:
	if (ret < 0)
		printk(KERN_ERR "set firmware active failed\n");
	return ret;
}

int cdn_dp_echo_test(struct cdn_dp_device *dp)
{
	u8 msg[20] = "11235813123456";
	u8 recv[32] = "";
	int i, ret;
	printk(KERN_ERR "echo test:%s\n", msg);
	ret = cdn_dp_mailbox_send(dp, MB_MODULE_ID_GENERAL,
				  GENERAL_TEST_ECHO,
				  sizeof(msg), msg);
	if (ret == 0)
	{
		ret = cdn_dp_mailbox_read_receive(dp, recv, sizeof(recv));
		printk(KERN_ERR "recv data:");
		for(i = 0; i < sizeof(recv); ++i)
		{
			printk(KERN_ERR "0x%02x ", recv[i]);
		}
		printk(KERN_ERR "\n");
	...
	{
		printk(KERN_ERR "mailbox_send fail:%d\n", ret);
	}
...
err_set_host_cap:
	if (ret)
		printk(KERN_ERR "set host cap failed: %d\n", ret);
	return ret;
}
...
int cdn_dp_event_config(struct cdn_dp_device *dp)
{
	...
	ret = cdn_dp_mailbox_send(dp, MB_MODULE_ID_DP_TX, DPTX_ENABLE_EVENT,
				  sizeof(msg), msg);
	if (ret)
		printk(KERN_ERR "set event config failed: %d\n", ret);
...
err_get_hpd:
	printk(KERN_ERR "get hpd status failed: %d\n", ret);
	return ret;
}
			  ...
			  unsigned int block, size_t length)
{
	...
	for (i = 0; i < 4; i++) {
        ...
        printk(KERN_ERR "1 cdn_dp_get_edid_block ret: %d\n", ret);
		if (ret)
			continue;
		...
		ret = cdn_dp_mailbox_validate_receive(dp, MB_MODULE_ID_DP_TX,
						      DPTX_GET_EDID,
						      sizeof(reg) + length);
        printk(KERN_ERR "2 cdn_dp_get_edid_block ret: %d\n", ret);
		if (ret)
			continue;

		ret = cdn_dp_mailbox_read_receive(dp, reg, sizeof(reg));
        printk(KERN_ERR "3 cdn_dp_get_edid_block ret: %d\n", ret);
		if (ret)
			continue;

		ret = cdn_dp_mailbox_read_receive(dp, edid, length);
        printk(KERN_ERR "4 cdn_dp_get_edid_block ret: %d\n", ret);
		if (ret)
			continue;
	...
	if (ret)
		printk(KERN_ERR "get block[%d] edid failed: %d\n", block,
			      ret);
...
err_training_start:
	printk(KERN_ERR "training failed: %d\n", ret);
	return ret;
}
...
int cdn_dp_get_training_status(struct cdn_dp_device *dp)
{
    ...
    printk(KERN_ERR "get training status %x%x%x%x%x %x%x%x%x%x\n",
        status[0], status[1], status[2], status[3], status[4],
        status[5], status[6], status[7], status[8], status[9]);

err_get_training_status:
	if (ret)
		printk(KERN_ERR "get training status failed: %d\n", ret);
	return ret;
}
...
int cdn_dp_train_link(struct cdn_dp_device *dp)
{
	...
	printk(KERN_ERR "use fw training\n");
	ret = cdn_dp_training_start(dp);
	if (ret) {
		printk(KERN_ERR "Failed to start training %d\n", ret);
		return ret;
	}

	ret = cdn_dp_get_training_status(dp);
	if (ret) {
		printk(KERN_ERR "Failed to get training stat %d\n", ret);
		return ret;
	}

	printk(KERN_ERR "rate:0x%x, lanes:%d\n", dp->link.rate,
			  dp->link.num_lanes);
	return 0;
}
...
int cdn_dp_set_video_status(struct cdn_dp_device *dp, int active)
{
	...
	ret = cdn_dp_mailbox_send(dp, MB_MODULE_ID_DP_TX, DPTX_SET_VIDEO,
				  sizeof(msg), &msg);
	if (ret)
		printk(KERN_ERR "set video status failed: %d\n", ret);
...
err_config_video:
	if (ret)
		printk(KERN_ERR "config video failed: %d\n", ret);
	return ret;
}
...
int cdn_dp_audio_stop(struct cdn_dp_device *dp, struct audio_info *audio)
{
	...
	if (ret) {
		printk(KERN_ERR "audio stop failed: %d\n", ret);
		return ret;
	}
...
int cdn_dp_audio_mute(struct cdn_dp_device *dp, bool enable)
{
	...
	if (ret)
		printk(KERN_ERR "audio mute failed: %d\n", ret);
...
err_audio_config:
	if (ret)
		printk(KERN_ERR "audio config failed: %d\n", ret);
	return ret;
}
...
err_hdcp_tx_status_rq:
	if (ret)
		printk(KERN_ERR "hdcp tx status failed: %d\n", ret);
	return ret;
}
