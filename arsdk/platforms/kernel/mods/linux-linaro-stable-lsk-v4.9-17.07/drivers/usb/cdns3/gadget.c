...
#include "trace.h"
#include "drd.h"
//#define CDNS_GADGET_ISOC_WORKAROUND2


//#define CDNS_GADGET_ISOC_WORKAROUND

#ifdef CDNS_GADGET_ISOC_WORKAROUND

#define                 padding_req_num         4
struct usb_request *    padding_req[padding_req_num];//wk:workaround for isoerr
unsigned int            padding_data[8];
int                     padding_flag = 0;
				 ...
				 struct usb_request *request)
{
	...
	int pcs;
	u16 total_tdl = 0;
#ifdef CDNS_GADGET_ISOC_WORKAROUND2
	unsigned int flag;
...
#ifdef CDNS_GADGET_ISOC_WORKAROUND2
	if (priv_dev->dev_ver < DEV_VER_V2 && (flag & EP_PENDING_REQUEST == 0))
				   ...
				   gfp_t gfp_flags)
{
	...
	//lock
	if(USB_ENDPOINT_XFER_ISOC == priv_ep->type && 2048 < request->length && 1 == padding_flag)
	{
		int i = 0;
		...
		for(i = 0; i < padding_req_num; ++i)
		{
			if (padding_req[i])
			{
				cdns3_gadget_ep_free_request(ep, padding_req[i]);
			}
			padding_req[i] = cdns3_gadget_ep_alloc_request(ep, GFP_KERNEL);

			if (padding_req[i])
			{
				padding_req[i]->buf = padding_data;
				padding_req[i]->length = 0;//sizeof(padding_data);
				padding_req[i]->complete = NULL;

				padding_req[i]->actual = 0;
				padding_req[i]->status = -EINPROGRESS;
				priv_req = to_cdns3_request(request);
				...
				cdns3_ep_run_transfer(priv_ep, padding_req[i]);
				list_add_tail(&padding_req[i]->list, &priv_ep->pending_req_list);
			}
		}
		padding_flag = 0;
	}
...
void cdns3_gadget_exit(struct cdns3 *cdns)
{
	...
	int i = 0;
		...
		if (padding_req[i])
                {
                        cdns3_gadget_ep_free_request(NULL, padding_req[i]);
                }
        }
#endif
}
