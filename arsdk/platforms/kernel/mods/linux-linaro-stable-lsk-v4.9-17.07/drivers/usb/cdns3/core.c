...
static int cdns3_role_start(struct cdns3 *cdns, enum usb_role role)
{
	...
	if (!cdns->roles[role])
	{
	    dev_err(cdns->dev, "role %d is null\n", role);
		return -ENXIO;
	}
...
/**
 * cdns3_hw_role_switch_by_extcon - switch roles based on extcon state
 * @cdns3: controller
 */
int cdns3_hw_role_switch_by_extcon(struct cdns3 *cdns, enum usb_role real_role)
{
	...
	if (ret) {
		...
		dev_err(cdns->dev, "set %d has failed, back to %d(ret: %d)\n",
			real_role, current_role, ret);
...
static int cdns3_probe(struct platform_device *pdev)
{
    ...
    printk(KERN_INFO "=========================================>cdns usb3.0 start phy init");
	...
	if(NULL != dev->parent)
		cdns->tcpc = (struct sirius_tcpc *)dev_get_drvdata(dev->parent);
	...
	if (cdns->edev && (cdns->dr_mode == USB_DR_MODE_OTG)) {
		if ((extcon_get_cable_state_(cdns->edev,
					     EXTCON_USB) > 0) ||
		    (extcon_get_cable_state_(cdns->edev,
					     EXTCON_USB_HOST) > 0))
		{
			union extcon_property_value property;
			int ret = extcon_get_property(cdns->edev, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY,
			    &property);
			if(!ret)
			{
	...
	dev_info(dev, "Cadence USB3 core: probe succeed\n");
