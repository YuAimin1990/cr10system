...
static struct usb_ss_ep_comp_descriptor as_out_ss_ep_comp = {
	.bLength		= sizeof(as_out_ss_ep_comp),
...
static struct usb_ss_ep_comp_descriptor as_in_ss_ep_comp = {
	.bLength		= sizeof(as_in_ss_ep_comp),
	.bDescriptorType	= USB_DT_SS_ENDPOINT_COMP,
	.bMaxBurst		= 0,
	.bmAttributes	= 0,
	.wBytesPerInterval	= cpu_to_le16(UAC1_OUT_EP_MAX_PACKET_SIZE),
...
static struct usb_descriptor_header *f_ss_audio_desc[] = {
	...
	(struct usb_descriptor_header *)&as_out_ep_desc,
	(struct usb_descriptor_header *)&as_out_ss_ep_comp,
	(struct usb_descriptor_header *)&as_iso_out_desc,
	...
	(struct usb_descriptor_header *)&as_in_ep_desc,
	(struct usb_descriptor_header *)&as_in_ss_ep_comp,
	(struct usb_descriptor_header *)&as_iso_in_desc,
	NULL,
};
...
static void setup_descriptor(struct f_uac1_opts *opts)
{
    ...
    i = 0;
    f_ss_audio_desc[i++] = USBDHDR(&iad_desc);
	f_ss_audio_desc[i++] = USBDHDR(&ac_interface_desc);
	f_ss_audio_desc[i++] = USBDHDR(ac_header_desc);

	if (EPOUT_EN(opts)) {
		f_ss_audio_desc[i++] = USBDHDR(&usb_out_it_desc);
		f_ss_audio_desc[i++] = USBDHDR(&io_out_ot_desc);
	}

	if (EPIN_EN(opts)) {
		f_ss_audio_desc[i++] = USBDHDR(&io_in_it_desc);
		f_ss_audio_desc[i++] = USBDHDR(&usb_in_ot_desc);
	}

	if (EPOUT_EN(opts)) {
		f_ss_audio_desc[i++] = USBDHDR(&as_out_interface_alt_0_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_out_interface_alt_1_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_out_header_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_out_type_i_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_out_ep_desc);
        f_ss_audio_desc[i++] = USBDHDR(&as_out_ss_ep_comp);
		f_ss_audio_desc[i++] = USBDHDR(&as_iso_out_desc);
	}
	if (EPIN_EN(opts)) {
		f_ss_audio_desc[i++] = USBDHDR(&as_in_interface_alt_0_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_in_interface_alt_1_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_in_header_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_in_type_i_desc);
		f_ss_audio_desc[i++] = USBDHDR(&as_in_ep_desc);
        f_ss_audio_desc[i++] = USBDHDR(&as_in_ss_ep_comp);
		f_ss_audio_desc[i++] = USBDHDR(&as_iso_in_desc);
	}
	f_ss_audio_desc[i] = NULL;
}
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	status = usb_assign_descriptors(f, f_audio_desc, f_audio_desc, f_ss_audio_desc,
					NULL);
	if (status)
		goto fail;
