...
static int da9062_disable_rtc(struct da9062 *chip)
{
	int ret;
	...
	ret = regmap_update_bits(chip->regmap,
				 ...
				 0 << DA9062AA_CRYSTAL_SHIFT);
	if (ret) {
		dev_err(chip->dev,
			"Failed to disable external RTC crystal (err = %d)\n",
			ret);
	...
	ret = regmap_update_bits(chip->regmap,
				 ...
				 0 << DA9062AA_RTC_EN_SHIFT);
	if (ret)
		dev_err(chip->dev,
			"Failed to disable RTC (err = %d)\n", ret);
...
static int da9062_fault_show(struct seq_file *s, void *unused)
{
	struct da9062           *chip = s->private;
	seq_printf(s, "0x%08x\n", chip->fault_log);
...
static int da9062_fault_open(struct inode *inode, struct file *file)
{
	return single_open(file, da9062_fault_show, inode->i_private);
}

static ssize_t da9062_fault_write(struct file *file, const char __user *ubuf, 
...
static const struct file_operations fault_log_fops = {
	.open			= da9062_fault_open, 
	.write			= da9062_fault_write,
	.read			= seq_read,
};

void da9062_debugfs_init(struct i2c_client *i2c)
{
	struct dentry		*root;
	struct da9062 		*chip;

	printk(KERN_INFO "da9062_debugfs_init\n");

	if(NULL == i2c)
	{
		printk(KERN_ERR "i2c is null!\n");
		return;
	}

	chip = (struct da9062 *)i2c_get_clientdata(i2c);
	if(NULL == chip)
	{
		printk(KERN_ERR "da9062 is null!\n");
		return;
	}

	root = debugfs_create_dir("da9062", NULL);
	if(NULL == root)
	{
		printk(KERN_ERR "debugfs dir create error!\n");
		return;
	}

	chip->root = root;
	debugfs_create_file("fault_log", S_IRUGO | S_IWUSR, root, chip, &fault_log_fops);

	return;
}

void da9062_debugfs_exit(struct i2c_client *i2c)
{
	struct dentry           *root;
        struct da9062           *chip;
	...
	debugfs_remove_recursive(chip->root);
	...
	const struct i2c_device_id *id)
{
	...
	chip->reset_gpio = devm_gpiod_get(chip->dev, NULL,
			open_source ? GPIOD_IN : GPIOD_OUT_LOW);
	if (IS_ERR(chip->reset_gpio)) {
		dev_warn(chip->dev, "Could not get reset GPIO\n");
//		return PTR_ERR(chip->reset_gpio);
	...
	if (device_property_read_bool(chip->dev, "internal-crystal-rtc-quirk")) {
		dev_info(chip->dev, "Disable RTC for internal-crystal-rtc-quirk\n");
		da9062_disable_rtc(chip);
	}
	da9062_debugfs_init(i2c);
