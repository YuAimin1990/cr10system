...
static void set_max_pktsize(struct dwc2_udc *dev, enum usb_device_speed speed)
{
	...
	for (i = 3; i < DWC2_MAX_ENDPOINTS; i++)
...
int dwc2_udc_probe(struct dwc2_plat_otg_data *pdata)
{
	...
	memset(usb_ctrl, 0xff, sizeof(struct usb_ctrlrequest));
	invalidate_dcache_range((unsigned long)usb_ctrl, (unsigned long)usb_ctrl +
				ROUND(sizeof(struct usb_ctrlrequest), CONFIG_SYS_CACHELINE_SIZE));
...
static const struct udevice_id dwc2_udc_otg_ids[] = {
	{ .compatible = "artosyn,dwc2,peripheral" },
