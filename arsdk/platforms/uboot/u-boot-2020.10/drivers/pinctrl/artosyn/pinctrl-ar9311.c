...
struct ar9311_pin_info group_info[] = {
	{23, 0x7c, 0x04},
	{22, 0x88, 0x1c},
	{26, 0x94, 0x34},
	{6,  0xa4, 0x50},
...
struct ar9311_pinctrl_priv {
	void __iomem *pb_sw_reg_base;
};

static int ar9311_pinctrl_pb_msc_config(struct udevice *dev)
{
	struct ar9311_pinctrl_priv *priv = NULL;
...
static int ar9311_pinctrl_probe(struct udevice *dev)
{
	struct ar9311_pinctrl_priv *priv = dev_get_priv(dev);
	u32 size, i, j;
	u32 pads_num;
	u32 *pads_info = NULL;
	u32 pad;
	...
	//u32 pad_sels_num;
	int ret = 0;
	struct ar9311_pad_info pad_info[110];

#define PINCTRL_BASE_NUM 	2
	unsigned int tmp[PINCTRL_BASE_NUM];
	u32 idx = 0;
	for(i = 0; i < (sizeof(group_info) / sizeof(struct ar9311_pin_info)); ++i) {
	...
	ret = ar9311_pinctrl_pb_msc_config(dev);
...
static int ar9311_pinctrl_set_state_simple(struct udevice *dev,
...
const struct pinctrl_ops single_pinctrl_ops = {
	.set_state_simple = ar9311_pinctrl_set_state_simple,
};

static const struct udevice_id ar9311_pinctrl_match[] = {
	{ .compatible = "artosyn,ar9311-pinctrl" },
	{ /* sentinel */ }
};

U_BOOT_DRIVER(ar9311_pinctrl) = {
	.name = "artosyn_ar9311_pinctrl",
	.id = UCLASS_PINCTRL,
	.of_match = ar9311_pinctrl_match,
	.priv_auto_alloc_size = sizeof(struct ar9311_pinctrl_priv),
	.ops = &single_pinctrl_ops,
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
	.bind		= dm_scan_fdt_dev,
#endif
	.probe		= ar9311_pinctrl_probe,
};
