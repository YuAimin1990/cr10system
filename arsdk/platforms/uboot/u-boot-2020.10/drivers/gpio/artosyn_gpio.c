...
/*
 * Header providing constants for Artosyn pinctrl bindings.
 *
 * Copyright (c) 2022 Artosyn
 */
...
struct gpio_artosyn_priv {
	struct reset_ctl_bulk	resets;
};

struct gpio_artosyn_platdata {
	const char	*name;
	int		bank;
	int		pins;
	fdt_addr_t	base;
...
static int artosyn_gpio_direction_input(struct udevice *dev, unsigned pin)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);

	setbits_le32(plat->base + GPIO_SWPORT_DDR(plat->bank), 1 << pin);
	return 0;
}

static int artosyn_gpio_direction_output(struct udevice *dev, unsigned pin,
				     int val)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
...
static int artosyn_gpio_get_value(struct udevice *dev, unsigned pin)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
...
static int artosyn_gpio_set_value(struct udevice *dev, unsigned pin, int val)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
...
static int artosyn_gpio_get_function(struct udevice *dev, unsigned offset)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
	u32 gpio;
...
static const struct dm_gpio_ops gpio_artosyn_ops = {
	.direction_input	= artosyn_gpio_direction_input,
	.direction_output	= artosyn_gpio_direction_output,
	.get_value		= artosyn_gpio_get_value,
	.set_value		= artosyn_gpio_set_value,
	.get_function		= artosyn_gpio_get_function,
};

static int gpio_artosyn_reset(struct udevice *dev)
{
	int ret;
	struct gpio_artosyn_priv *priv = dev_get_priv(dev);
...
static int gpio_artosyn_probe(struct udevice *dev)
{
	struct gpio_dev_priv *priv = dev_get_uclass_priv(dev);
	struct gpio_artosyn_platdata *plat = dev->platdata;

	if (!plat) {
		/* Reset on parent device only */
		return gpio_artosyn_reset(dev);
	}
...
static int gpio_artosyn_bind(struct udevice *dev)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
	...
	for (node = dev_read_first_subnode(dev); ofnode_valid(node);
		...
		plat->bank = bank;
		plat->pins = ofnode_read_u32_default(node, "arto,nr-gpios", 0);
...
static int gpio_artosyn_remove(struct udevice *dev)
{
	struct gpio_artosyn_platdata *plat = dev_get_platdata(dev);
	struct gpio_artosyn_priv *priv = dev_get_priv(dev);
...
static const struct udevice_id gpio_artosyn_ids[] = {
	{ .compatible = "artosyn,gpio" },
	{ }
};

U_BOOT_DRIVER(gpio_artosyn) = {
	.name		= "gpio-artosyn",
	.id		= UCLASS_GPIO,
	.of_match	= gpio_artosyn_ids,
	.ops		= &gpio_artosyn_ops,
	.bind		= gpio_artosyn_bind,
	.probe		= gpio_artosyn_probe,
	.remove		= gpio_artosyn_remove,
	.priv_auto_alloc_size   = sizeof(struct gpio_artosyn_priv),
};
