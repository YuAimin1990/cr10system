...
retry:
	...
	if (host->quirks & SDHCI_QUIRK_HOST_CMD_RETRY && retry) {
		host->ops->cmd_post_retry(host, cmd, stat);
	}
	...
	if (host->quirks & SDHCI_QUIRK_HOST_CMD_RETRY && !retry) {
		if (host->ops->cmd_pre_retry(host, cmd, stat)) {
			retry = 1;
			printf("%s: Retry for host command %d\n", __func__, cmd->cmdidx);
			goto retry;
		}
	}
...
int sdhci_power_cycle(struct udevice *dev)
{
	struct mmc *mmc = mmc_get_mmc_dev(dev);
	struct sdhci_host *host = mmc->priv;
	u8 pwr = 0;
...
#endif
	.host_power_cycle = sdhci_power_cycle,
};
#else
