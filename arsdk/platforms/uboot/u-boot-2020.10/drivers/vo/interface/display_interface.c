...
{
   ar_always("interfae_type=%d",interface_type);
   ...
   switch(interface_type)
   {
	  ...
	  break;
	  default:
	  ar_err("not supported display interfaceb");
	  break;
   }
   return 0;
}
...
int set_display_interface(int interface_type, int subintf)
{
   ...
   ar_always("interfae_type=%d subintf=%d",interface_type, subintf);
   ...
   switch(interface_type)
   {
	  ...
	  break;
	  default:
	  ar_err("not supported display interface");
	  break;
   }
   return 0;
}
...
{
	ar_always("interfae_type=%d",interfae_type);
	display_intgerface_ops_t *interface_ops=NULL;
