...
int dsi_set_timing(STRU_DSI_CFG *cfg)
{
    ...
    float pixel_freq = 0;
    unsigned int dphy_clk_int = 0;
    unsigned int pixel_freq_int = 0;
#endif
    ...
    ar_always("dpi_bytes_per_pixel=%d, vid_num_lanes=%d, vid_null_size=%d, vid_pkt_size=%d, dphy_clk_mhz=%f, freq=%d",
              dpi_bytes_per_pixel, vid_num_lanes, vid_null_size, vid_pkt_size, cfg->dphy_clk_mhz, freq);
    ar_always("vid_hsa_time=%d, vid_hbp_time=%d, vid_hfp_time=%d, vid_hact_time=%d, vid_hline_time=%d",
              vid_hsa_time, vid_hbp_time, vid_hfp_time, vid_hact_time, vid_hline_time);
    ar_always("vid_vsa_lines=%d, vid_vbp_lines=%d, vid_vfp_lines=%d, vid_vactive_lines=%d",
              vid_vsa_lines, vid_vbp_lines, vid_vfp_lines, vid_vactive_lines);

    if ((ret = dphy_freq_conf_get(freq, &Info)) != 0)
    {
        ar_err("unsupport dphy freq(%f, %d) in conf table!", cfg->dphy_clk_mhz, freq);
        return -1;
    }
    ...
    dsi_reg_write(DSI_HOST_REG_VID_PKT_SIZE, vid_pkt_size); // VID_PKT_SIZE,the number of pixels in a signle video packet
    ...
    pixel_freq = cfg->dphy_clk_mhz * vid_num_lanes / 24.0;

    dphy_clk_int = (unsigned int)(cfg->dphy_clk_mhz * 1000000);
    pixel_freq_int = (unsigned int)(pixel_freq * 1000000);

    ar_debug("dphy_clk_mhz:%d pixel_freq:%d \n", dphy_clk_int, pixel_freq_int);

    ar9311_abb_set_mipi_pll(dphy_clk_int);

    ar_hal_set_pix_clk(pixel_freq * 2);
#endif
...
#elif defined(CONFIG_ARTOSYN_AR9311)
    dsi_reg_write(DPHY_REG_MIPI_ANALOG_TX0_1, 0x200);
#endif
...
#elif defined(CONFIG_ARTOSYN_AR9311)
    dsi_reg_write(DPHY_REG_MIPI_ANALOG_TX1_1, 0x200);
#endif
...
{
    uint8_t test_data[4] = {0};
    if (dsi_read(0x14, addr, test_data, 0x3) == 0)
    {
        int id = (test_data[3] << 24) | (test_data[2] << 16) | (test_data[1] << 8) | test_data[0];
        printf("panel_id : 0x%x \n", id);
        return id;
