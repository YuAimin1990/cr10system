    ...
    )
{
    ...
    ar_always("framebuffer->fb_format=0x%x",framebuffer->fb_format);
    ...
    )
{
    ...
    AR_SET_REG_BITS(config,dc->out_format,4,5);
    AR_SET_REG_BITS(config,dc->dvp_out_en,16,16);
    AR_SET_REG_BITS(config,dc->dsi_out_en,17,17);
    AR_SET_REG_BITS(config,1,20,20);                 // enable data fifo clear when one frame done
    AR_SET_REG_BITS(config,dc->rgb2yuv_en,0,0);
