...
#include <asm/gpio.h>
#endif
#if defined(CONFIG_USB_FUNCTION_ARTOUSB)
#include <asm/arch/boot.h>
#include <asm/arch/image.h>
#include <asm/arch/f_artousb.h>
#include <button.h>
#endif
#if defined(CONFIG_MMC) && defined(CONFIG_VO)
#include <vo/vo.h>
#endif
...
int is_boot_from_outside(void)
{
	...
	{
		printf("detect usb upgrade!\n");
		return 1;
	}
	else if (BOOT_DEVICE_UART == ((pHeader->boot_info & 0xffff0000) >> 16))
	{
		printf("detect uart upgrade!\n");
		return 1;
	}
	...
	else if (AR_BOOT_DEVICE_UART == ((pHeader->boot_info & 0xffff0000) >> 16))
	{
		printf("detect uart upgrade!\n");
		return 1;
	}
...
#if defined(CONFIG_USB_FUNCTION_ARTOUSB)
static int initr_usb_upgrade(void)
{
	...
	ret = button_get_by_label("Uboot upgrade", &dev);
	if (0 == ret)
	{
		if (BUTTON_ON == button_get_state(dev))
		{
			printf("detect usb upgrade!\n");
			artosyn_hid_upgrade(0);
		}
...
#if 0
static int gpio_set_direct(uint32_t group, uint32_t port, uint32_t inner_gpio, uint32_t dir)
{
	uint32_t addr;
	uint32_t value;

	if ((group >= MAX_GPIO_GROUP) || (port >= MAX_GPIO_PORT) ||
		(inner_gpio >= MAX_GPIO_PIN)) // group_num[port]))
		return -1;

	addr = GPIO_DIR_BASE + group * GPIO_GROUP_STEP + port * GPIO_PORT_DIR_STEP;
	printf("set GPIO_%c%d_%d(addr=0x%x) dir=%d", 'A' + port, group, inner_gpio, addr, dir);
...
static int gpio_set_val(uint32_t group, uint32_t port, uint32_t inner_gpio, uint32_t val)
{
	uint32_t addr;
	uint32_t value;
	...
	printf("set GPIO_%c%d_%d(addr=0x%x) val=%d", 'A' + port, group, inner_gpio, addr, val);
	value = read_reg32(addr);
	...
	printf("addr:%02x value:%02x", addr, value);
...
{
	printf("set PWR_KEY_CTL_OUT 0 \n");
	gpio_set_direct(0, 0, 1, GPIO_DIR_OUTPUT);
	gpio_set_val(0, 0, 1, GPIO_DATA_LOW);
	printf("set PWR_KEY_CTL_OUT 1 \n");
	gpio_set_direct(0, 0, 1, GPIO_DIR_OUTPUT);
	gpio_set_val(0, 0, 1, GPIO_DATA_HIGH);
	return 0;
}
