			   ...
			   ulong sector, struct image_header *header)
{
	...
	struct sirius_image_header *ar_header = (struct sirius_image_header *)spl_get_load_buffer(-0x1000, bd->blksz);
	unsigned long offset = 0;
	if(0x4152544f == ar_header->magic)
	{
		offset = sizeof(*ar_header) + ar_header->hash_size + ar_header->sig_len;
	}
	...
	image_size_sectors = (spl_image->size + offset + mmc->read_bl_len - 1) /
			     mmc->read_bl_len;
	/* Read the header too to avoid extra memcpy */
	count = blk_dread(mmc_get_blk_desc(mmc), sector, image_size_sectors,
			  (void *)((ulong)spl_image->load_addr - offset));
	debug("read %x sectors to %lx offset %lu\n", image_size_sectors,
	      spl_image->load_addr, offset);
	if (count != image_size_sectors) {
		printf("%s: error count = %ld\n", __func__, count);
		return -EIO;
	}
			      ...
			      struct mmc *mmc, unsigned long sector)
{
	...
	ar_header = (struct sirius_image_header *)spl_get_load_buffer(-0x1000, bd->blksz);

	/* read image header to find the image size & load address */
	count = blk_dread(bd, sector, 1, ar_header);
	...
	debug("ar_header read 0x%x %u %u\n", ar_header->magic, ar_header->sig_len, ar_header->hash_size);
	if(0x4152544f != ar_header->magic)
	{
	...
	if(secure_boot_enabled())
	{
		if(0 != spl_verify_sw("uboot", (unsigned char *)hash_buffer, ar_header->hash_size,
				...
				(unsigned char *)spl_image->load_addr, ar_header->img_len))
		{
		       ...
		       struct spl_boot_device *bootdev)
{
	...
	spl_mmc_load(spl_image, bootdev, NULL, spl_mmc_boot_partition(1), 0);
	spl_mmc_load(spl_image, bootdev, NULL, spl_mmc_boot_partition(0), 0);
	return 0;
#else
