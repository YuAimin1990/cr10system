/*
 * Artosync spacc driver for bootloader & uboot
 *
 * Support Algorithms:
 * AES CBC
 * SHA256, HMAC-SHA256
...
static int spacc_cipher_set_iv(unsigned char *iv, unsigned long length, unsigned long offs)
{
	...
	memcpy((void *)(unsigned long)(SPACC_CIPH_CTX_0 + SPACC_CIPH_CTX_SZ * job.key_ctx + AES_IV_OFFSET), (void *)iv, length);
...
{
	memcpy((void *)iv, (void *)SPACC_CIPH_CTX_0 + SPACC_CIPH_CTX_SZ * job.key_ctx + AES_IV_OFFSET, length);
}
...
static void spacc_start_cipher_operation(void)
{
	...
	spacc_write_reg32(SPACC_SRC_PTR, (unsigned int)(&job.src_ddt));
	spacc_write_reg32(SPACC_DST_PTR, (unsigned int)(&job.dst_ddt));
