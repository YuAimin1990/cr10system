...
/* fnamecopy, files, dirs export to use in the uboot upgrade */
static char fnamesBuf[1024];
static int filecnt;
static int copylen;
		...
		if (dent->attr & ATTR_VOLUME) {
			if ((dent->attr & ATTR_VFAT) == ATTR_VFAT &&
				...
				if (!dent)
					continue;
				itr->name = itr->l_name;
				/* for artosyn uboot upgrade only */
				if (itr->is_root && !fat_itr_isdir(itr)) {
					if (0 == memcmp(itr->l_name, "artosyn", 7) && (copylen + strlen(itr->l_name) + 1) < sizeof(fnamesBuf)) {
						memcpy(fnamesBuf + copylen, itr->l_name, strlen(itr->l_name));
						copylen += (strlen(itr->l_name) + 1);
						filecnt ++;
					}
				}
				break;
...
char *do_fat_getfnamecopy(int *files_cnt, int *dirs_cnt)
{
	if (files_cnt) {
		*files_cnt = filecnt;
	}
