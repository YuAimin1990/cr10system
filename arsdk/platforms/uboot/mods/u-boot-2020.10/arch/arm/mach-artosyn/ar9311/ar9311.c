...
static struct mm_region ar9311_mem_map[] = {
	{
		.virt = 0x0UL,
		.phys = 0x0UL,
		.size = 0x10000000UL,
...
struct mm_region *mem_map = ar9311_mem_map;
...
{
	int val = readl(ARTOSYN_BOOT_MODE_VALUE_REG) & 0x07;
...
u32 spl_boot_device(void)
{
	...
	writel(readl(BOOT_DEVICE_STATUS_BASE_REG) | 0x80000000, BOOT_DEVICE_STATUS_BASE_REG);
...
{
	/* set ip to nonsec */
	writel(0xff, 0x0842006c);
	writel(0x3ffff, 0x08420074);
}
...
static void tzc400_init(unsigned int base_addr,	//Base address
	...
	writel(action, (unsigned long)(base_addr + ACTION));
	writel(gate_keeper, (unsigned long)(base_addr + GATE_KEEPER));
	writel(spec_ctrl, (unsigned long)(base_addr + SPECULATION_CTRL));
}
...
static void tzc400_rgn_config(unsigned int base_addr,
	...
	writel(region_base_low, (unsigned long)(base_addr + RGN_BASE_LOW + 0x20 * region_num));
	writel(region_base_high, (unsigned long)(base_addr + RGN_BASE_HIGH + 0x20 * region_num));
	writel(region_top_low, (unsigned long)(base_addr + RGN_TOP_LOW + 0x20 * region_num));
	writel(region_top_high, (unsigned long)(base_addr + RGN_TOP_HIGH + 0x20 * region_num));
	writel(region_attribute, (unsigned long)(base_addr + RGN_ATTRIBUTE + 0x20 * region_num));
	writel(region_id_access, (unsigned long)(base_addr + RGN_ID_ACCESS + 0x20 * region_num));
}
...
static void secondary_cpu_poweron(void)
{
	int i;
	unsigned int tmp;

	memset(0x100000, 0, 64);

	writel(0xacce55, 0x0a106200);

	/* release cpu1 reset */
	tmp = readl(CPU_RESET_CTRL);
	writel((tmp | CPU_POR_RESET_1 | CPU_CORE_RESET_1), CPU_RESET_CTRL);

	tmp = readl(0x0a102024);
	writel(tmp & ~(1 << 5 |1 << 1), 0x0a102024);

	tmp = readl(0x0a106204);
	tmp |= (1 << 17);
	writel(tmp, 0x0a106204);
	tmp &= ~(1 << 1);
	writel(tmp, 0x0a106204);
	tmp &= ~(1 << 9);
	writel(tmp, 0x0a106204);
...
int arch_cpu_init(void)
{
	...
	//reset normal wdt
	tmp = readl(0x01a00000);
	tmp &= ~(1 << 29);
	writel(tmp, 0x01a00000);

	//reset sec wdt
	tmp = readl(0x0842007c);
	tmp &= ~(1 << 0);
	writel(tmp, 0x0842007c);
	udelay(5);
	tmp = readl(0x0842007c);
	tmp |= (1 << 0);
	writel(tmp, 0x0842007c);
	...
	writel(0xf, 0x00402c68);
	writel(0xf, 0x00402c88);
	writel(0xf, 0x00402ca8);
	writel(0xf, 0x00402cc8);

	/* set secure ip to nonsecure */
	writel(0xf, 0x00403868);
	/* set spacc to nonsecure */
	writel(0xf, 0x00405068);
	/* set noc_main nonsecure */
	writel(0xf, 0x00406868);

	/* set ip to sec */
	writel(0x0, 0x0842006c);
	writel(0x0, 0x08420074);
	...
	if(AR_BOOT_DEVICE_USB == ((pHeader->boot_info & 0xffff0000) >> 16) ||
	   AR_BOOT_DEVICE_UART == ((pHeader->boot_info & 0xffff0000) >> 16))
	{
		int ret, offset=-1;
		unsigned int user_modify[39];
		...
		if (offset >= 0) {
			ret = fdtdec_get_int_array(gd->fdt_blob, offset, "user-modify0",
					   user_modify, 39);

			if(0 == ret) {
				gd->ram_size = (phys_size_t)user_modify[3] * 128;
				printf("DRAM size is: 0x%08lx, 0x%08x \n", gd->ram_size, user_modify[3]);
			}else {
				printf("ret :%d %d\n", ret);
			}
		}
...
//	gd->ram_top = board_get_usable_ram_top(gd->ram_size);
	   ...
	   AR_BOOT_DEVICE_UART == ((pHeader->boot_info & 0xffff0000) >> 16))
	{
		...
		uboot_entry = (void (*)(void))((unsigned long)(image_get_ep(header)));
		...
		if(uboot_entry != (void (*)(void))uboot)
		{
