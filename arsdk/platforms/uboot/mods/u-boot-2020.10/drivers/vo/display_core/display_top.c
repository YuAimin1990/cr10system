...
static display_handle_t ar_creat_display_layer(void)
{
   static int layer_num=0;
   layer_num++;
   if(layer_num<=2){
      disp_obj_t *disp_obj=ar_malloc(sizeof(disp_obj_t));
      memset(disp_obj, 0,sizeof(disp_obj_t));
      return (display_handle_t)disp_obj;
   }else
   {
      ar_err("can not creat layer.");
	  return 0;
   }
}
display_handle_t ar_get_display_layer(int layer_id)
{
   ar_lock(get_display_server()->lock);
   display_handle_t handle=find_handle_by_layer_id(layer_id);
   if(!handle)
   {
       handle=ar_creat_display_layer();
	   register_handle_to_server(handle);
       ar_init_display(handle,layer_id);
   }
   ar_unlock(get_display_server()->lock);
   return handle;
}

static int flush_disp_buffer(disp_obj_t *obj)
{
	ar_queue_t *disp_done_queue=(ar_queue_t *)obj->disp_done_queue;
	ar_queue_t *disp_queue=(ar_queue_t *)obj->disp_queue;
 	void *buffer=NULL;
...
{
	ar_queue_t *disp_done_queue=(ar_queue_t *)obj->disp_done_queue;
	ar_queue_t *hold_odd=(ar_queue_t *)obj->hold_queue_odd;
	ar_queue_t *hold_even=(ar_queue_t *)obj->hold_queue_even;
	void *buffer=NULL;
...
static void disp_done_queue_insert_wrapper(disp_obj_t *obj, ar_queue_t * done_queue, display_buffer_t *buffer)
{
    ...
    #if 0
    ar_printf("id %u, done %u, show %u, cfg %u delay time %d\n",
             buffer->frame_id,
...
int ar_display_res_get(display_hal_t *hal, display_interface_res_infor_t *res_infor,
                                        disp_res_infor_t *timing_info, AR_BOOL *interlace)
{
    int ret = AR_HAL_VO_SUCCESS;
    int   index = 0;

    if (hal->vo_dev.attr.e_timing_template > AR_HAL_VO_OUTPUT_USER)
    {
    ...
    if (hal->vo_dev.attr.e_timing_template == AR_HAL_VO_OUTPUT_USER)
    {
        ...
        timing_info->h_total_pixels = hal->vo_dev.attr.timing_customize.hpw +
                                      hal->vo_dev.attr.timing_customize.hbp +
                                      hal->vo_dev.attr.timing_customize.hdp +
                                      hal->vo_dev.attr.timing_customize.hfp;

        timing_info->start_hsync   =  hal->vo_dev.attr.timing_customize.hdp +
                                      hal->vo_dev.attr.timing_customize.hfp;

        timing_info->end_hsync     =  hal->vo_dev.attr.timing_customize.hdp +
                                      hal->vo_dev.attr.timing_customize.hfp +
                                      hal->vo_dev.attr.timing_customize.hpw;

        timing_info->v_valid_lines  = hal->vo_dev.attr.timing_customize.vdp;

        timing_info->v_total_lines  = hal->vo_dev.attr.timing_customize.vpw +
                                      hal->vo_dev.attr.timing_customize.vbp +
                                      hal->vo_dev.attr.timing_customize.vdp +
                                      hal->vo_dev.attr.timing_customize.vfp;

        timing_info->start_vsync    = hal->vo_dev.attr.timing_customize.vdp +
                                      hal->vo_dev.attr.timing_customize.vfp;

        timing_info->end_vsync      = hal->vo_dev.attr.timing_customize.vdp +
                                      hal->vo_dev.attr.timing_customize.vfp +
    ...
    else
    {
        ...
        if (res == NULL)
        {
            ar_err("error timing template %d",hal->vo_dev.attr.e_timing_template);
            ret = HAL_ERR_VO_DEV_TIMING_TEMPLATE;
            goto End;
        }
        ...
        if(index>=size)
        {
            ar_err("can not find a timing for this res, (%d %d %d)",res->width,res->height,(int)(res->fps*10000));
            ret = HAL_ERR_VO_DEV_TIMING_TEMPLATE;
            goto End;
        }
...
{
    display_handle_t *handle = ar_get_display_layer(AR_HAL_VO_LAYER_ID_VIDEO_0);
    ...
    ret = ar_display_res_get(hal, &obj->res_infor, &obj->res_timing, (AR_BOOL *)&obj->interlace_mod);
    if (ret != AR_HAL_VO_SUCCESS)
        goto End;
    ...
    if(bit_count_per_channel == 0)
    {
        ...
        ar_always("use hal bit_count_per_channel=%d", bit_count_per_channel);
    }
    ...
    {
        ar_err("err bit_count_per_channel=%d",obj->res_infor.bit_count_per_channel);
        out_bitdepth=DPI_D24;
    }

    ar_always("out_bitdepth=%d",out_bitdepth);
    ...
    {
        ar_always("use yuv csc");
        ar_display_set_csc(handle,AR_VIDEO_FORMAT_RGB,ar_display_hal_csc_trans(hal->vo_dev.e_csc), 0);
    }
    else
    {
        ar_always("use rgb csc");
        ar_display_set_csc(handle,AR_VIDEO_FORMAT_RGB,AR_VIDEO_MATRIX_BT709_LIMIT,0);
    }
    #else
    ar_display_set_csc(handle,AR_VIDEO_FORMAT_RGB,ar_display_hal_csc_trans(hal->vo_dev.e_csc), 0);
    #endif
    ...
    //set interlace
    if (obj->interlace_mod != AR_SYSTEM_INTERLACE_MOD_NULL)
    {
        ...
        *(unsigned int *)0x645B00A4 = (unsigned int )0xd9183;       // BT656
        ...
        ar_always("display bufconfig =%08x\n",*(unsigned int *)0x645c1518 );
    }

    power_on_display(obj);
    ar_always("vo dev init finished.");
...
{
    disp_obj_t *obj=(disp_obj_t *)ar_get_display_layer(AR_HAL_VO_LAYER_ID_VIDEO_0);
    int ret = AR_HAL_VO_SUCCESS;

    dc_display_reset(obj);    // clear all register
    obj->current_display_interface=get_current_display_interface();
    reset_display_interface(obj->current_display_interface);
    ar_always("vo dev deinit finished.");

    return ret;
}

int ar_init_display(display_handle_t handle,int overlay_id)
{
    ...
    obj->interlace_mod=AR_SYSTEM_INTERLACE_MOD_NULL;
    ...
    obj->display_post_proc=(void *)ar_create_signal();
    obj->frame_buffer_vsync=(void *)ar_create_signal();
    //init the queue
    obj->disp_queue=ar_creat_queue(16,"display_queue");
    obj->disp_done_queue=ar_creat_queue(16,"display_done_queue");
    obj->hold_queue_odd=ar_creat_queue(16,"hold_queue_odd");
    obj->hold_queue_even=ar_creat_queue(16,"hold_queue_even");
    obj->queue_lock = ar_creat_lock();
...
int ar_disp_get_buffer_width(uint32_t width)
{
  ...
  ar_always("width=%d ret_width=%d bit_num_per_burst=%d bitdepth=%d burst_num=%d ",\
    width,ret_width,bit_num_per_burst,bitdepth,burst_num);
  return ret_width;
}

int ar_disp_get_buffer_width_div2(uint32_t width)
{
  uint32_t bit_num_per_burst=128;
  uint32_t bitdepth=8;
...
int ar_cfg_display_interlace_mode(display_handle_t handle,int interlace_mode)
{
	...
	ar_always("interlace_mod=%d",obj->interlace_mod);
    ...
    if(obj->overlay_id==0)
    {
        ...
        if(obj->interlace_mod==AR_SYSTEM_INTERLACE_MOD_NULL)//progressive
        {
        ...
        }else
        {
            ...
            {
                case AR_SYSTEM_INTERLACE_MOD_1080I:
                    gpio_setpin(obj->cpld_sd_hd_pin_num, 1);
                break;
                case AR_SYSTEM_INTERLACE_MOD_720_480I_NTSC:
                case AR_SYSTEM_INTERLACE_MOD_720_576I_PAL:
     ...
     {
            ar_always("over layer layer, not to cfg fpga");
     }
#endif
	 return 0;
}
...
void ar_display_zoom(display_handle_t handle,float zoom)
{
	...
	obj->dc_inst.zoom_addr_offset[0]=(obj->dc_inst.framebuffer.height-h)/2*obj->dc_inst.framebuffer.fb_stride[0]+(obj->dc_inst.framebuffer.width-w)/2;
    obj->dc_inst.zoom_addr_offset[2]=obj->dc_inst.zoom_addr_offset[1]=(obj->dc_inst.framebuffer.height-h)/4*obj->dc_inst.framebuffer.fb_stride[1]+(obj->dc_inst.framebuffer.width-w)/4;
    ar_always("zoom=%d w=%d h=%d offset(%d %d %d)",float2int(zoom),w,h,\
...
int ar_display_interface_cfg_csc(int csc_pra)
{
    ...
    disp_obj_t *obj=(disp_obj_t *)handle;
    obj->csc_pra = csc_pra;
    int ret = set_display_interface_ctl(obj->current_display_interface, SET_DVP_OUT_CSC_PRA,&obj->csc_pra,sizeof(obj->csc_pra));
    return ret;
}
...
int ar_pause_display(display_handle_t handle)
{
	...
	ar_always("stop the display hw .....");

    ar_lock(obj->queue_lock);
    obj->start_flag=0;
...
int ar_display_set_csc(display_handle_t handle,int format,int convert_matric, int change_range)
{
    ...
    ar_always("csc format %d, matrix %d change_range: %d",format, convert_matric, change_range);

    if ((change_range) &&
          (convert_matric == AR_VIDEO_MATRIX_BT601_FULL) &&
            (convert_matric == AR_VIDEO_MATRIX_BT709_FULL) &&
              (convert_matric == AR_VIDEO_MATRIX_NODATA_FULL) )
    {
        ar_err("full range can't change csc");
        return -1;
    }

    return set_display_interface_ctl(obj->current_display_interface,DISPLAY_INTERFACE_CTL_SET_HDMI_CSC, &csc,sizeof(hdmi_csc_t));
}
...
int ar_display_set_layer_csc(display_handle_t handle, STRU_AR_HAL_VO_LAYER_CSC *csc)
{
    disp_obj_t *obj=(disp_obj_t *)handle;
    int  ret = AR_HAL_VO_SUCCESS;
    ar_always("set layer(%d) csc  input:%d, output:%d.", obj->overlay_id,
...
static void display_first_overlay_frame(disp_obj_t *obj,display_buffer_t *buffer)
{
    ar_always("display_first overlay frame");
...
static void display_video_first_frame(disp_obj_t *obj,display_buffer_t *buffer)
{
    ...
    {
         fb->scale = 1;
         ar_always("vo layer's rect(%d, %d) does not match the vo dev's(%d, %d).",
                 obj->width, obj->height, obj->dc_inst.panel_hline, obj->dc_inst.panel_vline);
    }

    ar_always("frame_width=%d frame_height=%d hline=%d vline=%d",
                obj->width, obj->height, obj->dc_inst.panel_hline, obj->dc_inst.panel_vline);
    ar_always("enter disp y_stride=0x%x,cb_stride=0x%x,cr_stride=0x%x\n",y_stride,cb_stride,cr_stride);
    obj->dis_frame_num = 0;
    obj->adjust_status=0;
    ...
    ar_always("First video frame show.");
    ar_func_exit();
 }

int ar_display_layer_enable(display_handle_t handle,int width,int height,int format,float fps, int luma_stride, int chroma_stride)
{
    disp_obj_t *obj=(disp_obj_t *)handle;
    if(obj->start_flag)
    {
        ar_err("layer has already enabled!");
    ...
    ar_always("width=%d height=%d y stride %u, chroma stride %u.format=%d fps=%d", width,height,obj->y_stride, obj->cbcr_stride,format,(int)fps);
...
int ar_display_layer_disable(display_handle_t handle)
{
    ...
    //display_buffer_t *buffer=NULL;
    //int x=0;
	ar_always("disable the layer %d.", obj->overlay_id);
    dc_display_pause(obj);

    obj->start_flag=0;
    ar_display_layer_flush_all_buffer(handle, AR_TRUE);
...
static int ar_display_enqueue_normal(display_handle_t handle,display_buffer_t *buffer)
{
  int status=0;
  disp_obj_t *obj=(disp_obj_t *)handle;
...
//  uint32_t x=0;
  ...
  if(obj->is_first_frame==1)
  {
	 ...
	 if(obj->overlay_id==0){
		 ar_always("start the display hw, delay %d .....", delay_time);
	 ...
	 obj->repeat_count=0;
	 obj->dis_irq_count=0;
     obj->first_time=ar_get_timestamp_us()*1000000;
	 obj->first_deta=0;

  }else{
    //cfg the next address
	dc_set_frameaddr2(&obj->dc_inst,buffer);
	buffer->timestamps_us=ar_get_timestamp_us()*1000000;
    if(obj->next_buf){ //set the lasted queue buffer, as next buffer ,so back the last next buffer
        obj->dis_loss_count++;
        disp_done_queue->queue_insert(disp_done_queue,obj->next_buf);
    }
    obj->next_buf=buffer;
    obj->next_buf_cfg_time=ar_get_timestamp_us()*1000000;
  }
  ar_unlock(obj->queue_lock);
  return status;
}

static int ar_display_enqueue_normal_fifo(display_handle_t handle,display_buffer_t *buffer)
{
  int status=0;
  disp_obj_t *obj=(disp_obj_t *)handle;
//  uint32_t x=0;
  ...
  buffer->timestamps_enqueue = ar_get_timestamp_us();
  ...
  if(disp_queue->get_queue_size(disp_queue) > obj->fifo_depth) {
    ...
    tmp_buffer->timestamps_done = ar_get_timestamp_us();
    obj->dis_loss_count++;
  ...
  if(obj->is_first_frame==1)
  {
     ...
     obj->dis_loss_count=0;
     obj->first_time=ar_get_timestamp_us()*1000000;
     obj->first_time=ar_get_timestamp_us();
     obj->current_buf=cfg_buffer;
	 obj->next_buf=NULL;
     cfg_buffer->timestamps_cfg = ar_get_timestamp_us();
...
int ar_display_enqueue(display_handle_t handle,display_buffer_t *buffer)
{
    ...
    if(obj->src_push_first_time==0)
        obj->src_push_first_time=ar_get_timestamp();

    obj->src_push_cnt++;
    if (obj->src_push_cnt == 1000) {
        obj->src_push_cnt = 0;
        end_time = ar_get_timestamp();
	...
	if(obj->interlace_mod==AR_SYSTEM_INTERLACE_MOD_NULL)
	{
        if(obj->enable_fifo_mode)
	        ar_display_enqueue_normal_fifo(handle,buffer);
        else
            ar_display_enqueue_normal(handle,buffer);
	}else
	{
		//ar_display_enqueue_interlace(handle,buffer);
	}
	return 0;
}
...
static void power_on_display(disp_obj_t *obj)
{
	 ...
	 AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),1,0,0);
	 AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),1,1,1);
	 AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),0,2,2);
     AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),1,12,12);    // enable display clock
#elif defined(CONFIG_ARTOSYN_AR9311)
    if (obj->current_display_interface != DISPLAY_INTERFACE_MIPI)
    {
        fre=dc_power_on(p_dc);
    }

    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),1,0,0);
    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),1,1,1);
    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),0,2,2);
    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),1,12,12);    // enable display clock
#endif
	 ar_exit_critical();
...
void ar_display_init(void)
{
    if(!display_server)
    {
        display_server=ar_malloc(sizeof(display_server_t));
        memset(display_server,0,sizeof(display_server_t));
        display_server->lock=ar_creat_lock();

        /* hal init */
        display_server->hal.lock = ar_creat_lock();
...
int uboot_ar_display_server_init(void)
{
    ar_display_init();
   // ar_vo_test();
    ...
    for (int i=0; i<3; i++)
    {
        ar_always("uboot_ar_display_server_init %d", i);
        ar_delay(500);
    }
#endif
    return 0;
...
static int ar_display_probe(struct udevice *dev)
{
    printf("abc probe\r\n");
    return 0;
}

static int ar_display_bind(struct udevice *dev)
{
    printf("abc bind\r\n");
    return 0;
}
