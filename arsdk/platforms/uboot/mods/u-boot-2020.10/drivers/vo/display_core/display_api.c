...
static const STRU_DC_CSC layer_csc_reg_tbl[AR_HAL_VO_LAYER_ID_MAX] = {
    {
...
static const STRU_DC_CSC_VALUE layer_csc_reg_value_tbl[AR_HAL_VO_CSC_DEFAULT] = {
    {
...
static const STRU_DC_CSC_VALUE output_csc_reg_value_tbl[AR_HAL_VO_CSC_DEFAULT] = {
    {
...
static int register_display_obj(disp_obj_t *obj)
{
   ...
   for(i=0;i<MAX_DISPLAY_OBJ_NUM;i++)
   {
       ...
       {
          ar_err("obj %p have register",obj);
          return -1;
       }else if(g_display_obj[i]==NULL)
       {
          g_display_obj[i]=obj;
		  ar_always("find a slot %d to register",i);
		  return 0;
       }
   }
   ar_err("no slot for display obj");
   return -1;
}
...
static int unregister_display_obj(disp_obj_t *obj)
{
   ...
   for(i=0;i<MAX_DISPLAY_OBJ_NUM;i++)
   {
       if(g_display_obj[i]==obj)
       {
          ar_err("obj %p have register, unregister it",obj);
          return 0;
       }
   }
   ar_err("the obj %p not register, can not unregister",obj);
   return -1;
}
#endif
...
//////////////////////////////////////////////////////////////////////////////////
/////////////////////////// APIs /////////////////////////////////////////////////
void dc_hard_reset(void)
{
    /* There is a mutual exclusion problem here, but ge2d may call this function first.
     in case ge2d hung when reset display */
    ar_always("hard reset the display!");

#ifdef CONFIG_ARTOSYN_AR9301
    AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),0,9,10);       // cclk pclk sw_reset
    AR_SET_REG_BITS(__REG32__(CGU_REG_BASE+0x4018),0x03,9,10);    // cclk pclk sw_reset
#elif defined(CONFIG_ARTOSYN_AR9311)
    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),0,9,10);       // cclk pclk sw_reset
    AR_SET_REG_BITS(__REG32__(SYSCTRL_REG_BASE+0x6018),0x03,9,10);    // cclk pclk sw_reset
#endif
...
void dc_display_reset(disp_obj_t *obj)
{
	...
	if(obj->overlay_id==0){
	  ...
	  #if 1
	   ar_always("soft reset the display");
...
//	   reg_interrupt_set(&obj->dc_inst);
	...
	{
	   ar_always("reset the display overlay, do nothing");
	}
}
...
//    unsigned int i;
    ...
    unsigned int *kernel_p=ar_malloc(128*sizeof(int));
...
int dc_set_output_csc(int csc_type)
{
    ...
    if (csc_type > AR_HAL_VO_CSC_DEFAULT)
        return -1;
    else if (csc_type == AR_HAL_VO_CSC_DEFAULT)
        csc_type = AR_HAL_VO_CSC_BT709_FULL;

    csc_reg = &output_csc_reg_tbl;
    csc_val = &output_csc_reg_value_tbl[csc_type];

    ar_always("csc_type=%d", csc_type);
    ...
    AR_SET_REG_BITS(reg_val,csc_val->YClipLow,0,7);
    AR_SET_REG_BITS(reg_val,csc_val->YClipHigh,16,23);
    reg_write_dc(csc_reg->YClip,reg_val);

    reg_val=reg_read_dc(csc_reg->UClip);
    AR_SET_REG_BITS(reg_val,csc_val->UClipLow, 0, 7);
    AR_SET_REG_BITS(reg_val,csc_val->UClipHigh,16,23);
    reg_write_dc(csc_reg->UClip,reg_val);

    reg_val=reg_read_dc(csc_reg->VClip);
    AR_SET_REG_BITS(reg_val,csc_val->VClipLow ,0 ,7);
    AR_SET_REG_BITS(reg_val,csc_val->VClipHigh,16,23);
    reg_write_dc(csc_reg->VClip,reg_val);

    return 0;
}

int dc_set_layer_csc(ENUM_AR_HAL_VO_LAYER_ID LayerId, ENUM_AR_HAL_VO_CSC csc_type)
{
    unsigned int reg_val=0;
    const STRU_DC_CSC  *csc_reg;
    const STRU_DC_CSC_VALUE  *csc_val;

    if (LayerId >= AR_HAL_VO_LAYER_ID_MAX)
        return -1;
    ...
    ar_always("layerid = %d, csc_type=%d", LayerId, csc_type);
	...
	)
{
    ...
    if(index>=size)
    {
      ar_err("can not find  a timing for this res, (%d %d %d)",hline,vline,(int)(fps*10000));
	  ar_err("so we find a 1920 1080 60 as default");
	  for(index=0;index<size;index++)
	  {
    ...
    ar_always("index=%d %d %d %d %d %d %d %d %d %d",index,dc->panel_hline,dc->panel_htotal,dc->panel_hsync_start,dc->panel_hsync_end,\
    ...
    )
{
    ...
    ar_always("%d %d %d %d %d %d %d %d %d",dc->panel_hline,dc->panel_htotal,dc->panel_hsync_start,dc->panel_hsync_end,\
...
{
#if defined(CONFIG_ARTOSYN_AR9311)
     float source_clk=0;
     int mult_cnt = 2;
     int ret;
#endif
	 ...
	 {
	    fre=((dc->panel_htotal*dc->panel_vtotal*dc->fps/2)+(dc->panel_htotal*(dc->panel_vtotal+1)*dc->fps/2))/1000.0/1000.0;
	 }
	 #if 0
     ...
     int clk=ar_clk_get(AR_CLK_MOD_DISP);
	 ar_clk_set(AR_CLK_MOD_DISP,clk);
     ...
     *((volatile uint32_t *)0x60630000)=tmp;
     ...
     ar_debug("0x60630000=0x%x 0x60634018=0x%x", *((volatile uint32_t *)0x60630000),*((volatile uint32_t *)0x60634018));
     ar_hal_set_pix_clk(fre);
	 #endif
     ar_always("fre2: %f",  fre);
     //ar9301_cgu_init();
#ifdef CONFIG_ARTOSYN_AR9301
     ar_hal_set_pix_clk(fre*2);
#elif defined(CONFIG_ARTOSYN_AR9311)
    source_clk = fre*2;
    /* source_clk must more than 500MHz if choose MIPI_TX_PLL_DIV2.*/
    while(source_clk <= 500)
    {
        source_clk = fre*mult_cnt;
        mult_cnt+=2;
    }

    ret = ar9311_abb_set_mipi_div2(source_clk * 1000000);
    if(0 != ret)
    {
        ar_err("claim MIPI_TX_PLL_DIV2 %f MHz failed, ret:%d\n", source_clk, ret);
    }
...
void dc_init(disp_obj_t *obj,DisNotify notify)
{
    ...
    ar_always("dc init ,register irq process notify");
	ret=register_display_obj(obj);
    if(ret)
    {
        ar_err("can not register obj, create more than 2 layer");
		return;
    }

    if(obj->overlay_id==0)
    {
        ar_always("dc init overlay id ==0 ,register irq funciton");
       // ar_irq_reg(DISPLAY_ENG_IRQ_NUM, dc_irq, NULL);
    ...
    {
       ar_always("not back layer, not ops irq overlay_id=%d",obj->overlay_id);
    }
	//we malloc cursor mem
	 ar_always("dc init ,register irq process notify exit");
}
