...
#define AR_CLK_ENABLE_I2S_CLK_FOR_SENSOR 0x40b00018

#define AR_CLK_AU_PLL_0    (0x6063213c)
#define AR_CLK_AU_PLL_1    (0x60632144)

#define AR_CLK_AU_PLL_DIG_REG_0   (0x60632A30)
#define AR_CLK_AU_PLL_DIG_REG_1   (0x60632A34)
...
static int ar_cfg_get_dvp_timing(int width,int height,float fps);
enum tp2803_timing
{
...
static int 	tp2803_i2c_write(int address,int data)
{
    ...
    //ar_always("address=0x%x, w_data=0x%x,read_data=0x%x",address,data,read_data);
	return 0;
}
...
static void tp2803_reset(int delay1, int delay2,int delay3)
{
      ar_always("enter %d %d %d",delay1,delay2,delay3);
	  pin_share_config(tp2803_rest_pad, THIS_PAD_GPIO_MODE);
	  sirius_gpio_set_direction(tp2803_rest_group,tp2803_rest_port,tp2803_rest_number, GPIO_IS_OUTPUT);
	  sirius_gpio_set_value(tp2803_rest_group,tp2803_rest_port, tp2803_rest_number, GPIO_IS_HIGH);
	  ar_delay(delay1);
	  sirius_gpio_set_value(tp2803_rest_group,tp2803_rest_port, tp2803_rest_number, GPIO_IS_LOW);
	  ar_delay(delay2);
	  sirius_gpio_set_value(tp2803_rest_group,tp2803_rest_port, tp2803_rest_number, GPIO_IS_HIGH);
	  ar_delay(delay3);
      ar_always("exit");
...
static int ar_cfg_get_dvp_timing(int width,int height,float fps)
{
   enum tp2803_timing input_opt = TP2803_1920_1080_30P;
	   // config dvp
   ar_always("fps = %d with w=%d h=%d", (int)fps,width,height);
   if((width==1920) && (height== 1080)) {
	 ...
	 {
	    ar_err("not supported fps = %d with w=%d h=%d", (int)fps,width,height);
	 }
   }
   ...
   ar_always("input_opt=%d",input_opt);
...
static void tp2803_set_timing_v0(enum tp2803_timing vmode)
{
  ...
  ar_always("read 0x00:%d,excepted 0x08 vmode=%d",data,vmode);
...
static int set_i2s_MCLK_2803(int freq)
{
	...
	// ar_always("divr_manual_float %d/10000", (int)(divr_manual_float*10000));
	divr_manual = (unsigned int)( divr_manual_float*(1<<PLL_DIVR_INTER_POS) );

	ar_always("input clk %d/10000 - divr manual 0x%x", (int)(freq*10000), divr_manual);

	//set
	int val = read_reg32(AR_CLK_ENABLE_I2S_CLK_FOR_SENSOR);
	val |= (1<<4); //i2s1 -> master
	val &= ~(1<<19);
	val &= ~(1<<28);
	val &= ~(0x7 << 20);

	write_reg32(AR_CLK_ENABLE_I2S_CLK_FOR_SENSOR, val);    //enable PLL

	write_reg32(AR_CLK_AU_PLL_0, 0x40);//B79
	write_reg32(AR_CLK_AU_PLL_1, 0x08);//B81 + PosDiv2 = 1

	// write_reg32(AR_CLK_AU_PLL_DIG_REG_0, 0x00ED9999);
	write_reg32(AR_CLK_AU_PLL_DIG_REG_0, divr_manual);
	write_reg32(AR_CLK_AU_PLL_DIG_REG_1, 0x0000501E);
	write_reg32(AR_CLK_AU_PLL_DIG_REG_1, 0x00305016);
	write_reg32(AR_CLK_AU_PLL_DIG_REG_1, 0x0030501E);
...
*/
		...
		//ar_clk_set_i2s_clock_for_sensor_double(27);
		tp2803_i2c_init();
		tp2803_reset(10,30,10);
		tp2803_set_timing(ar_cfg_get_dvp_timing(res_infor->h_valid_pixels, res_infor->v_valid_lines, res_infor->fps));
