...
void ar_ite_hdmi_tx_init(void *args);
...
static int ar_cfg_get_dvp_timing(int width,int height,float fps)
{
   ...
   ar_always("dvp timing %d.", input_opt);
...
static int set_display_interface_res_infor_dev(disp_res_infor_t *res_infor)
{
    if(res_infor)
	{
        ...
        {
          case AR_VIDEO_MATRIX_NODATA_FULL:
          {
              user_itu=2;
              user_limit=0;
          }
          break;
          case AR_VIDEO_MATRIX_NODATA_LIMIT:
          {
             user_itu=2;
             user_limit=1;
          }
          break;
          case AR_VIDEO_MATRIX_BT601_FULL:
          {
              user_itu=0;
              user_limit=0;
          }
          break;
          case AR_VIDEO_MATRIX_BT601_LIMIT:
          {
             user_itu=0;
             user_limit=1;
          }
          break;
          case AR_VIDEO_MATRIX_BT709_FULL:
          {
             user_itu=1;
             user_limit=0;
          }
          break;
          case AR_VIDEO_MATRIX_BT709_LIMIT:
          {
             user_itu=1;
             user_limit=1;
          }
          break;
          default:
          ar_err("not supported color matric");
          break;
        }
        switch(hdmi_tx_dev->format)
        {
          case AR_VIDEO_FORMAT_RGB:
            hdmi_format=0; //HDMI_RGB444;
            break;
          case AR_VIDEO_FORMAT_Y444:
            hdmi_format=1;//HDMI_YUV444;
            break;
          case AR_VIDEO_FORMAT_Y42B:
		...
		//if(!get_display_dts("ui_mode"))
        {
            ...
            ar_always("orig = %d, user_itu=%d user_limit=%d hdmi_format=%d change_range=%d",
            ...
            hdmi_tx_set_timing(ar_cfg_get_dvp_timing(res_infor->h_valid_pixels, res_infor->v_valid_lines, res_infor->fps),hdmi_format);
		}
...
static int set_display_interface_ctl_dev(int clt_code,void *pra, int size)
{
   ...
   switch(clt_code)
   {
      case DISPLAY_INTERFACE_CTL_SET_HDMI_CSC:
      {
          ...
          ar_always("csc format %d, matrix %d change_range %d",
