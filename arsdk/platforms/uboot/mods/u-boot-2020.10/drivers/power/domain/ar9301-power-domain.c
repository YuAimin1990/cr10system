...
#define PWR_CTRL_ID_MASK                            GENMASK(AR9301_PWR_CTRL_GATING_SHIFT-1, 0)
#define PWR_CTRL_STATUS_MASK                        GENMASK(31, AR9301_PWR_CTRL_GATING_SHIFT)
...
static struct pwr_ctrl_tab ar_pwr_ctrl_tab[AR9301_PWR_CTRL_MAX] =
{
	[AR9301_PWR_CTRL_SOC_VENC] = {
...
static int pwr_ctrl_power_off(struct power_domain *power_domain)
{
    ...
    switch(power_domain->id)
    {
        ...
        case AR9301_PWR_CTRL_SOC_VENC:
        case AR9301_PWR_CTRL_SOC_ISP:
        case AR9301_PWR_CTRL_SOC_DLA:
        case AR9301_PWR_CTRL_A53_CORE1:
        case AR9301_PWR_CTRL_A53_CORE2:
        case AR9301_PWR_CTRL_A53_CORE3:
        case AR9301_PWR_CTRL_CEVA_CORE0:
            ...
            ret = readl_poll_timeout(ack_addr, tmp, PWR_CTRL_POWEROFF_ACKED(tmp, tab->bit_offset[PWR_CTRL_BIT_OFFSET_TYPE_ACK_M]), PWR_CTRL_POLL_TIMEOUT_INTERVAL);
            if (ret < 0)
            {
            ...
            ret = readl_poll_timeout(ack_addr, tmp, PWR_CTRL_POWEROFF_ACKED(tmp, tab->bit_offset[PWR_CTRL_BIT_OFFSET_TYPE_ACK_D]), PWR_CTRL_POLL_TIMEOUT_INTERVAL);
            if (ret < 0)
            {
...
static int pwr_ctrl_power_on(struct power_domain *power_domain)
{
    ...
    switch(power_domain->id)
    {
        ...
        case AR9301_PWR_CTRL_CEVA_CORE0:
            ...
            ret = readl_poll_timeout(ack_addr, tmp, PWR_CTRL_POWERON_ACKED(tmp, tab->bit_offset[PWR_CTRL_BIT_OFFSET_TYPE_ACK_M]), PWR_CTRL_POLL_TIMEOUT_INTERVAL);
            if (ret < 0)
            {
                log_err("%s: %lu mother not acked, ret=%x\n", __func__, power_domain->id, ret);
                return ret;
            ...
            ret = readl_poll_timeout(ack_addr, tmp, PWR_CTRL_POWERON_ACKED(tmp, tab->bit_offset[PWR_CTRL_BIT_OFFSET_TYPE_ACK_D]), PWR_CTRL_POLL_TIMEOUT_INTERVAL);
            if (ret < 0)
            {
                log_err("%s: %lu daughter not acked, ret=%x\n", __func__, power_domain->id, ret);
                return ret;
...
static int ar_pwr_ctrl_init(struct udevice *dev)
{
    u32 array[AR9301_PWR_CTRL_MAX] = {0};
    u32 size = 0;
    u32 i = 0;
    int ret = 0;
    u32 id = 0;
...
static int ar_pwr_ctrl_probe(struct udevice *dev)
{
...
static int ar_pwr_ctrl_remove(struct udevice *dev)
{
...
static int ar_pwr_ctrl_ofdata_to_platdata(struct udevice *dev)
{
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
    struct pwr_ctrl_plat *plat = dev_get_platdata(dev);
    struct pwr_ctrl *pcd = dev_get_priv(dev);
    ...
    pcd->tab = ar_pwr_ctrl_tab;

    ar_pwr_ctrl_init(dev);
#endif
    return 0;
}

static const struct udevice_id ar_pwr_ctrl_ids[] = {
    {
        .compatible = "artosyn,ar9301-pwr_ctrl",
    },
    { /* sentinel */ }
};

struct power_domain_ops ar_pwr_ctrl_ops = {
...
U_BOOT_DRIVER(ar_pwr_ctrl) = {
    .name = "ar_pwr_ctrl",
    .id = UCLASS_POWER_DOMAIN,
    .ops = &ar_pwr_ctrl_ops,
    .probe = ar_pwr_ctrl_probe,
    .remove = ar_pwr_ctrl_remove,
    .of_match = ar_pwr_ctrl_ids,
    .priv_auto_alloc_size = sizeof(struct pwr_ctrl),
    .ofdata_to_platdata = ar_pwr_ctrl_ofdata_to_platdata,
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
