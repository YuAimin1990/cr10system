...
#define W25N02KW_STATUS_ECC_1_7_BITFLIPS	(1 << 4)
#define W25N02KW_STATUS_ECC_8_BITFLIPS	(3 << 4)
#define W25N02KW_REG_STATUS_BFD		0x10
...
{
	u8 status_bfd;
	struct spi_mem_op op = SPINAND_GET_FEATURE_OP(W25N02KW_REG_STATUS_BFD,
						      &status_bfd);
	int ret;
	...
	case W25N02KW_STATUS_ECC_1_7_BITFLIPS:
		ret = spi_mem_exec_op(spinand->slave, &op);
		if (ret)
			return ret;
		status_bfd = status_bfd >> 4;
		if ((1 <= status_bfd) && (status_bfd <= 7))
			return status_bfd;
		else
			return -EBADMSG;

	case W25N02KW_STATUS_ECC_8_BITFLIPS:
		return 8;
