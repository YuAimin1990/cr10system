...
/*
 * (C) Copyright 2021 Artosyn Electronics Co., Ltd
 *
 * Artosyn SD Host Controller Interface
 */
...
#define ABB_REG32			0x01072080
#define  CLK_SEL_EMMC_50M		0x0
#define  CLK_SEL_EMMC_100M		0x1
#define  CLK_SEL_EMMC_200M		0x2
...
#define ABB_REG32			0x0a108080
#define  CLK_SEL_EMMC_50M		0x1
#define  CLK_SEL_EMMC_100M		0x2
#define  CLK_SEL_EMMC_200M		0x3
...
#define P_EMBEDDED_CNTRL_ADDR		0xF6C
#define SDHCI_EMBEDDED_CTRL		P_EMBEDDED_CNTRL_ADDR
#define SDHCI_P_VENDOR_SPECIFIC_AREA	0x500
#define SDHCI_MSHC_CTRL_R		SDHCI_P_VENDOR_SPECIFIC_AREA + 0x8
#define SDHCI_EMMC_CTRL_R		SDHCI_P_VENDOR_SPECIFIC_AREA + 0x2C
#define SDHCI_SW_CG_DIS			0x10
#define SDHCI_CARD_IS_EMMC		0x01

//#define FPGA_EMMC
struct artosyn_sdhc_plat {
...
static void snps_sdhci_global_reset(void) {
...
#elif defined(CONFIG_ARTOSYN_AR9311)
	writel((1 << 28), RGU_EMMC_CORE_RSTN_ADDR);
	writel((1 << 0), RGU_EMMC_HRSTN_ADDR);
#endif
	...
	reg_val = readl(0x01073094);
	reg_val |= 0x18;
	writel(reg_val, 0x01073094);
#endif
#endif
...
#ifdef CONFIG_ARTOSYN_AR9301
	struct artosyn_sdhc_plat *plat = dev_get_platdata(host->mmc->dev);
...
static int snps_sdhci_cmd_post_retry(struct sdhci_host *host, struct mmc_cmd *cmd, u32 stat)
{
	if (((cmd->cmdidx == MMC_CMD_READ_SINGLE_BLOCK) && (stat & SDHCI_INT_DATA_TIMEOUT)) ||
	    ((cmd->cmdidx == MMC_CMD_WRITE_SINGLE_BLOCK) && (stat & SDHCI_INT_DATA_TIMEOUT)) ||
...
static int snps_sdhci_cmd_pre_retry(struct sdhci_host *host, struct mmc_cmd *cmd, u32 stat)
{
	unsigned int reg_val;
	...
	writel(reg_val, 0x01021018);
#elif defined(CONFIG_ARTOSYN_AR9311)
	reg_val = readl(0x08051018);
	reg_val ^= 1 << 13;
	writel(reg_val, 0x08051018);
#endif
	if (((cmd->cmdidx == MMC_CMD_READ_SINGLE_BLOCK) && (stat & SDHCI_INT_DATA_TIMEOUT)) ||
	    ((cmd->cmdidx == MMC_CMD_WRITE_SINGLE_BLOCK) && (stat & SDHCI_INT_DATA_TIMEOUT)) ||
	    cmd->cmdidx == MMC_CMD_READ_MULTIPLE_BLOCK ||
...
#if CONFIG_IS_ENABLED(DM_REGULATOR) && CONFIG_IS_ENABLED(DM_REGULATOR_GPIO)
#ifdef CONFIG_ARTOSYN_AR9311
	if (mmc->vqmmc_supply) {
		pin_mod = readl(PINMUX_GPIO_MSC);
		if (mmc->signal_voltage != MMC_SIGNAL_VOLTAGE_180) {
			pin_mod &= ~(PINMUX_GPIO_MODE_1V8);
			writel(pin_mod, PINMUX_GPIO_MSC);
		}
...
static int snps_sdhci_probe(struct udevice *dev)
{
	...
	struct artosyn_sdhc_plat *plat = dev_get_platdata(dev);
	struct artosyn_sdhc *prv = dev_get_priv(dev);
	struct sdhci_host *host = &prv->host;
	int max_frequency, len, ret, hw_reset_enable = 0;
	struct clk clk;
	unsigned int val;

#ifdef CONFIG_SPL_BUILD
	if (dev_read_prop(dev, "hw-reset-enable", &len)) {
		debug("EMMC HW reset enable by dts in SPL.\n");
		hw_reset_enable = 1;
	} else {
		debug("EMMC HW reset not enable by dts in SPL.\n");
		hw_reset_enable = 0;
	}
#endif
	if (hw_reset_enable == 0)
	...
	val = readl(ABB_REG32);
	val &= ~(0x3 << 6);
	if (host->max_clk == 50000000)
		val |= (CLK_SEL_EMMC_50M << 6);
	else if (host->max_clk == 100000000)
		val |= (CLK_SEL_EMMC_100M << 6);
	else if (host->max_clk == 200000000)
		val |= (CLK_SEL_EMMC_200M << 6);
	else
		printf("unsupport max frequence\n");
	writel(val, ABB_REG32);
	...
	if (hw_reset_enable) {
		plat->mmc.rca = 0x55AA;
		ret = mmc_switch(&plat->mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_RST_N_FUNCTION, 1);
		if (ret) {
			printf("Set HW reset failed, ret %d\n", ret);
		}
	}
#endif
	ret = sdhci_probe(dev);
	...
	val = sdhci_readb(host, SDHCI_EMMC_CTRL_R);
	val |= SDHCI_CARD_IS_EMMC;
	sdhci_writeb(host, val, SDHCI_EMMC_CTRL_R);
	sdhci_writel(host, 0x0001, SDHCI_EMBEDDED_CTRL);
...
static int artosyn_sdhci_bind(struct udevice *dev)
{
	struct artosyn_sdhc_plat *plat = dev_get_platdata(dev);
...
U_BOOT_DRIVER(snps_sdhci_drv) = {
	...
	.ops		= &sdhci_ops,
	.bind		= artosyn_sdhci_bind,
	.probe		= snps_sdhci_probe,
	.priv_auto_alloc_size = sizeof(struct artosyn_sdhc),
	.platdata_auto_alloc_size = sizeof(struct artosyn_sdhc_plat),
};
