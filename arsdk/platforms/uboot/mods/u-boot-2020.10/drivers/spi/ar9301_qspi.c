...
/*
 * Copyright (C) 2021
 * Artosyn Corporation <www.artosyn.cn>
 */
...
struct ar9301_qspi {
	...
	struct ar9301_qspi_debug_info *dbg;
};

/* dma handshake select Huffman(1) / ahb dma(0) */
static void ar9301_qspi_dma_select(int huffman)
{
	if (huffman)
...
int ar9301_hdecomp_read_init(struct ar9301_qspi *aq, unsigned int addr, unsigned int len)
{
	int item_num = 0;
	unsigned int words, bursts, ws, bs, tmp;
	void *hdecomp_base = (void *)HDECOMP_BASE;

	ar9301_qspi_dma_select(1);
...
static void ahb_dmac_set_hs(struct ar9301_qspi *aq, int src_per, int dst_per)
{
	unsigned int tmp;
...
static void ahb_dmac_clearint(struct ar9301_qspi *aq)
{
	unsigned int tmp;
...
static int ahb_dmac_init(struct ar9301_qspi *aq, int read)
{
	unsigned int tmp;
...
static void ahb_dmac_channel_enable(struct ar9301_qspi *aq)
{
	unsigned tmp;
...
static void ahb_dmac_channel_disable(struct ar9301_qspi *aq)
{
	unsigned tmp;
...
static int ahb_dmac_check_finish_block(struct ar9301_qspi *aq)
{
	unsigned int tmp;
...
static int ahb_dmac_single_block_tran(struct ar9301_qspi *aq, unsigned int addr, unsigned int len, int read)
{
	int ret;
	unsigned int words;
...
static int ar9301_qspi_dma_read(struct ar9301_qspi *aq, unsigned int addr, unsigned int len)
{
...
static int ar9301_qspi_dma_write(struct ar9301_qspi *aq, unsigned int addr, unsigned int len)
{
...
/* qspi related codes */
static void ar9301_qspi_registers_dump(struct ar9301_qspi *aq)
{
	int i, tmp;
...
static __maybe_unused void ar9301_qspi_set_spi_mode(struct ar9301_qspi *aq, int mod)
{
	unsigned int tmp;
...
static __maybe_unused void ar9301_qspi_set_flash_cs(struct ar9301_qspi *aq, int cs)
{
	int tmp;
...
static void ar9301_qspi_set_timing_cfg(struct ar9301_qspi *aq, int val)
{
	unsigned int tmp;
...
static __maybe_unused void ar9301_qspi_clear_fifo(struct ar9301_qspi *aq)
{
	unsigned int tmp;
...
static void ar9301_qspi_dma_enable(struct ar9301_qspi *aq)
{
	unsigned int tmp;
...
static void ar9301_qspi_dma_disable(struct ar9301_qspi *aq)
{
	writel(0, aq->regs + QSPI_THRESHOLD);
}

static __maybe_unused void ar9301_qspi_int_enable(struct ar9301_qspi *aq)
{
	writel(QSPI_INT_EN_DONE_INT_EN, aq->regs + QSPI_INT_EN);
}

static void ar9301_qspi_int_disable(struct ar9301_qspi *aq)
{
	writel(0, aq->regs + QSPI_INT_EN);
}

static __maybe_unused void ar9301_qspi_clear_interrupt(struct ar9301_qspi *aq)
{
	/* write 1 clear interrupt */
	writel(1, aq->regs + QSPI_INT_ST);
}

static __maybe_unused void ar9301_qspi_dtr_enable(struct ar9301_qspi *aq)
{
	int tmp;
...
static __maybe_unused void ar9301_qspi_dtr_disable(struct ar9301_qspi *aq)
{
	int tmp;
...
#ifdef CONFIG_ARTOSYN_AR9301
/* TODO: 133M */
static __maybe_unused void ar9301_qspi_set_clk(struct ar9301_qspi *aq, unsigned int rate)
{
...
static int ar9301_qspi_waiting_rfifo_not_empty(struct ar9301_qspi *aq)
{
	unsigned int tmp;
	unsigned long timeout;
...
static int ar9301_qspi_read_fifo(struct ar9301_qspi *aq, unsigned char *buf, unsigned int len)
{
	...
	if (mod) {
		ret = ar9301_qspi_waiting_rfifo_not_empty(aq);
		if (ret)
			return ret;
...
static int ar9301_qspi_write_fifo(struct ar9301_qspi *aq, unsigned char *buf, unsigned int len)
{
...
static int ar9301_qspi_waiting_cmd_finish(struct ar9301_qspi *aq)
{
	unsigned tmp;
	unsigned long timeout;
...
static void ar9301_qspi_linewidth_convert(const struct spi_mem_op *op, unsigned int *width)
{
...
static int ar9301_qspi_send_cmd(struct ar9301_qspi *aq, const struct spi_mem_op *op)
{
	int ret;
	...
	ar9301_qspi_linewidth_convert(op, width);

	/* waiting for the previous cmd finish */
	ret = ar9301_qspi_waiting_cmd_finish(aq);
	if (ret)
		return ret;
...
static int ar9301_qspi_nor_fast_read(struct ar9301_qspi *aq)
{
	int ret, i;
	...
	for (i = 0; i < TRAINING_READ_CNT; i++) {
		ret = ar9301_qspi_send_cmd(aq, &op);
		if (ret) {
			pr_err("%s [%d]: qspi send command failed\n", __func__, __LINE__);
			goto out;
		}

		ret = ar9301_qspi_read_fifo(aq, op.data.buf.in, op.data.nbytes);
		if (ret)
			goto out;
	...
	if (i == TRAINING_READ_CNT)
		...
		return -1;
out:
	ar9301_qspi_registers_dump(aq);
	return ret;
}

static int spinand_read_reg_op(struct ar9301_qspi *aq, u8 reg, u8 *val)
{
	int ret;
	unsigned int tmp;
	unsigned char buf;
...
static int spinand_read_status(struct ar9301_qspi *aq, u8 *status)
{
	return spinand_read_reg_op(aq, REG_STATUS, status);
}

static int spinand_wait(struct ar9301_qspi *aq, u8 *s)
{
...
static int ar9301_qspi_nand_fast_read(struct ar9301_qspi *aq)
{
	int ret, i;
	unsigned int tmp, buf;
	...
	for (i = 0; i < TRAINING_READ_CNT; i++) {
		ret = ar9301_qspi_send_cmd(aq, &op_pageload);
		if (ret) {
			pr_err("%s [%d]: qspi send command failed\n", __func__, __LINE__);
			goto out;
		}
...
static __maybe_unused int ar9301_qspi_training(struct ar9301_qspi *aq)
{
	int ret, i, first = -1, last = -1;
	int (*fast_read)(struct ar9301_qspi *aq) = ar9301_qspi_nor_fast_read;

again:
	for (i = 0; i < TRAINING_MAX; i++) {
		ar9301_qspi_set_timing_cfg(aq, i);
		ret = fast_read(aq);
		if (!ret) {
			if (first < 0)
				first = i;
	...
	if (first < 0 && fast_read == ar9301_qspi_nor_fast_read) {
		fast_read = ar9301_qspi_nand_fast_read;
		goto again;
	} else if (first >= 0) {
		/* found the right region, set the middle value */
		ar9301_qspi_set_timing_cfg(aq, (first + last) / 2);
...
static int ar9301_qspi_init(struct ar9301_qspi *aq)
{
	/* interrupt disable */
	ar9301_qspi_int_disable(aq);
#if 0
	ar9301_set_pinshare(PIN_NO(76), PIN_FUNC_QSPI_SCK);
	ar9301_set_pinshare(PIN_NO(77), PIN_FUNC_QSPI_CS);
	ar9301_set_pinshare(PIN_NO(80), PIN_FUNC_QSPI_DATA_0);
	ar9301_set_pinshare(PIN_NO(81), PIN_FUNC_QSPI_DATA_1);
	ar9301_set_pinshare(PIN_NO(82), PIN_FUNC_QSPI_DATA_2);
	ar9301_set_pinshare(PIN_NO(83), PIN_FUNC_QSPI_DATA_3);
#endif

	/* use flash cs 0 */
	ar9301_qspi_set_flash_cs(aq, 0);
	/* dtr disable */
	ar9301_qspi_dtr_disable(aq);
	aq->cur_die = 0;
	aq->cur_cs = 0;
	aq->cur_dtr = 0;

	aq->dbg = (struct ar9301_qspi_debug_info *)((unsigned long)&aq->dbg + sizeof(void *));
	...
	if (aq->timing >= 0) {
		...
		if (ret) {
			 ...
			 * it must work
			 */
			ar9301_qspi_set_clk(aq, 25000000);
			ar9301_qspi_set_timing_cfg(aq, 0);
		}
	}
#ifdef CONFIG_ARTOSYN_AR9301
	/* TODO: for 9311 ahb dma access sram */
...
static int ar9301_spi_set_speed(struct udevice *bus, uint hz)
{
	return 0;
}

static int ar9301_spi_set_mode(struct udevice *bus, uint mode)
{
	return 0;
}

static int ar9301_spi_adjust_op_size(struct spi_slave *spi,
...
static int ar9301_qspi_opcode_is_dtr(unsigned char opcode)
{
	switch (opcode) {
...
static int ar9301_spi_mem_exec_op(struct spi_slave *spi,
				   const struct spi_mem_op *op)
{
	int ret, mod;
	struct udevice *bus = spi->dev->parent;
	struct ar9301_qspi *aq = bus->platdata;
	...
	if (plat->cs != aq->cur_cs) {
		ar9301_qspi_set_flash_cs(aq, plat->cs);
		if (aq->dbg->dbg_flag & DBG_CS)
			pr_info("change flash cs to %d\n", plat->cs);
		aq->cur_cs = plat->cs;
	}

	if (ar9301_qspi_opcode_is_dtr(op->cmd.opcode) != aq->cur_dtr) {
		if (aq->cur_dtr)
			ar9301_qspi_dtr_disable(aq);
		else
			ar9301_qspi_dtr_enable(aq);
...
#define DMA_TRANS_MIN 	16000000
	if (op->data.dir == SPI_MEM_DATA_IN) {
		if (op->data.nbytes < DMA_TRANS_MIN) {
			...
			ar9301_qspi_dma_enable(aq);
			invalidate_dcache_range((unsigned long)op->data.buf.in, (unsigned long)op->data.buf.in + roundup(op->data.nbytes, ARCH_DMA_MINALIGN));
			ret = ar9301_qspi_dma_read(aq, (unsigned long)op->data.buf.in, op->data.nbytes - mod);
			ar9301_qspi_dma_disable(aq);
			if (ret)
				goto out;
			if (mod) {
				ret = ar9301_qspi_read_fifo(aq, op->data.buf.in + op->data.nbytes - mod, mod);
				if (ret)
					goto out;
			}
		...
		if (op->data.nbytes < DMA_TRANS_MIN) {
			ret = ar9301_qspi_write_fifo(aq, (unsigned char *)op->data.buf.out, op->data.nbytes);
			if (ret)
				goto out;
		} else {
			mod = op->data.nbytes % 4;
			flush_dcache_range((unsigned long)op->data.buf.out, (unsigned long)op->data.buf.out + roundup(op->data.nbytes, ARCH_DMA_MINALIGN));
			ar9301_qspi_dma_enable(aq);
			ret = ar9301_qspi_dma_write(aq, (unsigned long)op->data.buf.out, op->data.nbytes - mod);
			ar9301_qspi_dma_disable(aq);
			if (ret)
				goto out;
			if (mod) {
				ret = ar9301_qspi_write_fifo(aq, (unsigned char *)op->data.buf.out + op->data.nbytes - mod, mod);
				if (ret)
					goto out;
			}
		}
...
static int ar9301_spi_ofdata_to_platdata(struct udevice *bus)
{
	struct ar9301_qspi *aq = bus->platdata;
...
static int ar9301_spi_probe(struct udevice *bus)
{
	struct ar9301_qspi *aq = bus->platdata;
...
static int ar9301_spi_remove(struct udevice *dev)
{
	return 0;
}

static const struct spi_controller_mem_ops ar9301_spi_mem_ops = {
	.adjust_op_size = ar9301_spi_adjust_op_size,
	.exec_op = ar9301_spi_mem_exec_op,
};

static const struct dm_spi_ops ar9301_spi_ops = {
	.set_speed	= ar9301_spi_set_speed,
	.set_mode	= ar9301_spi_set_mode,
	.mem_ops	= &ar9301_spi_mem_ops,
};

static const struct udevice_id ar9301_spi_ids[] = {
	{ .compatible = "artosyn,ar9301-qspi" },
	{ }
};

U_BOOT_DRIVER(ar9301_qspi) = {
	.name = "ar9301_qspi",
	.id = UCLASS_SPI,
	.of_match = ar9301_spi_ids,
	.ops = &ar9301_spi_ops,
	.ofdata_to_platdata = ar9301_spi_ofdata_to_platdata,
	.platdata_auto_alloc_size = sizeof(struct ar9301_qspi) + sizeof(struct ar9301_qspi_debug_info),
	.probe = ar9301_spi_probe,
	.remove = ar9301_spi_remove,
};
