...
struct ar9301_clock {
	...
	struct ar9301_abb_freqs abb_freqs;
};

struct ar9301_clock_plat {
	struct regmap *map;
};
...
static void ar9301_abb_set_emmc(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_sdc0(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_sdc1(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_pixel(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_audio(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	u32 intg;
...
static void ar9301_abb_set_audio1(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	u32 intg;
	...
	abb->aupll1_reg_b40 = 1;
	udelay(5);
	abb->aupll1_reg_b40 = 0;

	abb->aupll1_reg_b45 = 0x20;

	abb_freqs->abb_audio1_freq = freq;
}

static void ar9301_abb_set_audio2(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	u32 intg;
...
static void ar9301_abb_set_backup(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_a53(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_a53_cgu *a53_cgu = priv->a53_cgu;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	unsigned int val;
	uint32_t tmp_freq = 0;
	...
	a53_cgu->cgu_pll_config |= 0x1 << 26;
	a53_cgu->cgu_pll_config |= 0x1 << 27;
	a53_cgu->cgu_reserved1 |= 0xf;
	udelay(5);
	...
	tmp_freq = freq <= 900000000 ? freq : 900000000;

	val = a53_cgu->cgu_pll_config & 0xffff00ff;
	val |= (tmp_freq / 10000000) << 8;
	a53_cgu->cgu_pll_config = val;
	...
	//wait for 900M lock
	for(i = 0; i < 10; i++) {
		if (a53_cgu->cgu_pll_ctrl_1 & 0x1)
			break;

		if (i == 9) {
		    abb_freqs->abb_a53_freq = freq;
		    printf("lock failed, i = %d\n", i);
                    return;
		}
		udelay(10);
	}
...
static void ar9301_abb_set_ceva(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_ceva_cgu *ceva_cgu = priv->ceva_cgu;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	unsigned int val;
...
static void ar9301_abb_set_dla(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_top_regs *top = priv->top;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_noc(struct ar9301_clock *priv, u32 freq)
{
	struct ar9301_abb *abb = priv->abb;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
...
static void ar9301_abb_set_quirks(struct ar9301_clock *priv)
{
	struct ar9301_abb *abb = priv->abb;
...
static ulong ar9301_clk_get_rate(struct clk *clk)
{
#if 0
	struct ar9301_clock *priv = dev_get_priv(clk->dev);
	ulong rate = 0;
...
static ulong ar9301_clk_set_sel(struct ar9301_cgu *cgu, u32 clk_id, u32 sel)
{
	switch (clk_id) {
	// hdecomp
	case CLK_CORE_HDECOMP:
	...
	//case CLK_PERIP_I2SM:
	//	cgu->perip_clk_ctrl &= ~CLK_AUDIO_SEL_MASK;
	//	ar9301_abb_set_audio(priv, rate);
	//	break;
	default:
...
static ulong ar9301_clk_set_rate(struct clk *clk, ulong rate)
{
#ifndef CONFIG_SPL_BUILD
	struct ar9301_clock *priv = dev_get_priv(clk->dev);
	struct ar9301_cgu *cgu = priv->cgu;
	struct ar9301_abb_freqs *abb_freqs = &priv->abb_freqs;
	uint32_t raw;
	ulong div;
#endif
	...
	//case CLK_EMMC_CLK:
		...
		if (rate <= 50000000) {
			ar9301_abb_set_emmc(priv, 50000000);
			div = 50000000 / rate / 2;
			cgu->emmc_clk_ctrl |= div << CLK_EMMC_FREQ_SEL_SHIFT;
			cgu->emmc_clk_ctrl |= 1 << CLK_EMMC_CLK_SEL_SHIFT;
		} else if (rate <= 100000000) {
			ar9301_abb_set_emmc(priv, 100000000);
		} else {
			ar9301_abb_set_emmc(priv, 200000000);
		}
		break;
	//case CLK_EMMC_DRV:
	//	if ((rate != 0) && (rate != 1)) {
	...
	case CLK_PERIP_I2SM:
		...
		break;
	case CLK_PLL_PIXEL:
		ar9301_abb_set_pixel(priv, rate);
		break;
	default:
		log_debug("Unknown clock %lu\n", clk->id);
		return -ENOENT;
...
static int __maybe_unused ar9301_clk_set_parent(struct clk *clk,
						struct clk *parent)
{
//	struct clk *c, *cp;
	int ret = 0;
...
static int ar9301_clk_enable_byid(struct ar9301_cgu *cgu, u32 clk_id)
{
	switch (clk_id) {
	// coresight
	case CLK_CFG_CS:
...
static int ar9301_clk_enable(struct clk *clk)
{
	struct ar9301_clock *priv = dev_get_priv(clk->dev);
	struct ar9301_cgu *cgu = priv->cgu;

	return ar9301_clk_enable_byid(cgu, clk->id);
}

static int ar9301_clk_disable_byid(struct ar9301_cgu *cgu, u32 clk_id)
{
	switch (clk_id) {
	// coresight
	case CLK_CFG_CS:
...
static int ar9301_clk_disable(struct clk *clk)
{
	struct ar9301_clock *priv = dev_get_priv(clk->dev);
	struct ar9301_cgu *cgu = priv->cgu;

	return ar9301_clk_disable_byid(cgu, clk->id);
}

static struct clk_ops ar9301_clk_ops = {
	.get_rate = ar9301_clk_get_rate,
	.set_rate = ar9301_clk_set_rate,
#if CONFIG_IS_ENABLED(OF_CONTROL) && !CONFIG_IS_ENABLED(OF_PLATDATA)
	.set_parent = ar9301_clk_set_parent,
#endif
	.enable = ar9301_clk_enable,
	.disable = ar9301_clk_disable,
};

static int ar9301_abb_init(struct udevice *dev)
{
	struct ar9301_clock *priv = dev_get_priv(dev);
	int ret;

	ret = dev_read_u32_array(dev, "abb-freqs",
				 (u32 *)&priv->abb_freqs,
				 sizeof(struct ar9301_abb_freqs) / sizeof(u32));
	if (ret) {
		log_err("%s: read abb-freqs failed %d\n", __func__, ret);
		return -EINVAL;
	}
	ar9301_abb_set_emmc(priv, priv->abb_freqs.abb_emmc_freq);
	ar9301_abb_set_sdc0(priv, priv->abb_freqs.abb_sdc0_freq);
	ar9301_abb_set_sdc1(priv, priv->abb_freqs.abb_sdc1_freq);
	ar9301_abb_set_pixel(priv, priv->abb_freqs.abb_pixel_freq);
	ar9301_abb_set_audio(priv, priv->abb_freqs.abb_audio_freq);
	ar9301_abb_set_audio1(priv, priv->abb_freqs.abb_audio1_freq);
	ar9301_abb_set_audio2(priv, priv->abb_freqs.abb_audio2_freq);
	ar9301_abb_set_backup(priv, priv->abb_freqs.abb_backup_freq);
	ar9301_abb_set_a53(priv, priv->abb_freqs.abb_a53_freq);
	ar9301_abb_set_ceva(priv, priv->abb_freqs.abb_ceva_freq);
	ar9301_abb_set_dla(priv, priv->abb_freqs.abb_dla_freq);
	ar9301_abb_set_noc(priv, priv->abb_freqs.abb_noc_freq);
	ar9301_abb_set_quirks(priv);

	return 0;
}

static int ar9301_cgu_init(struct udevice *dev)
{
	struct ar9301_clock *priv = dev_get_priv(dev);
	...
	if (size < 0) {
		log_err("%s: ar9301 clk-default-gating size %d error\n",
			__func__, size);
		return -EINVAL;
	}
	...
	for (i = 0; i < size / sizeof(u32); i++) {
		if (array[i] & CLK_INFO_MASK)
			ar9301_clk_enable_byid(priv->cgu,
					       array[i] & CLK_ID_MASK);
		else
			ar9301_clk_disable_byid(priv->cgu,
					        array[i] & CLK_ID_MASK);
	}

	size = dev_read_size(dev, "clk-default-sel");
	if (size < 0) {
		log_err("%s: ar9301 clk-default-sel size %d error\n", __func__,
			size);
		return -EINVAL;
	}
	ret = dev_read_u32_array(dev, "clk-default-sel", (u32 *)array,
				 size / sizeof(u32));
	for (i = 0; i < size / sizeof(u32); i++) {
		ar9301_clk_set_sel(priv->cgu, array[i] & CLK_ID_MASK,
...
static int ar9301_clk_probe(struct udevice *dev)
{
#ifdef CONFIG_SPL_BUILD
//	struct ar9301_clock_plat *plat = dev_get_platdata(dev);
//	struct ar9301_clock *priv = dev_get_priv(dev);
...
static int ar9301_clk_ofdata_to_platdata(struct udevice *dev)
{
...
#define CGU_BASE_NUM 	10
	...
	ret = dev_read_u32_array(dev, "reg", (unsigned int *)tmp, CGU_BASE_NUM);
	if (ret) {
	...
	priv->abb = (void *)tmp[2];
	priv->top = (void *)tmp[4];
	priv->ceva_cgu = (void *)tmp[6];
	priv->a53_cgu = (void *)tmp[8];
...
static const struct udevice_id ar9301_cgu_ids[] = {
	{ .compatible = "artosyn,ar9301-cgu" },
	{ }
};

U_BOOT_DRIVER(clk_ar9301) = {
	.name		= "artosyn_ar9301_cgu",
	.id		= UCLASS_CLK,
	.of_match	= ar9301_cgu_ids,
	.priv_auto_alloc_size = sizeof(struct ar9301_clock),
	.ofdata_to_platdata = ar9301_clk_ofdata_to_platdata,
	.ops		= &ar9301_clk_ops,
	.probe		= ar9301_clk_probe,
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
	.platdata_auto_alloc_size = sizeof(struct ar9301_clock_plat),
#endif
};
