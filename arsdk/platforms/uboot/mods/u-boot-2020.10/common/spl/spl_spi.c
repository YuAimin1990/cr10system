			      ...
			      struct spl_boot_device *bootdev)
{
	...
	err = spi_flash_read(flash, payload_offs, 512, (void *)ar_header);
	if (err) {
...
#endif
	{
		if (IS_ENABLED(CONFIG_SPL_LOAD_FIT_FULL) &&
			...
			err = spl_parse_image_header(spl_image, header);
			if (err) {
				printf("%s[%d]: parse image %s error!\n", __FUNCTION__, __LINE__, "u-boot");
				return err;
			}
			err = spi_flash_read(flash, payload_offs + sizeof(*ar_header) + ar_header->hash_size + ar_header->sig_len,
					     ar_header->img_len,
