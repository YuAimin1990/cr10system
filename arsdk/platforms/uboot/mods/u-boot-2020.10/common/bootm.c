		    ...
		    int boot_progress)
{
	...
	unsigned long addr = simple_strtoul(argv[0], NULL, 16);
	char * tmp_str = (char *)argv[0];

	struct sirius_image_header *ar_header = (struct sirius_image_header *)addr;
	...
	data = signature_buffer + ar_header->sig_len;
	...
	if(secure_boot_enabled())
	{
		...
		if(0 != spl_verify_sw("kernel", (unsigned char *)hash_buffer, ar_header->hash_size,
			(unsigned char *)signature_buffer, ar_header->sig_len,
			(unsigned char *)data, ar_header->img_len))
		{
			printf("%s[%d]: verify %s error!\n", __FUNCTION__, __LINE__, "kernel");
			hang();
		}
		/*dcache_disable();
		if(0 != spl_verify((unsigned char *)hash_buffer, ar_header->hash_size, 
			(unsigned char *)signature_buffer, ar_header->sig_len, 
			(unsigned char *)data, ar_header->img_len))
		{
			printf("%s[%d]: verify %s error!\n", __FUNCTION__, __LINE__, "kernel");
	...
	addr += (sizeof(*ar_header) + ar_header->hash_size + ar_header->sig_len);
	sprintf(tmp_str, "0x%lx", addr);
