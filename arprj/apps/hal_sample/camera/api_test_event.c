   ...
   while(1)
   {
		...
		switch(p_event_info->event_code)
		{
            ...
            case NOTIFY_EVENT_AEC_UPDATE:
			{
                ...
                ar_always("luma=%f real_gain=%f exp_time_us=%d line_count=%d exp_index=%d",\
            ...
            case NOTIFY_EVENT_AWB_UPDATE:
			{
				 ...
				 ar_always("cct=%d r_gain=%f b_gain=%f",p_awb_out->cct,p_awb_out->r_gain,p_awb_out->b_gain);
			}
			break;
            case NOTIFY_EVENT_AF_UPDATE:
			{
				STRU_AF_OUT_T *p_af_out=(STRU_AF_OUT_T *)event_data;
				ar_always("lens_position=%d move_step=%d dir=%d b_move_lens=%d",\
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
                      ...
                      ar_hal_vin_create_event_client(fd_pipe,pipe_id,&event_obj.event_client_id);
					  ...
					  {
					      ar_hal_vin_register_event(fd_pipe,pipe_id,event_obj.event_client_id,i);
					  }
					  int delay_ms=5000;
					  ...
					  for(int i=NOTIFY_EVENT_ERR;i<NOTIFY_EVENT_MAX;i++)
					  {
					      ar_hal_vin_unregister_event(fd_pipe,pipe_id,event_obj.event_client_id,i);
					  }
					  ar_osal_thread_join(event_obj.event_thread);
					  ar_hal_vin_delete_event_client(fd_pipe,pipe_id,event_obj.event_client_id);
...
__test_camera_case__(pipe_event)=
{
   "--pipe_event",
   api_test_pipe_event,
   {
