...
static int api_test_set_get_shapeness(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_get_sharpness(fd_pipe,pipe_id,&value);
					  ar_printf("current shapeness is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-shapeness",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_set_shapeness(fd_pipe,pipe_id,value);
						  ar_hal_vin_get_sharpness(fd_pipe,pipe_id,&value);
						  ar_printf("current shapeness is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_shapeness)=
{
   "--set_get_shapeness",
   api_test_set_get_shapeness,
   {
      "test if -shapeness is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-shapeness [0--100]:",
	  NULL,
   },
