...
static int api_test_set_af_roi(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
    			      STRU_CAM_ROI_T value={0};
                      index=camera_find_cmd_pra_index_by_name("-af_roi",p_cmd);
					  if(index>=0){
                          value.x=atof(p_cmd->args[index +1]);
                          value.y=atof(p_cmd->args[index +2]);
                          value.width=atof(p_cmd->args[index +3]);
                          value.height=atof(p_cmd->args[index +4]);

                      }else{
                          ar_printf("-af_roi does not exists, will get -x -y -width -height\n");
                          ...
                          if(index>=0){
                              value.y=atof(p_cmd->args[index +1]);
                          }else{
                          ...
                          if(index>=0){
                              value.width=atof(p_cmd->args[index +1]);
                          }else{
                              ar_err("-width does not exists, exit");
                              return -1;
                          }
                          ar_always("get -height\n");
                          index=camera_find_cmd_pra_index_by_name("-height",p_cmd);
                          if(index>=0){
                              value.height=atof(p_cmd->args[index +1]);
                          }else{
                              ar_err("-height does not exists, exit");
                              return -1;
                          }
                      }
					  ar_always("switch to x %f, y %f, width %f, height %f\n",value.x,value.y,value.width,value.height);
                      ar_hal_vin_set_af_roi(fd_pipe,pipe_id,&value);
				  }else
				  {
...
__test_camera_case__(set_af_roi)=
{
   "--set_af_roi",
   api_test_set_af_roi,
   {
	  ...
	  "-dev_id [id]",
      "-af_roi [0.0(x) 0.0(y) 1.0(width) 1.0(height)]",
	  "-x [0.0 -- 1.0, if -af_roi exists, be ignored]",
	  "-y [0.0 -- 1.0, if -af_roi exists, be ignored]",
	  "-width [0.0 -- 1.0, if -af_roi exists, be ignored]:",
	  "-height [0.0 -- 1.0, if -af_roi exists, be ignored]:",
	  NULL,
   },
};
