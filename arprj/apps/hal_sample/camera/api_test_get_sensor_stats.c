...
static int api_test_get_sensor_stats(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_src_get_sensor_stats(fd_pipe,pipe_id,&value);
					  ar_printf("current sensor stats is %d\n",value);
				  }else
				  {
...
__test_camera_case__(get_sensor_stats)=
{
   "--get_sensor_stats",
   api_test_get_sensor_stats,
   {
