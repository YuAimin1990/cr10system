...
static int api_test_set_get_csc(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
    			      STRU_CSC_PRA_T value={0};
                      ar_hal_vin_get_csc(fd_pipe,pipe_id,&value);
                      ar_printf("current type=%d, full_range=%d\n", value.type, value.full_range);

					  index=camera_find_cmd_pra_index_by_name("-type",p_cmd);
					  if(index>=0){
					  	  value.type=atoi(p_cmd->args[index +1]);
					  }
					  index=camera_find_cmd_pra_index_by_name("-full_range",p_cmd);
					  if(index>=0){
					  	  value.full_range=atoi(p_cmd->args[index +1]);
					  }

                      ar_printf("switch to type=%d, full_range=%d\n", value.type, value.full_range);
                      ar_hal_vin_set_csc(fd_pipe,pipe_id,&value);

                      ar_hal_vin_get_csc(fd_pipe,pipe_id,&value);
                      ar_printf("current type=%d, full_range=%d after switch\n", value.type, value.full_range);
				  }else
				  {
...
__test_camera_case__(set_get_csc)=
{
   "--set_get_csc",
   api_test_set_get_csc,
   {
      "test -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-type [0 -- 1, if missed, be not changed], 0: BT709, 1: BT601",
      "-full_range [0 -- 1, if missed, be not changed], 0: not full range, 1: full range",
	  NULL,
   },
};
