...
static int api_test_set_get_ae_roi(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
                      ...
                      ar_hal_vin_get_ae_roi(fd_pipe,pipe_id,&value);
                      ...
                      ar_hal_vin_set_ae_roi(fd_pipe,pipe_id,&value);
                      ar_hal_vin_get_ae_roi(fd_pipe,pipe_id,&value);
...
__test_camera_case__(set_get_ae_roi)=
{
   "--set_get_ae_roi",
   api_test_set_get_ae_roi,
   {
