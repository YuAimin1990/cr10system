...
static int api_test_set_get_backight(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
   ...
   }else
   {
         ...
         int mode=0;
		 index=camera_find_cmd_pra_index_by_name("-mode",p_cmd);
		 if(index>0)
		 {
			 mode=atoi(p_cmd->args[index+1]);
		 }
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
    			      STRU_BLACK_LIGHT_T value={0};
                      ar_hal_vin_get_back_light_mode(fd_pipe,pipe_id,&value);
					  ar_printf("current mode is {%d %d}\n",value.mode,value.strength);
					  index=camera_find_cmd_pra_index_by_name("-strength",p_cmd);
					  if(index>=0){
					  	  int strength=atoi(p_cmd->args[index +1]);						  
						  value.mode=mode;
						  value.strength=strength;
					  	  ar_printf("switch to {%d %d}\n",value.mode,value.strength);
						  ar_hal_vin_set_back_light_mode(fd_pipe,pipe_id,&value);
						  ar_hal_vin_get_back_light_mode(fd_pipe,pipe_id,&value);
						  ar_printf("current backlight is {%d,%d} after switch\n",value.mode,value.strength);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_backight)=
{
   "--set_get_backight",
   api_test_set_get_backight,
   {
      "test if -hue is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-mode:[0:BLACK_LIGHT_MODE_OFF,1 BLACK_LIGHT_COMPENSTATE,2:STRONG_LIGHT_SUPPRESSION",	  
	  "-strength [0--100]:",
	  NULL,
   },
};
