...
static int api_test_set_get_af_mode(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_af_get_mode(fd_pipe,pipe_id,&value);
					  ar_printf("current af_mode is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-af_mode",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_af_set_mode(fd_pipe,pipe_id,value);
						  ar_hal_vin_af_get_mode(fd_pipe,pipe_id,&value);
						  ar_printf("current af_mode is %d after switch\n",value);
					  }
				  }else
				  {
				      ar_err("open fd pipe faiaf");
				  }
...
__test_camera_case__(set_get_af_mode)=
{
   "--set_get_af_mode",
   api_test_set_get_af_mode,
   {
      "test if -af_mode is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-af_mode [0--4]: 0: AF_AUTO, 1: AF_CAF, 2: AF_MACRO, 3: AF_INFINITE, 4: AF_MANUAL",
	  NULL,
   },
};
