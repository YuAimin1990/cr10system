...
static int api_test_set_af_focus(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
					  ar_printf("trigger focus \n");
                      ar_hal_vin_trigger_focus(fd_pipe,pipe_id);
				  }else
				  {
...
__test_camera_case__(set_af_focus)=
{
   "--set_af_focus",
   api_test_set_af_focus,
   {
