...
static int api_test_set_get_hue(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_get_hue(fd_pipe,pipe_id,&value);
					  ar_printf("current hue is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-hue",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_set_hue(fd_pipe,pipe_id,value);
						  ar_hal_vin_get_hue(fd_pipe,pipe_id,&value);
						  ar_printf("current hue is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_hue)=
{
   "--set_get_hue",
   api_test_set_get_hue,
   {
