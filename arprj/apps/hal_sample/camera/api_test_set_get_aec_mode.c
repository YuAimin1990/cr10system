...
static int api_test_set_get_aec_mode(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value = 0;
                      ar_hal_vin_aec_get_mode(fd_pipe,pipe_id,&value);
					  ar_printf("current aec_mode is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-aec_mode",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);

					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_aec_set_mode(fd_pipe,pipe_id,value);
						  ar_hal_vin_aec_get_mode(fd_pipe,pipe_id,&value);
						  ar_printf("current aec_mode is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_aec_mode)=
{
   "--set_get_aec_mode",
   api_test_set_get_aec_mode,
   {
      "test if -aec_mode is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-aec_mode [0 -- 6], 0: AEC_AUTO, 1: AEC_FULL_SWEEP, 2: AEC_AVERGATE, 3: AEC_CENTER_WEIGHT, 4: AEC_SPOT, 5: AEC_MANUAL, 6: AEC_LOCKED",
	  NULL,
   },
};
