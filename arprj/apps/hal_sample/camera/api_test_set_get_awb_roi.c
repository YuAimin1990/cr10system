...
static int api_test_set_get_awb_roi(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
                      ...
                      ar_hal_vin_get_awb_roi(fd_pipe,pipe_id,&value);
					  ar_printf("current x %f, y %f, width %f, height %f\n",value.x,value.y,value.width,value.height);
                      ...
                      ar_hal_vin_set_awb_roi(fd_pipe,pipe_id,&value);
                      ar_hal_vin_get_awb_roi(fd_pipe,pipe_id,&value);
					  ar_printf("switch to x %f, y %f, width %f, height %f after switch\n",value.x,value.y,value.width,value.height);
				  }else
				  {
...
__test_camera_case__(set_get_awb_roi)=
{
   "--set_get_awb_roi",
   api_test_set_get_awb_roi,
   {
      "test if -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-x [0.0 -- 1.0, if missed, be not changed]",
	  "-y [0.0 -- 1.0, if missed, be not changed]",
	  "-width [0.0 -- 1.0, if missed, be not changed]:",
	  "-height [0.0 -- 1.0, if missed, be not changed]:",
	  NULL,
   },
};
