...
static int api_test_set_sensce(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
    			      STRU_SENSE_INFO_T pra;
                      ar_hal_vin_get_sensce(fd_pipe,pipe_id,&pra);
					  ar_printf("current sensce is %s\n",pra.tuning_name);
					  index=camera_find_cmd_pra_index_by_name("-sensce",p_cmd);
					  if(index>=0){
					  	  ar_printf("switch to %s\n",p_cmd->args[index +1]);
					      strcpy(pra.tuning_name,p_cmd->args[index +1]);
						  ar_hal_vin_set_sensce(fd_pipe,pipe_id,&pra);
						  ar_hal_vin_get_sensce(fd_pipe,pipe_id,&pra);
						  ar_printf("current sensce is %s after switch\n",pra.tuning_name);
					  }
				  }else
				  {
...
__test_camera_case__(set_sensce)=
{
   "--set_sensce",
   api_test_set_sensce,
   {
	  ...
	  "-dev_id [id]",
	  "-sensce [name]: eg --sensce /usrdata/local/factory/tunning/cam_imx307/imx307_tuning_night_dwt_80.bin ",
	  NULL,
   },
};
