...
static void *event_loop_thread(void* arg)
{
		...
		switch(p_event_info->event_code)
		{
			case META_TYPE_AWB_GROUP:
			{
			     STRU_META_DATA_TYPE_T *p_meta=(STRU_META_DATA_TYPE_T *)event_data;
 				 STRU_AWB_ALGO_LIB_INPUT_T *p_algo_lib_input_phy=p_meta->meta_data.awb_stats_group.p_stats;
				 STRU_AWB_ALGO_LIB_OUTPUT_T *p_algo_lib_out_phy=p_meta->meta_data.awb_stats_group.p_out;
				 STRU_AWB_CB_EVENT_RUN_PRA_T run_pra;
				 run_pra.camera_id=p_event_pbj->pipe_id;
                 run_pra.p_awb_algo_lib_input=p_algo_lib_input_phy;
				 run_pra.p_awb_algo_lib_output=p_algo_lib_out_phy;
				 //run the aec with the stats phy addr
				 ar_hal_vin_awb_run(p_event_pbj->fd_pipe,p_event_pbj->pipe_id,&run_pra);
				 //map the output and convert the out to aec output
				 void *p_algo_lib_out_maped=ar_hal_vin_map_phy_addr(fd_mem,p_algo_lib_out_phy,sizeof(STRU_AWB_ALGO_LIB_OUTPUT_T),&offset_ret);
				 STRU_AWB_ALGO_LIB_OUTPUT_T *p_algo_lib_out=(STRU_AWB_ALGO_LIB_OUTPUT_T *)(p_algo_lib_out_maped+offset_ret);

				 STRU_AWB_OUT_T awb_out;
				 awb_out.r_gain=p_algo_lib_out->r_gain;
				 awb_out.g_gain=p_algo_lib_out->g_gain;
				 awb_out.b_gain=p_algo_lib_out->b_gain;
				 awb_out.cct=p_algo_lib_out->cct;

                 ar_always("%f %f %f cct:%f",awb_out.r_gain,awb_out.g_gain,awb_out.b_gain,awb_out.cct);
				 ar_hal_vin_awb_update(p_event_pbj->fd_pipe,p_event_pbj->pipe_id,&awb_out);
   ...
   STRU_AWB_CB_EVENT_RUN_PRA_T run_pra={0};
   ar_hal_vin_awb_run(p_event_pbj->fd_pipe,p_event_pbj->pipe_id,&run_pra);
   free(p_event_info);
   free(p_opt);
...
static int api_test_pipe_usr_awb(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
					  ...
					  for(int i=META_TYPE_AWB_GROUP;i<=META_TYPE_AWB_GROUP;i++)
					  {
...
__test_camera_case__(usr_awb)=
{
   "--usr_awb",
   api_test_pipe_usr_awb,
   {
      "api_test_pipe_usr_aec, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
