...
static int api_test_set_get_mirror_state(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_mirror_get_state(fd_pipe,pipe_id,&value);
					  ar_printf("current mirror_state is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-mirror_state",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d",value);
						  ar_hal_vin_mirror_set_state(fd_pipe,pipe_id,value);
						  ar_hal_vin_mirror_get_state(fd_pipe,pipe_id,&value);
						  ar_printf("current mirror_state is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_mirror_state)=
{
   "--set_get_mirror_state",
   api_test_set_get_mirror_state,
   {
      "test if -mirror_state is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-mirror_state [0--1]:",
	  NULL,
   },
