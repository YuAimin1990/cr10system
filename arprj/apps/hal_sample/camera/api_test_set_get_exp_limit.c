...
static int api_test_set_get_exp_limit(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
            ...
            if(opend)
            {
                ...
                if(ret>=0 && pipe_id>=0)
                {
                    ...
                    if(fd_pipe>=0)
                    {
                        ...
                        ar_hal_vin_get_exp_limit(fd_pipe,pipe_id,&value);
                        ar_printf("current exp_limit %d\n",value);

                        index=camera_find_cmd_pra_index_by_name("-exp_limit",p_cmd);
                        if(index>=0){
                            value=atoi(p_cmd->args[index +1]);

                            ar_printf("switch to exp_limit %d\n",value);
                            ar_hal_vin_set_exp_limit(fd_pipe,pipe_id,value);

                            ar_hal_vin_get_exp_limit(fd_pipe,pipe_id,&value);
                            ar_printf("current exp_limit %d after switch\n",value);
                        }
                    }else
                    {
...
__test_camera_case__(set_get_exp_limit)=
{
    "--set_get_exp_limit",
    api_test_set_get_exp_limit,
    {
        "test if -exp_limit, is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
        "-dev_id [id]",
        "-exp_limit [0 -- 30000us]",
        NULL,
    },
