...
static int api_test_set_get_led_state(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
    			      STRU_CAM_PRA_LED_PRA_T value={0};
                      ar_hal_vin_led_get_state(fd_pipe,pipe_id,&value);
					  ar_printf("current led_state is %d\n",value.status);
					  index=camera_find_cmd_pra_index_by_name("-led_state",p_cmd);
					  if(index>=0){
					  	  int status=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",status);
					      value.status=status;
						  ar_hal_vin_led_set_state(fd_pipe,pipe_id,value);
						  ar_hal_vin_led_get_state(fd_pipe,pipe_id,&value);
						  ar_printf("current led_state is %d after switch\n",value.status);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_led_state)=
{
   "--set_get_led_state",
   api_test_set_get_led_state,
   {
      "test if -led_state is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-led_state [0--1]:",
	  NULL,
   },
