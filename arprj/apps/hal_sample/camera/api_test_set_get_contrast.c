		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_get_contrast(fd_pipe,pipe_id,&value);
					  ar_printf("current contrast is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-contrast",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_set_contrast(fd_pipe,pipe_id,value);
						  ar_hal_vin_get_contrast(fd_pipe,pipe_id,&value);
						  ar_printf("current contrast is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_contrast)=
{
   "--set_get_contrast",
   api_test_set_get_contrast,
   {
      "test if -contrast is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-contrast [0--100]:",
	  NULL,
   },
