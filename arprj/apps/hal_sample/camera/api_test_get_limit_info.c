...
static int api_test_set_get_contrast(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  if(fd_pipe>=0)
				  {
					  STRU_CAM_PRA_GET_LIMIT_INFO_PRA_T info={0};
                      ar_hal_vin_get_pipe_limit_info(fd_pipe,pipe_id,&info);
					  ar_printf("max_exp_times_us=(%d %d %d)\n",info.max_exp_times_us[0],info.max_exp_times_us[1],info.max_exp_times_us[2]);
					  ar_printf("min_exp_times_us=(%d %d %d)\n",info.min_exp_times_us[0],info.min_exp_times_us[1],info.min_exp_times_us[2]);
					  ar_printf("max_gain=(%f %f %f)\n",info.max_gain[0],info.max_gain[1],info.max_gain[2]);
					  ar_printf("min_gain=(%f %f %f)\n",info.min_gain[0],info.min_gain[1],info.min_gain[2]);
					  ar_printf("max_fps min_fps=(%d %d)\n",info.max_fps,info.min_fps);
					  ar_printf("max_lens_pos min_lens_pos=(%d %d)\n",info.max_lens_pos,info.min_lens_pos);
...
__test_camera_case__(get_limit_info)=
{
   "--get_limit_info",
   api_test_set_get_contrast,
   {
