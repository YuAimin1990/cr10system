...
static int api_test_set_get_hdr_status(STRU_CAMERA_T *p_camera,STRU_CAMERA_CMD_T *p_cmd)
{
		 ...
		 if(ret<0 || opend)
		 {
			...
			if(opend)
			{
			   ...
			   if(ret>=0 && pipe_id>=0)
			   {
				  ...
				  {
    			      int value=0;
                      ar_hal_vin_get_hdr_status(fd_pipe,pipe_id,&value);
					  ar_printf("current hdr status is %d\n",value);
					  index=camera_find_cmd_pra_index_by_name("-hdr_status",p_cmd);
					  if(index>=0){
					  	  value=atoi(p_cmd->args[index +1]);
					  	  ar_printf("switch to %d\n",value);
						  ar_hal_vin_set_hdr_status(fd_pipe,pipe_id,value);
						  ar_hal_vin_get_hdr_status(fd_pipe,pipe_id,&value);
						  ar_printf("current hdr status is %d after switch\n",value);
					  }
				  }else
				  {
...
__test_camera_case__(set_get_hdr_status)=
{
   "--set_get_hdr_status",
   api_test_set_get_hdr_status,
   {
      "test if -hdr_status is missed,only get, -remote [remote binder server name]: this indicate the api run on remote app,",
	  "-dev_id [id]",
	  "-hdr_status [0:fusion 1:long 2:middle 3:short 4 auto]",
	  NULL,
   },
};
