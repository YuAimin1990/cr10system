...
static int uvc_h26x_deinit(uvc_format_data_t *data, void * handle);

static int uvc_h26x_init(uvc_format_data_t *data, void * handle, int argc, char *argv[])
{
...
static struct uvc_frame_buf_info * uvc_h26x_get_frame(uvc_format_data_t *data)
{
    AR_S32                      s32Ret = 0, i = 0;
    ...
    if(priv_data->i_frame_for_start < __MAX_I_FRAME_FOR_H26x_START__)
    {
        AR_MPI_VENC_RequestIDR(server_l->cam_index, AR_TRUE);
    }
...
static int uvc_h26x_release_frame(uvc_format_data_t *data, struct uvc_frame_buf_info * buf_info)
{
    int ret = 0;
...
static struct uvc_format_opt uvc_h264_opt =
{
    .format_init = uvc_h26x_init,
    .format_deinit = uvc_h26x_deinit,
    .get_frame = uvc_h26x_get_frame,
    .release_frame = uvc_h26x_release_frame,
};
...
static struct uvc_format_opt uvc_h265_opt =
{
    .format_init = uvc_h26x_init,
    .format_deinit = uvc_h26x_deinit,
    .get_frame = uvc_h26x_get_frame,
