...
AR_U16 OSD_MAKECOLOR_U16(AR_U8 r, AR_U8 g, AR_U8 b, OSD_COMP_INFO compinfo)
{
    AR_U8 r1, g1, b1;
    AR_U16 pixel = 0;
    AR_U32 tmp = 15;
...
AR_S32 GetBmpInfo(const char* filename,     OSD_BITMAPFILEHEADER*  pBmpFileHeader
...
int LoadBMP(const char* filename, OSD_LOGO_T* pVideoLogo)
{
    ...
    pVideoLogo->width = (AR_U16)bmpInfo.bmiHeader.biWidth;
    pVideoLogo->height = (AR_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
    w = pVideoLogo->width;
    h = pVideoLogo->height;
...
#endif
    /* RGB8888 or RGB1555 */
    pOrigBMPBuf = (AR_U8*)malloc(h * stride);
    if (NULL == pOrigBMPBuf)
    {
            ...
            if (Bpp == 3)
            {
                switch (enFmt)
                {
                    ...
                    case OSD_COLOR_FMT_RGB4444:
                        ...
                        pDst = (AR_U16*)(pRGBBuf + i * pVideoLogo->stride + j * 2);
                        r = *(pStart);
                        g = *(pStart + 1);
                        b = *(pStart + 2);
...
int LoadBMPCanvas(const char* filename, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    w = (AR_U16)bmpInfo.bmiHeader.biWidth;
    h = (AR_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
