...
int SAMPLE_COMM_VO_LcdInit(VO_DEV VoDev, int eLcdId);
...
AR_S32 SAMPLE_COMM_VO_StartDev(VO_DEV VoDev, VO_PUB_ATTR_S* pstPubAttr)
{
	...
	{
		pstPubAttr->stSyncInfo.u16Hmid  = 0;
        pstPubAttr->stSyncInfo.u16Bvact = 0;
        pstPubAttr->stSyncInfo.u16Bvbb  = 0;
        pstPubAttr->stSyncInfo.u16Bvfb  = 0;
        pstPubAttr->stSyncInfo.bIdv = AR_FALSE;
        pstPubAttr->stSyncInfo.bIhs = AR_FALSE;
        pstPubAttr->stSyncInfo.bIvs = AR_FALSE;

		pstPubAttr->enIntfSync = VO_OUTPUT_USER;
        pstPubAttr->stSyncInfo.bSynm = AR_TRUE;
        pstPubAttr->stSyncInfo.bIop  = AR_TRUE;
        pstPubAttr->stSyncInfo.u8Intfb = 0;
		pstPubAttr->enIntfType = VO_INTF_MIPI;
	}
    s32Ret = AR_MPI_VO_SetPubAttr(VoDev, pstPubAttr);
	...
	if (pstPubAttr->enIntfType == VO_INTF_MIPI)
	{
        ...
        AR_MPI_VO_SetUserIntfSyncInfo(VoDev, &stUserInfo);
	}
	#endif
...
AR_S32 SAMPLE_COMM_VO_StartLayer(VO_LAYER VoLayer, const VO_VIDEO_LAYER_ATTR_S* pstLayerAttr)
{
    ...
    s32Ret = AR_MPI_VO_SetVideoLayerAttr(VoLayer, pstLayerAttr);
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return AR_FAILURE;
...
AR_S32 SAMPLE_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    ...
    SAMPLE_PRT("u32Width:%d, u32Height:%d, u32Square:%d, u32WndNum:%d\n", u32Width, u32Height, u32Square, u32WndNum);
...
static AR_VOID SAMPLE_PRIVATE_VO_GetDefWbcAttr(VO_WBC_ATTR_S *pstWbcAttr, VO_WBC_SOURCE_S *pstSource)
{
    if (NULL == pstWbcAttr)
    {
...
static AR_S32 SAMPLE_PRIVATE_VO_StartWbc(VO_WBC VoWbc,VO_WBC_ATTR_S *pstWbcAttr, VO_WBC_SOURCE_S *pstSource)
{
    AR_S32 s32Ret = AR_SUCCESS;

    s32Ret = AR_MPI_VO_SetWBCSource(VoWbc,pstSource);
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return AR_FAILURE;
    }
    s32Ret = AR_MPI_VO_SetWBCAttr(VoWbc,pstWbcAttr);
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return AR_FAILURE;
    }
    s32Ret = AR_MPI_VO_EnableWBC(VoWbc);
        if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return AR_FAILURE;
...
static AR_S32 SAMPLE_PRIVATE_VO_StopWbc(VO_WBC VoWbc)
{
    AR_S32 s32Ret = AR_SUCCESS;

    s32Ret = AR_MPI_VO_DisableWBC(VoWbc);
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return AR_FAILURE;
...
AR_S32 SAMPLE_COMM_VO_StartWBC(SAMPLE_VO_WBC_CONFIG * pWbcConfig)
{
    ...
    *******************************/
    ...
    {
        s32Ret = AR_MPI_VO_SetWBCDepth(pWbcConfig->VoWbc, 5);
        if (AR_SUCCESS != s32Ret)
        {
            printf("Dev(%d) AR_MPI_VO_SetWbcDepth errno %#x\n", pWbcConfig->VoWbc, s32Ret);
            return s32Ret;
        }
    }
...
AR_VOID SAMPLE_COMM_VO_HdmiConvertSync(VO_INTF_SYNC_E enIntfSync,
    AR_HDMI_VIDEO_FMT_E *penVideoFmt)
{
    switch (enIntfSync)
    {
        case VO_OUTPUT_PAL:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_PAL;
            break;
        case VO_OUTPUT_NTSC:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_NTSC;
            break;
        case VO_OUTPUT_1080P24:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_24;
            break;
        case VO_OUTPUT_1080P25:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_25;
            break;
        case VO_OUTPUT_1080P30:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_30;
            break;
        case VO_OUTPUT_720P50:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_720P_50;
            break;
        case VO_OUTPUT_720P60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_720P_60;
            break;
        case VO_OUTPUT_1080I50:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080i_50;
            break;
        case VO_OUTPUT_1080I60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080i_60;
            break;
        case VO_OUTPUT_1080P50:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_50;
            break;
        case VO_OUTPUT_1080P60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_60;
            break;
        case VO_OUTPUT_576P50:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_576P_50;
            break;
        case VO_OUTPUT_480P60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_480P_60;
            break;
        case VO_OUTPUT_800x600_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_800X600_60;
            break;
        case VO_OUTPUT_1024x768_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1024X768_60;
            break;
        case VO_OUTPUT_1280x1024_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1280X1024_60;
            break;
        case VO_OUTPUT_1366x768_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1366X768_60;
            break;
        case VO_OUTPUT_1440x900_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1440X900_60;
            break;
        case VO_OUTPUT_1280x800_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1280X800_60;
            break;
        case VO_OUTPUT_1920x2160_30:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1920x2160_30;
            break;
        case VO_OUTPUT_1600x1200_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1600X1200_60;
            break;
        case VO_OUTPUT_1920x1200_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_VESA_1920X1200_60;
            break;
        case VO_OUTPUT_2560x1440_30:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_2560x1440_30;
            break;
        case VO_OUTPUT_2560x1600_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_2560x1600_60;
            break;
        case VO_OUTPUT_3840x2160_30:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_3840X2160P_30;
            break;
        case VO_OUTPUT_3840x2160_60:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_3840X2160P_60;
        break;
        default:
            *penVideoFmt = AR_HDMI_VIDEO_FMT_1080P_60;
            break;
...
AR_S32 SAMPLE_COMM_VO_HdmiStart(VO_INTF_SYNC_E enIntfSync)
{
    ...
    AR_HDMI_VIDEO_FMT_E enVideoFmt;
    AR_HDMI_ID_E        enHdmiId    = AR_HDMI_ID_0;

    SAMPLE_COMM_VO_HdmiConvertSync(enIntfSync, &enVideoFmt);

    CHECK_RET(AR_MPI_HDMI_Init(), "AR_MPI_HDMI_Init");
    CHECK_RET(AR_MPI_HDMI_Open(enHdmiId), "AR_MPI_HDMI_Open");
    CHECK_RET(AR_MPI_HDMI_GetAttr(enHdmiId, &stAttr), "AR_MPI_HDMI_GetAttr");
    ...
    CHECK_RET(AR_MPI_HDMI_SetAttr(enHdmiId, &stAttr), "AR_MPI_HDMI_SetAttr");
    CHECK_RET(AR_MPI_HDMI_Start(enHdmiId), "AR_MPI_HDMI_Start");
...
AR_S32 SAMPLE_COMM_VO_HdmiStartByDyRg(VO_INTF_SYNC_E enIntfSync, DYNAMIC_RANGE_E enDyRg)
{
    ...
    switch(enDyRg)
    {
        ...
        case DYNAMIC_RANGE_HDR10:
            stAttr.enVidOutMode    = AR_HDMI_VIDEO_MODE_YCBCR422;
            break;
        default:
            stAttr.enDeepColorMode = AR_HDMI_DEEP_COLOR_24BIT;
            break;
...
AR_S32 SAMPLE_COMM_VO_HdmiStop(AR_VOID)
{
    ...
    CHECK_RET(AR_MPI_HDMI_Stop(enHdmiId),"AR_MPI_HDMI_Stop");
    CHECK_RET(AR_MPI_HDMI_Close(enHdmiId),"AR_MPI_HDMI_Close");
    CHECK_RET(AR_MPI_HDMI_DeInit(),"AR_MPI_HDMI_DeInit");
...
#define LCD_ID_7INCH_LCD (0)
int SAMPLE_COMM_VO_GetDisplayParam(VO_PUB_ATTR_S *stPubAttr, int eLcdId)
{
    switch (eLcdId)
    {
        ...
        default:
            SAMPLE_PRT("no matched panel id: %d!\n", eLcdId);
            break;
    }
    return 0;
}
...
AR_S32 SAMPLE_COMM_VO_StartVO(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
    ...
    *********************************/
	...
	if (enVoIntfType == VO_INTF_MIPI)
	{
		SAMPLE_COMM_VO_GetDisplayParam(&stVoPubAttr, LCD_ID_7INCH_LCD);
	}
    ...
    ********************************/
    ...
    stLayerAttr.u32DispFrmRt = (float)u32DispFrmRt;
    if (AR_SUCCESS != s32Ret)
    {
	...
	AR_MPI_VO_SetDevFrameRate(VoDev, stLayerAttr.u32DispFrmRt);
    ...
    ********************************/
    ...
    {
        s32Ret = AR_MPI_VO_SetDisplayBufLen(VoLayer, pstVoConfig->u32DisBufLen);
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VO_SetDisplayBufLen failed with %#x!\n",s32Ret);
            SAMPLE_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }
    ...
    {
        s32Ret = AR_MPI_VO_SetVideoLayerPartitionMode(VoLayer, enVoPartMode);
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VO_SetVideoLayerPartitionMode failed!\n");
            SAMPLE_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }
	SAMPLE_PRT("stLayerAttr, disp_w = %d, disp_h =%d, pic_w=%d, pic_h=%d\n", stLayerAttr.stDispRect.u32Width, stLayerAttr.stDispRect.u32Height, stLayerAttr.stImageSize.u32Width, stLayerAttr.stImageSize.u32Height);
    ...
    {
        s32Ret = AR_MPI_VO_GetVideoLayerCSC(VoLayer, &stVideoCSC);
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VO_GetVideoLayerCSC failed!\n");
            SAMPLE_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
        stVideoCSC.enCscMatrix =VO_CSC_MATRIX_BT709_TO_RGB_PC;
        s32Ret = AR_MPI_VO_SetVideoLayerCSC(VoLayer, &stVideoCSC);
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VO_SetVideoLayerCSC failed!\n");
            SAMPLE_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }
    ...
    ********************************/
    if(VO_INTF_MIPI == enVoIntfType)
    {
        ...
        SAMPLE_COMM_VO_LcdInit(VoDev, LCD_ID_7INCH_LCD);
    }
...
AR_S32 SAMPLE_COMM_VO_StartLayerChn(SAMPLE_COMM_VO_LAYER_CONFIG_S * pstVoLayerConfig)
{
    ...
    AR_S32                  s32Ret            = AR_SUCCESS;
    VO_VIDEO_LAYER_ATTR_S   stLayerAttr;
    AR_U32                  u32Frmt, u32Width, u32Height;
    ...
    ********************************/
    ...
    {
        s32Ret = AR_MPI_VO_SetDisplayBufLen(VoLayer, pstVoLayerConfig->u32DisBufLen);
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VO_SetDisplayBufLen failed with %#x!\n",s32Ret);
            return s32Ret;
...
AR_VOID SAMPLE_COMM_VO_Exit()
{
    ...
    for (i=0; i<16; i++)
    {
        AR_MPI_VO_DisableChn(1, i);
    }

    for (i=0; i<1; i++)
    {
        AR_MPI_VO_DisableChn(2, i);
    }


    for (i = 0; i < VO_MAX_LAYER_NUM; i++)
    {
        AR_MPI_VO_DisableVideoLayer(i);
    }
...
*/
static AR_VOID SAMPLE_PRIVATE_VO_InitScreen1080x1920(AR_S32 fd)
{
    AR_S32 s32Ret;
    cmd_info_t cmd_info;
    ...
    cmd_info.cmd = NULL;
    s32Ret = ioctl(fd, AR_MIPI_TX_SET_CMD, &cmd_info);
    if (AR_SUCCESS != s32Ret)
    {
...
*/
static AR_VOID SAMPLE_PRIVATE_VO_InitScreen720x1280(AR_S32 s32fd)
{
    AR_S32     fd     = s32fd;
    AR_S32     s32Ret;
    AR_U8      cmd[30];
    ...
    //armm 0x11170070 0xa99790c7;armm 0x11170070 0x89a888b7;armm 0x11170070 0x8888a98a;armm 0x11170070 0x77777888;armm 0x11170070 0x00667777;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x90;
    cmd[2] = 0x97;
    cmd[3] = 0xa9;
    ...
    // armm 0x11170070 0x999970c7;armm 0x11170070 0x88b887b7;armm 0x11170070 0x8888a989;armm 0x11170070 0x77778888;armm 0x11170070 0x00777777;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x70;
    cmd[2] = 0x99;
    cmd[3] = 0x99;
    ...
    // armm 0x11170070 0x998970c7;armm 0x11170070 0x88a888a7;armm 0x11170070 0x88889989;armm 0x11170070 0x78888888;armm 0x11170070 0x00777777;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x70;
    cmd[2] = 0x89;
    cmd[3] = 0x99;
    ...
    // armm 0x11170070 0xa89790c7;armm 0x11170070 0x98a878a7;armm 0x11170070 0x88889988;armm 0x11170070 0x78888888;armm 0x11170070 0x00777777;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x90;
    cmd[2] = 0x97;
    cmd[3] = 0xa8;
    ...
    // armm 0x11170070 0x898790c7;armm 0x11170070 0x89988898;armm 0x11170070 0x88889889;armm 0x11170070 0x88888888;armm 0x11170070 0x00777778;
    cmd[0] = 0xc7;
    cmd[1] = 0x90;
    cmd[2] = 0x87;
    cmd[3] = 0x89;
    ...
    // armm 0x11170070 0x988790c7;armm 0x11170070 0x88988897;armm 0x11170070 0x88889889;armm 0x11170070 0x88888888;armm 0x11170070 0x00777888;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x90;
    cmd[2] = 0x87;
    cmd[3] = 0x98;
    ...
    //armm 0x11170070 0x888790c7;armm 0x11170070 0x88988798;armm 0x11170070 0x88888888;armm 0x11170070 0x88888888;armm 0x11170070 0x00888888;armm 0x1117006c 0x00001329;
    cmd[0] = 0xc7;
    cmd[1] = 0x90;
    cmd[2] = 0x87;
    cmd[3] = 0x88;
    ...
    // armm 0x11170070 0x010001d6;armm 0x11170070 0x014d0100;armm 0x11170070 0x019a01b3;armm 0x11170070 0x0000009a;armm 0x1117006c 0x00000d29;
    cmd[0] = 0xd6;
    cmd[1] = 0x01;
    cmd[2] = 0x00;
    cmd[3] = 0x01;
    ...
    // armm 0x11170070 0x159a01d6;armm 0x11170070 0x159a159a;armm 0x11170070 0x019a019a;armm 0x11170070 0x0000009a;armm 0x1117006c 0x00000d29;
    cmd[0] = 0xd6;
    cmd[1] = 0x01;
    cmd[2] = 0x9a;
    cmd[3] = 0x15;
    ...
    //armm 0x11170070 0x001133d6;armm 0x11170070 0x66771166;armm 0x11170070 0x11666611;armm 0x11170070 0x00000066;armm 0x1117006c 0x00000d29;
    cmd[0] = 0xd6;
    cmd[1] = 0x33;
    cmd[2] = 0x11;
    cmd[3] = 0x00;
    ...
    // armm 0x11170070 0x661166d6;armm 0x11170070 0x00661166;armm 0x1117006c 0x00000729;
    cmd[0] = 0xd6;
    cmd[1] = 0x66;
    cmd[2] = 0x11;
    cmd[3] = 0x66;
    ...
    //armm 0x11170070 0x331133d6;armm 0x11170070 0x333c1133;armm 0x11170070 0x11333311;armm 0x11170070 0x00000033;armm 0x1117006c 0x00000d29;
    cmd[0] = 0xd6;
    cmd[1] = 0x33;
    cmd[2] = 0x11;
    cmd[3] = 0x33;
    ...
    // armm 0x11170070 0x331133d6;armm 0x11170070 0x00331133;armm 0x1117006c 0x00000729;
    cmd[0] = 0xd6;
    cmd[1] = 0x33;
    cmd[2] = 0x11;
    cmd[3] = 0x33;
    ...
    //armm 0x11170070 0x00aaaad9;armm 0x1117006c 0x00000329;
    cmd[0] = 0xd9;
    cmd[1] = 0xaa;
    cmd[2] = 0xaa;
    cmd[3] = 0x00;
    ...
    // armm 0x11170070 0x8f877fca;armm 0x11170070 0xafa79f97;armm 0x11170070 0xcfc7bfb7;armm 0x11170070 0xefe7dfd7;armm 0x11170070 0x000000f7;armm 0x1117006c 0x00001129;
    cmd[0] = 0xca;
    cmd[1] = 0x7f;
    cmd[2] = 0x87;
    cmd[3] = 0x8f;
    ...
    // armm 0x11170070 0xffffffff;armm 0x1117006c 0x00000429;
    cmd[0] = 0xff;
    cmd[1] = 0xff;
    cmd[2] = 0xff;
    cmd[3] = 0xff;
...
*/
static void SAMPLE_PRIVATE_VO_InitMipiTxScreen(VO_INTF_SYNC_E enVoIntfSync, AR_S32 fd)
{
...
static void SAMPLE_PRIVATE_VO_LcdGpioInti(void)
{
...
static int SAMPLE_PRIVATE_7INCH_LCD_Init(VO_DEV VoDev)
{

    SAMPLE_PRIVATE_VO_LcdGpioInti();
    ...
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("set vo dev_%d dsi attr failed with %#x!\n", VoDev, s32Ret);
        return AR_FAILURE;
    }
    ...
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("enable vo dev_%d dsi failed with %#x!\n", VoDev, s32Ret);
        return AR_FAILURE;
    }
	return AR_SUCCESS;
}
...
{
	int ret = -1;
	SAMPLE_PRT("init panel, id: %d!\n", eLcdId);
	switch (eLcdId)
	{
		case LCD_ID_7INCH_LCD:
			ret = SAMPLE_PRIVATE_7INCH_LCD_Init(VoDev);
			break;
		default:
			SAMPLE_PRT("no matched panel id: %d!\n", eLcdId);
			break;
...
void SAMPLE_COMM_VO_StartMipiTx(VO_INTF_SYNC_E enVoIntfSync)
{
    ...
    fd = open("/dev/ar_mipi_tx", O_RDWR);
    if (fd < 0)
    {
        SAMPLE_PRT("open ar_mipi_tx dev failed\n");
        return;
    }
    ...
    */
    s32Ret = ioctl(fd, AR_MIPI_TX_SET_DEV_CFG, pstMipiTxConfig);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    */
    s32Ret = ioctl(fd, AR_MIPI_TX_ENABLE);
    if (AR_SUCCESS != s32Ret)
    {
