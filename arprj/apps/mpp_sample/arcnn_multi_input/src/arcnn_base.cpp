...
AR_S32 CARCNN::GetLocalInput(AR_CHAR * pchFileName, AR_IMG_SET_S *stImg, AR_U32 u32BatchId)
{
    ...
    if(u32FileType == RGB_RAW_DATA_FILE || u32FileType == BGR_RAW_DATA_FILE || u32FileType == RGBD_RAW_DATA_FILE)
    {
	    ...
	    for(in_idx = 0; in_idx < u32InputTensorNum; in_idx++)
	    {
	    	AR_MPI_NPU_GetInputTensorParam(handle, in_idx, &stTensor);
	    ...
	    if(in_idx == u32InputTensorNum)
	    {
	    	Log(ARCNN_LOG_ERR, "Input File %s Not Compare With Tensor %s !", pchFileName, stTensor.achName);
	    	return -1;
	    }

		if(u32IfcPreProcess[in_idx])
		{
		    stImg->astInputImg[in_idx].bPreIfcProcess = AR_TRUE;
			...
			if(file.is_open())
			{
				...
				pstImg = &(stImg->astInputImg[in_idx].astBatchImg[u32BatchId]);
		...
		{
		    stImg->astInputImg[in_idx].bPreIfcProcess = AR_FALSE;
            ifstream file(pchFileName, ios::in|ios::binary|ios::ate);
	        if(file.is_open())
	        {
	        	s32FileSize = file.tellg();
	...
	else if(u32FileType == JPG_BMP_PNG_IMG_FILE)
    {
		...
		printf("in_idx %d FileName %s stTensor.achName %s \r\n",in_idx,pchFileName,stTensor.achName);
	...
	else if(u32FileType == FLOAT_RAW_DATA_FILE)
    {
		...
		{
			AR_MPI_NPU_GetInputTensorParam(handle, in_idx, &stTensor);
			AR_MPI_NPU_GetInputTensorExtParam(handle, in_idx, &stTensorExt);
			if(u32InputTensorNum ==1)
			{
				break;
			}
        ...
        AR_MEM_S stFileBuff;
        stFileBuff.u64Len= s32FileSize;
        int ret = AR_MPI_NPU_MallocBuff((AR_CHAR *)"FileBuff", &stFileBuff);
        if(ret)
        {
            Log(ARCNN_LOG_ERR, "Malloc NPU input error: size %d.\n", stFileBuff.u64Len);
            return -1;
        ...
        memset((AR_CHAR *)stFileBuff.u64VirtAddr, 0, stFileBuff.u64Len);
        file.read((AR_CHAR *)stFileBuff.u64VirtAddr, s32FileSize);
        file.close();
		...
		AR_CHAR * fpReadBuff = (AR_CHAR *)stFileBuff.u64VirtAddr;
		//adjust the input tensor is NCHW format or not
        if((!strcmp(stTensorExt.achDdrFormat, "blocking-NHWC") && stTensor.u32KSizeNorm == 1)
            || !strcmp(stTensorExt.achDdrFormat, "NCHW"))
        {
            AR_U32 u32TensorSize = stTensor.u32KStep * stTensor.u32OriChannels;
            memset((AR_CHAR *)stTensorAddr.u64VirtAddr, 0, u32TensorSize);			
#if 1                    		
            ProfileStart("float_to_fix");
            #ifdef NHWC_NCHW_NEON
            AR_NPU_FloatCvtFix_With_Stride_Neon((AR_FLOAT *)fpReadBuff, (AR_CHAR *)stTensorAddr.u64VirtAddr, &stTensor);
            #else
            AR_NPU_FloatCvtFix_With_Stride((AR_FLOAT *)fpReadBuff, (AR_CHAR *)stTensorAddr.u64VirtAddr, &stTensor);
            #endif
			ProfileEnd("float_to_fix");	
#else
            //for float convert to fix call dsp
            AR_DSP_PRE_MSG_S stDspMsg = {0};
            AR_DSP_TASK_S stDspTask = {0};
            
            stDspMsg.u32TensorInPhy = stFileBuff.u64PhyAddr & 0x0FFFFFFFF;
            stDspMsg.u32TensorOutPhy = stTensorAddr.u64PhyAddr & 0x0FFFFFFFF;
            stDspMsg.u32Width = stTensor.u32Width;
            stDspMsg.u32Height = stTensor.u32Height;
            stDspMsg.u32Num = stTensor.u32Num;
            stDspMsg.u32Precision = stTensor.u32Precision;
            stDspMsg.u32OriChannels = stTensor.u32OriChannels;
            stDspMsg.u32KNormNum = stTensor.u32KNormNum;
            stDspMsg.u32KSizeLast = stTensor.u32KSizeLast;
            stDspMsg.u32KSizeNorm = stTensor.u32KSizeNorm;
            stDspMsg.u32KStep = stTensor.u32KStep;
            stDspMsg.dScaleFactor = stTensor.dScaleFactor;
            stDspMsg.s32ZeroPoint = stTensor.s32ZeroPoint;
            stDspMsg.u32RowStep = stTensor.u32RowStep;
            stDspMsg.u32TensorStep = stTensor.u32TensorStep;
            
            strncpy(stDspTask.s8Name, "ArArmFloatCvtFix", 31);
            stDspTask.u64MsgBodyAddr = (AR_U64)&stDspMsg;
            stDspTask.u32MsgLen = sizeof(AR_DSP_PRE_MSG_S);
            
            ret = AR_MPI_DSP_RemoteCall(0, &stDspTask, 1);
            if(ret < 0)
            {		
                Log(ARCNN_LOG_ERR, "Input Tensor call Dsp failed!");
            	AR_MPI_NPU_FreeBuff(&stFileBuff);
            	return -1;
            }
#endif
        }
        ...
        fp1 = fopen(name, "wb+");
        fwrite((AR_CHAR *)stTensorAddr.u64VirtAddr, 1, u32TensorSize, fp1);
        fclose(fp1);
	#endif
    }
...
AR_S32 CARCNN::FillInputBuff(AR_CHAR * pchFileName, AR_U32 u32InputId, AR_U32 u32BatchId)
{
    ...
    Log(ARCNN_LOG_DEBUG, "Get NOIFC [%s][%d] to input ..\n", pchFileName, s32FileSize);
    return 0;
}
...
AR_S32 CARCNN::GetLocalImage(AR_CHAR * pchFileName, AR_IMG_S * pstImg)
{
	...
	else if(u32FileType == JPG_BMP_PNG_IMG_FILE)
    {
		...
		else
		{
			...
			pstImg->enFormat= AR_IMG_RGB;
			pstImg->astChannels[0].u32AddrPhy  = (AR_UINTPTR)stPchbuff.u64PhyAddr;
			pstImg->astChannels[0].uptrAddrVirt = (AR_UINTPTR)(pchDstBuffVirt + u32ChSize);
			pstImg->astChannels[1].u32AddrPhy = (AR_UINTPTR)(stPchbuff.u64PhyAddr + u32ChSize);
			pstImg->astChannels[1].uptrAddrVirt = (AR_UINTPTR)(pchDstBuffVirt + u32ChSize);
			pstImg->astChannels[2].u32AddrPhy = (AR_UINTPTR)(stPchbuff.u64PhyAddr + 2 * u32ChSize);
			pstImg->astChannels[2].uptrAddrVirt = (AR_UINTPTR)(pchDstBuffVirt + 2 *u32ChSize);
    ...
    Log(ARCNN_LOG_DEBUG, "Get IFC [%s][%d] to input ..\n", pchFileName, s32FileSize);
    return 0;
}
