    ...
    switch (enSnsType)
    {
	 ...
	 case SC_SC530AI_MIPI_5M_30FPS_10BIT:
        ...
        case ISP_VIN_IR_THERMAL_GST212W4:
            ...
            pstComboAttr->img_rect.width = 256 + 1;
        ...
        case ISP_RAW_SIMULATION:
            if(stSnsIspIn0Obj.pfnGetDefaultAttr)
            {
                ...
                memcpy(pstComboAttr, &stDefaultAttr.stComboAttr, sizeof(*pstComboAttr));
                IPC_CFG_VIO_S * pstVio = IPC_CFG_VIO_GetParam();
                pstComboAttr->img_rect.x = pstVio->vi[0].stComboAttr.stRect.s32X;
                pstComboAttr->img_rect.x = pstVio->vi[0].stComboAttr.stRect.s32Y;
                pstComboAttr->img_rect.width = pstVio->vi[0].stComboAttr.stRect.u32Width;
                pstComboAttr->img_rect.height = pstVio->vi[0].stComboAttr.stRect.u32Height;
                pstComboAttr->mipi_attr.input_data_type = pstVio->vi[0].stComboAttr.enMipiDataType;
                break;
            }
            else
            {
                PRINT_ERR("Get Combo default attr failed.\n");
                return -1;
            }
...
AR_S32 IPC_PF_VI_GetDevAttrBySns(IPC_SNS_TYPE_E enSnsType, VI_DEV_ATTR_S* pstViDevAttr)
{
	...
	case SC_SC530AI_MIPI_5M_30FPS_10BIT:
        ...
        case ISP_VIN_IR_THERMAL_GST212W4:
            ...
            pstViDevAttr->au32ComponentMask[0] = 0xfffc0000;
            pstViDevAttr->au32ComponentMask[1] = 0;
            pstViDevAttr->stSynCfg.stTimingBlank.u32HsyncAct = 256 + 1;
            pstViDevAttr->stSynCfg.stTimingBlank.u32VsyncVact = 200;
            pstViDevAttr->stSize.u32Width = 256 + 1;
            pstViDevAttr->stSize.u32Height = 200;
            pstViDevAttr->stBasAttr.stSacleAttr.stBasSize.u32Width = 256 + 1;
        ...
        case ISP_RAW_SIMULATION:
            ...
            {
                stSnsIspIn0Obj.pfnGetDefaultAttr(0, &stDefaultAttr);
                memcpy(pstViDevAttr, &stDefaultAttr.stDevAttr, sizeof(*pstViDevAttr));
                IPC_CFG_VIO_S * pstVio = IPC_CFG_VIO_GetParam();
                pstViDevAttr->stSize.u32Width = pstVio->vi[0].stDevAttr.u32Width;
                pstViDevAttr->stSize.u32Height = pstVio->vi[0].stDevAttr.u32Height;
                pstViDevAttr->stBasAttr.stSacleAttr.stBasSize.u32Width = pstVio->vi[0].stDevAttr.u32BayerScaleWidth;
                pstViDevAttr->stBasAttr.stSacleAttr.stBasSize.u32Height = pstVio->vi[0].stDevAttr.u32BayerScaleHeight;
                break;
            }
            else
            {
                PRINT_ERR("Get Dev default attr failed.\n");
                return -1;
            }
        default:
            memcpy(pstViDevAttr, &DEV_ATTR_IMX307_2M_BASE, sizeof(VI_DEV_ATTR_S));
...
AR_S32 IPC_PF_VI_GetPipeAttrBySns(IPC_SNS_TYPE_E enSnsType, VI_PIPE_ATTR_S* pstPipeAttr)
{
	...
	case SC_SC530AI_MIPI_5M_30FPS_10BIT:
        ...
        case ISP_RAW_SIMULATION:
            ...
            {
                stSnsIspIn0Obj.pfnGetDefaultAttr(0, &stDefaultAttr);
                memcpy(pstPipeAttr, &stDefaultAttr.stPipeAttr, sizeof(*pstPipeAttr));
                IPC_CFG_VIO_S * pstVio = IPC_CFG_VIO_GetParam();
                pstPipeAttr->u32MaxW = pstVio->vi[0].stPipeAttr.u32MaxW;
                pstPipeAttr->u32MaxH = pstVio->vi[0].stPipeAttr.u32MaxH;
                pstPipeAttr->enPixFmt = pstVio->vi[0].stPipeAttr.enPixFmt;
                pstPipeAttr->enBitWidth = pstVio->vi[0].stPipeAttr.enBitWidth;
                pstPipeAttr->stNrAttr.enPixFmt = pstVio->vi[0].stPipeAttr.enNrPixFmt;
                pstPipeAttr->stNrAttr.enBitWidth = pstVio->vi[0].stPipeAttr.enNrBitWidth;
                pstPipeAttr->enEnableNoCmp = VIN_PIPE_NOCPM_ENABLE; //NO compress
                pstPipeAttr->enCompressMode = COMPRESS_MODE_NONE;
                pstPipeAttr->stNrAttr.enCompressMode = pstVio->vi[0].stPipeAttr.enNrCompressMode;
                break;
            }
            else
            {
                PRINT_ERR("Get Pipe default attr failed.\n");
                return -1;
            }
		default:
            memcpy(pstPipeAttr, &PIPE_ATTR_3840x2160_RAW12_420_3DNR_RFR, sizeof(VI_PIPE_ATTR_S));
...
AR_S32 IPC_PF_VI_GetChnAttrBySns(IPC_SNS_TYPE_E enSnsType, VI_CHN_ATTR_S* pstChnAttr)
{
	...
	case SC_SC530AI_MIPI_5M_30FPS_10BIT:
        ...
        case ISP_RAW_SIMULATION:
            ...
            {
                stSnsIspIn0Obj.pfnGetDefaultAttr(0, &stDefaultAttr);
                memcpy(pstChnAttr, &stDefaultAttr.stChnAttr, sizeof(*pstChnAttr));
                IPC_CFG_VIO_S * pstVio = IPC_CFG_VIO_GetParam();
                pstChnAttr->stSize = pstVio->vi[0].stChnAttr.stSize;
                pstChnAttr->enPixelFormat = pstVio->vi[0].stChnAttr.enPixelFormat;
                break;
            }
            else
            {
                PRINT_ERR("Get Chn default attr failed.\n");
                return -1;
            }
        default:
            memcpy(pstChnAttr, &CHN_ATTR_3840x2160_420_SDR8_LINEAR, sizeof(VI_CHN_ATTR_S));
