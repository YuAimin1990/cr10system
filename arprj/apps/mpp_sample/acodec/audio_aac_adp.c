...
#define AR_AUDIO_ASSERT(x) {if (AR_TRUE != (x))return -1;}

static AR_S32 AencCheckAACAttr(const AENC_ATTR_AAC_S *pstAACAttr)
{
	if (pstAACAttr->enBitWidth != AUDIO_BIT_WIDTH_16)
	{
		printf("[Func]:%s [Line]:%d [Info]:%s\n",
			__FUNCTION__, __LINE__, "invalid bitwidth for AAC encoder");
		return AR_ERR_AENC_ILLEGAL_PARAM;
	}
...
static AR_S32 AencCheckAACLCConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32MinBitRate = 0;
	AR_S32 s32MaxBitRate = 0;
	AR_S32 s32RecommemdRate = 0;
...
static AR_S32 AencCheckEAACConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32MinBitRate = 0;
	AR_S32 s32MaxBitRate = 0;
	AR_S32 s32RecommemdRate = 0;
...
static AR_S32 AencCheckEAACPLUSConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32MinBitRate = 0;
	AR_S32 s32MaxBitRate = 0;
	AR_S32 s32RecommemdRate = 0;
...
static AR_S32 AencCheckAACLDConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32MinBitRate = 0;
	AR_S32 s32MaxBitRate = 0;
	AR_S32 s32RecommemdRate = 0;
...
static AR_S32 AencCheckAACELDConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32MinBitRate = 0;
	AR_S32 s32MaxBitRate = 0;
	AR_S32 s32RecommemdRate = 0;
...
AR_S32 AencAACCheckConfig(AACENC_CONFIG *pconfig)
{
	AR_S32 s32Ret = 0;
...
AR_S32 OpenAACEncoder(AR_VOID *pEncoderAttr, AR_VOID **ppEncoder)
{
	...
	AR_AUDIO_ASSERT(pEncoderAttr != NULL);
	AR_AUDIO_ASSERT(ppEncoder != NULL);
	...
	if(NULL == pstEncoder)
	{
		...
		return AR_ERR_AENC_NOMEM;
	}
	memset(pstEncoder, 0, sizeof(AENC_AAC_ENCODER_S));
	*ppEncoder = (AR_VOID *)pstEncoder;
		...
		AR_U8 *pu8Outbuf,  AR_U32 *pu32OutLen)
{
	...
	AR_U32 u32PtNums;
	AR_S32 i;
	AR_S16 aData[AACENC_BLOCKSIZE*2*MAX_CHANNELS];
	AR_S16 s16Len = 0;

	AR_U32 u32WaterLine;

	AR_AUDIO_ASSERT(pEncoder != NULL);
	AR_AUDIO_ASSERT(pstData != NULL);
	AR_AUDIO_ASSERT(pu8Outbuf != NULL);
	AR_AUDIO_ASSERT(pu32OutLen != NULL);
	...
	if (AUDIO_SOUND_MODE_STEREO == pstEncoder->stAACAttr.enSoundMode)
	{
		...
		{
			///!!!!!
			aData[2*i] = *((AR_S16 *)pstData->u64VirAddr[0] + i);
			aData[2*i+1] = *((AR_S16 *)pstData->u64VirAddr[1] + i);
		}
		chanels = 2;
	}
	else/* if inbuf is momo, copy left to right */
	{
		AR_S16 *temp = (AR_S16 *)pstData->u64VirAddr[0];
		s16Len = u32WaterLine;
		for (i = s16Len-1; i >= 0 ; i--)
		{
			aData[i] = *(temp + i);
	...
	//if (cnt_aenc > 0)
	{
		fwrite((AR_U8 *)aData, 1, (AUDIO_SOUND_MODE_STEREO == pstEncoder->stAACAttr.enSoundMode)?u32count*2:u32count, pfdin_ENC);
	}
#endif

	s32Ret = AACEncoderFrame(pstEncoder->pstAACState, aData, pstData->u32Len * chanels, pu8Outbuf, (AR_S32*)pu32OutLen);
	...
	//if (cnt_aenc > 0)
	{
		AR_U8 *pu8TestNum = pu8Outbuf + 1;
		//printf("Enc:(%02x %02x), ~~~~~~~~~strlen:%d~~~~~~\n", *pu8Outbuf, *pu8TestNum, *((AR_S32*)pu32OutLen));
		fwrite((AR_U8 *)pu8Outbuf, 1, *((AR_S32*)pu32OutLen), pfdout_ENC);
		cnt_aenc --;
	}
#endif
	return s32Ret;
...
AR_S32 OpenAACDecoder(AR_VOID *pDecoderAttr, AR_VOID **ppDecoder)
{
    ...
    AR_AUDIO_ASSERT(pDecoderAttr != NULL);
    AR_AUDIO_ASSERT(ppDecoder != NULL);
    ...
    {
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "no memory");
        //return AR_ERR_ADEC_NOMEM;
        return -1;
    }
    memset(pstDecoder, 0, sizeof(ADEC_AAC_DECODER_S));
    *ppDecoder = (AR_VOID *)pstDecoder;

    /* create decoder */
    //printf("~~~~~~~~ADEC trans type:%d ~~~~~~~~~\n", pstAttr->enTransType);
    pstDecoder->pstAACState = AACInitDecoder(pstAttr->enTransType);
    if (!pstDecoder->pstAACState)
    {
        free(pstDecoder);
        printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__, "AACInitDecoder failed");
        return AR_ERR_ADEC_DECODER_ERR;
    }
    ...
    cnt_adec = 10000;
#endif
	printf("open trans type:%d decoder ok\n", pstAttr->enTransType);
    return AR_SUCCESS;
}
                        ...
                        AR_U16 *pu16Outbuf,AR_U32 *pu32OutLen,AR_U32 *pu32Chns)
{
    AR_S32 s32Ret = AR_SUCCESS;
    ADEC_AAC_DECODER_S *pstDecoder = NULL;
    AR_S32 s32Samples, s32FrmLen, s32SampleBytes;
    AACFrameInfo aacFrameInfo;

    AR_AUDIO_ASSERT(pDecoder != NULL);
    AR_AUDIO_ASSERT(pu8Inbuf != NULL);
    AR_AUDIO_ASSERT(ps32LeftByte != NULL);
    AR_AUDIO_ASSERT(pu16Outbuf != NULL);
    AR_AUDIO_ASSERT(pu32OutLen != NULL);
    AR_AUDIO_ASSERT(pu32Chns != NULL);
    ...
    *pu32Chns = 1;/*voice encoder only one channle */
...
#ifdef DUMP_AACDEC
    AR_U32 u32PreLen = *ps32LeftByte;
#endif

    //AR_S32 valid = s32FrmLen;
    /*Notes: pInbuf will updated*/
	// ps32LeftByte为输入数据剩余长度
    s32Ret = AACDecodeFrame(pstDecoder->pstAACState, pu8Inbuf, ps32LeftByte, (AR_S16 *)pu16Outbuf);

	//流式解码 输入数据不足完整一帧时需返回 HAL_ERR_ADEC_BUF_LACK, SDK会进行缓存到下一帧合并处理
	if(ERR_AAC_INVALID_ADTS_HEADER == s32Ret || ERR_AAC_INVALID_ADIF_HEADER == s32Ret){
		return AR_ERR_ADEC_BUF_LACK;
	}else if(s32Ret){
    ...
    s32SampleBytes = s32Samples * sizeof(AR_U16);
    *pu32Chns = aacFrameInfo.nChans;
    *pu32OutLen = s32SampleBytes;

//	printf("u32Chns:%d u32OutLen:%d\n", *pu32Chns, *pu32OutLen);
#ifdef DUMP_AACDEC
    //if (cnt_adec > 0)
    {
        //printf("Dec: ~~~~~~~~(%d, %d)~~~~~~~\n", aacFrameInfo.outputSamps, *pu32OutLen);
        fwrite((AR_U8 *)pu8BaseBuf, 1, (u32PreLen-(*ps32LeftByte)), pfdin_DEC);
        fwrite((AR_U8 *)pu16Outbuf, 1, (*pu32OutLen) * (*pu32Chns), pfdout_DEC);
        fwrite((AR_U8 *)pu16Outbuf, 1, (*pu32OutLen), pfdout_DEC_L);
        cnt_adec --;
    }
#endif
    ...
    if (MAX_AUDIO_FRAME_LEN != s32SampleBytes)
    {
        ...
        memmove((AR_U8*)pOutbuf + MAX_AUDIO_FRAME_LEN,
            (AR_U8*)pOutbuf + s32SampleBytes, s32SampleBytes);
    }
#endif
...
AR_S32 ResetAACDecoder(AR_VOID *pDecoder)
{
    ...
    /* create decoder */
    pstDecoder->pstAACState = AACInitDecoder(pstDecoder->stAACAttr.enTransType);
    if (!pstDecoder->pstAACState)
    {
...
AR_S32 AENC_AacInit(AR_VOID)
{
    ...
    s32Ret = AR_MPI_AENC_RegisterEncoder(&s32Handle, &stAac);
    if (s32Ret)
    {
        return s32Ret;
    }
...
AR_S32 ADEC_AacInit(AR_VOID)
{
    ...
    s32Ret = AR_MPI_ADEC_RegisterDecoder(&s32Handle, &stAac);
    if (s32Ret)
    {
        return s32Ret;
    }
