...
static int entry_index(int h, int w, int c, int byteUnit, AR_NPU_TENSOR_S &pTensorInfo)
{
	/*
...
SSD_detection::SSD_detection(NET_TYPE net_type, AR_NPU_TENSOR_S *outputs_tensor, int tensorNum, int class_num = 21, float nms_threshold = 0.5f, float dect_threshold = 0.45f, int input_width = 300, int input_height = 300)
{
	...
	this->pTensor = (AR_NPU_TENSOR_S*)malloc(tensorNum*sizeof(AR_NPU_TENSOR_S));
	memcpy(this->pTensor, outputs_tensor, tensorNum*sizeof(AR_NPU_TENSOR_S));
...
void SSD_detection::fixedToFloatHWCSSD(float *dst, short * src, AR_NPU_TENSOR_S &tensor, int index)
{
...
void SSD_detection::fixedToFloatHWCSSD(float *dst, char *src, AR_NPU_TENSOR_S &tensor, int index)
{
	int channel = tensor.u32OriChannels;
	int width = tensor.u32Width;
	int height = tensor.u32Height;
...
void SSD_detection::fixedToFloatHWCSSD(float *dst, float *src, AR_NPU_TENSOR_S &tensor, int index)
{
	int channel = tensor.u32OriChannels;
	int width = tensor.u32Width;
	int height = tensor.u32Height;
...
void SSD_detection::CHW_FloatToHWC_Float(float *dst,float *src, AR_NPU_TENSOR_S &tensor, int index)
{
	int channel = tensor.u32OriChannels;
	int width = tensor.u32Width;
	int height = tensor.u32Height;
...
void SSD_detection::CHW_ShortToHWC_Float(float *dst,short *src, AR_NPU_TENSOR_S &tensor, int index)
{
	int channel = tensor.u32OriChannels;
	int width = tensor.u32Width;
	int height = tensor.u32Height;
	...
	for (int h = 0; h < height; h++)
	{
		for (int w = 0; w < width; w++)
		{
			for (int c = 0; c < channel; c++)
			{
			    dst[index + h*width*channel + w*channel + c] = (src[c*height*width + h*width+w]- tensor.s32ZeroPoint)*tensor.dScaleFactor;
			}
		}
	}
}
void SSD_detection::CHW_CharToHWC_Float(float *dst,char *src, AR_NPU_TENSOR_S &tensor, int index)
{
	int channel = tensor.u32OriChannels;
	int width = tensor.u32Width;
	int height = tensor.u32Height;
...
void SSD_detection::SetDetectParam(int class_num, int input_width, int input_height, float nms_threshold, float dect_threshold)
{
...
void SSD_detection::PermuteSSD(short *ssdBuff)
{
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i + 1].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i + 1].u32Offset) / sizeof(short);
		short *src = ssdBuff + offset;
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i].u32Offset) / sizeof(short);
		short *src = ssdBuff + offset;
...
void SSD_detection::PermuteSSD(char *ssdBuff)
{
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int width = pTensor[2 * i + 1].u32Width;
		int height = pTensor[2 * i + 1].u32Height;
		int offset = (pTensor[2 * i + 1].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i + 1].u32Offset) / sizeof(char);
		char *src = ssdBuff + offset;
		fixedToFloatHWCSSD(scores_flat, src, pTensor[2 * i + 1], dst_offset);
		dst_offset += width*height*channel;
	}
	dst_offset = 0;
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int width = pTensor[2 * i].u32Width;
		int height = pTensor[2 * i].u32Height;
		int offset = (pTensor[2 * i].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i].u32Offset) / sizeof(char);
		char *src = ssdBuff + offset;
		fixedToFloatHWCSSD(deltas_flat, src, pTensor[2 * i], dst_offset);
		dst_offset += width*height*channel;
	}
...
void SSD_detection::CHW_PermuteSSD(float *ssdBuff)
{
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i + 1].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i + 1].u32Offset) / sizeof(float);
		float *src = ssdBuff + offset;
		CHW_FloatToHWC_Float(scores_flat, src, pTensor[2 * i + 1], dst_offset);
		dst_offset += width*height*channel;
	}
	dst_offset = 0;
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i].u32Offset) / sizeof(float);
		float *src = ssdBuff + offset;
		CHW_FloatToHWC_Float(deltas_flat, src, pTensor[2 * i], dst_offset);
		dst_offset += width*height*channel;
	}
}
...
void SSD_detection::CHW_PermuteSSD(short *ssdBuff)
{
	int dst_offset = 0;
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i + 1].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i + 1].u32Offset) / sizeof(short);
		short *src = ssdBuff + offset;
		CHW_ShortToHWC_Float(scores_flat, src, pTensor[2 * i + 1], dst_offset);
		dst_offset += width*height*channel;
	}
	dst_offset = 0;
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i].u32Offset) / sizeof(short);
		short *src = ssdBuff + offset;
		CHW_ShortToHWC_Float(deltas_flat, src, pTensor[2 * i], dst_offset);
		dst_offset += width*height*channel;
	}
}


void SSD_detection::CHW_PermuteSSD(char *ssdBuff)
{
	int dst_offset = 0;
	...
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i + 1].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i + 1].u32Offset) / sizeof(char);
		char *src = ssdBuff + offset;
		CHW_CharToHWC_Float(scores_flat, src, pTensor[2 * i + 1], dst_offset);
		dst_offset += width*height*channel;
	}
	dst_offset = 0;
	for (int i = 0; i < tensorNum / 2; i++)
	{
		...
		int offset = (pTensor[2 * i].u32Bank * 32 * 1024 * 1024 + pTensor[2 * i].u32Offset) / sizeof(char);
		char *src = ssdBuff + offset;
		CHW_CharToHWC_Float(deltas_flat, src, pTensor[2 * i], dst_offset);
		dst_offset += width*height*channel;
	}
}
...
#if 1
	box0 = std::min(box0, 1.f);  delta[0] = std::max(box0, 0.f);
	box1 = std::min(box1, 1.f);  delta[1] = std::max(box1, 0.f);
	box2 = std::min(box2, 1.f);  delta[2] = std::max(box2, 0.f);
	box3 = std::min(box3, 1.f);  delta[3] = std::max(box3, 0.f);
#else
	delta[0] = box0;
	delta[1] = box1;
	delta[2] = box2;
...
void SSD_detection::SSD_PostProcess(short *ssdBuff, vector<detection_output> &dets)
{
	...
	std::partial_sort(idex_vector.begin(), idex_vector.begin() + num_dets, idex_vector.end(), std::greater<std::pair<float, int>>());
...
void SSD_detection::SSD_CHW_PostProcess(short *ssdBuff, vector<detection_output> &dets)
{
	assert(4 == dect_param.per_box_num);
	vector<detection_output>result_temp;
	CHW_PermuteSSD(ssdBuff);
...
void SSD_detection::SSD_CHW_PostProcess(char *ssdBuff, vector<detection_output> &dets)
{
	assert(4 == dect_param.per_box_num);
	vector<detection_output>result_temp;
	CHW_PermuteSSD(ssdBuff);
