...
vector<STrack*> BYTETracker::joint_stracks(vector<STrack*> &tlista, vector<STrack> &tlistb)
{
	map<int, int> exists;
	vector<STrack*> res;
	for (AR_U32 i = 0; i < tlista.size(); i++)
	{
		exists.insert(pair<int, int>(tlista[i]->track_id, 1));
		res.push_back(tlista[i]);
	}
	for (AR_U32 i = 0; i < tlistb.size(); i++)
	{
...
void BYTETracker::remove_duplicate_stracks(vector<STrack> &resa, vector<STrack> &resb, vector<STrack> &stracksa, vector<STrack> &stracksb)
{
	...
	for (AR_U32 i = 0; i < pdist.size(); i++)
	{
		for (AR_U32 j = 0; j < pdist[i].size(); j++)
		{
			if (pdist[i][j] < 0.15)
			{
	...
	for (AR_U32 i = 0; i < stracksa.size(); i++)
	{
	...
	for (AR_U32 i = 0; i < stracksb.size(); i++)
	{
	...
	vector<vector<int> > &matches, vector<int> &unmatched_a, vector<int> &unmatched_b)
{
	...
	for (AR_U32 i = 0; i < rowsol.size(); i++)
	{
		if (rowsol[i] >= 0)
		{
			vector<int> match;
	...
	for (AR_U32 i = 0; i < colsol.size(); i++)
	{
		if (colsol[i] < 0)
		{
			unmatched_b.push_back(i);
...
vector<vector<float> > BYTETracker::ious(vector<vector<float> > &atlbrs, vector<vector<float> > &btlbrs)
{
	...
	for (AR_U32 i = 0; i < ious.size(); i++)
	{
		ious[i].resize(btlbrs.size());
	}

	//bbox_ious
	for (AR_U32 k = 0; k < btlbrs.size(); k++)
	{
		vector<float> ious_tmp;
		float box_area = (btlbrs[k][2] - btlbrs[k][0] + 1)*(btlbrs[k][3] - btlbrs[k][1] + 1);
		for (AR_U32 n = 0; n < atlbrs.size(); n++)
		{
...
vector<vector<float> > BYTETracker::iou_distance(vector<STrack*> &atracks, vector<STrack> &btracks, int &dist_size, int &dist_size_size)
{
	...
	for (AR_U32 i = 0; i < atracks.size(); i++)
	{
		atlbrs.push_back(atracks[i]->tlbr);
	}
	for (AR_U32 i = 0; i < btracks.size(); i++)
	{
	...
	for (AR_U32 i = 0; i < _ious.size();i++)
	{
		vector<float> _iou;
		for (AR_U32 j = 0; j < _ious[i].size(); j++)
		{
	...
	bool extend_cost, float cost_limit, bool return_cost)
{
	...
	AR_U32 n_rows = cost.size();
	AR_U32 n_cols = cost[0].size();
	...
	if (extend_cost || cost_limit < LONG_MAX)
	{
		...
		for (AR_U32 i = 0; i < cost_c_extended.size(); i++)
		...
		if (cost_limit < LONG_MAX)
		{
			for (AR_U32 i = 0; i < cost_c_extended.size(); i++)
			{
				for (AR_U32 j = 0; j < cost_c_extended[i].size(); j++)
				{
		...
		{
			float cost_max = -1;
			for (AR_U32 i = 0; i < cost_c.size(); i++)
			{
				for (AR_U32 j = 0; j < cost_c[i].size(); j++)
				{
		...
		for (AR_U32 i = n_rows; i < cost_c_extended.size(); i++)
		{
			for (AR_U32 j = n_cols; j < cost_c_extended[i].size(); j++)
			{
				cost_c_extended[i][j] = 0;
			}
		}
		for (AR_U32 i = 0; i < n_rows; i++)
		{
			for (AR_U32 j = 0; j < n_cols; j++)
			{
