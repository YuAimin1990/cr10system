...
public:
    ...
    AR_VOID RegisterPostCB(PostProcessCB cbFuncPtr);
...
private:
	...
	yolov8_Param yolov8Param = {0};

	cv::Mat src_img;
	cv::Mat dst_img;
	
	AR_U32 u32TensorsId[6] = {0};
	...
	std::thread drawOsdThread;
	std::mutex osdMutex;
	std::queue<IPC_CTRL_OSD_S> osdQueue;
