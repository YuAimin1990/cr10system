...
#define IPC_RPC_CMD_DRAW_VPSS_GEOM_OSD (0x76540000+52)
...
BlobData* createBlob(int num, int channels, int height, int width)
{
	...
	blob->data = new float[blob->count];
...
static float toFloat(signed char s8, AR_NPU_TENSOR_S *pTensor)
{
    AR_S32 s32Value = s8;
    return (s32Value-pTensor->s32ZeroPoint)*pTensor->dScaleFactor;
}

static float toFloat1(short s16, AR_NPU_TENSOR_S *pTensor)
{
    AR_S32 s32Value = s16;
    return (s32Value-pTensor->s32ZeroPoint)*pTensor->dScaleFactor;
}
		...
		for (k = 0; k < k_norm_num; k++)
		{
			for (c = 0; c < k_size_norm; c++)
			{
				for(h = 0; h < height * batch; h++)
				{
				    ...
				    for(w = 0; w < width; w++)
                    {
                        *dst++ = toFloat1(*(src + w * k_size_last + c), pTensor);
                    }
				}
			}
		}
    ...
    else
    {
		...
		for (k = 0; k < k_norm_num; k++)
		{
			for (c = 0; c < k_size_norm; c++)
			{
				for(h = 0; h < height * batch; h++)
				{
				    ...
				    for(w = 0; w < width; w++)
                    {
                        *dst++ = toFloat(*(src + w * k_size_norm + c), pTensor);
                    }
				}
			}
		}
		...
		for (c = 0; c < k_size_last; c++)
		{
			for (h = 0; h < height * batch; h++)
			{
				...
				for (w = 0; w < width; w++)
				{
					*dst++ = toFloat(*(src + w * k_size_last + c), pTensor);
				}
			}
		}
    }
...
void hwcTochwNeon(const char * hwc, AR_NPU_TENSOR_S * pTensor, float *chw)
{
	int c = 0, w = 0, h = 0, k = 0;
	unsigned int width = pTensor->u32Width;
	unsigned int height = pTensor->u32Height;
	...
	//the norm block
    if(precision == 16){
        return hwcTochw(hwc, pTensor, chw);
    }else{
        float* dst = chw;
        AR_S8 * src = (AR_S8 *)hwc;
        for (k = 0; k < k_norm_num; k++)
	...
	const float interThreshold, const int minSubsetCnt, const float minSubsetScore, const float scaleFactor,std::vector<float>& poseKeypoints,  std::vector<int>& keypointShape)
{
    ...
    //std::cout << "Keypoint Shape: " << keypointShape[0] <<", " << keypointShape[1] << ", " << keypointShape[2] << std::endl;
	if (numberPeople > 0)
...
{
    const std::vector<unsigned int> pairs = POSE_COCO_PAIRS_RENDER;
    const auto numberKeypoints = keyshape[1];
    float x_scaler = 0.0, y_scaler = 0.0;

    IPC_CTRL_GEOM_OSD_S stOsd = {0};
    memset(&stOsd, 0, sizeof(stOsd));
    stOsd.u32VpssGrp = u32VpssGrp;

    stOsd.u32ImgWidth = imagewidth;
    stOsd.u32ImgHeight = imageheight;
    ...
    for (auto person = 0; person < keyshape[0]; person++)
    {
        ...
        for (auto pair = 0u; pair < pairs.size(); pair += 2)
        {
            //std::cout << "Draw Line " << pair << std::endl;
            const auto index1 = (person * numberKeypoints + pairs[pair]) * keyshape[2];
            const auto index2 = (person * numberKeypoints + pairs[pair + 1]) * keyshape[2];
            if (keypoints[index1 + 2] > threshold && keypoints[index2 + 2] > threshold && (stOsd.u32RectNum < IPC_ALG_OSD_MAX_NUM-1))
            {
                stOsd.stOsdRect[stOsd.u32RectNum].u32X0 = intRound(keypoints[index1]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32Y0 = intRound(keypoints[index1+1]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32X1 = intRound(keypoints[index2]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32Y1 = intRound(keypoints[index2+1]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32RGB = 0x00FF;//blue
                stOsd.u32RectNum++;
            }
        }
        ...
        {
            const auto faceIndex = (person * numberKeypoints + part) * keyshape[2];
            if (keypoints[faceIndex + 2] > threshold && (stOsd.u32RectNum < IPC_ALG_OSD_MAX_NUM-1))
            {
                stOsd.stOsdRect[stOsd.u32RectNum].u32X0 = intRound(keypoints[faceIndex]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32Y0 = intRound(keypoints[faceIndex+1]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32X1 = intRound(keypoints[faceIndex]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32Y1 = intRound(keypoints[faceIndex+1]);
                stOsd.stOsdRect[stOsd.u32RectNum].u32RGB = 0xFF00;//blue
                stOsd.u32RectNum++;
            }
        }
    }
    if(stOsd.u32RectNum <= 0){
        return 0;
    }

    BINDER_RET_S stBinderRet = {0};
    AR_S32 s32Ret = ar_ipc_send_buffer(pstClient, u32Handle, IPC_RPC_CMD_DRAW_VPSS_GEOM_OSD, &stOsd, sizeof(stOsd), &stBinderRet, 0);
    if(s32Ret<0)
    {
        printf("draw osd err ret=%d\r\n",s32Ret);
    }
	return 0;
}
