...
static int sd_init_rec_param(IPC_REC_PARAM_S *rec_init_param, int id)
{
    ...
    IPC_CFG_AUDIO_S * pstAudioCfg = NULL;
    ...
    pstAudioCfg = IPC_CFG_AUDIO_GetParam();
    ...
    if ((pstRecordCfg->stRecord[u32Chn][u32Stream].audioRecEnable == 1) && (pstAudioCfg->mode != 0))
    {
    ...
    rec_init_param->a_chans = pstAudioCfg->chans;
    rec_init_param->a_rate = pstAudioCfg->sampleRate;
    rec_init_param->a_bits = pstAudioCfg->sampleBitWidth;
...
static AR_S32 IPC_MID_RECMNG_RecOneFile(AR_U32 u32Id, MEDIABUF_HANDLE hMbufReader, AR_U32 u32Type)
{
    ...
    */
	...
	while (1)
    {
    	if(g_rec_suspend)
        {	
            PRINT_INFO("rec suspend, so to end writing file.\n");
            break;
        }
...
void IPC_MID_RECMNG_Init()
{
	...
	AR_S32 ret = 0;
    IPC_CFG_RECORD_S * pstRecord = IPC_CFG_RECORD_GetParam();
    IPC_CFG_ALARM_S * pstAlarmCfg = IPC_CFG_ALARM_GetParam();
    ...
    for(i = 0; i < IPC_MAX_PIPELINE_NUM; i++)
    {
        for(j = 0; j < IPC_MAX_VENC_STREAM_PER_CHN; j++)
        {
            ...
            if(pstMdCfg->bEnable)
            {
                IPC_MID_RECMNG_CreateAlarmThread(i, j);
            }
        }
    }

    //check which channel should be record when alarm(gpio in or md happens) comes.
    if(pstAlarmCfg->alarmIn.enable)
    {
        AR_U32 u32Chn, u32Stream;
        u32Chn = pstAlarmCfg->alarmIn.u32RecChn / IPC_MAX_VENC_STREAM_PER_CHN;
        u32Stream = pstAlarmCfg->alarmIn.u32RecChn % IPC_MAX_VENC_STREAM_PER_CHN;

        IPC_MID_RECMNG_CreateAlarmThread(u32Chn, u32Stream); //it's ok to call it twice.
    }
...
void IPC_MID_RECMNG_Suspend()
{
    g_rec_suspend = 1;
    printf("IPC_MID_RECMNG_Suspend. \n");
}

void IPC_MID_RECMNG_Resume()
{
    g_rec_suspend = 0;
    printf("IPC_MID_RECMNG_Resume. \n");
}
