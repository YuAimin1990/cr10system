...
void FAST_FUNC net_ipstr2byte(unsigned char * const bytes,const char *ip)
{
...
void FAST_FUNC net_ipbyte2str(unsigned char * const ip_num, char *ip)
{
	unsigned int *ip_addr = NULL;
	struct in_addr in;
...
int FAST_FUNC net_macstrtobyte(unsigned char* mac, const char* mac_str, int mac_len)
{
    int i;
    char c, val;
    for (i = 0; i < mac_len; i++) {
...
int FAST_FUNC net_bytetomacstr(char * const mac, const unsigned char *mac_num, int mac_len)
{
	int str_len = 0, i = 0;
...
int FAST_FUNC net_get_if_addr(char *eth, char * const ip, char * const mac)
{
    int sock;
    //
...
#if 0
int FAST_FUNC net_getipbyname(const char *hostname, struct in_addr* in_addr)
{
...
int FAST_FUNC net_chartoascii(const char *str, char *ascii_str)
{
    const char *src = str;
    char *des = ascii_str;
