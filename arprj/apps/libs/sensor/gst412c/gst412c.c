#if !defined(__GST412C_CMOS_H_)
#define __GST412C_CMOS_H_
...
#define GST412C_CALIB_TARGET      8192
...
static gst412c_pipe_ctx_t g_gst412c_pipe_ctx[VI_MAX_PIPE_NUM] = {
    [0 ... VI_MAX_PIPE_NUM-1] = {
        .s8I2cDevId = -1,
        .s32I2cFd = -1,
        .s32VinFd = -1,
        .s32NucFd = -1,
        .enCalibState = GST412C_CALIB_NONE,
...
/*
412c setting, 15M mclk, 50Hz, 400x308
开窗参数: (400+80)*(308+4)+240 @P_MC 15MHZ
*/
static gst412c_reg_t g_gst412c_setting_0[] = {
//   addr, val
    {0x02, 0x90},       //0x02  行有效[7:0]
    {0x03, 0x01},		//0x03  行有效[8]
    {0x04, 0x34},		//0x04  场有效[7:0]
    {0x05, 0x01},		//0x05  场有效[8]
    {0x06, 0x50},		//0x06  行消隐[7:0]
    {0x07, 0x00},		//0x07  行消隐[9:8]
    {0x08, 0x04},		//0x08  场消隐[7:0]
    {0x09, 0x00},		//0x09  场消隐[9:8]
    {0x0a, 0xF0},		//0x0a  最后行场消隐点数
    {0x0b, 0x00},		//0x0b  最后行场消隐点数
    {0x0c, 0x20},		//0x0c  帧频
    {0x0d, 0x03},		//0x0d  帧频系数
    
    //***ADC时钟频率计算公式：AD_CLK_OUT = MC/2^pre_div * pll_m/pll_n 
    //***如果按照当前的配置计算： 14.75/2^1 * 64 / 2 = 236MHz,GST412探测器ADC时钟频率设置为236MHz左右
    {0x2F, 0x41},		//0x2F  //pre_div为寄存器低3bit（bit[2:0]）
    {0x30, 0x40},		//0x30  //pll_m
...
AR_S32 gst412c_read_register(VI_PIPE ViPipe, AR_U8 addr, AR_U8 *data)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
    return ar_hal_i2c_master_read(pipe_ctx->s32I2cFd, GST412C_I2C_SLAVE_ADDR, 7, &addr, 1, data, 1);
}

AR_S32 gst412c_write_register(VI_PIPE ViPipe, AR_U8 addr, AR_U8 data)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
    AR_U8 buf[2];
    ...
    return ar_hal_i2c_master_write(pipe_ctx->s32I2cFd, GST412C_I2C_SLAVE_ADDR, 7, buf, 2);
}

static AR_S32 gst412c_init_ctx(VI_PIPE ViPipe)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
...
static void gst412c_deinit_ctx(VI_PIPE ViPipe)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
...
#if 0
static void gst412c_adc_calib_auto_en(VI_PIPE ViPipe)
{
    gst412c_write_register(ViPipe, 0x7c, 0xa9);      //to page1;
    gst412c_write_register(ViPipe, 0x2f, 0x00);  //0x80);
    gst412c_write_register(ViPipe, 0x7c, 0xa8);      //to page0;
}

static void gst412c_adc_calib(VI_PIPE ViPipe)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
    ...
    gst412c_write_register(ViPipe, 0x7c, 0xa9);      //to page1;
    gst412c_write_register(ViPipe, 0x1c, 0x07);      //ADC_CALIB_ENABLE_FLAG[2:0] = 7;

    gst412c_write_register(ViPipe, 0x10, (ADC_CALIB_SIZE_USER & 0x1f));      //ADC_CALIB_SIZE_USER

    gst412c_write_register(ViPipe, 0x1D, 0x03);
    ar_delay_us(192);       //3行 64*3
    gst412c_read_register(ViPipe, 0x14, (AR_U8 *)&rd_data_l);
    gst412c_read_register(ViPipe, 0x15, (AR_U8 *)&rd_data_h);
    pipe_ctx->niossysctl.gNmiddle_first = (rd_data_h << 8) + rd_data_l;

    gst412c_write_register(ViPipe, 0x1D, 0x01);
    ar_delay_us(192);       //3行 64*3
    gst412c_read_register(ViPipe, 0x14, (AR_U8 *)&rd_data_l);
    gst412c_read_register(ViPipe, 0x15, (AR_U8 *)&rd_data_h);
    pipe_ctx->niossysctl.gNmin_first = (rd_data_h << 8) + rd_data_l;

    gst412c_write_register(ViPipe, 0x1D, 0x07);
    ar_delay_us(192);       //3行 64*3
    gst412c_read_register(ViPipe, 0x14, (AR_U8 *)&rd_data_l);
    gst412c_read_register(ViPipe, 0x15, (AR_U8 *)&rd_data_h);
    ...
    gst412c_write_register(ViPipe, 0x08, (0x00ff & pipe_ctx->niossysctl.gNmin_first));
    //ADC_CALIB_NMIN_NUM_USER[14:8]
    gst412c_write_register(ViPipe, 0x09, (0x007f & (pipe_ctx->niossysctl.gNmin_first >> 8)));
...
static void rd_reg(VI_PIPE ViPipe)
{
    ...
    volatile unsigned char  rd_data = 0,rd_otp_ready_cnt = 0;

    gst412c_write_register(ViPipe, 0x7c, 0xa8);
    gst412c_write_register(ViPipe, 0x01, 0x01);
    usleep(5000);
    gst412c_write_register(ViPipe, 0x7c, 0xa9);  //to page1;
    gst412c_write_register(ViPipe, 0x39,0x00);
    gst412c_write_register(ViPipe, 0x3a,0x01);
    while((rd_data == 2)&&(rd_otp_ready_cnt < 200))
    {
	  gst412c_read_register(ViPipe, 0x39, (AR_U8 *)&rd_data);
	   rd_data = rd_data & 0x02;
	   rd_otp_ready_cnt++;
    }

    gst412c_read_register(ViPipe,0x33,(AR_U8 *)&rd_data_l);
    gst412c_read_register(ViPipe,0x34,(AR_U8 *)&rd_data_h);
    x1_cursor = ((rd_data_h & 0x07) << 8)|(rd_data_l & 0xff);
	if(x1_cursor >= 400)
	{
		x1_cursor = 0;
	}
    gst412c_read_register(ViPipe,0x35,(AR_U8 *)&rd_data_l);
    gst412c_read_register(ViPipe,0x36,(AR_U8 *)&rd_data_h);
	...
	gst412c_read_register(ViPipe,0x32,(AR_U8 *)&rd_data);
...
{
	gst412c_write_register(ViPipe,0x7c,0xa9); //to page1;
	gst412c_write_register(ViPipe,0x00,0x00);//niossysctl.tblr_time = 0;
	gst412c_write_register(ViPipe,0x7c,0xa8); //to page0;
}
...
static void bc_replace(VI_PIPE ViPipe, unsigned char bp1_adj_en, unsigned short bp1_col_position, unsigned char bp2_adj_en, unsigned short bp2_col_position)
{
	...
	gst412c_write_register(ViPipe,0x7c, 0xa9);
	//setting bc1;
	wr_bp_data1 = bp1_col_position + 5;
	wr_data = wr_bp_data1&0x00ff;
	gst412c_write_register(ViPipe,0x0e, wr_data);
	wr_data = ((0x01&bp1_adj_en)<<7) + ((wr_bp_data1>>8)&0x03);
	gst412c_write_register(ViPipe,0x0f, wr_data);

	//setting bc2;
	wr_bp_data2 = bp2_col_position + 5;
	wr_data = wr_bp_data2&0x00ff;
	gst412c_write_register(ViPipe,0x10, wr_data);
	wr_data = ((0x01&bp2_adj_en)<<7) + ((wr_bp_data2>>8)&0x03);
	gst412c_write_register(ViPipe,0x11, wr_data);
	usleep(10);
...
static void wr_reg(VI_PIPE ViPipe)
{
    ...
    gst412c_write_register(ViPipe,0x7c,0xab); 
    gst412c_write_register(ViPipe,0x01,0x03);
    gst412c_write_register(ViPipe,0x05,0x03);
    gst412c_write_register(ViPipe,0x06,0xff); 
    gst412c_write_register(ViPipe,0x09,0x40); 
    gst412c_write_register(ViPipe,0x0a,0xc0); 
    gst412c_write_register(ViPipe,0x0d,0x00); 
    gst412c_write_register(ViPipe,0x0f,0x08); 
    gst412c_write_register(ViPipe,0x10,0x20); 
    gst412c_write_register(ViPipe,0x11,0x20); 

    gst412c_write_register(ViPipe,0x12,0x07); 
    gst412c_write_register(ViPipe,0x18,0x27);
    gst412c_write_register(ViPipe,0x19,0x07); 

    gst412c_write_register(ViPipe,0x1B,0xFF); 
    gst412c_write_register(ViPipe,0x1d,0x3e);
    gst412c_write_register(ViPipe,0x1e,0x7f);
    gst412c_write_register(ViPipe,0x20,0xff);
    gst412c_write_register(ViPipe,0x21,0xff);
    gst412c_write_register(ViPipe,0x7c,0xa9);
    gst412c_write_register(ViPipe,0x06,0x36);
    gst412c_write_register(ViPipe,0x07,0x0c);
    gst412c_write_register(ViPipe,0x27,0x0E); 
    gst412c_write_register(ViPipe,0x7c,0xa8); 
    gst412c_write_register(ViPipe,0x20,0x07); 
    gst412c_write_register(ViPipe,0x1f,0x0c);
    gst412c_write_register(ViPipe,0x2c,0x02); 
    gst412c_write_register(ViPipe,0x35,0x3d); 

	if(sensor_version == 4) 
	{
	   gst412c_write_register(ViPipe,0x7c,0xab);//to page3;
	   gst412c_write_register(ViPipe,0x18,0x24);//RD 
	   gst412c_write_register(ViPipe,0x12,0x04);//RD 
	   gst412c_write_register(ViPipe,0x7c,0xa9);//to page1
	   gst412c_write_register(ViPipe,0x07,0x0d);
	   gst412c_write_register(ViPipe,0x24,0x80);
	   gst412c_write_register(ViPipe,0x3D,0x00);
	   gst412c_write_register(ViPipe,0x42,0x05); 
	   gst412c_write_register(ViPipe,0x7c,0xa8); 
	}
...
static void RaselSet(VI_PIPE ViPipe, AR_U8 u8Rasel)
{
	...
	gst412c_write_register(ViPipe,0x7C, 0xAB);//page3
	if(u8Rasel <= 8)//0x20
	{
		temp_data = u8Rasel;
		temp_data = 0xff >> (8 - temp_data);
		gst412c_write_register(ViPipe,0x20, temp_data); 
		gst412c_write_register(ViPipe,0x21, 0xff); 
	}
	gst412c_write_register(ViPipe,0x7C, 0xA8);//page0
}
//**************************************************************************
//////// Hssd设置 0-127
//**************************************************************************
...
{
    gst412c_write_register(ViPipe,0x7C, 0xAB);//page3
    gst412c_write_register(ViPipe,0x09,u8Hssd & 0x7f);
    gst412c_write_register(ViPipe,0x7C, 0xA8);//page0
}
...
static void RaselHssdRegGet(VI_PIPE ViPipe, AR_U8 *pRasel, AR_U8 *pHssd)
{
	AR_U8 rdata = 0;
	gst412c_write_register(ViPipe,0x7C, 0xAB);//page3
	gst412c_read_register(ViPipe, 0x20, &rdata);
	*pRasel = rdata;
    gst412c_read_register(ViPipe, 0x09, &rdata);
	*pHssd = rdata&0x7f;
    gst412c_write_register(ViPipe,0x7C, 0xA8);//page0
}
...
{
	gst412c_write_register(ViPipe,0x7C, 0xAB);//page3
    gst412c_write_register(ViPipe,0x20, u8RaselReg);
	gst412c_write_register(ViPipe,0x21, 0xff);
    gst412c_write_register(ViPipe,0x7C, 0xA8);//page0
}
...
static void IntSet(VI_PIPE ViPipe, AR_U8 u8Int )
{
    ...
    gst412c_write_register(ViPipe,0x7C, 0xA8);///page0
    gst412c_write_register(ViPipe,0x2E, ( u8Int & 0x0F00)>>8);
    gst412c_write_register(ViPipe,0x2D, u8Int & 0x00FF);
    }
...
static void GainSet(VI_PIPE ViPipe, AR_U8 u8Gain )
{
    ...
    gst412c_write_register(ViPipe,0x7C, 0xA8);///page0
    gst412c_write_register(ViPipe,0x28, ( u8Gain & 0xFF));
    }
...
static void gst412c_stream_on(VI_PIPE ViPipe)
{
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
	volatile unsigned int i;
	volatile unsigned char rd_data=0;
	//volatile unsigned char flag = 0,Wr_Data = 0,wr10_data = 0;
    ...
    //ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A104, 0, 0, 0);
    usleep(5000);
    ar_hal_gpio_set_value(gpNum_rst, 1);
    //ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A104, 1, 0, 0);
    usleep(5000);
    tblr_time(ViPipe);
	...
	for (i = 0; i < sizeof(g_gst412c_setting_0) / sizeof(gst412c_reg_t); i++) {
		gst412c_write_register(ViPipe, g_gst412c_setting_0[i].addr, g_gst412c_setting_0[i].val);
		usleep(10);
		gst412c_read_register(ViPipe, g_gst412c_setting_0[i].addr, (AR_U8 *)&rd_data);
		usleep(10);
    }
	rd_data = 1;
	usleep(5000);
    ...
    {
	   gst412c_read_register(ViPipe,0x01, (AR_U8 *)&rd_data);
	...
	gst412c_read_register(ViPipe, 0x3d, (AR_U8 *)&rd_data);
	usleep(10);
	gst412c_read_register(ViPipe, 0x3e, (AR_U8 *)&rd_data);
	usleep(10);
...
static AR_S32 gst412c_power_on(VI_PIPE ViPipe, ar_dev_power_attr_t *p_dev_power_attr)
{
    AR_S32 ret = 0;
...
#if 0
	//set gpio output
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A108, 0, 2, 2);
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A108, 0, 1, 1);
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A0C0, 0, 18, 18);
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A108, 0, 0, 0);

	//set reset 0
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A104, 0, 0, 0);

	//power
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A104, 1, 2, 2);
	ar_delay(1);
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A104, 1, 1, 1);
	ar_delay(1);
	ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10A0BC, 1, 18, 18);
	ar_delay(10);
#endif

    //set clk reg
    //ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10404C, 0, 24, 26);
    //ar_hal_vin_write_reg(g_gst412c_pipe_ctx[ViPipe].s32VinFd, 0xA10404C, 0x13, 16, 22);
	//set nuc clk output
    ret = ar_hal_vin_nuc_set_out_clk(g_gst412c_pipe_ctx[ViPipe].s32NucFd,
        NUC_OUT_CLK_SRC_600MHZ, 20, NUC_CLK_PHASE_180, AR_TRUE);
    if (ret) {
        ar_err("Pipe[%d] gst412c set nuc_out_clk failed", ViPipe);
        return ret;
    }

    ar_delay(1);
    ar_always("Pipe[%d] gst412c power on", ViPipe);
...
static AR_S32 gst412c_power_off(VI_PIPE ViPipe, ar_dev_power_attr_t *p_dev_power_attr)
{
    AR_S32 ret = 0;
    ...
    ret = ar_hal_vin_nuc_set_out_clk(g_gst412c_pipe_ctx[ViPipe].s32NucFd,
        NUC_OUT_CLK_SRC_600MHZ, 40, NUC_CLK_PHASE_180, AR_FALSE);
    if (ret) {
        ar_err("Pipe[%d] gst412c set nuc_out_clk failed", ViPipe);
        return ret;
    }
    ...
    ar_always("Pipe[%d] gst412c power off", ViPipe);

    return ret;
}

static AR_S32 gst412c_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    g_gst412c_pipe_ctx[ViPipe].stImageMode = *pstSensorImageMode;

    return 0;
}

void gst412c_init(VI_PIPE ViPipe)
{
    STRU_NUC_CFG_T nuc_cfg = {
        ...
        .fs_head = 0xffffc1,
        .fs_head_bitnum = 24,
        .fs_head_ver_loc = 312 - 308 - 1,   //0,
        .fs_head_hor_loc = 462,
        .ls_head = 0xffffc0,
        .ls_head_bitnum = 24,
        .ls_head_ver_loc_start = 312 - 308 - 1, //0,
        .ls_head_ver_loc_end = 312 - 2, //307,
    };

    //printf("nuc cfg %d %d %d %d\n", nuc_cfg.active_width, nuc_cfg.active_height, nuc_cfg.total_width, nuc_cfg.total_height);
    ar_hal_vin_nuc_cfg(g_gst412c_pipe_ctx[ViPipe].s32NucFd, &nuc_cfg);
    gst412c_stream_on(ViPipe);
}

void gst412c_exit(VI_PIPE ViPipe)
{
    ar_hal_vin_nuc_stop(g_gst412c_pipe_ctx[ViPipe].s32NucFd);
}

static ISP_CMOS_DNG_COLORPARAM_S g_stGst412cDngColorParam = {
    .stWbGain1 = {256, 256, 256},
    .stWbGain2 = {256, 256, 256},
};

static AR_S32 gst412c_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    if (ViPipe < 0 || ViPipe >= ISP_MAX_PIPE_NUM) {
        ar_err("the pipe id [%d] is invalid", ViPipe);
        return AR_ERR_VI_INVALID_PIPEID;
    ...
    pstDef->stSensorMode.u32SensorID = GST412C_SNS_ID;
    pstDef->stSensorMode.u8SensorMode = 0;
    memcpy(&pstDef->stDngColorParam, &g_stGst412cDngColorParam, sizeof(ISP_CMOS_DNG_COLORPARAM_S));
    strcpy(pstDef->TuningPraBinName, "/usrdata/local/factory/tunning/cam_imx307/imx307_tuning_preview_dwt_80.bin");

    return AR_SUCCESS;
}

AR_S32 gst412c_get_ir_sns_attr(VI_PIPE ViPipe, IR_SNS_ATTR_S *pstIrSnsAttr)
{
    if (ViPipe < 0 || ViPipe >= ISP_MAX_PIPE_NUM) {
        ar_err("the pipe id [%d] is invalid", ViPipe);
        return AR_ERR_VI_INVALID_PIPEID;
...
AR_S32 gst412c_set_ir_param(VI_PIPE ViPipe, AR_VOID *pParamData, AR_U32 u32ParamSize)
{
    AR_U8 *p = NULL;
...
AR_S32 gst412c_get_ir_param(VI_PIPE ViPipe, AR_VOID *pParamData, AR_U32 *pu32ParamSize)
{
    AR_U8 *p = NULL;
...
AR_S32 gst412c_get_sns_temperature(VI_PIPE ViPipe, AR_S32 *ps32Temperature)
{
#if 0
    AR_U16 dvtemp_d5 = 0;
    AR_U16 dvtemp_d6 = 0;
    ...
    gst412c_write_register(ViPipe, 0x7c, 0xa9);
    ar_delay_us(100);
    gst412c_read_register(ViPipe, 0x14, (AR_U8 *)(&dvtemp_d5));
    gst412c_read_register(ViPipe, 0x15, (AR_U8 *)(&dvtemp_d6));
...
static AR_S32 gst412c_start_sns_calib(VI_PIPE ViPipe, IR_SNS_CALIB_CTX_S *pstCalibCtx)
{
    AR_S32 s32Ret = 0;
    gst412c_pipe_ctx_t *pstPipeCtx = &g_gst412c_pipe_ctx[ViPipe];
    ...
    pstPipeCtx->enCalibState = GST412C_CALIB_RA_ADJ;
...
FAIL:
    pstPipeCtx->enCalibState = GST412C_CALIB_NONE;
    pstCalibCtx->stOutput.enCalibStatus = IR_CALIB_STATUS_FAIL;
    return s32Ret;
}

static AR_S32 gst412c_ra_adj_calib(VI_PIPE ViPipe, IR_SNS_CALIB_CTX_S *pstCalibCtx)
{
    gst412c_pipe_ctx_t *pstPipeCtx = &g_gst412c_pipe_ctx[ViPipe];
    AR_U8 *pIrSnsParam = (AR_U8 *)pstCalibCtx->stOutput.pCalibParam;
    ...
    if (bCalibDone) {
        ...
        pIrSnsParam[1] = 0;
        pstPipeCtx->u16MinMeanDelta = 0xffffUL;
        pstPipeCtx->enCalibState = GST412C_CALIB_HSSD;
    }
...
static AR_S32 gst412c_hssd_calib(VI_PIPE ViPipe, IR_SNS_CALIB_CTX_S *pstCalibCtx)
{
    AR_S32 s32Ret = 0;
    gst412c_pipe_ctx_t *pstPipeCtx = &g_gst412c_pipe_ctx[ViPipe];
    AR_U8 *pIrSnsParam = (AR_U8 *)pstCalibCtx->stOutput.pCalibParam;
...
static AR_S32 gst412c_sns_calib(VI_PIPE ViPipe, IR_SNS_CALIB_CTX_S *pstCalibCtx)
{
    AR_S32 s32Ret = 0;
    gst412c_pipe_ctx_t *pstPipeCtx = &g_gst412c_pipe_ctx[ViPipe];

    if (pstCalibCtx->stInput.stCalibFrm.u32FrameId == 0) {
        s32Ret = gst412c_start_sns_calib(ViPipe, pstCalibCtx);
        if (s32Ret) {
            ar_err("pipe[%d] start sensor calib failed", ViPipe);
        } else {
            ar_always("pipe[%d] sensor calib started", ViPipe);
        }
    } else if (pstPipeCtx->enCalibState == GST412C_CALIB_RA_ADJ) {
        s32Ret = gst412c_ra_adj_calib(ViPipe, pstCalibCtx);
        if (s32Ret) {
            ar_err("pipe[%d] ra_adj calib failed", ViPipe);
        }
    } else if (pstPipeCtx->enCalibState == GST412C_CALIB_HSSD) {
        s32Ret = gst412c_hssd_calib(ViPipe, pstCalibCtx);
        if (s32Ret) {
...
#if 0
static AR_S32 gst412c_ooc_calib(VI_PIPE ViPipe, GST412C_CALIB_INFO_S *pCalibInfo)
{
    AR_S32 s32Ret = 0;
    gst412c_pipe_ctx_t *pstPipeCtx = &g_gst412c_pipe_ctx[ViPipe];
    AR_U16 *pu16Raw = NULL;
    ...
    for (i = 0; i < pCalibInfo->stROI.u32Height; i++) {
        for (j = 0; j < pCalibInfo->stROI.u32Width; j++) {
            if (pu16Raw[j] <= GST412C_CALIB_TARGET) {
                u16PixDelta = GST412C_CALIB_TARGET - pu16Raw[j];
            } else {
                u16PixDelta = pu16Raw[j] - GST412C_CALIB_TARGET;
            }
    ...
    if (!bCalibDone) {
        pCalibInfo->enCalibResult = GST412C_CALIB_ONGOING;
    } else {
        pstPipeCtx->enCalibState = GST412C_CALIB_NONE;
        pCalibInfo->enCalibResult = GST412C_CALIB_DONE;
...
FAIL:
    pstPipeCtx->enCalibState = GST412C_CALIB_NONE;
    pCalibInfo->enCalibResult = GST412C_CALIB_FAIL;
    return s32Ret;
}
#endif

AR_S32 gst412c_switch_shutter(VI_PIPE ViPipe, AR_BOOL bOpen)
{
    AR_S32 s32Ret = 0;
...
static AR_S32 gst412c_init_sensor_exp_function( ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    CMOS_CHECK_POINTER(pstSensorExpFunc);
    memset(pstSensorExpFunc, 0, sizeof(ISP_SENSOR_EXP_FUNC_S));

    pstSensorExpFunc->pfn_cmos_sensor_init = gst412c_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = gst412c_exit;
    pstSensorExpFunc->pfn_cmos_sensor_global_init = AR_NULL;
    pstSensorExpFunc->pfn_cmos_set_image_mode = gst412c_set_image_mode;
    pstSensorExpFunc->pfn_cmos_set_wdr_mode = AR_NULL;
    pstSensorExpFunc->pfn_cmos_get_isp_default = gst412c_get_isp_default;
    pstSensorExpFunc->pfn_cmos_set_pixel_detect = AR_NULL;
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = AR_NULL;
    pstSensorExpFunc->pfn_cmos_sns_power_on = gst412c_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = gst412c_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = AR_NULL;
    pstSensorExpFunc->pfn_cmos_get_ir_sns_attr = gst412c_get_ir_sns_attr;
    pstSensorExpFunc->pfn_cmos_set_ir_param = gst412c_set_ir_param;
    pstSensorExpFunc->pfn_cmos_get_ir_param = gst412c_get_ir_param;
    pstSensorExpFunc->pfn_cmos_get_sns_temperature = gst412c_get_sns_temperature;
    pstSensorExpFunc->pfn_cmos_sns_calib = gst412c_sns_calib;
    pstSensorExpFunc->pfn_cmos_switch_shutter = gst412c_switch_shutter;

    return AR_SUCCESS;
}

#if 0
static AR_S32 gst412c_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static AR_S32 gst412c_init_ae_exp_function(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    CMOS_CHECK_POINTER(pstExpFuncs);
    memset(pstExpFuncs, 0, sizeof(AE_SENSOR_EXP_FUNC_S));
    //pstExpFuncs->pfn_cmos_get_ae_default = gst412c_get_ae_default;
    return AR_SUCCESS;
}

static AR_S32 gst412c_init_awb_exp_function(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    CMOS_CHECK_POINTER(pstExpFuncs);
...
static AR_S32 gst412c_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    AR_S32 ret = 0;
    gst412c_pipe_ctx_t *pipe_ctx = &g_gst412c_pipe_ctx[ViPipe];
...
static AR_S32 gst412c_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    AR_S32 s32Ret;
    ...
    s32Ret = gst412c_init_ctx(ViPipe);
    if (s32Ret != AR_SUCCESS) {
        ar_err("init ctx failed, pipe[%d] ret[%d]", ViPipe, s32Ret);
        return s32Ret;
    }

    stSnsAttrInfo.eSensorId = GST412C_SNS_ID;
    s32Ret  = gst412c_init_sensor_exp_function(&stIspRegister.stSnsExp);
    ...
    s32Ret  = gst412c_init_ae_exp_function(&stAeRegister.stSnsExp);
    ...
    s32Ret  = gst412c_init_awb_exp_function(&stAwbRegister.stSnsExp);
...
static AR_S32 gst412c_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    AR_S32 s32Ret;
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, GST412C_SNS_ID);
    if (AR_SUCCESS != s32Ret) {
        ar_err("sensor unregister callback function failed!");
        return s32Ret;
    }

    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, GST412C_SNS_ID);
    if (AR_SUCCESS != s32Ret) {
        ar_err("sensor unregister callback function to ae lib failed!");
        return s32Ret;
    }

    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, GST412C_SNS_ID);
    if (AR_SUCCESS != s32Ret) {
        ar_err("sensor unregister callback function to awb lib failed!");
        return s32Ret;
    }

    gst412c_deinit_ctx(ViPipe);
    return AR_SUCCESS;
}

static AR_S32 gst412c_read(VI_PIPE ViPipe, AR_S32 s32Addr)
{
    AR_U8 data = 0;
    gst412c_read_register(ViPipe, (AR_U8)s32Addr, &data);
    return (AR_S32)data;
}

static AR_S32 gst412c_write(VI_PIPE ViPipe, AR_S32 s32Addr, AR_S32 s32Data)
{
    return gst412c_write_register(ViPipe, (AR_U8)s32Addr, (AR_U8)s32Data);
}


ISP_SNS_OBJ_S stSnsGst412cObj = {
    .pfnRegisterCallback    = gst412c_register_callback,
    .pfnUnRegisterCallback  = gst412c_unregister_callback,
    .pfnStandby             = AR_NULL,
    .pfnRestart             = AR_NULL,
    .pfnMirrorFlip          = AR_NULL,
    .pfnWriteReg            = gst412c_write,
    .pfnReadReg             = gst412c_read,
    .pfnSetBusInfo          = gst412c_set_bus_info,
    .pfnSetInit             = AR_NULL,
};
