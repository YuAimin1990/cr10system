#if !defined(__ISPIN_INF_CMOS_H_)
#define __ISPIN_INF_CMOS_H_
...
#define ISP_IN_INF_ID 143

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastIspInInf[ISP_MAX_PIPE_NUM] = {AR_NULL};

#define ISPIN_INF_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastIspInInf[dev])
#define ISPIN_INF_SENSOR_SET_CTX(dev, pstCtx)   (g_pastIspInInf[dev] = pstCtx)
#define ISPIN_INF_SENSOR_RESET_CTX(dev)         (g_pastIspInInf[dev] = AR_NULL)

ISP_SNS_COMMBUS_U g_aunIspInInfBusInfo[ISP_MAX_PIPE_NUM] =
{
    [0] = { .s8I2cDev = 1},
...
typedef struct arISPIN0_STATE_S
{
    AR_U8       u8Hcg;
    AR_U32      u32BRL;
    AR_U32      u32RHS1_MAX;
...
ISPIN0_STATE_S g_astisp_in_infState[ISP_MAX_PIPE_NUM] = {{0}};

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/

/* change params for your infrared sensor valid input*/
#define INF_SNS_WIDTH   (640)
#define INF_SNS_HEIGHT  (512)
#define INF_SNS_FPS     (25)

#define ISPIN0_FULL_LINES_MAX  (0x3FFFF)
#define ISPIN0_FULL_LINES_MAX_2TO1_WDR  (0x8AA)    // considering the YOUT_SIZE and bad frame
#define ISPIN0_FULL_LINES_MAX_3TO1_WDR  (0x7FC)
#define ISPIN0_HMAX (INF_SNS_WIDTH+200)

/*****IspIn0 Register Address*****/

#define ISPIN0_VMAX_1080P30_LINEAR  (INF_SNS_HEIGHT+100)
...
static AR_S32 isp_in_inf_cmos_power_on(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    ar_always("%s \n",__FUNCTION__);
	return 0;
}

static AR_S32 isp_in_inf_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    ar_always("%s \n",__FUNCTION__);
	return 0;
}
static AR_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    ISPIN_INF_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER(pstSnsState);

	pstAeSnsDft->f32Fps=INF_SNS_FPS;
	pstAeSnsDft->f32MaxFps=INF_SNS_FPS;
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    // AR_U32 u32VMAX = ISPIN0_VMAX_1080P30_LINEAR;
...
static AR_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    switch (pstSnsState->enWDRMode)
    {
        ...
        break;*/
        default:
            strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_common_inf/common_infrared_tuning_preview_thermal.bin");
            //strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_common_inf/infrared_timo256_tuning_preview.bin");
            //strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_common_inf/infrared_elf1_tuning_preview.bin");
            break;
    }
    ...
    pstDef->stSensorMode.u32SensorID = ISP_IN_INF_ID;
...
static AR_S32 isp_in_inf_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
...
static void isp_in_inf_init(VI_PIPE ViPipe)
{
   ar_always("%s \n",__FUNCTION__);
   return;
}

static void isp_in_inf_exit(VI_PIPE ViPipe)
{
    ar_always("%s \n",__FUNCTION__);
    return;
}
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init = isp_in_inf_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = isp_in_inf_exit;
    ...
    pstSensorExpFunc->pfn_cmos_sns_power_on = isp_in_inf_cmos_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = isp_in_inf_cmos_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = isp_in_inf_cmos_sensor_ctl;
    return AR_SUCCESS;
}
...
static AR_S32 isp_in_inf_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunIspInInfBusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static AR_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    ISPIN_INF_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    ISPIN_INF_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static AR_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    ISPIN_INF_SENSOR_RESET_CTX(ViPipe);
}
...
static AR_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = ISP_IN_INF_ID;
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, ISP_IN_INF_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, ISP_IN_INF_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, ISP_IN_INF_ID);
    if (AR_SUCCESS != s32Ret)
    {
...
static void isp_in_inf_standby(VI_PIPE ViPipe)
{
    // TODO:
    return;
}

static void isp_in_inf_restart(VI_PIPE ViPipe)
{
    // TODO:
    return;
}
static int isp_in_inf_read_register(VI_PIPE ViPipe, int addr)
{
    return AR_SUCCESS;
}
static int isp_in_inf_write_register(VI_PIPE ViPipe, int addr, int data)
{
    return AR_SUCCESS;
}

ISP_SNS_OBJ_S stSnsIspInInfThermalObj =
{
    .pfnRegisterCallback    = sensor_register_callback,
    .pfnUnRegisterCallback  = sensor_unregister_callback,
    .pfnStandby             = isp_in_inf_standby,
    .pfnRestart             = isp_in_inf_restart,
    .pfnMirrorFlip          = AR_NULL,
    .pfnWriteReg            = isp_in_inf_write_register,
    .pfnReadReg             = isp_in_inf_read_register,
    .pfnSetBusInfo          = isp_in_inf_set_bus_info,
