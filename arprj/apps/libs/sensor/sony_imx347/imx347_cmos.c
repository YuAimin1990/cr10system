...
ISP_SNS_STATE_S *g_pastImx347[ISP_MAX_PIPE_NUM] = {AR_NULL};
...
#define IMX347_SENSOR_RESET_CTX(dev)         (g_pastImx347[dev] = AR_NULL)
...
extern const unsigned int imx347_i2c_addr;
extern unsigned int imx347_addr_byte;
extern unsigned int imx347_data_byte;

typedef struct arIMX347_STATE_S
{
    AR_U8       u8Hcg;
    AR_U32      u32BRL;
    AR_U32      u32RHS1_MAX;
...
extern void imx347_default_reg_init(VI_PIPE ViPipe);
extern int  imx347_flip_off_mirro_off(VI_PIPE ViPipe);
extern int  imx347_flip_off_mirro_on(VI_PIPE ViPipe);
extern int  imx347_flip_on_mirro_off(VI_PIPE ViPipe);
extern int  imx347_flip_on_mirro_on(VI_PIPE ViPipe);

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define IMX347_FULL_LINES_MAX  (0xFFFFF)
#define IMX347_FULL_LINES_MAX_2TO1_WDR  (0x8AA)    // considering the YOUT_SIZE and bad frame
#define IMX347_FULL_LINES_MAX_3TO1_WDR  (0x7FC)

/*****Imx347 Register Address*****/
#define IMX347_SHR_ADDR   (0x3058)
#define IMX347_SHR1_ADDR   (0x305c)

#define IMX347_GAIN_ADDR  (0x30e8)
#define IMX347_SGAIN_ADDR  (0x30ea)
#define IMX347_HCG_ADDR   (0x3019)
#define IMX347_VMAX_ADDR  (0x3030)
#define IMX347_HMAX_ADDR  (0x3034)
#define IMX347_RHS1_ADDR  (0x3068)
#define IMX347_RHS2_ADDR  (0x306c)
//#define IMX347_Y_OUT_SIZE_ADDR (0x3418)
#define IMX347_INCREASE_LINES (1) /* make real fps less than stand fps because NVR require*/

#define IMX347_VMAX_4MP30_LINEAR  (3959+IMX347_INCREASE_LINES)
#define IMX347_VMAX_720P60TO30_WDR  (750+IMX347_INCREASE_LINES)
#define IMX347_VMAX_1080P60TO30_WDR (1220+IMX347_INCREASE_LINES)
#define IMX347_VMAX_1080P120TO30_WDR (1125+IMX347_INCREASE_LINES)
#define IMX347_VMAX_4MP30_WDR  (1649+IMX347_INCREASE_LINES)

//sensor fps mode
#define IMX347_SENSOR_4M_25FPS_LINEAR_MODE      (1)
...
#define IMX347_RES_IS_720P(w, h)       ((w) <= 1280 && (h) <= 720)
#define IMX347_RES_IS_4M(w, h)      ((w) <= 3000 && (h) <= 2000)
#define SENSOR_NM (0)
...
static AR_S32 imx347_cmos_power_on(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
	...
	ISP_TRACE(AR_DBG_INFO, "imx347_cmos_power_on  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
...
static AR_S32 imx347_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    int power_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->power_gpio[0],p_dev_power_attr->power_gpio[1],p_dev_power_attr->power_gpio[2]);
    int reset_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->reset_gpio[0],p_dev_power_attr->reset_gpio[1],p_dev_power_attr->reset_gpio[2]);
	...
	ISP_TRACE(AR_DBG_INFO, "imx347_cmos_power_off  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
	//set the gpio dir to out
	ar_hal_gpio_set_dir(power_gpio,1);
	ar_hal_gpio_set_dir(reset_gpio,1);
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32VMAX = IMX347_VMAX_4MP30_LINEAR;
    ...
    {
        case IMX347_SENSOR_1080P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 16.5))
            {
                u32VMAX = IMX347_VMAX_1080P60TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
            ...
            u32VMAX = (u32VMAX > IMX347_FULL_LINES_MAX_2TO1_WDR) ? IMX347_FULL_LINES_MAX_2TO1_WDR : u32VMAX;
            break;

        case IMX347_SENSOR_1080P_30FPS_3t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 16.5))
            {
                u32VMAX = IMX347_VMAX_1080P120TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
            ...
            u32VMAX = (u32VMAX > IMX347_FULL_LINES_MAX_3TO1_WDR) ? IMX347_FULL_LINES_MAX_3TO1_WDR : u32VMAX;
            break;

        case IMX347_SENSOR_4M_25FPS_LINEAR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX347_VMAX_4MP30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
            ...
            u32VMAX = (u32VMAX > IMX347_FULL_LINES_MAX) ? IMX347_FULL_LINES_MAX : u32VMAX;
            break;
        case IMX347_SENSOR_4M_30FPS_2t1_WDR_MODE:
        {
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX347_VMAX_4MP30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
        ...
        /*case IMX347_SENSOR_720P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX347_VMAX_720P60TO30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            2n + 5
            RHS1 <= FSC - BRL*2 -21
            (2 * VMAX_IMX347_1080P30_WDR - 2 * gu32BRL - 21) - (((2 * VMAX_IMX347_1080P30_WDR - 2 * 1109 - 21) - 5) %2)
        */

        g_astimx347State[ViPipe].u32RHS1_MAX = (u32VMAX - g_astimx347State[ViPipe].u32BRL) * 2 - 21;
    ...
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            RHS2 <= FSC - BRL*3 -25
        */
        g_astimx347State[ViPipe].u32RHS2_MAX = u32VMAX * 4 - g_astimx347State[ViPipe].u32BRL * 3 - 25;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {    	
        u32FullLines = (u32FullLines > 2 * IMX347_FULL_LINES_MAX_2TO1_WDR) ? 2 * IMX347_FULL_LINES_MAX_2TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;			
        g_astimx347State[ViPipe].u32RHS1_MAX = pstSnsState->au32FL[0] - g_astimx347State[ViPipe].u32BRL * 2 - 21;
    }
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > 4 * IMX347_FULL_LINES_MAX_3TO1_WDR) ? 4 * IMX347_FULL_LINES_MAX_3TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 2) << 2;
        g_astimx347State[ViPipe].u32RHS2_MAX = pstSnsState->au32FL[0] - g_astimx347State[ViPipe].u32BRL * 3 - 25;
    }
    else if ( WDR_MODE_2To1_FRAME == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > IMX347_FULL_LINES_MAX) ? IMX347_FULL_LINES_MAX : u32FullLines;
...
static AR_U32 gain_table[262] =
{
    ...
    1262786, 1307163, 1353100, 1400651, 1449872, 1500824, 1553566, 1608162, 1664676, 1723177, 1783733, 1834717,
    ...
    else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx347State[ViPipe].u32RHS1_MAX - 3)) ? (g_astimx347State[ViPipe].u32RHS1_MAX - 3) : u32IntTimeMaxTmp;
    ...
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32RHS2_Max = ((g_astimx347State[ViPipe].u32RHS2_MAX - 8) * 0x40) / (au32Ratio[0] + 0x40);
...
static AR_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    switch (pstSnsState->enWDRMode)
    {
        default:
        case WDR_MODE_NONE:
		strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_imx347/imx347_tuning_preview.bin");
		break;
        case WDR_MODE_2To1_LINE:
        case WDR_MODE_3To1_LINE:
		strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_imx347/imx347_hdr_tuning_preview.bin");
        break;
    }
    ...
    pstDef->stSensorMode.u32SensorID = IMX347_ID;
...
static AR_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, AR_BOOL bEnable)
{
    ...
    {
        if (IMX347_SENSOR_4M_25FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            u32FullLines_5Fps = (IMX347_VMAX_4MP30_LINEAR * 30) / 5;
        }
    ...
    //u32FullLines_5Fps = (u32FullLines_5Fps > IMX347_FULL_LINES_MAX) ? IMX347_FULL_LINES_MAX : u32FullLines_5Fps;
    u32MaxIntTime_5Fps = 4;

    if (bEnable) /* setup for ISP pixel calibration mode */
    {
        imx347_write_register (ViPipe, IMX347_GAIN_ADDR, 0x00);
        imx347_write_register (ViPipe, IMX347_GAIN_ADDR + 1, 0x00);

        imx347_write_register (ViPipe, IMX347_VMAX_ADDR, u32FullLines_5Fps & 0xFF);
        imx347_write_register (ViPipe, IMX347_VMAX_ADDR + 1, (u32FullLines_5Fps & 0xFF00) >> 8);
        imx347_write_register (ViPipe, IMX347_VMAX_ADDR + 2, (u32FullLines_5Fps & 0xF0000) >> 16);

        imx347_write_register (ViPipe, IMX347_SHR_ADDR, u32MaxIntTime_5Fps & 0xFF);
        imx347_write_register (ViPipe, IMX347_SHR_ADDR + 1,  (u32MaxIntTime_5Fps & 0xFF00) >> 8);
        imx347_write_register (ViPipe, IMX347_SHR_ADDR + 2, (u32MaxIntTime_5Fps & 0xF0000) >> 16);

    }
    else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0x7FFFF) ? 0x7FFFF : pstSnsState->u32FLStd;
        imx347_write_register (ViPipe, IMX347_VMAX_ADDR, pstSnsState->u32FLStd & 0xFF);
        imx347_write_register (ViPipe, IMX347_VMAX_ADDR + 1, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        imx347_write_register (ViPipe, IMX347_VMAX_ADDR + 2, (pstSnsState->u32FLStd & 0xF0000) >> 16);
...
static AR_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, AR_U8 u8Mode)
{
    ...
    switch (u8Mode)
    {
        case WDR_MODE_NONE:
            ...
            //pstSnsState->u32FLStd = IMX347_VMAX_4MP30_LINEAR;
            //g_astimx347State[ViPipe].u8Hcg = 0x2;
        ...
        case WDR_MODE_2To1_LINE:
            ...
            g_astimx347State[ViPipe].u8Hcg    = 0x1;
            #if 0
            if (IMX347_SENSOR_1080P_30FPS_2t1_WDR_MODE == pstSnsState->u8ImgMode)
            {
                pstSnsState->u32FLStd = IMX347_VMAX_1080P60TO30_WDR * 2;
                g_astimx347State[ViPipe].u32BRL  = 1109;
                ISP_TRACE(AR_DBG_INFO, "2to1 line WDR 1080p mode(60fps->30fps)\n");
            }
            else if (IMX347_SENSOR_720P_30FPS_2t1_WDR_MODE == pstSnsState->u8ImgMode)
            {
                pstSnsState->u32FLStd = IMX347_VMAX_720P60TO30_WDR * 2;
                g_astimx347State[ViPipe].u32BRL  = 735;
                ISP_TRACE(AR_DBG_INFO, "2to1 line WDR 720p mode(60fps->30fps)\n");
            }
            else if (IMX347_SENSOR_4M_30FPS_2t1_WDR_MODE == pstSnsState->u8ImgMode)
            {
                pstSnsState->u32FLStd = IMX347_VMAX_4MP30_WDR * 2;
                g_astimx347State[ViPipe].u32BRL  = 1538;
        ...
        case WDR_MODE_3To1_LINE:
            ...
            pstSnsState->u32FLStd  = IMX347_VMAX_1080P120TO30_WDR * 4;
            g_astimx347State[ViPipe].u32BRL   = 1109;
            g_astimx347State[ViPipe].u8Hcg    = 0x0;
...
static AR_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((AR_FALSE == pstSnsState->bSyncInit) || (AR_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx347BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx347_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx347_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx347_data_byte;
        }

        //Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX347_SHR_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX347_SHR_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX347_SHR_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //make shutter and gain effective at the same time
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX347_GAIN_ADDR;  //gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX347_GAIN_ADDR + 1;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX347_VMAX_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX347_VMAX_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX347_VMAX_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX347_HCG_ADDR;

        if ( WDR_MODE_2To1_FRAME == pstSnsState->enWDRMode)
        {
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX347_SHR_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX347_SHR_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX347_SHR_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX347_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX347_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX347_VMAX_ADDR + 2;
        ...
        else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX347_GAIN_ADDR;  //gain
            pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX347_GAIN_ADDR + 1;  //gain
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX347_SGAIN_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX347_SGAIN_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX347_HCG_ADDR;
             pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            //SEF1
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX347_SHR1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX347_SHR1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX347_SHR1_ADDR + 2;
            //VMAX
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX347_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX347_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX347_VMAX_ADDR + 2;
        ...
        else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX347_RHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX347_RHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX347_RHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX347_RHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX347_RHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr = IMX347_RHS2_ADDR + 2;
    ...
    if(pstSnsRegsInfo->bConfig) imx347_default_reg_init(ViPipe);
    return AR_SUCCESS;
}
...
static AR_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30)
    {
        if (WDR_MODE_NONE == pstSnsState->enWDRMode)
        {
            if (IMX347_RES_IS_4M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX347_SENSOR_4M_25FPS_LINEAR_MODE;
                pstSnsState->u32FLStd = IMX347_VMAX_4MP30_LINEAR;
            }
            else
            {
        ...
        {
            if (IMX347_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX347_SENSOR_720P_30FPS_2t1_WDR_MODE;
                g_astimx347State[ViPipe].u32BRL  = 735;
            }
            else if (IMX347_RES_IS_4M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX347_SENSOR_4M_30FPS_2t1_WDR_MODE;
                g_astimx347State[ViPipe].u32BRL = 1538;
                pstSnsState->u32FLStd = IMX347_VMAX_4MP30_WDR;
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
        ...
        else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
        {
            if (IMX347_RES_IS_4M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX347_SENSOR_1080P_30FPS_3t1_WDR_MODE;
...
static AR_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = AR_FALSE;
    pstSnsState->u8ImgMode = IMX347_SENSOR_4M_25FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd = IMX347_VMAX_4MP30_LINEAR;
    pstSnsState->au32FL[0] = IMX347_VMAX_4MP30_LINEAR;
    pstSnsState->au32FL[1] = IMX347_VMAX_4MP30_LINEAR;

    memset(&pstSnsState->astRegsInfo[0], 0, sizeof(ISP_SNS_REGS_INFO_S));
    memset(&pstSnsState->astRegsInfo[1], 0, sizeof(ISP_SNS_REGS_INFO_S));
}

static AR_S32 imx347_cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    AR_S32 ret = 0;
    ...
    switch (mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = imx347_flip_off_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR:
        {
            ret = imx347_flip_off_mirro_on(ViPipe);
            break;
        }
        case ISP_SNS_FLIP:
        {
            ret = imx347_flip_on_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR_FLIP:
        {
            ret = imx347_flip_on_mirro_on(ViPipe);
            break;
        }
...
static AR_S32 imx347_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
    ...
    switch(u8CtlCode)
    {
        ...
        {
            ret = imx347_cmos_set_mirror_flip(ViPipe, mode);
            break;
        }
        default:
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init = imx347_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = imx347_exit;
    ...
    pstSensorExpFunc->pfn_cmos_sns_power_on = imx347_cmos_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = imx347_cmos_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = imx347_cmos_sensor_ctl;
    return AR_SUCCESS;
}
...
static AR_S32 imx347_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx347BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static AR_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    IMX347_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    IMX347_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static AR_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    IMX347_SENSOR_RESET_CTX(ViPipe);
}
...
static AR_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = IMX347_ID;
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, IMX347_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, IMX347_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, IMX347_ID);
    if (AR_SUCCESS != s32Ret)
    {
