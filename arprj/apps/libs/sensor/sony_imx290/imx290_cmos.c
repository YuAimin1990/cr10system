...
#define SENSOR0_RESET_GROUP      1
#define SENSOR0_RESET_PORT       2
#define SENSOR0_RESET_NUM        0
#endif
#define SENSOR0_POWER_GROUP      0
#define SENSOR0_POWER_PORT       0
#define SENSOR0_POWER_NUM        2
...
ISP_SNS_STATE_S *g_pastImx290[ISP_MAX_PIPE_NUM] = {AR_NULL};
...
#define IMX290_SENSOR_RESET_CTX(dev)         (g_pastImx290[dev] = AR_NULL)
...
typedef struct arIMX290_STATE_S
{
    AR_U8       u8Hcg;
    AR_U32      u32BRL;
    AR_U32      u32RHS1_MAX;
...
extern void imx290_trigger(VI_PIPE ViPipe, AR_S32 s32Frames);
extern void imx290_stream_on(VI_PIPE ViPipe);
...
extern void imx290_default_reg_init(VI_PIPE ViPipe);
extern int  imx290_flip_off_mirro_off(VI_PIPE ViPipe);
extern int  imx290_flip_off_mirro_on(VI_PIPE ViPipe);
extern int  imx290_flip_on_mirro_off(VI_PIPE ViPipe);
extern int  imx290_flip_on_mirro_on(VI_PIPE ViPipe);
...
#define IMX290_SGAIN_ADDR (0x30f2)
...
#define IMX290_FLIP_MIRRO (0x3007)
#define IMX290_INCREASE_LINES (1) /* make real fps less than stand fps because NVR require*/

#define IMX290_VMAX_1080P30_LINEAR  (1124+IMX290_INCREASE_LINES)
#define IMX290_VMAX_1080P30_WDR     (1349+IMX290_INCREASE_LINES)
#define IMX290_VMAX_720P60TO30_WDR  (750+IMX290_INCREASE_LINES)
#define IMX290_VMAX_1080P60TO30_WDR (1219+IMX290_INCREASE_LINES)
...
#define SONY_IMX290_MIPI_2M_30FPS_12BIT_VC_RAW	   (75)
#define IMX290_RES_IS_720P(w, h)       ((w) <= 1280 && (h) <= 720)
#define IMX290_RES_IS_1080P(w, h)      ((w) <= 1920 && (h) <= 1080)
#define IMX290_RES_IS_WDR(w, h)      ((w) <= 1952 && (h) <= 2678)

#define SENSOR_NM (0)


#if 0
static AR_S32 imx290_cmos_power_on(VI_PIPE ViPipe)
{
...
static AR_S32 imx290_cmos_power_off(VI_PIPE ViPipe)
{
    AR_U32 reset_val = 0, pow_val = 0, val = 0;
    AR_MPI_ISP_GetRegister(ViPipe, 0x07890004, &val);
...
static AR_S32 imx290_cmos_power_on(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
	...
	ar_printf("imx290_cmos_power_on power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
...
static AR_S32 imx290_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    int power_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->power_gpio[0],p_dev_power_attr->power_gpio[1],p_dev_power_attr->power_gpio[2]);
    int reset_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->reset_gpio[0],p_dev_power_attr->reset_gpio[1],p_dev_power_attr->reset_gpio[2]);
	...
	ar_printf("imx290_cmos_power_off  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
	//set the gpio dir to out
	ar_hal_gpio_set_dir(power_gpio,1);
	ar_hal_gpio_set_dir(reset_gpio,1);
...
static AR_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	pstAeSnsDft->f32MaxFps = (pstSnsState->u8ImgMode == IMX290_SENSOR_1080P_30FPS_LINEAR_MODE ? 30 : g_astImx290ImageMode[ViPipe].f32Fps);
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32VMAX = IMX290_VMAX_1080P30_LINEAR;
    ...
    switch (pstSnsState->u8ImgMode)
    {
        case IMX290_SENSOR_1080P_30FPS_2t1_WDR_MODE:
            ...
            {
                u32VMAX = IMX290_VMAX_1080P30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
        ...
        case IMX290_SENSOR_1080P_30FPS_3t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 16.5))
            {
                u32VMAX = IMX290_VMAX_1080P120TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
        ...
        case IMX290_SENSOR_1080P_30FPS_LINEAR_MODE:
            ...
            {
                u32VMAX = IMX290_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);		// dg : 20220318
                //u32VMAX = IMX290_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
        ...
        case IMX290_SENSOR_1080P_60FPS_LINEAR_MODE:							// dg : 20220318
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX290_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
        ...
        case IMX290_SENSOR_720P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX290_VMAX_720P60TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
    ...
    printf("%s %d: f32MaxFps=%f, f32Fps=%f, u32FLStd=%d\n", __func__, __LINE__, pstAeSnsDft->f32MaxFps, pstAeSnsDft->f32Fps, pstSnsState->u32FLStd);
    return;
...
static AR_VOID cmos_inttime_update(VI_PIPE ViPipe, AR_U32 u32IntTime)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        else   /* long exposure */
        {
            ...
            if (IMX290_SENSOR_1080P_30FPS_2t1_WDR_MODE == pstSnsState->u8ImgMode)
            {
...
static AR_VOID cmos_again_calc_table(VI_PIPE ViPipe, AR_U32 *pu32AgainLin, AR_U32 *pu32AgainDb)
{
    ...
    {
        g_astimx290State[ViPipe].u8Hcg |= 0x10;
        u32HCGOn = 1;
    }
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32HCG = g_astimx290State[ViPipe].u8Hcg;
    AR_U32 u32Tmp;
    static AR_BOOL bFirst[ISP_MAX_PIPE_NUM] = {[0 ... (ISP_MAX_PIPE_NUM - 1)] = 1};
    IMX290_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
...
static AR_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    case WDR_MODE_NONE:
        if (ViPipe == 0)
        {
            strcpy(pstDef->TuningPraBinName, "/usrdata/local/factory/tunning/cam_imx307/imx307_tuning_preview_rgb.bin");
        }
        else
        {
            strcpy(pstDef->TuningPraBinName, "/usrdata/local/factory/tunning/cam_imx307/imx307_tuning_preview_ir.bin");
        }
        break;
    case WDR_MODE_2To1_LINE:
    case WDR_MODE_3To1_LINE:
	...
	pstDef->stWdrSwitchAttr.au32VcCnt = pstSnsState->u8Hdr & 0xf;
	 pstDef->stWdrSwitchAttr.au32VcMask =  (pstSnsState->u8Hdr >> 4) & 0xf;
    pstDef->stSensorMode.u32SensorID = IMX290_ID + ViPipe;
    pstDef->stSensorMode.u8SensorMode = pstSnsState->u8ImgMode;
	pstDef->bFoucs=pstSnsState->focus;
    ...
    {
        default:
        case IMX290_SENSOR_1080P_60FPS_LINEAR_MODE:					// dg : right or not?		20220318
        case IMX290_SENSOR_1080P_30FPS_LINEAR_MODE:
        case IMX290_SENSOR_1080P_30FPS_3t1_WDR_MODE:
...
static AR_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, AR_BOOL bEnable)
{
    ...
    {
        if (IMX290_SENSOR_1080P_30FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            u32FullLines_5Fps = (IMX290_VMAX_1080P30_LINEAR * 30) / 5;
        }
        else if (IMX290_SENSOR_1080P_60FPS_LINEAR_MODE == pstSnsState->u8ImgMode)			// dg : right or not		20220318
        {
            u32FullLines_5Fps = (IMX290_VMAX_1080P30_LINEAR * 60) / 5;
        }
...
static AR_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((AR_FALSE == pstSnsState->bSyncInit) || (AR_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX290_SGAIN_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX290_HCG_ADDR;

            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX290_SHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX290_SHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX290_SHS2_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX290_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX290_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX290_VMAX_ADDR + 2;
...
static AR_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->u8SnsMode == SONY_IMX290_MIPI_2M_30FPS_12BIT_VC_RAW)
	{
		//low 4bits: vc_cnt, high 4bits = vc mask; 
		pstSnsState->u8Hdr = 0;
		pstSnsState->u8Hdr = 0x02 | (0x03 << 4);
	}
	else
		pstSnsState->u8Hdr = 0;
    ...
    if (pstSensorImageMode->f32Fps <= 30)
    {
        ...
        else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            else if (IMX290_RES_IS_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX290_SENSOR_1080P_30FPS_2t1_WDR_MODE;
                pstSnsState->u32FLStd = IMX290_VMAX_1080P60TO30_WDR * 2 + 340;
                g_astimx290State[ViPipe].u32BRL = 1109;
            }
            else
            {
    ...
    else
    {
        if (WDR_MODE_NONE == pstSnsState->enWDRMode)
        {
            ...
            if (IMX290_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = IMX290_SENSOR_1080P_60FPS_LINEAR_MODE;
                pstSnsState->u32FLStd = IMX290_VMAX_1080P30_LINEAR;
            }
            else
            {
...
static AR_S32 imx290_cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    AR_S32 ret = 0;
    ...
    switch (mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = imx290_flip_off_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR:
        {
            ret = imx290_flip_off_mirro_on(ViPipe);
            break;
        }
        case ISP_SNS_FLIP:
        {
            ret = imx290_flip_on_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR_FLIP:
        {
            ret = imx290_flip_on_mirro_on(ViPipe);
            break;
        }
...
static AR_S32 imx290_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
    ...
    switch(u8CtlCode)
    {
        ...
        {        
            ...
            ret = imx290_cmos_set_mirror_flip(ViPipe, mode);
            break;
        }
		case SENSOR_CTL_SET_LED:
		{
		     STRU_CAM_PRA_LED_PRA_T *pra = (STRU_CAM_PRA_LED_PRA_T *)pSensorCtl->pCtlData;
			 ar_printf("SENSOR_CTL_SET_LED %d %d \n",pra->status,pra->led_current[0]);
		}
		break;
		case SENSOR_CTL_SET_IR_CUTTER:
		{
		   STRU_CAM_PRA_IR_CUTTER_PRA_T *pra=(STRU_CAM_PRA_IR_CUTTER_PRA_T *)pSensorCtl->pCtlData;		   
		   ar_printf("SENSOR_CTL_SET_IR_CUTTER %d \n",pra->status);
...
static AR_S32 cmos_sns_move_focus(VI_PIPE ViPipe,ISP_SNS_MOVE_FOUCS_S *move)
{
    ar_printf("ViPipe:%d move to %d \n",ViPipe,move->dac_value);
	return 0;
}

static AR_S32 cmos_sns_get_otp_data(VI_PIPE ViPipe,STRU_OTP_CALLIBRATION_DATA_T *stOtpData)
{
    //here to read the eeprom or opt mem to fill the calibration data
	if(stOtpData)
	{
	    stOtpData->awb_valid=0;
		stOtpData->awb_gold_valid=0;
		stOtpData->awb_r_gr_gb_b[0]=128;
		stOtpData->awb_r_gr_gb_b[1]=128;
		stOtpData->awb_r_gr_gb_b[2]=128;		
		stOtpData->awb_r_gr_gb_b[3]=128;
		stOtpData->lsc_valid=0;
		stOtpData->lsc_gold_valid=0;
		for(int i=0;i<300;i++)
		{
		    stOtpData->lsc[i]=15*1024;
		}
	}
}
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sns_stream_on= imx290_stream_on;
	pstSensorExpFunc->pfn_cmos_sns_trigger = imx290_trigger;
    ...
    pstSensorExpFunc->pfn_cmos_sns_power_on = imx290_cmos_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = imx290_cmos_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = imx290_cmos_sensor_ctl;
	pstSensorExpFunc->pfn_cmos_sns_move_focus=cmos_sns_move_focus;
	pstSensorExpFunc->pfn_cmos_sns_get_otp_data=cmos_sns_get_otp_data;
    return AR_SUCCESS;
}
...
static AR_S32 imx290_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static AR_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = IMX290_ID + ViPipe;
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, IMX290_ID + ViPipe);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, IMX290_ID + ViPipe);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, IMX290_ID + ViPipe);
    if (AR_SUCCESS != s32Ret)
    {
...
static AR_S32 SetFocus(VI_PIPE ViPipe, AR_S32 focus)  //0 disable foucus, 1:enable foucus
{   
    ISP_SNS_STATE_S *pastSnsStateCtx = AR_NULL;
    IMX290_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
	pastSnsStateCtx->focus=focus;
	ar_always("foucus=%d",pastSnsStateCtx->focus);
}
...
{
   {
       .stComboAttr=  
	   {
		   ...
		   {
				   {GROUP_1,PORT_C,1},//reset
				   {GROUP_0,PORT_A,2},//power
				   {-1,-1,-1},//common
				   /*
                     #define CLK_SENSOR0		64
                     #define CLK_SENSOR1		65
...
static int GetDefaultAttr(AR_U8 u8SnsMode,SRTU_SENSOR_DEFAULT_ATTR_T *pstDefaultAttr)
{
    ...
    if (u8SnsMode == 1)
    {
        pstDefaultAttr->stComboAttr.devno = 1;
        pstDefaultAttr->stComboAttr.dev_power_attr.mclk_src = CLK_SENSOR1;
        pstDefaultAttr->stComboAttr.dev_power_attr.reset_gpio[0] = GROUP_1;
        pstDefaultAttr->stComboAttr.dev_power_attr.reset_gpio[1] = PORT_C;
        pstDefaultAttr->stComboAttr.dev_power_attr.reset_gpio[2] = 0;
    }
    return 0;
}
