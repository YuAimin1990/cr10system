#if !defined(__ISPIN0_CMOS_H_)
#define __ISPIN0_CMOS_H_
...
ISP_SNS_STATE_S *g_pastIspIn0[ISP_MAX_PIPE_NUM] = {AR_NULL};

#define ISPIN0_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastIspIn0[dev])
#define ISPIN0_SENSOR_SET_CTX(dev, pstCtx)   (g_pastIspIn0[dev] = pstCtx)
#define ISPIN0_SENSOR_RESET_CTX(dev)         (g_pastIspIn0[dev] = AR_NULL)
...
ISPIN0_STATE_S g_astisp_in_0State[ISP_MAX_PIPE_NUM] = {{0}};
...
static AR_S32 isp_in_0_cmos_power_on(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    ar_always("%s \n",__FUNCTION__);
	return 0;
}

static AR_S32 isp_in_0_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    ar_always("%s \n",__FUNCTION__);
	return 0;
}
static AR_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    ISPIN0_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER(pstSnsState);

	pstAeSnsDft->f32Fps=30;
	pstAeSnsDft->f32MaxFps=30;
    pstAeSnsDft->u32FullLinesStd = ISPIN0_VMAX_1080P30_LINEAR;
    ...
    {
        pstAeSnsDft->u32LinesPer500ms = ISPIN0_VMAX_1080P30_LINEAR* pstAeSnsDft->f32Fps / 2;
    }
    else
    {
    ...
    switch (pstSnsState->enWDRMode)
    {
        ...
        case WDR_MODE_NONE:   /*linear mode*/
            ...
            pstAeSnsDft->u32MaxIntTime = ISPIN0_VMAX_1080P30_LINEAR - 2;
...
static AR_S32 isp_in_0_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
...
static void isp_in_0_init(VI_PIPE ViPipe)
{
   ar_always("%s \n",__FUNCTION__);
   return;
}
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = NULL;
...
static AR_S32 isp_in_0_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static AR_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    ISPIN0_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    ISPIN0_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static AR_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    ISPIN0_SENSOR_RESET_CTX(ViPipe);
}
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, ISP_IN_0_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, ISP_IN_0_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, ISP_IN_0_ID);
    if (AR_SUCCESS != s32Ret)
    {
