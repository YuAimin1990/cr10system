#if !defined(__PLECO_CMOS_H_)
#define __PLECO_CMOS_H_
...
ISP_SNS_STATE_S *g_pastPleco[ISP_MAX_PIPE_NUM] = {AR_NULL};

#define PLECO_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastPleco[dev])
#define PLECO_SENSOR_SET_CTX(dev, pstCtx)   (g_pastPleco[dev] = pstCtx)
#define PLECO_SENSOR_RESET_CTX(dev)         (g_pastPleco[dev] = AR_NULL)

ISP_SNS_COMMBUS_U g_aunPlecoBusInfo[ISP_MAX_PIPE_NUM] =
{
    [0] = { .s8I2cDev = 1},
	[1] = { .s8I2cDev = 1},
    [2 ... ISP_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1}
};
...
static ISP_CMOS_SENSOR_IMAGE_MODE_S g_astPlecoImageMode[ISP_MAX_PIPE_NUM];
...
extern const unsigned int pleco_i2c_addr;
extern unsigned int pleco_addr_byte;
extern unsigned int pleco_data_byte;

typedef struct arPLECO_STATE_S
{
    AR_U8       u8Hcg;
    AR_U32      u32BRL;
    AR_U32      u32RHS1_MAX;
...
PLECO_STATE_S g_astplecoState[ISP_MAX_PIPE_NUM] = {{0}};

extern void pleco_init(VI_PIPE ViPipe);
extern void pleco_exit(VI_PIPE ViPipe);
void pleco_trigger(VI_PIPE ViPipe);

extern void pleco_standby(VI_PIPE ViPipe);
extern void pleco_restart(VI_PIPE ViPipe);
extern int  pleco_write_register(VI_PIPE ViPipe, int addr, int data);
extern int  pleco_read_register(VI_PIPE ViPipe, int addr);
extern void pleco_default_reg_init(VI_PIPE ViPipe);
extern int  pleco_flip_off_mirro_off(VI_PIPE ViPipe);
extern int  pleco_flip_off_mirro_on(VI_PIPE ViPipe);
extern int  pleco_flip_on_mirro_off(VI_PIPE ViPipe);
extern int  pleco_flip_on_mirro_on(VI_PIPE ViPipe);

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define PLECO_FULL_LINES_MAX  (0x3FFFF)
#define PLECO_FULL_LINES_MAX_2TO1_WDR  (0x8AA)    // considering the YOUT_SIZE and bad frame
#define PLECO_FULL_LINES_MAX_3TO1_WDR  (0x7FC)

/*****PLECO Register Address*****/
#define PLECO_SHS1_ADDR (0x3020)
#define PLECO_SHS2_ADDR (0x3024)
#define PLECO_SHS3_ADDR (0x3028)
#define PLECO_GAIN_ADDR (0x3014)
#define PLECO_SGAIN_ADDR (0x30f2)
#define PLECO_HCG_ADDR  (0x3009)
#define PLECO_VMAX_ADDR (0x3018)
#define PLECO_HMAX_ADDR (0x301c)
#define PLECO_RHS1_ADDR (0x3030)
#define PLECO_RHS2_ADDR (0x3034)
#define PLECO_Y_OUT_SIZE_ADDR (0x3418)
#define PLECO_FLIP_MIRRO (0x3007)
#define PLECO_INCREASE_LINES (1) /* make real fps less than stand fps because NVR require*/

#define PLECO_VMAX_1080P30_LINEAR  (1124+PLECO_INCREASE_LINES)
#define PLECO_VMAX_1080P30_WDR     (1349+PLECO_INCREASE_LINES)
#define PLECO_VMAX_720P60TO30_WDR  (750+PLECO_INCREASE_LINES)
#define PLECO_VMAX_1080P60TO30_WDR (1219+PLECO_INCREASE_LINES)
#define PLECO_VMAX_1080P120TO30_WDR (1125+PLECO_INCREASE_LINES)
...
#define PLECO_RES_IS_720P(w, h)       ((w) <= 1280 && (h) <= 720)
#define PLECO_RES_IS_1080P(w, h)      ((w) <= 1920 && (h) <= 1080)
#define PLECO_RES_IS_WDR(w, h)      ((w) <= 1952 && (h) <= 2678)
#define PLECO_RES_IS_1920_2892(w, h)      ((w) <= 1920 && (h) <= 2892)
...
#if 0
static AR_S32 pleco_cmos_power_on(VI_PIPE ViPipe)
{
...
static AR_S32 pleco_cmos_power_off(VI_PIPE ViPipe)
{
    AR_U32 reset_val = 0, pow_val = 0, val = 0;
    AR_MPI_ISP_GetRegister(ViPipe, 0x07890004, &val);
...
//	ISP_TRACE(AR_DBG_ERR, "mclk_src %d \n",p_dev_power_attr->mclk_src);
	...
	int power_gpio_test = ar_hal_gpio_name_to_num(1, 2, 2);
	
//	ISP_TRACE(AR_DBG_ERR, "pleco_cmos_power_on  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
	ar_always("#@@@@@@@@@@@pleco power on");
#if 1
	...
	usleep(10000);
    //ar_hal_gpio_set_value(power_gpio_test,1);
    usleep(10000);
	return 0;

}

static AR_S32 pleco_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
    int power_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->power_gpio[0],p_dev_power_attr->power_gpio[1],p_dev_power_attr->power_gpio[2]);
    int reset_gpio=ar_hal_gpio_name_to_num(p_dev_power_attr->reset_gpio[0],p_dev_power_attr->reset_gpio[1],p_dev_power_attr->reset_gpio[2]);
	...
	ar_always("pleco_cmos_power_off  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
	//set the gpio dir to out
	ar_hal_gpio_set_dir(power_gpio,1);
	ar_hal_gpio_set_dir(reset_gpio,1);
...
static AR_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    PLECO_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER(pstSnsState);
    pstAeSnsDft->f32Fps = g_astPlecoImageMode[ViPipe].f32Fps;
	pstAeSnsDft->f32MaxFps = (pstSnsState->u8ImgMode == PLECO_SENSOR_1080P_30FPS_LINEAR_MODE ? 30 : g_astPlecoImageMode[ViPipe].f32Fps);
    pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
    pstAeSnsDft->u32FlickerFreq = 50 * 256;
    pstAeSnsDft->u32FullLinesMax = PLECO_FULL_LINES_MAX;
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32VMAX = PLECO_VMAX_1080P30_LINEAR;
    ...
    {
        case PLECO_SENSOR_1080P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 16.5))
            {
                u32VMAX = PLECO_VMAX_1080P30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
            ...
            u32VMAX = (u32VMAX > PLECO_FULL_LINES_MAX_2TO1_WDR) ? PLECO_FULL_LINES_MAX_2TO1_WDR : u32VMAX;
            break;

        case PLECO_SENSOR_1080P_30FPS_3t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 16.5))
            {
                u32VMAX = PLECO_VMAX_1080P120TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
            ...
            u32VMAX = (u32VMAX > PLECO_FULL_LINES_MAX_3TO1_WDR) ? PLECO_FULL_LINES_MAX_3TO1_WDR : u32VMAX;
            break;

        case PLECO_SENSOR_1080P_30FPS_LINEAR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = PLECO_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);		// dg : 20220318
                //u32VMAX = PLECO_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
            ...
            u32VMAX = (u32VMAX > PLECO_FULL_LINES_MAX) ? PLECO_FULL_LINES_MAX : u32VMAX;
            break;

        case PLECO_SENSOR_1080P_60FPS_LINEAR_MODE:							// dg : 20220318
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = PLECO_VMAX_1080P30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
        ...
        case PLECO_SENSOR_720P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = PLECO_VMAX_720P60TO30_WDR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support Fps: %f\n", f32Fps);
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            2n + 5
            RHS1 <= FSC - BRL*2 -21
            (2 * VMAX_PLECO_1080P30_WDR - 2 * gu32BRL - 21) - (((2 * VMAX_PLECO_1080P30_WDR - 2 * 1109 - 21) - 5) %2)
        */

        g_astplecoState[ViPipe].u32RHS1_MAX = (u32VMAX - g_astplecoState[ViPipe].u32BRL) * 2 - 21;
    ...
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            RHS2 <= FSC - BRL*3 -25
        */
        g_astplecoState[ViPipe].u32RHS2_MAX = u32VMAX * 4 - g_astplecoState[ViPipe].u32BRL * 3 - 25;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        //u32FullLines = (u32FullLines > 2 * PLECO_FULL_LINES_MAX_2TO1_WDR) ? 2 * PLECO_FULL_LINES_MAX_2TO1_WDR : u32FullLines;
        //pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;
        //g_astplecoState[ViPipe].u32RHS1_MAX = pstSnsState->au32FL[0] - g_astplecoState[ViPipe].u32BRL * 2 - 21;
    }
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > 4 * PLECO_FULL_LINES_MAX_3TO1_WDR) ? 4 * PLECO_FULL_LINES_MAX_3TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 2) << 2;
        g_astplecoState[ViPipe].u32RHS2_MAX = pstSnsState->au32FL[0] - g_astplecoState[ViPipe].u32BRL * 3 - 25;
    }
    else if ( WDR_MODE_2To1_FRAME == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > PLECO_FULL_LINES_MAX) ? PLECO_FULL_LINES_MAX : u32FullLines;
...
static AR_VOID cmos_inttime_update(VI_PIPE ViPipe, AR_U32 u32IntTime)
{
    ...
    static AR_S32 u32SHS1[ISP_MAX_PIPE_NUM]  = {0};
    static AR_S32 u32SHS2[ISP_MAX_PIPE_NUM]  = {0};
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        if (bFirst[ViPipe]) /* short exposure */
        {
            ...
            //ar_always("u32ShortIntTime = %d \n",u32ShortIntTime[ViPipe]);
            u32SHS1[ViPipe] = pstSnsState->u32FLShort - u32ShortIntTime[ViPipe];
            if (u32SHS1[ViPipe] <= 2)
            {
                u32SHS1[ViPipe] = 2;
            }
            else if (u32SHS1[ViPipe] >= pstSnsState->u32FLShort - 2)
            {
                u32SHS1[ViPipe] = pstSnsState->u32FLShort - 2;
            }
        ...
        else   /* long exposure */
        {
            ...
            u32SHS2[ViPipe] = pstSnsState->au32FL[1] - u32LongIntTime[ViPipe];
            if (u32SHS2[ViPipe] <= pstSnsState->u32FLShort + 2) //this is not same as datasheet with >= RSH1 + 2
            {
                u32SHS2[ViPipe] = pstSnsState->u32FLShort + 2;
            }
            else if (u32SHS2[ViPipe] >= pstSnsState->au32FL[1] - 2)
            {
                u32SHS2[ViPipe] =  pstSnsState->au32FL[1] - 2;
            }

            if (PLECO_SENSOR_1080P_30FPS_2t1_WDR_MODE == pstSnsState->u8ImgMode)
            {
            ...
            //ar_always("u32ShortIntTime = %d u32SHS1 = %d, u32SHS2 = %d \n",u32ShortIntTime[ViPipe], u32SHS1[ViPipe], u32SHS2[ViPipe]);

            pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = (u32SHS2[ViPipe] & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = ((u32SHS2[ViPipe] & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = ((u32SHS2[ViPipe] & 0x30000) >> 16);

            pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = (u32SHS1[ViPipe] & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = ((u32SHS1[ViPipe] & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = ((u32SHS1[ViPipe] & 0x30000) >> 16);
...
static AR_VOID cmos_again_calc_table(VI_PIPE ViPipe, AR_U32 *pu32AgainLin, AR_U32 *pu32AgainDb)
{
    ...
    if (*pu32AgainLin >= 65536) //64x1024 gain
    {
        g_astplecoState[ViPipe].u8Hcg |= 0x10;
        u32HCGOn = 1;
    }
...
# if 0  //not to do digital dcg of imx307
    AR_U32 u32HCGOn = 0;

    if (*pu32DgainLin >= 65536) //64x1024 gain
    {
        g_astplecoState[ViPipe].u8Hcg |= 0x10;
        u32HCGOn = 1;
    }
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32HCG = g_astplecoState[ViPipe].u8Hcg;
    AR_U32 u32Tmp;
    static AR_BOOL bFirst[ISP_MAX_PIPE_NUM] = {[0 ... (ISP_MAX_PIPE_NUM - 1)] = 1};
    PLECO_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
    ...
    {
         if (bFirst[ViPipe]) //short gain
         {
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = (u32Tmp & 0xFF);
            //u32HCG = u32HCG & 0xf;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = (u32HCG & 0xFF);
            bFirst[ViPipe] = AR_FALSE;
         }
         else //long gain
         {
    ...
    else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astplecoState[ViPipe].u32RHS1_MAX - 3)) ? (g_astplecoState[ViPipe].u32RHS1_MAX - 3) : u32IntTimeMaxTmp;
    ...
    else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32RHS2_Max = ((g_astplecoState[ViPipe].u32RHS2_MAX - 8) * 0x40) / (au32Ratio[0] + 0x40);
...
static AR_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    pstDef->stSensorMode.u32SensorID = PLECO_ID;
    ...
    {
        default:
        case PLECO_SENSOR_1080P_60FPS_LINEAR_MODE:					// dg : right or not?		20220318
        case PLECO_SENSOR_1080P_30FPS_LINEAR_MODE:
        case PLECO_SENSOR_1080P_30FPS_3t1_WDR_MODE:
...
static AR_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, AR_BOOL bEnable)
{
    ...
    {
        if (PLECO_SENSOR_1080P_30FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            u32FullLines_5Fps = (PLECO_VMAX_1080P30_LINEAR * 30) / 5;
        }
        else if (PLECO_SENSOR_1080P_60FPS_LINEAR_MODE == pstSnsState->u8ImgMode)			// dg : right or not		20220318
        {
            u32FullLines_5Fps = (PLECO_VMAX_1080P30_LINEAR * 60) / 5;
        }
    ...
    //u32FullLines_5Fps = (u32FullLines_5Fps > PLECO_FULL_LINES_MAX) ? PLECO_FULL_LINES_MAX : u32FullLines_5Fps;
    u32MaxIntTime_5Fps = 4;

    if (bEnable) /* setup for ISP pixel calibration mode */
    {
//        pleco_write_register (ViPipe, PLECO_GAIN_ADDR, 0x00);

//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR, u32FullLines_5Fps & 0xFF);
//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR + 1, (u32FullLines_5Fps & 0xFF00) >> 8);
//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR + 2, (u32FullLines_5Fps & 0xF0000) >> 16);

//        pleco_write_register (ViPipe, PLECO_SHS1_ADDR, u32MaxIntTime_5Fps & 0xFF);
//        pleco_write_register (ViPipe, PLECO_SHS1_ADDR + 1,  (u32MaxIntTime_5Fps & 0xFF00) >> 8);
//        pleco_write_register (ViPipe, PLECO_SHS1_ADDR + 2, (u32MaxIntTime_5Fps & 0xF0000) >> 16);

    }
    else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0x1FFFF) ? 0x1FFFF : pstSnsState->u32FLStd;
//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR, pstSnsState->u32FLStd & 0xFF);
//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR + 1, (pstSnsState->u32FLStd & 0xFF00) >> 8);
//        pleco_write_register (ViPipe, PLECO_VMAX_ADDR + 2, (pstSnsState->u32FLStd & 0xF0000) >> 16);
...
static AR_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((AR_FALSE == pstSnsState->bSyncInit) || (AR_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunPlecoBusInfo[ViPipe].s8I2cDev;
        pstSnsState->astRegsInfo[0].u8Cfg2ValidDelayMax = 2;
        pstSnsState->astRegsInfo[0].u32RegNum = 1;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = pleco_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = pleco_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = pleco_data_byte;
        }

        //Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = PLECO_SHS1_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = PLECO_SHS1_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = PLECO_SHS1_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //make shutter and gain effective at the same time
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = PLECO_GAIN_ADDR;  //gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = PLECO_HCG_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = PLECO_VMAX_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = PLECO_VMAX_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = PLECO_VMAX_ADDR + 2;

        if ( WDR_MODE_2To1_FRAME == pstSnsState->enWDRMode)
        {
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = PLECO_SHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = PLECO_SHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = PLECO_SHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = PLECO_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = PLECO_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = PLECO_VMAX_ADDR + 2;
        ...
        else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = PLECO_SGAIN_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = PLECO_HCG_ADDR;

            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = PLECO_SHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = PLECO_SHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = PLECO_SHS2_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = PLECO_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = PLECO_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = PLECO_VMAX_ADDR + 2;
        ...
        else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = PLECO_SHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = PLECO_SHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = PLECO_SHS2_ADDR + 2;
            ...
            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = PLECO_RHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = PLECO_RHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = PLECO_RHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = PLECO_RHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = PLECO_RHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr = PLECO_RHS2_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[17].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[17].u32RegAddr = PLECO_SHS3_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[18].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[18].u32RegAddr = PLECO_SHS3_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[19].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[19].u32RegAddr = PLECO_SHS3_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[20].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[20].u32RegAddr = PLECO_Y_OUT_SIZE_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[21].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[21].u32RegAddr = PLECO_Y_OUT_SIZE_ADDR + 1;
	...
	ar_always("@@@vipipe = %d, s8I2cDev = %d", ViPipe, pstSnsState->astRegsInfo[0].unComBus.s8I2cDev);	
...
static AR_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    g_astPlecoImageMode[ViPipe] = *pstSensorImageMode;
    ...
    if (pstSensorImageMode->f32Fps <= 30)
    {
        if (WDR_MODE_NONE == pstSnsState->enWDRMode)
        {
//            g_astplecoState[ViPipe].u8Hcg = 0x2;

//            if (PLECO_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
//            {
//                u8SensorImageMode = PLECO_SENSOR_1080P_30FPS_LINEAR_MODE;
//                pstSnsState->u32FLStd = PLECO_VMAX_1080P30_LINEAR;
//            }
//            else
//            {
...
//            }
            ...
            if (PLECO_RES_IS_1920_2892(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_DUAL_SHUFFLE_MASTER_1920x2892_MODE;
                pstSnsState->u32FLStd = PLECO_VMAX_1080P30_LINEAR;
				ar_always("cmos_set_image_mode:PLECO_SENSOR_DUAL_SHUFFLE_MASTER_1920x2892_MODE\n");
            }
            else
            {
                ISP_TRACE(AR_DBG_ERR, "Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
        ...
        {
            g_astplecoState[ViPipe].u8Hcg    = 0x1;

            if (PLECO_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_720P_30FPS_2t1_WDR_MODE;
                pstSnsState->u32FLStd = PLECO_VMAX_720P60TO30_WDR * 2;
                g_astplecoState[ViPipe].u32BRL  = 735;
            }
            else if (PLECO_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_1080P_30FPS_2t1_WDR_MODE;
                pstSnsState->u32FLStd = PLECO_VMAX_1080P60TO30_WDR * 2;
                g_astplecoState[ViPipe].u32BRL = 1109;
            }
            else if (PLECO_RES_IS_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_1080P_30FPS_2t1_WDR_MODE;
                pstSnsState->u32FLStd = PLECO_VMAX_1080P60TO30_WDR * 2 + 340;
                g_astplecoState[ViPipe].u32BRL = 1109;
            }
            else
            {
        ...
        {
            g_astplecoState[ViPipe].u8Hcg    = 0x0;

            if (PLECO_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_1080P_30FPS_3t1_WDR_MODE;
                pstSnsState->u32FLStd  = PLECO_VMAX_1080P120TO30_WDR * 4;
                g_astplecoState[ViPipe].u32BRL = 1109;
            }
            else
            {
    ...
    else
    {
        if (WDR_MODE_NONE == pstSnsState->enWDRMode)
        {
            ...
            if (PLECO_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
            {
                u8SensorImageMode = PLECO_SENSOR_1080P_60FPS_LINEAR_MODE;
                pstSnsState->u32FLStd = PLECO_VMAX_1080P30_LINEAR;
            }
            else
            {
...
static AR_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = AR_FALSE;
    pstSnsState->u8ImgMode = PLECO_SENSOR_1080P_30FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd = PLECO_VMAX_1080P30_LINEAR;
    pstSnsState->au32FL[0] = PLECO_VMAX_1080P30_LINEAR;
    pstSnsState->au32FL[1] = PLECO_VMAX_1080P30_LINEAR;
...
//    int ret = 0;
//    if (enable)
//        ret = pleco_write_register(ViPipe, 16, 0x01);
//    else
//        ret = pleco_write_register(ViPipe, 16, 0x00);
...
//        printf("[ERROR] %s[%d] inter_time %dus out of range\n", __func__, __LINE__, integrationTime);
//        return -1;
//    }
//    
...
//    pleco_group_hold(ViPipe, true);
//    pleco_write_register(ViPipe, 526, valueL); // F0
//    pleco_write_register(ViPipe, 527, valueH);
//    
//    pleco_write_register(ViPipe, 528, valueL); // F1
//    pleco_write_register(ViPipe, 529, valueH);
//    pleco_group_hold(ViPipe, false);
...
//    //F1
//    ret = pleco_i2c_read8(VI_PIPE ViPipe, 526, &valueL);
//    if (ret < 0) {
//        return ret;
//    }
//    ret = pleco_i2c_read8(VI_PIPE ViPipe, 527, &valueH);
//    if (ret < 0) {
//        return ret;
//    }

//    //频率1和频率2的积分时间一样, 我们这边只读取一个就好了
//#if 0
//    //F2
//    ret = pleco_i2c_read8(528, &valueL);
//    if (ret < 0) {
//        return ret;
//    }
//    ret = pleco_i2c_read8(529, &valueH);
//    if (ret < 0) {
//        return ret;
//    }
//#endif
...
static AR_S32 pleco_cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    AR_S32 ret = 0;
    ...
    switch (mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = pleco_flip_off_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR:
        {
            ret = pleco_flip_off_mirro_on(ViPipe);
            break;
        }
        case ISP_SNS_FLIP:
        {
            ret = pleco_flip_on_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR_FLIP:
        {
            ret = pleco_flip_on_mirro_on(ViPipe);
            break;
        }
...
static AR_S32 pleco_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
    ...
    switch(u8CtlCode)
    {
        ...
        {
            ret = pleco_cmos_set_mirror_flip(ViPipe, mode);
            break;
        }
        default:
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init = pleco_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = pleco_exit;
	pstSensorExpFunc->pfn_cmos_sns_trigger = pleco_trigger;
    ...
    pstSensorExpFunc->pfn_cmos_sns_power_on = pleco_cmos_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = pleco_cmos_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = pleco_cmos_sensor_ctl;
    return AR_SUCCESS;
}
...
static AR_S32 pleco_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunPlecoBusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
	ar_always("@@@@@@@ ViPipe = %d, g_aunPlecoBusInfo[ViPipe].s8I2cDev = %d", ViPipe, g_aunPlecoBusInfo[ViPipe].s8I2cDev);
    return AR_SUCCESS;
}
...
static AR_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    PLECO_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    PLECO_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static AR_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = PLECO_ID;
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, PLECO_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, PLECO_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, PLECO_ID);
    if (AR_SUCCESS != s32Ret)
    {
...
ISP_SNS_OBJ_S stSnsPlecoObj =
{
    .pfnRegisterCallback    = sensor_register_callback,
    .pfnUnRegisterCallback  = sensor_unregister_callback,
    .pfnStandby             = pleco_standby,
    .pfnRestart             = pleco_restart,
    .pfnMirrorFlip          = AR_NULL,
    .pfnWriteReg            = pleco_write_register,
    .pfnReadReg             = pleco_read_register,
    .pfnSetBusInfo          = pleco_set_bus_info,
