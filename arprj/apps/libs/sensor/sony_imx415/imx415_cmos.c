...
ISP_SNS_STATE_S *g_pastImx415[ISP_MAX_PIPE_NUM] = {AR_NULL};
...
#define IMX415_SENSOR_RESET_CTX(dev)         (g_pastImx415[dev] = AR_NULL)
...
extern const unsigned int imx415_i2c_addr;
extern unsigned int imx415_addr_byte;
extern unsigned int imx415_data_byte;

typedef struct arIMX415_STATE_S
{
    AR_U8       u8Hcg;
    AR_U32      u32BRL;
    AR_U32      u32RHS1_MAX;
...
extern void imx415_default_reg_init(VI_PIPE ViPipe);
extern int  imx415_flip_off_mirro_off(VI_PIPE ViPipe);
extern int  imx415_flip_off_mirro_on(VI_PIPE ViPipe);
extern int  imx415_flip_on_mirro_off(VI_PIPE ViPipe);
extern int  imx415_flip_on_mirro_on(VI_PIPE ViPipe);
extern void imx415_trigger(VI_PIPE ViPipe, AR_S32 s32Frames);

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define IMX415_FULL_LINES_MAX  (0x0FFFFF)
#define IMX415_FULL_LINES_MAX_2TO1_WDR  (0x8AA)    // considering the YOUT_SIZE and bad frame
#define IMX415_FULL_LINES_MAX_3TO1_WDR  (0x7FC)

/*****Imx415 Register Address*****/
#define IMX415_SHR_ADDR (0x3050)
#define IMX415_GAIN_ADDR (0x3090)
#define IMX415_VMAX_ADDR (0x3024)
#define IMX415_HMAX_ADDR (0x3028)
#define IMX415_INCREASE_LINES (1) /* make real fps less than stand fps because NVR require*/

#define IMX415_VMAX_4KP30_LINEAR  (2250+IMX415_INCREASE_LINES)
#define IMX415_VMAX_4KP60_LINEAR  2250 //(2250+IMX415_INCREASE_LINES)
// #define IMX415_VMAX_720P60TO30_WDR  (750+IMX415_INCREASE_LINES)
// #define IMX415_VMAX_1080P60TO30_WDR (1220+IMX415_INCREASE_LINES)
// #define IMX415_VMAX_1080P120TO30_WDR (1125+IMX415_INCREASE_LINES)
...
#define SONY_IMX415_MIPI_WDR2TO1_VC_12BIT		(4)
...
// #define IMX415_RES_IS_720P(w, h)       ((w) <= 1280 && (h) <= 720)
#define IMX415_RES_IS_4K(w, h)      ((w) <= 4000 && (h) <= 3000)
#define IMX415_RES_IS_4M(w, h)      ((w) <= 2688 && (h) <= 1520)
...
*/
static AR_S32 imx415_cmos_power_on(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
	...
	ar_always("imx415_cmos_power_on  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
...
static AR_S32 imx415_cmos_power_off(VI_PIPE ViPipe,ar_dev_power_attr_t  *p_dev_power_attr)
{
	...
	ar_always("imx415_cmos_power_off  power_gpio=%d reset_gpio=%d\n",power_gpio,reset_gpio);
	//set the gpio dir to out
	ar_hal_gpio_set_dir(power_gpio,1);
	ar_hal_gpio_set_dir(reset_gpio,1);
...
static AR_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if(pstSnsState->u8ImgMode==IMX415_SENSOR_4K_30FPS_LINEAR_MODE)
    {
       pstAeSnsDft->f32MaxFps=30;
	   pstAeSnsDft->f32Fps=30;
    }else if(pstSnsState->u8ImgMode==IMX415_SENSOR_4K_60FPS_LINEAR_MODE)
    {
       pstAeSnsDft->f32MaxFps=60;
	   pstAeSnsDft->f32Fps=60;
    }
	else if (pstSnsState->u8ImgMode== SONY_IMX415_MIPI_HIGH_SPEED_12BIT)
	{
	   pstAeSnsDft->f32MaxFps=30;
	   pstAeSnsDft->f32Fps=30;	
	}
	else if (pstSnsState->u8ImgMode == SONY_IMX415_MIPI_WDR2TO1_VC_12BIT)
	{
		pstAeSnsDft->f32MaxFps=30;
	   pstAeSnsDft->f32Fps=30;
	}	
    ...
    switch (pstSnsState->enWDRMode)
    {
        case WDR_MODE_NONE:   /*linear mode*/
            ...
            pstAeSnsDft->u32MaxAgain = 0xFFFFFFFF;
            pstAeSnsDft->u32MinAgain = 0;
            pstAeSnsDft->u32MaxDgain = 0xFFFFFFFF;
            pstAeSnsDft->u32MinDgain = 0;
            pstAeSnsDft->u32MaxIntTime = 0xFFFFFFFF; // pstSnsState->u32FLStd - 2;
            pstAeSnsDft->u32MinIntTime = 1;

            pstAeSnsDft->u32LFMaxShortTime = 0xFFFFFFFF;
            break;
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32VMAX = IMX415_VMAX_4KP30_LINEAR;
    ...
    {
        case IMX415_SENSOR_4K_30FPS_LINEAR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX415_VMAX_4KP30_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
        ...
        case IMX415_SENSOR_4K_60FPS_LINEAR_MODE:
            if ((f32Fps <= pstAeSnsDft->f32MaxFps) && (f32Fps >= 0.5))
            {
                u32VMAX = IMX415_VMAX_4KP60_LINEAR * pstAeSnsDft->f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            2n + 5
            RHS1 <= FSC - BRL*2 -21
            (2 * VMAX_IMX415_1080P30_WDR - 2 * gu32BRL - 21) - (((2 * VMAX_IMX415_1080P30_WDR - 2 * 1109 - 21) - 5) %2)
        */

        g_astimx415State[ViPipe].u32RHS1_MAX = (u32VMAX - g_astimx415State[ViPipe].u32BRL) * 2 - 21;
    ...
    }else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        /*
            ...
            RHS2 <= FSC - BRL*3 -25
        */
        g_astimx415State[ViPipe].u32RHS2_MAX = u32VMAX * 4 - g_astimx415State[ViPipe].u32BRL * 3 - 25;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > 2 * IMX415_FULL_LINES_MAX_2TO1_WDR) ? 2 * IMX415_FULL_LINES_MAX_2TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;
        g_astimx415State[ViPipe].u32RHS1_MAX = pstSnsState->au32FL[0] - g_astimx415State[ViPipe].u32BRL * 2 - 21;

    }else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        u32FullLines = (u32FullLines > 4 * IMX415_FULL_LINES_MAX_3TO1_WDR) ? 4 * IMX415_FULL_LINES_MAX_3TO1_WDR : u32FullLines;
        pstSnsState->au32FL[0] = (u32FullLines >> 2) << 2;
        g_astimx415State[ViPipe].u32RHS2_MAX = pstSnsState->au32FL[0] - g_astimx415State[ViPipe].u32BRL * 3 - 25;
...
static AR_VOID cmos_inttime_update(VI_PIPE ViPipe, AR_U32 u32IntTime)
{
    ...
    }else
    {
        ...
        if(u32Value < 8)
            u32Value = 8;
        if(u32Value > (pstSnsState->au32FL[0] - 5))
            u32Value = pstSnsState->au32FL[0] - 5;
...
{
    ISP_SNS_STATE_S *pstSnsState = AR_NULL;
    AR_U32 u32HCG = g_astimx415State[ViPipe].u8Hcg;
    AR_U32 u32Tmp;
    ...
    // if (u32Again >= 21)
    // {
    //     u32HCG = u32HCG | 0x01;  // bit[0] HCG  .Reg0x3009[7:0]
    ...
    pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = (u32HCG & 0x01);
    ...
    else if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        }else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx415State[ViPipe].u32RHS1_MAX - 3)) ? (g_astimx415State[ViPipe].u32RHS1_MAX - 3) : u32IntTimeMaxTmp;
    ...
    }else if (WDR_MODE_3To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        */
        ...
        }else
        {
            ...
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
            u32RHS2_Max = ((g_astimx415State[ViPipe].u32RHS2_MAX - 8) * 0x40) / (au32Ratio[0] + 0x40);
...
static AR_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    switch (pstSnsState->enWDRMode)
    {
        default:
        case WDR_MODE_NONE:
		strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_imx415/imx415_tuning_preview.bin");
		break;
        case WDR_MODE_2To1_LINE:
        case WDR_MODE_3To1_LINE:
		strcpy(pstDef->TuningPraBinName,"/usrdata/local/factory/tunning/cam_imx415/imx415_tuning_preview_hdr.bin");
        break;
    }
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        pstDef->stWdrSwitchAttr.au32ShortOffset = 0x14;
        pstDef->stWdrSwitchAttr.au32MidOffset = 109;
        pstDef->stWdrSwitchAttr.au32LongOffset = 109;
    }
...
static AR_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, AR_BOOL bEnable)
{
    ...
    {
        if (IMX415_SENSOR_4K_30FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            u32FullLines_5Fps = (IMX415_VMAX_4KP30_LINEAR * 30) / 5;

        }else if (IMX415_SENSOR_4K_60FPS_LINEAR_MODE == pstSnsState->u8ImgMode)
        {
            // u32FullLines_5Fps = (IMX415_VMAX_4KP60_LINEAR * 60) / 5;
            return;
    ...
    //u32FullLines_5Fps = (u32FullLines_5Fps > IMX415_FULL_LINES_MAX) ? IMX415_FULL_LINES_MAX : u32FullLines_5Fps;
    u32MaxIntTime_5Fps = 4;

    if (bEnable) /* setup for ISP pixel calibration mode */
    {
        imx415_write_register (ViPipe, IMX415_GAIN_ADDR, 0x00);

        imx415_write_register (ViPipe, IMX415_VMAX_ADDR, u32FullLines_5Fps & 0xFF);
        imx415_write_register (ViPipe, IMX415_VMAX_ADDR + 1, (u32FullLines_5Fps & 0xFF00) >> 8);
        imx415_write_register (ViPipe, IMX415_VMAX_ADDR + 2, (u32FullLines_5Fps & 0xF0000) >> 16);

        imx415_write_register (ViPipe, IMX415_SHR_ADDR, u32MaxIntTime_5Fps & 0xFF);
        imx415_write_register (ViPipe, IMX415_SHR_ADDR + 1,  (u32MaxIntTime_5Fps & 0xFF00) >> 8);
        imx415_write_register (ViPipe, IMX415_SHR_ADDR + 2, (u32MaxIntTime_5Fps & 0xF0000) >> 16);

    }else /* setup for ISP 'normal mode' */
    {
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0x1FFFF) ? 0x1FFFF : pstSnsState->u32FLStd;
        imx415_write_register (ViPipe, IMX415_VMAX_ADDR, pstSnsState->u32FLStd & 0xFF);
        imx415_write_register (ViPipe, IMX415_VMAX_ADDR + 1, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        imx415_write_register (ViPipe, IMX415_VMAX_ADDR + 2, (pstSnsState->u32FLStd & 0xF0000) >> 16);
...
static AR_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((AR_FALSE == pstSnsState->bSyncInit) || (AR_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX415_SHR_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX415_SHR_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX415_SHR_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //make shutter and gain effective at the same time
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX415_GAIN_ADDR;  //gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX415_GAIN_ADDR + 1;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX415_VMAX_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX415_VMAX_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX415_VMAX_ADDR + 2;
    ...
    if (WDR_MODE_NONE == pstSnsState->enWDRMode)
    {
        if(pstSnsRegsInfo->bConfig) imx415_default_reg_init(ViPipe);
    }
    return AR_SUCCESS;
}
...
static AR_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30)
    {
        ...
        {          
            ...
            if(IMX415_RES_IS_4M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
			{
                 ...
                 u8SensorImageMode = SONY_IMX415_MIPI_HIGH_SPEED_12BIT;
                 pstSnsState->u32FLStd = IMX415_VMAX_4KP30_LINEAR;
			}
			else if (IMX415_RES_IS_4K(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
			{
				 ISP_TRACE(AR_DBG_ERR, "u8SensorImageMode:%d\n",u8SensorImageMode);
                 u8SensorImageMode = IMX415_SENSOR_4K_30FPS_LINEAR_MODE;
                 pstSnsState->u32FLStd = IMX415_VMAX_4KP30_LINEAR;
			}	
			else
    ...
    {
        if (IMX415_RES_IS_4K(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
        {
            ISP_TRACE(AR_DBG_ERR, "u8SensorImageMode:%d\n",u8SensorImageMode);
            u8SensorImageMode = IMX415_SENSOR_4K_60FPS_LINEAR_MODE;
            pstSnsState->u32FLStd = IMX415_VMAX_4KP60_LINEAR;
            // g_astimx415State[ViPipe].u8Hcg = 0x2;
...
static AR_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = AR_FALSE;
    pstSnsState->u8ImgMode = IMX415_SENSOR_4K_30FPS_LINEAR_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd = IMX415_VMAX_4KP30_LINEAR;
    pstSnsState->au32FL[0] = IMX415_VMAX_4KP30_LINEAR;
    pstSnsState->au32FL[1] = IMX415_VMAX_4KP30_LINEAR;

    memset(&pstSnsState->astRegsInfo[0], 0, sizeof(ISP_SNS_REGS_INFO_S));
    memset(&pstSnsState->astRegsInfo[1], 0, sizeof(ISP_SNS_REGS_INFO_S));
}

static AR_S32 imx415_cmos_set_mirror_flip(VI_PIPE ViPipe, int mode)
{
    AR_S32 ret = 0;
    ...
    switch (mode)
    {
        case ISP_SNS_NORMAL:
        {
            ret = imx415_flip_off_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR:
        {
            ret = imx415_flip_off_mirro_on(ViPipe);
            break;
        }
        case ISP_SNS_FLIP:
        {
            ret = imx415_flip_on_mirro_off(ViPipe);
            break;
        }
        case ISP_SNS_MIRROR_FLIP:
        {
            ret = imx415_flip_on_mirro_on(ViPipe);
            break;
        }
...
static AR_S32 imx415_cmos_sensor_ctl(VI_PIPE ViPipe, ISP_CMOS_SENSOR_CTL *pSensorCtl)
{
    if (!pSensorCtl)
    {
    ...
    switch(u8CtlCode)
    {
        ...
        {
            ret = imx415_cmos_set_mirror_flip(ViPipe, mode);
            break;
        }
        default:
...
static AR_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sns_trigger = imx415_trigger;	
    ...
    pstSensorExpFunc->pfn_cmos_sns_power_on = imx415_cmos_power_on;
    pstSensorExpFunc->pfn_cmos_sns_power_off = imx415_cmos_power_off;
    pstSensorExpFunc->pfn_cmos_sns_ctl = imx415_cmos_sensor_ctl;
...
static AR_S32 imx415_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static AR_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = AR_MPI_ISP_SensorUnRegCallBack(ViPipe, IMX415_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, IMX415_ID);
    if (AR_SUCCESS != s32Ret)
    {
    ...
    s32Ret = AR_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, IMX415_ID);
    if (AR_SUCCESS != s32Ret)
    {
