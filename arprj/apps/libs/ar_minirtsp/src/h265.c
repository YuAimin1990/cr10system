...
static int h265_parse_slice(uint8_t *p_buffer, int len)
{
...
static const uint8_t *h265_find_startcode_internal(const uint8_t *p, const uint8_t *end)
{
...
{
	const uint8_t *out = h265_find_startcode_internal(p, end);
...
int h265_get_frame_type(uint8_t *buf, int len)
{
	int i;
	uint32_t code = -1;
	...
	for (i = 0; i < len; i++) {
        ...
        if ((code & 0xffffff00) == 0x100) {
			...
			if (type == 0x13) {
				return h265_parse_slice(buf + i, 10);
			}
		}
	}
	return 0;
...
ssize_t h265_get_vps(uint8_t *src, size_t src_len, uint8_t *vps, size_t vps_len)
{
	if (!src || !src_len || !vps || vps_len < 32)
		return -1;
	...
	while (r < end) {
		...
		int type = (r[0] & 0x7E)>>1;
		if (type == 0x20) {	/* vps */
			memcpy(vps, start_code, sizeof(start_code));

			int size = min(r1 - r, vps_len - sizeof(start_code));
			memcpy(vps + sizeof(start_code), r, size);
...
ssize_t h265_get_sps(uint8_t *src, size_t src_len, uint8_t *sps, size_t sps_len)
{
...
ssize_t h265_get_pps(uint8_t *src, size_t src_len, uint8_t *pps, size_t pps_len)
{
...
int h265_sps_get_dimension(uint8_t *sps, size_t size, int *width, int *height)
{
	decoder_t dec;
