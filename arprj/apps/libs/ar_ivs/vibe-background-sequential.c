...
vibeModel_Sequential_t *libvibeModel_Sequential_New()
{
  ...
  model->numberOfSamples         = NUM_SAMPLES;
  model->matchingThreshold       = MATCHING_THREADS;
  model->matchingNumber          = MATCHING_NUMBER;
  model->updateFactor            = UPDATE_FACTOR;
...
int32_t libvibeModel_Sequential_Free(vibeModel_Sequential_t *model)
{
  ...
  if (model->historyBuffer) {
  	free(model->historyBuffer);
  }
  if (model->jump) {
  	free(model->jump);
  }
  if (model->historyImage) {
...
int32_t libvibeModel_Sequential_Segmentation_8u_C1R(
  ...
  uint8_t* pabsvalue = (uint8_t*)malloc(width * height*sizeof(uint8_t));
  if(!pabsvalue)
     return -1;
  memset(pabsvalue, 0, width * height);
  
  for (int index = width * height - 1; index >= 0; --index) {
    pabsvalue[index]=abs_uint(image_data[index] - historyImage[index]);   
  }

  for (int index = width * height - 1; index >= 0; --index) {
    if (pabsvalue[index] > matchingThreshold)
      segmentation_map[index] = matchingNumber;
  }

  /* Next historyImages. */
  uint8_t* psegvalue = (uint8_t*)malloc(width * height*sizeof(uint8_t));
  if(!psegvalue){
  	 free(pabsvalue);
     return -1;
  }
  memset(psegvalue, 0, width * height);
  
  for (int i = 1; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
    uint8_t *pels = historyImage + i * width * height;

    for (int index = width * height - 1; index >= 0; --index) {
        pabsvalue[index]=abs_uint(image_data[index] - pels[index]); 
	    psegvalue[index] = segmentation_map[index]-1;
    }

	for (int index = width * height - 1; index >= 0; --index) {
        if (pabsvalue[index] <= matchingThreshold)
          segmentation_map[index]=psegvalue[index];
    }
  }
  ...
  for (int index = 0; index <= width * height-1; index++) {
	 psegvalue[index] = segmentation_map[index];
  }

  for (int index = width * height - 1; index >= 0; --index) {
    if (psegvalue[index] > 0) {
  ...
  for (y = 1; y < height - 1; ++y) {
    ...
    while (indX < width - 1) {
      ...
      if (updating_mask[index] == COLOR_BACKGROUND) {
		...
		int tmp = position[shift] * width * height;
        if (position[shift] < NUMBER_OF_HISTORY_IMAGES) {
          historyImage[index + tmp] = value;
          historyImage[index_neighbor + tmp] = value;
