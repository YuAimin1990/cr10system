...
struct _fbg *fbg_dispmanxSetup(uint32_t displayNumber, VC_IMAGE_TYPE_T image_type) {
    ...
    struct _fbg *fbg = fbg_customSetup(info.width, info.height, components, 1, 1, (void *)dispmanx_context, fbg_dispmanxDraw, fbg_dispmanxFlip, NULL, fbg_dispmanxFree, NULL);
    if (!fbg) {
