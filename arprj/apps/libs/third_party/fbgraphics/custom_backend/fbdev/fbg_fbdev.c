...
void fbg_fbdevFlush(struct _fbg *fbg, int line_offset, int line_num );

struct _fbg *fbg_fbdevSetup(char *user_fb_device, int page_flipping, int one_framebuffer) {
    ...
    struct _fbg *fbg = fbg_customSetup(fbdev_context->vinfo.xres, fbdev_context->vinfo.yres, components, 0, 0, (void *)fbdev_context, fbg_fbdevDraw, fbg_fbdevFlip, NULL, fbg_fbdevFree, fbg_fbdevFlush);
    if (!fbg) {
    ...
    fbdev_context->one_framebuffer = one_framebuffer;
    // setup page flipping
    if (fbdev_context->page_flipping) {
        fbg->disp_buffer = fbdev_context->buffer;
        fbg->back_buffer = fbdev_context->buffer + fbg->width * fbg->components * fbg->height;
    } else if(fbdev_context->one_framebuffer) {
        fbg->back_buffer = fbdev_context->buffer;
        fbg->disp_buffer = fbdev_context->buffer;
    } else {
...
void fbg_fbdevFree(struct _fbg *fbg) {
    ...
    if ((!fbdev_context->one_framebuffer)
       &&(!fbdev_context->page_flipping)) {
        fbg->back_buffer = fbdev_context->buffer;
        free(fbg->back_buffer);
...
void fbg_fbdevFlush(struct _fbg *fbg, int line_offset, int line_num ) {
   arfb_flush_zone_t flush_zone = {0};
   struct _fbg_fbdev_context *fbdev_context = fbg->user_context;

   flush_zone.line_offset = line_offset;
   flush_zone.line_num = line_num;
   if (ioctl(fbdev_context->fd, AR_FRAMEBUFFER_IO_CMD_FLUSH_ZONE, &flush_zone) == -1) {
        fprintf(stderr, "fbg_fbdevFlush: AR_FRAMEBUFFER_IO_CMD_FLUSH_ZONE failed!\n");
   }
