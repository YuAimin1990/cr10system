...
{
	if(!dst || !src0 || !src1 || !srcSize)
		return -1;

	unsigned char* end = dst+srcSize;
	while(dst<end)
	{
		*dst++ = alaw_encode(*src0++);
		*dst++ = alaw_encode(*src1++);
	}
...
unsigned int AR_G711_Ulaw_Stereo_Encode(unsigned char *dst, short *src0, short *src1, unsigned int srcSize)
{
	...
	while(dst<end)
	{
		*dst++ = ulaw_encode(*src0++);
		*dst++ = ulaw_encode(*src1++);
	}
...
{
	short *dst_l = dst;
	short *dst_r = dst + (srcSize>>1);
	short* end = dst_r;
	while(dst_l<end)
	{
		*dst_l++ = alaw_decode(*src++);
		*dst_r++ = alaw_decode(*src++);
	}
...
unsigned int AR_G711_Stereo_UlawDecode(short *dst, const unsigned char *src, unsigned int srcSize)
{
	...
	while(dst_l<end)
	{
		*dst_l++ = ulaw_decode(*src++);
		*dst_r++ = ulaw_decode(*src++);
	}
