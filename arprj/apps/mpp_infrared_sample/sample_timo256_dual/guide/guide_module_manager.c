...
static int guide_ita_sdk_init(int w, int h)
{
	...
	registry.I2COpen = arI2COpen;
	registry.I2CClose = arI2CClose;
	registry.I2CRead = arI2CRead;
	registry.I2CWrite = arI2CWrite;
	...
	#endif
	result = ITA_ConfigureDetector(instance, 12000000);
	if (result < ITA_OK)
	{
	    printf("ita failed 0\n");
	    return -1;
...
int guide_init()
{
    return guide_ita_sdk_init(256, 192);
}
