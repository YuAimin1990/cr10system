...
AR_S32 SAMPLE_COMM_VENC_MemConfig(AR_VOID)
{
    ...
    for (i = 0; i < 64; i++)
    {
        ...
        {
            SAMPLE_PRT("AR_MPI_SYS_SetMemConfig with %#x!\n", s32Ret);
            return AR_FAILURE;
        }
    }
    return AR_SUCCESS;
...
******************************************************************************/
AR_S32 SAMPLE_COMM_VENC_GetFilePostfix(PAYLOAD_TYPE_E enPayload, char* szFilePostfix)
{
...
AR_S32 SAMPLE_COMM_VENC_Getdcfinfo(char* SrcJpgPath, char* DstThmPath)
{
...
******************************************************************************/
AR_S32 SAMPLE_COMM_VENC_SaveStream(FILE* pFd, VENC_STREAM_S* pstStream)
{
    AR_S32 i;
...
******************************************************************************/
AR_S32 SAMPLE_COMM_VENC_SaveStream_PhyAddr(FILE* pFd, VENC_STREAM_BUF_INFO_S *pstStreamBuf, VENC_STREAM_S* pstStream)
{
    AR_U32 i,j;
...
AR_S32 SAMPLE_COMM_VENC_Creat(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode, AR_U32  u32Profile, VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    ******************************************/
    ...
    stVencChnAttr.stVencAttr.bByFrame        = AR_TRUE;/*get stream mode is slice mode or frame mode?*/
    ...
    switch (enType)
    {
        case PT_H265:
        {
            ...
            stVencChnAttr.stVencAttr.stAttrH265e.bRcnRefShareBuf = AR_FALSE;
        }
        break;
        case PT_H264:
        {
            ...
            stVencChnAttr.stVencAttr.stAttrH264e.bRcnRefShareBuf = AR_FALSE;
        }
        break;
        case PT_MJPEG:
        {
    ...
    // For a channel which needs to call create and destroy many times,
    // it's better to sleep 1s before call AR_MPI_VENC_CreateChn or after
    // AR_MPI_VENC_DestroyChn.
    sleep(1);
    s32Ret = AR_MPI_VENC_CreateChn(VencChn, &stVencChnAttr);
    if (AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VENC_CreateChn [%d] faild with %#x! ===\n", \
                   VencChn, s32Ret);
        return s32Ret;
    }
...
AR_S32 SAMPLE_COMM_VENC_Start(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode, AR_U32  u32Profile, VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    ******************************************/
    ...
    if (AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VENC_StartRecvPic faild with%#x! \n", s32Ret);
        return AR_FAILURE;
    }
    return AR_SUCCESS;
}
...
AR_S32 SAMPLE_COMM_VENC_Stop(VENC_CHN VencChn)
{
    ...
    ******************************************/
    ...
    if (AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VENC_StopRecvPic vechn[%d] failed with %#x!\n", \
                   VencChn, s32Ret);
        return AR_FAILURE;
    }    /******************************************
    ...
    ******************************************/
    ...
    if (AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VENC_DestroyChn vechn[%d] failed with %#x!\n", \
                   VencChn, s32Ret);
        return AR_FAILURE;
    }
    return AR_SUCCESS;
...
AR_S32 SAMPLE_COMM_VENC_SnapProcess(VENC_CHN VencChn, AR_U32 SnapCnt, AR_BOOL bSaveJpg, AR_BOOL bSaveThm)
{
    ...
    ******************************************/
    ...
    if (s32VencFd < 0)
    {
        SAMPLE_PRT("AR_MPI_VENC_GetFd faild with%#x!\n", s32VencFd);
        return AR_FAILURE;
    }
    ...
    for(i=0; i<SnapCnt; i++)
    {
        ...
        else
        {
            if (FD_ISSET(s32VencFd, &read_fds))
            {
                ...
                {
                    SAMPLE_PRT("AR_MPI_VENC_QueryStatus failed with %#x!\n", s32Ret);
                    return AR_FAILURE;
                }
                /*******************************************************
                ...
                if (AR_SUCCESS != s32Ret)
                {
                    SAMPLE_PRT("AR_MPI_VENC_GetStream failed with %#x!\n", s32Ret);
                ...
                if (AR_SUCCESS != s32Ret)
                {
                    SAMPLE_PRT("AR_MPI_VENC_ReleaseStream failed with %#x!\n", s32Ret);
    ...
    ******************************************/
    ...
    if (s32Ret != AR_SUCCESS)
    {
        SAMPLE_PRT("AR_MPI_VENC_StopRecvPic failed with %#x!\n",  s32Ret);
        return AR_FAILURE;
    }
    return AR_SUCCESS;
}
...
AR_VOID* SAMPLE_COMM_QpmapSendFrameProc(AR_VOID* p)
{
    AR_U32 i,j,VeChnCnt;
    AR_S32 s32Ret;
    ...
    if(AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_SYS_MmzAlloc err:0x%x",s32Ret);
        return NULL;
    }
    ...
    if(AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VPSS_GetChnAttr err:0x%x",s32Ret);
    ...
    if(AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_VPSS_SetChnAttr err:0x%x",s32Ret);
    ...
    while(AR_TRUE == pstPara->bThreadStart)
    {
        ...
        if(AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VPSS_GetChnFrame err:0x%x\n",s32Ret);
            continue;
        }
        ...
        for(VeChnCnt=0; VeChnCnt<pstPara->s32Cnt; VeChnCnt++)
        {
            ...
            if(AR_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("AR_MPI_VENC_SendFrame err:0x%x\n",s32Ret);
                break;
            }
        }
        if(AR_SUCCESS != s32Ret)
        {
            ...
            if(AR_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("AR_MPI_VPSS_ReleaseChnFrame err:0x%x",s32Ret);
                goto err_out;
            }
            continue;
        }
...
err_out:
    ...
    if(AR_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("AR_MPI_SYS_MmzFree err:0x%x",s32Ret);
        return NULL;
    }
...
AR_S32 SAMPLE_COMM_VENC_QpmapSendFrame(VPSS_GRP VpssGrp,VPSS_CHN VpssChn,VENC_CHN VeChn[],AR_S32 s32Cnt,SIZE_S stSize)
{
    ...
    return pthread_create(&gs_VencQpmapPid, 0, SAMPLE_COMM_QpmapSendFrameProc, (AR_VOID*)&stQpMapSendFramePara);
}
...
******************************************************************************/
AR_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(AR_VOID* p)
{
    AR_S32 i;
    AR_S32 s32ChnTotal;
    VENC_CHN_ATTR_S stVencChnAttr;
    ...
    struct timeval TimeoutVal;
    fd_set read_fds;
    AR_U32 u32PictureCnt[VENC_MAX_CHN_NUM]={0};
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        if (s32Ret != AR_SUCCESS)
        {
            SAMPLE_PRT("AR_MPI_VENC_GetChnAttr chn[%d] failed with %#x!\n", \
                       VencChn, s32Ret);
            return NULL;
        }
        ...
        if (VencFd[i] < 0)
        {
            SAMPLE_PRT("AR_MPI_VENC_GetFd failed with %#x!\n",
                       VencFd[i]);
            return NULL;
        }
        if (maxfd <= VencFd[i])
        ...
        if (AR_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("AR_MPI_VENC_GetStreamBufInfo failed with %#x!\n", s32Ret);
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if (AR_SUCCESS != s32Ret)
                    {
                        SAMPLE_PRT("AR_MPI_VENC_QueryStatus chn[%d] failed with %#x!\n", i, s32Ret);
                        break;
                    }
                    ...
                    *******************************************************/
                    ...
                    if (AR_SUCCESS != s32Ret)
                    {
                        SAMPLE_PRT("AR_MPI_VENC_ReleaseStream failed!\n");
                        free(stStream.pstPack);
                        stStream.pstPack = NULL;
                        break;
                    }
...
******************************************************************************/
AR_VOID* SAMPLE_COMM_VENC_GetVencStreamProc_Svc_t(void* p)
{
    AR_S32 i = 0;
    AR_S32 s32Cnt = 0;
    AR_S32 s32ChnTotal;
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if (AR_SUCCESS != s32Ret)
                    {
                        SAMPLE_PRT("AR_MPI_VENC_Query chn[%d] failed with %#x!\n", i, s32Ret);
                        break;
                    }
                    /*******************************************************
                    step 2.2 :suggest to check both u32CurPacks and u32LeftStreamFrames at the same time,for example:
...
AR_S32 SAMPLE_COMM_VENC_StartGetStream(VENC_CHN VeChn[],AR_S32 s32Cnt)
{
    ...
    gs_stPara.bThreadStart = AR_TRUE;
    gs_stPara.s32Cnt = s32Cnt;
    for(i=0; i<s32Cnt; i++)
    {
        gs_stPara.VeChn[i] = VeChn[i];
    }
    return pthread_create(&gs_VencPid, 0, SAMPLE_COMM_VENC_GetVencStreamProc, (AR_VOID*)&gs_stPara);
}
...
AR_S32 SAMPLE_COMM_VENC_StartGetStream_Svc_t(AR_S32 s32Cnt)
{
    gs_stPara.bThreadStart = AR_TRUE;
    gs_stPara.s32Cnt = s32Cnt;
    return pthread_create(&gs_VencPid, 0, SAMPLE_COMM_VENC_GetVencStreamProc_Svc_t, (AR_VOID*)&gs_stPara);
}
...
{
    if (AR_TRUE == gs_stPara.bThreadStart)
    {
        gs_stPara.bThreadStart = AR_FALSE;
...
AR_VOID SAMPLE_COMM_VENC_ReadOneFrame( FILE* fp, AR_U8* pY, AR_U8* pU, AR_U8* pV,
...
AR_S32 SAMPLE_COMM_VENC_PlanToSemi(AR_U8* pY, AR_S32 yStride,
