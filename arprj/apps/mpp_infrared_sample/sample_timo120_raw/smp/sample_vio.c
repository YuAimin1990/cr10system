...
static int IRAW_W = 120*2;
static int IRAW_H = 92;
static int IYUV_W = 192;
static int IYUV_H = 144;
static int gIYUV_ACT_W = 120;
static int gIYUV_ACT_H = 90;
...
typedef struct
{
    ...
    ar_lock_t lock;
    short *y16data;
}STRU_TEMP_MEASURE_PARAM_T;


static STRU_IR_RAW_PROCESS_PARAM_T g_RawProcessParam = {-1};
static INF_ALGO_OBJECT_S*           g_pstInfAlgObject = NULL;
static STRU_TEMP_MEASURE_PARAM_T   g_stTempMeasureParam = {0};

static int save_y16 = 0, save_lms = 0, save_tp = 0, save_raw = 0;
...
static void *debug_update_thread(void* arg)
{
    ...
    while(1)
    {
        ...
        if((access("/tmp/dump",F_OK))!=-1)
        {
            remove("/tmp/dump");
            printf("will dump..\n");
            save_y16 = save_lms = save_tp = save_raw = 1;
        }
...
static void *temperature_measure_thread(void* arg)
{
    //测温点
    int iPosX = 60;
    int iPosY = 45;
    ...
    {
        sleep(3);
        ar_lock(g_stTempMeasureParam.lock);
        iPosY16 = g_stTempMeasureParam.y16data[120*iPosY + iPosX];
	    ar_unlock(g_stTempMeasureParam.lock);
	    float fSurfaceT = 0;
	    guide_MeasureTempPoint(iPosY16 , &fSurfaceT);
	    //printf("temp: %d %.2f\n",iPosY16,fSurfaceT);
    }
    return 0;
}
...
int init_inf_alg_obj(int set_width, int set_height)
{
    ...
    INF_ALGO_ALL_PARAM_S stInfAlgParams = {
        .stGtmParam = {
            ...
            .target_mean_value = 260,
            .contrastBaseUp = 2,
            .contrastBaseDown = 0.7,
            .compensate1 = 100,
            .compensate2 = 600,
        },
        .stLmsParam = {
...
int onGetGdSdkY16Data(short *data,int len)
{
	...
	AR_S32 s32Ret;
	STRU_TIME_STATISTICS_T gTs;

	ar_lock(g_stTempMeasureParam.lock);
	memcpy(g_stTempMeasureParam.y16data,data,len*2);
	ar_unlock(g_stTempMeasureParam.lock);
    ...
    //memset(p_vaddr_align, 0, 512*IYUV_H);
    
    if(isDoLms){
        time_statistics_begin(&gTs);
        g_pstInfAlgObject->lms_process(g_pstInfAlgObject,data,data,-1);
        time_statistics_end(&gTs, isPrintTimeLog, "lms");
        if(save_lms){
            save_lms = 0;
    		dump_to_file("/tmp/lms.raw",data,len*2);
        }
    }

    unsigned char* tmp_gtm_buf = (unsigned char*)malloc(240*90);

    time_statistics_begin(&gTs);
    g_pstInfAlgObject->gtm_process(g_pstInfAlgObject,data,tmp_gtm_buf,-1);
    time_statistics_end(&gTs, isPrintTimeLog, "gtm");
    ...
    AR_U32 u32IspInRawStride = 512; //256 align of IYUV_W*2
#if 1
	CvMat Msrc;
    cvInitMatHeader(&Msrc, 90, 120, CV_16UC1, tmp_gtm_buf, CV_AUTOSTEP);
    CvMat *pMdst = cvCreateMat( IYUV_H, IYUV_W, CV_16UC1);
    cvResize( &Msrc, pMdst, CV_INTER_LINEAR);
    for(int i = 0;i<144;i++)
    {
        memcpy(p_vaddr_align+i*u32IspInRawStride, pMdst->data.ptr+i*IYUV_W*2, IYUV_W*2);
    }
    cvReleaseMat(&pMdst);
#else
	for(int i = 0;i<90;i++)
    {
        memcpy(p_vaddr_align+i*u32IspInRawStride, tmp_gtm_buf+i*240, 240);
    }
#endif
    free(tmp_gtm_buf);
	...
	pstVideoFrame->stVFrame.u32Stride[0]=u32IspInRawStride;;
...
AR_S32 SAMPLE_timo120_test(AR_S32 outMode)
{
    AR_S32             s32Ret;
    ...
    
    {
        ...
        g_enSnsType[s32WorkSnsId] = TIMO120_DVP_120x90_IR;
    	...
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.enSnsType = TIMO120_DVP_120x90_IR;
    ...
    VI_CHN			   ViChn_isp    	  = 0;
    {
        ...
        g_enSnsType[s32WorkSnsId] = ISP_VIN_COMMON_INF_120x90;
        
        stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32SnsId = s32WorkSnsId;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32BusId = 0;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.MipiDev  = 0;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.enSnsType = ISP_VIN_COMMON_INF_120x90;
    ...
    VI_CHN			   ViChn_isp1    	  = bNeedViCrop?1:0;
    {
        DYNAMIC_RANGE_E    enDynamicRange = DYNAMIC_RANGE_SDR8;
        PIXEL_FORMAT_E     enPixFormat    = PIXEL_FORMAT_YVU_PLANAR_420;
        VIDEO_FORMAT_E     enVideoFormat  = VIDEO_FORMAT_LINEAR;
        ...
        g_enSnsType[s32WorkSnsId] = ISP_VIN_COMMON_INF1_120x90;
        
        stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32SnsId = s32WorkSnsId;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32BusId = 0;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.MipiDev  = 0;
    	stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.enSnsType = ISP_VIN_COMMON_INF1_120x90;
	    ...
	    //if(bNeedViCrop){
	    //    stViConfig.astViInfo[s32WorkSnsId].stChnInfo.u32Width = gIYUV_ACT_W;
	    //    stViConfig.astViInfo[s32WorkSnsId].stChnInfo.u32Height = gIYUV_ACT_H;
	    //}
    }
    ...
    if(bNeedViCrop)
    {
        ...
        stCropInfo.stCropRect.u32Width = gIYUV_ACT_W;
        stCropInfo.stCropRect.u32Height = gIYUV_ACT_H;
        int ret2 = AR_MPI_VI_SetChnCrop(ViPipe_isp1, ViChn_isp1, &stCropInfo);
        printf("vi set channel crop ret=%d %dx%d \n",ret2, gIYUV_ACT_W, gIYUV_ACT_H);
    }
    ...
    VPSS_CHN VpssChn_inf0 = 0;
    {
        ...
        SIZE_S GrpOutSize = {gIYUV_ACT_W,gIYUV_ACT_H};
        SIZE_S ChnOutSize = {gIYUV_ACT_W,gIYUV_ACT_H};
    ...
    if(s32OutMode == 1 || s32OutMode == 2)
    {
        ...
        *************************************************/
        ...
        AR_U32 u32Width = 256;//gIYUV_ACT_W; //min width is 256
        AR_U32 u32Height = gIYUV_ACT_H;
    ...
    s32Ret = init_inf_alg_obj(120,90);
    if(s32Ret!=0){
    ...
    g_stTempMeasureParam.lock = ar_creat_lock();
    g_stTempMeasureParam.y16data = (short*)malloc(256*192*2);
    s32Ret = pthread_create(&g_stTempMeasureParam.thd, NULL, temperature_measure_thread, NULL);

    ITA_RESULT enItaResult;
    ITAISPResult stITAISPResult;
    ...
    while(1)
	{
        ...
        for(i = 0;i<92;i++)
        {
            memcpy(tmp_buf.pViraddr+i*240,(void*)pstFrameInfo->stVFrame.u64VirAddr[0]+i*256,240);
        }

        time_statistics_begin(&gTSTmp);
        enItaResult = guide_push_raw_data((unsigned char *)(tmp_buf.pViraddr+240*2), IRAW_W*90, &stITAISPResult);
        if (ITA_OK == enItaResult)
		{
		    onGetGdSdkY16Data(stITAISPResult.y16Data, stITAISPResult.info.y16Len);
		    time_statistics_end(&gTSTmp, isPrintTimeLog, "guide push raw");
		}
