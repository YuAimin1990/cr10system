...
static void *debug_update_thread(void* arg)
{
    ...
    while(1)
    {
        ...
        if((access("/tmp/bright_add",F_OK))!=-1)
        {
            remove("/tmp/bright_add");
            printf("will add bright..\n");
            int v = 0;
            g_pstInfAlgObject->image_control_get(g_pstInfAlgObject, ALG_IMG_CTL_BRIGHT, &v);
            printf("get v = %d\n", v);
            v = v+10;
            g_pstInfAlgObject->image_control_set(g_pstInfAlgObject, ALG_IMG_CTL_BRIGHT, v);
            printf("set v = %d\n", v);
        }
        if((access("/tmp/bright_sub",F_OK))!=-1)
        {
            remove("/tmp/bright_sub");
            printf("will sub bright..\n");
            int v = 0;
            g_pstInfAlgObject->image_control_get(g_pstInfAlgObject, ALG_IMG_CTL_BRIGHT, &v);
            printf("get v = %d\n", v);
            v = v-10;
        ...
        if((access("/tmp/contrast_add",F_OK))!=-1)
        {
            remove("/tmp/contrast_add");
            printf("will add contrast..\n");
            int v = 0;
            g_pstInfAlgObject->image_control_get(g_pstInfAlgObject, ALG_IMG_CTL_CONTRAST, &v);
            printf("get v = %d\n", v);
            v = v+10;
            g_pstInfAlgObject->image_control_set(g_pstInfAlgObject, ALG_IMG_CTL_CONTRAST, v);
            printf("set v = %d\n", v);
        }
        if((access("/tmp/contrast_sub",F_OK))!=-1)
        {
            remove("/tmp/contrast_sub");
            printf("will sub contrast..\n");
            int v = 0;
            g_pstInfAlgObject->image_control_get(g_pstInfAlgObject, ALG_IMG_CTL_CONTRAST, &v);
            printf("get v = %d\n", v);
            v = v-10;
...
AR_S32 SAMPLE_elf1_test(AR_S32 outMode)
{
    AR_S32             s32Ret;
    ...
    if(s32OutMode == 1 || s32OutMode == 2)
    {
        ...
        *************************************************/
        ...
        s32Ret = AR_MPI_VENC_GetRcParam(VencChn_id, &stRcParam);
        if (s32Ret) {
            printf("AR_MPI_VENC_GetRcParam failed\n");
            goto EXIT2;
        }
        stRcParam.stParamH265Cbr.u32MaxIQp = 50;
        stRcParam.stParamH265Cbr.u32MinIQp = 1;
        s32Ret = AR_MPI_VENC_SetRcParam(VencChn_id, &stRcParam);
        if (s32Ret) {
            printf("AR_MPI_VENC_SetRcParam failed\n");
            goto EXIT2;
        }
    ...
    while(1)
	{
        ...
        if(save_raw){
		    save_raw = 0;
            printf("will save raw, info: %u %u %u %u %u %p %p %p valid: 256*204\n",
