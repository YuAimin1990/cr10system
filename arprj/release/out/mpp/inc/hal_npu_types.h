...
#undef  MAX_NAME_LEN    // Undef the redundant macro MAX_NAME_LEN in ar_mpp_irq.h
...
#define MAX_INPUT_IMG_NUM 64
#define MAX_BATCH_IMG_NUM 64
...
typedef struct
{
    ...
    AR_CHAR  achTensorName[MAX_NAME_LEN];
    AR_IMG_S astBatchImg[MAX_BATCH_IMG_NUM];
} AR_INPUT_IMG_S;
...
typedef struct
{
    ...
    AR_CHAR achType[MAX_NAME_LEN]; //options: float/int16/int8/uint8
    AR_U32 u32Num;
    ...
    AR_U32 u32Width;
    AR_S32 s32ZeroPoint;
	AR_CHAR achLayoutType[MAX_NAME_LEN]; //options: integer/float
} AR_NPU_TENSOR_S;

typedef struct
{
	AR_CHAR achMemoryType[MAX_NAME_LEN];
	AR_CHAR achDdrFormat[MAX_NAME_LEN];
} AR_NPU_TENSOR_EXT_S;
...
typedef struct
{
    AR_NPU_TENSOR_EXT_S astInputTensorExt[8]; //tmp 8
    AR_NPU_TENSOR_EXT_S astOutputTensorExt[8];
