...
insmod /mod/cryptodev.ko
#insmod /mod/ar_dsp.ko
#insmod /mod/ar_efuse.ko
# insmod /mod/GT1X.ko

# mount backup partition to /backup
mkdir -p /backup
if [ -e /dev/mmcblk0p10 ]; then
    mount -t ext4 /dev/mmcblk0p10 /backup
    if [ $? -ne 0 ]; then
        /etc/mkfs.ext4 /dev/mmcblk0p10
        mount -t ext4 /dev/mmcblk0p10 /backup
    fi
fi

# mount storage partition to /data
mkdir -p /data
if [ -e /dev/mmcblk0p11 ]; then
    mount -t ext4 /dev/mmcblk0p11 /data
    if [ $? -ne 0 ]; then
        /etc/mkfs.ext4 /dev/mmcblk0p11
        mount -t ext4 /dev/mmcblk0p11 /data
    fi
fi
...
if [ -e "/usrdata/tools/dm_crypt.sh" ]; then
    cd /usrdata/tools/
    ./dm_crypt.sh &
fi
