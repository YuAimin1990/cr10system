...
def get_args():
	...
	parser.add_argument('--mpp_service', default="app", help='mpp_service type')
...
def copy_all_components(conf, output, boot_media, base_board, build_type, app, chip, mpp_service):
	...
	if build_type == "release":
		__build_type = "debug"    #Not strip debug info in compiling. Will handle 'strip' in later step.

	if mpp_service == "lib":
		cmd = "make BOOT_MEDIA=" + boot_media + " BASE_BOARD=" + base_board + " BUILD_TYPE=" + __build_type + " APP=" + app + " CHIP=" + chip + " MPP_SERVICE=lib all"
	else:
		cmd = "make BOOT_MEDIA=" + boot_media + " BASE_BOARD=" + base_board + " BUILD_TYPE=" + __build_type + " APP=" + app + " CHIP=" + chip + " all"
	run_os_command(cmd)
	...
	copy_sdk_components_after_build(conf, output, chip, build_type)

	md5_generate = ('python2 ./hosts/utility/bin/md5_cal.py ' + chip)
	os.system(md5_generate)
...
def copy_projects(conf, output, chip):
	projects_exclude = conf.get("projects", "exclude_path").split()

	projects_path    = "configs/" + chip + "/"
	projects         = os.listdir(projects_path)

	for project in projects_exclude:
...
def copy_sdk_components_before_build(conf, output, chip):
	if not conf.has_option("components", "base_components"):
		...
		run_os_command(cmd)
		cmd = "cp ../arsdk/Feature.param " + base_output_path + "/arsdk"
		run_os_command(cmd)
...
def copy_sdk_components(components, output):
	...
	for component in components.split():
		...
		elif component.find("app-excludemode") >= 0:
			cp_dir("./apps", prj_path)
			cmd = "rm -f " + prj_path + "/apps/.gitignore"
			run_os_command(cmd)

		elif component.find("app-exclude-") >= 0:
			cmd = "rm -rf " + prj_path + "apps/" + component.partition("app-exclude-")[2]
			run_os_command(cmd)

		elif component.find("kmod-excludemode") >= 0:
			src = platforms_real_path + "kmodule/"
			dst = sdk_path + "platforms/"
			cp_dir(src, dst)
			cmd = "rm -rf " + sdk_path + "platforms/kmodule/module_config.in " + sdk_path + "platforms/kmodule/module_config"
		...
		elif component.find("kmod-exclude-") >= 0:
			cmd = "rm -rf " + sdk_path + "platforms/kmodule/" + component.partition("kmod-exclude-")[2]
			run_os_command(cmd)
...
def copy_sdk_components_after_build(conf, output, chip, build_type):
	if not conf.has_option("components", "base_components"):
		print("components has't base_components option")
	else:
		base_components = conf.get("components", "base_components")
	...
	add_output_path = output + "add_on"
	#1. copy output
	#### Record all .so files with debug info, to ./debug directory.
	if build_type == "release":
		cmd = "rm -rf ./debug && mkdir -p debug"
		run_os_command(cmd)
		cmd = "find ./release -name *.so* | xargs -i cp --parent {} ./debug"
		run_os_command(cmd)
		cmd = "mv ./debug/release/* ./debug/ && rm -r ./debug/release/"
		run_os_command(cmd)
		cmd = "make strip BUILD_TYPE=" + build_type
		run_os_command(cmd)
...
#	procese_close_components(conf, add_output_path)
	replace_file_content(add_output_path+"/arprj/Makefile", {"ifeq ($(BASE_BOARD),ipcnand)": "ifneq ($(BASE_BOARD),)\n"})
	replace_file_content(add_output_path+"/arprj/Makefile", {"middlewares:": "middlewares:\nifeq (x$(BUILD_TYPE), xdebug1)\n	cp -fL $(gTOOLCHAIN_OPT_PATH)/../aarch64-linux-gnu/lib64/libasan.so $(OUTPUT_MIDDLEWARES_DIR)/lib\nendif\n"})
	...
	for component in spec_components.split():
		...
		if component.find("efuse") >= 0:
			cmd = "rm -rf " + base_output_path + "/arprj/release/out/hal/lib/libhal_efuse.so"
			run_os_command(cmd)
			cmd = "rm -rf " + base_output_path + "/arprj/release/out/hal/lib/libhal_efuse.a"
			run_os_command(cmd)
			cmd = "rm -rf " + base_output_path + "/arprj/release/out/hal/inc/hal_efuse.h"
			run_os_command(cmd)
...
#				run_os_command(cmd)
		...
		cmd = "rm -rf " + output + "/arprj/release/prebuilds/apps/bin"
		run_os_command(cmd)
...
def main():
	...
	if result == False:
		...
		sys.stderr.write("\n Notice: Please input command like below example, or input 'make sdk_help' to get more info.\n")
		sys.stderr.write("   make sdk VENDOR_NAME=ar MODEL=sdk MAJOR_V=0.14.00 MINOR_V=01 TAG=release\n\n")
		sys.exit(1)
	...
	create_sdk_version_file(chip, prefix + version + "-" + tag)
...
#	tar_dir =  os.path.abspath(output) + "/tmp/sdk_release" + str(os.getpid()) + "/"
	...
	tmppath = tar_dir + prefix + version + suffix + "/"

	print("tmppath:", tmppath, tar_dir)
	run_os_command("mkdir -p " + tmppath)

	# make and mv bases to tmppath
	copy_all_components(conf, tmppath, boot_media, base_board, build_type, app, chip, mpp_service)

	# cd to tar_dir and tar
	soure_tar_dir = tar_dir + prefix + version + suffix

	setup_file = soure_tar_dir + "/set_up.sh"
	if not os.path.exists(setup_file):
		f = open(setup_file, 'w')
		f.write('#! /bin/bash\n')
		f.write('tar -xvzf add_on.tar.gz\n')
		f.write('tar -xvzf base.tar.gz\n')
